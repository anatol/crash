 ...
 */
int is_netdump(char *file, ulong source_query)
{
	int i, fd, swap;
	Elf32_Ehdr *elf32;
	Elf32_Phdr *load32;
	...
	if ((elf32->e_ident[EI_CLASS] == ELFCLASS32) &&
	    ...
	    (swap32(elf32->e_version, swap) == EV_CURRENT) && (swap16(elf32->e_phnum, swap) >= 2)) {
		switch (swap16(elf32->e_machine, swap)) {
		case EM_386:
			if (machine_type_mismatch(file, "X86", NULL, source_query))
				goto bailout;
			break;

		case EM_ARM:
			if (machine_type_mismatch(file, "ARM", NULL, source_query))
				goto bailout;
			break;

		case EM_PPC:
			if (machine_type_mismatch(file, "PPC", NULL, source_query))
				goto bailout;
			break;

		default:
			if (machine_type_mismatch(file, "(unknown)", NULL, source_query))
				goto bailout;
		}

		if (endian_mismatch(file, elf32->e_ident[EI_DATA], source_query))
			goto bailout;
		...
		if ((load32->p_offset & (MIN_PAGE_SIZE - 1))
		   ...
		   (swap32(elf64->e_version, swap) == EV_CURRENT) && (swap16(elf64->e_phnum, swap) >= 2)) {
		switch (swap16(elf64->e_machine, swap)) {
		case EM_IA_64:
			if (machine_type_mismatch(file, "IA64", NULL, source_query))
				goto bailout;
			break;

		case EM_PPC64:
			if (machine_type_mismatch(file, "PPC64", NULL, source_query))
				goto bailout;
			break;

		case EM_X86_64:
			if (machine_type_mismatch(file, "X86_64", NULL, source_query))
				goto bailout;
			break;

		case EM_S390:
			if (machine_type_mismatch(file, "S390X", NULL, source_query))
				goto bailout;
			break;
		...
		case EM_AARCH64:
			if (machine_type_mismatch(file, "ARM64", NULL, source_query))
				goto bailout;
			break;
		...
		if (endian_mismatch(file, elf64->e_ident[EI_DATA], source_query))
			goto bailout;
		...
		if (CRASHDEBUG(2))
			error(INFO, "%s: not a %s ELF dumpfile\n",
			      file, source_query == NETDUMP_LOCAL ? "netdump" : "kdump");
	...
	case KDUMP_ELF32:
		...
		if ((nd->pt_load_segments = (struct pt_load_segment *)
		     malloc(sizeof(struct pt_load_segment) * nd->num_pt_load_segments)) == NULL) {
	...
	if ((source_query == KDUMP_LOCAL) && (pc->flags2 & GET_OSRELEASE))
		kdump_get_osrelease();

	if ((source_query == KDUMP_LOCAL) && (pc->flags2 & GET_LOG)) {
		pc->dfd = nd->ndfd;
 ...
 */
int file_elf_version(char *file)
{
	int fd, size;
	Elf32_Ehdr *elf32;
	Elf64_Ehdr *elf64;
	...
	if (STRNEQ(elf32->e_ident, ELFMAG) &&
	    ...
	    (elf32->e_ident[EI_DATA] == ELFDATA2LSB) && (elf32->e_ident[EI_VERSION] == EV_CURRENT)) {
		return (elf32->e_version);
	} else if (STRNEQ(elf64->e_ident, ELFMAG) &&
		   (elf64->e_ident[EI_CLASS] == ELFCLASS64) && (elf64->e_ident[EI_VERSION] == EV_CURRENT)) {
 ...
 */
static void check_dumpfile_size(char *file)
{
	int i;
	struct stat64 stat;
	...
	for (i = 0; i < nd->num_pt_load_segments; i++) {
		...
		segment_end = pls->file_offset + (pls->phys_end - pls->phys_start);
		...
		if (segment_end > stat.st_size) {
			error(WARNING, "%s: may be truncated or incomplete\n"
			      ...
			      "           bytes required: %lld\n"
			      "            dumpfile size: %lld\n\n",
			      file, pls->file_offset, pls->phys_end - pls->phys_start, segment_end, stat.st_size);
			return;
		}
	}
}
 ...
 */
int netdump_init(char *unused, FILE * fptr)
{
	if (!VMCORE_VALID())
		return FALSE;
 ...
 */
int read_netdump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	off_t offset;
	struct pt_load_segment *pls;
	int i;
	...
	case KDUMP_ELF64:
		if (nd->num_pt_load_segments == 1) {
			offset = (off_t) paddr + (off_t) nd->header_size - (off_t) nd->pt_load_segments[0].phys_start;
			break;
		}
		...
		for (i = offset = 0; i < nd->num_pt_load_segments; i++) {
			...
			if ((paddr >= pls->phys_start)
			    && (paddr < pls->phys_end)) {
				offset = (off_t) (paddr - pls->phys_start) + pls->file_offset;
				break;
			}
			if (pls->zero_fill && (paddr >= pls->phys_end) && (paddr < pls->zero_fill)) {
				memset(bufptr, 0, cnt);
				if (CRASHDEBUG(8))
	...
	if (CRASHDEBUG(8))
		fprintf(fp,
			"read_netdump: addr: %lx paddr: %llx cnt: %d offset: %llx\n",
	...
	if (FLAT_FORMAT()) {
		if (!read_flattened_format(nd->ndfd, offset, bufptr, cnt)) {
			if (CRASHDEBUG(8))
				fprintf(fp, "read_netdump: READ_ERROR: "
					"read_flattened_format failed for offset:" " %llx\n", (ulonglong) offset);
			return READ_ERROR;
		}
	} else {
		if (lseek(nd->ndfd, offset, SEEK_SET) == -1) {
			if (CRASHDEBUG(8))
				fprintf(fp, "read_netdump: SEEK_ERROR: " "offset: %llx\n", (ulonglong) offset);
			return SEEK_ERROR;
		}
		if (read(nd->ndfd, bufptr, cnt) != cnt) {
			if (CRASHDEBUG(8))
				fprintf(fp, "read_netdump: READ_ERROR: " "offset: %llx\n", (ulonglong) offset);
			return READ_ERROR;
		}
	}
 ...
 */
int write_netdump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	off_t offset;
	struct pt_load_segment *pls;
	int i;
 ...
 */
static void netdump_print(char *fmt, ...)
{
	char buf[BUFSIZE];
	va_list ap;
...
ulong get_netdump_panic_task(void)
{
	...
	if (nd->task_struct) {
		if (CRASHDEBUG(1))
			error(INFO, "get_netdump_panic_task: NT_TASKSTRUCT: %lx\n", nd->task_struct);
		return nd->task_struct;
	}
 ...
 check_ebp_esp:
		...
		if (IS_KVADDR(esp)) {
			...
			if (CRASHDEBUG(1))
				error(INFO, "get_netdump_panic_task: esp: %lx -> task: %lx\n", esp, task);
		...
		if (IS_KVADDR(ebp)) {
			...
			if (CRASHDEBUG(1))
				error(INFO, "get_netdump_panic_task: ebp: %lx -> task: %lx\n", ebp, task);
			for (i = 0; task && (i < NR_CPUS); i++) {
				if (task == tt->active_set[i])
					return task;
			}
		...
		user_regs = (char *)((char *)note64 + len + MEMBER_OFFSET("elf_prstatus", "pr_reg"));
		...
		if (nd->elf64->e_machine == EM_PPC64) {
			...
			if (CRASHDEBUG(1))
				error(INFO, "get_netdump_panic_task: NT_PRSTATUS esp: %lx\n", esp);
			if (IS_KVADDR(esp)) {
				task = stkptr_to_task(esp);
				if (CRASHDEBUG(1))
					error(INFO, "get_netdump_panic_task: esp: %lx -> task: %lx\n", esp, task);
 ...
 */
ulong get_netdump_switch_stack(ulong task)
{
#ifdef DAEMON
...
int netdump_memory_dump(FILE * fp)
{
	...
	if ((pc->flags & RUNTIME) && symbol_exists("dump_level")) {
		...
		if (readmem(symbol_value("dump_level"), KVADDR, &dump_level,
			...
			netdump_print("             dump_level: %d (0x%x) %s",
				      dump_level, dump_level, dump_level > 0 ? "(" : "");
...
#define DUMP_SAVE_PRIVATE  0x00000010	/* Save private pages */
			...
			if (dump_level & DUMP_EXCLUDE_CACHE)
				netdump_print("%sDUMP_EXCLUDE_CACHE", others++ ? "|" : "");
			if (dump_level & DUMP_EXCLUDE_CLEAN)
				netdump_print("%sDUMP_EXCLUDE_CLEAN", others++ ? "|" : "");
			if (dump_level & DUMP_EXCLUDE_FREE)
				netdump_print("%sDUMP_EXCLUDE_FREE", others++ ? "|" : "");
			if (dump_level & DUMP_EXCLUDE_ANON)
				netdump_print("%sDUMP_EXCLUDE_ANON", others++ ? "|" : "");
			if (dump_level & DUMP_SAVE_PRIVATE)
				netdump_print("%sDUMP_SAVE_PRIVATE", others++ ? "|" : "");
			netdump_print("%s\n", dump_level > 0 ? ")" : "");
		} else
	...
	if (XEN_CORE_DUMPFILE()) {
		...
		netdump_print("               cache_hits: %ld ", nd->xen_kdump_data->cache_hits);
		if (nd->xen_kdump_data->accesses)
			netdump_print("(%ld%%)", nd->xen_kdump_data->cache_hits * 100 / nd->xen_kdump_data->accesses);
		...
		for (i = 0; i < nd->xen_kdump_data->p2m_frames; i++)
			netdump_print("%lx ", nd->xen_kdump_data->p2m_mfn_frame_list[i]);
		if (i)
			netdump_print("\n");
	}
	...
	if (nd->num_prstatus_notes == 1)
		...
		for (i = 0; i < nd->num_prstatus_notes; i++) {
			if ((i % wrap) == 0)
				netdump_print("\n        ");
			netdump_print("%.*lx ", flen, nd->nt_prstatus_percpu[i]);
		}
	}
	netdump_print("\n");
	...
	if (nd->num_qemu_notes == 1)
		...
		for (i = 0; i < nd->num_qemu_notes; i++) {
			if ((i % wrap) == 0)
				netdump_print("\n        ");
			netdump_print("%.*lx ", flen, nd->nt_qemu_percpu[i]);
		}
	}
	netdump_print("\n");
...
static void dump_Elf32_Ehdr(Elf32_Ehdr * elf)
{
	...
	netdump_print("    e_ident[EI_VERSION]: %d ", elf->e_ident[EI_VERSION]);
...
static void dump_Elf32_Phdr(Elf32_Phdr * prog, int store_pt_load_data)
{
	...
	netdump_print("               p_filesz: %lu (%lx)\n", prog->p_filesz, prog->p_filesz);
	if (store_pt_load_data) {
		pls->phys_end = pls->phys_start + prog->p_filesz;
		pls->zero_fill = (prog->p_filesz == prog->p_memsz) ? 0 : pls->phys_start + prog->p_memsz;
	}
	netdump_print("                p_memsz: %lu (%lx)\n", prog->p_memsz, prog->p_memsz);
 ...
 */
static long vmcoreinfo_read_integer(const char *key, long default_value)
{
	char *string;
...
static size_t dump_Elf32_Nhdr(Elf32_Off offset, int store)
{
	...
	case NT_KDUMPINFO:
		...
		if (store) {
			uptr = (note->n_namesz == 5) ?
			    (ulong *) (ptr + ((note->n_namesz + 3) & ~3)) : (ulong *) (ptr + note->n_namesz);
	...
	case NT_XEN_KDUMP_CR3:
		...
		if (store)
			error(WARNING, "obsolete Xen n_type: %lx (NT_XEN_KDUMP_CR3)\n\n", note->n_type);
		/* FALL THROUGH */
	...
	case XEN_ELFNOTE_CRASH_INFO:
		...
		if (store) {
			...
			if ((note->n_type == NT_XEN_KDUMP_CR3) && ((note->n_descsz / sizeof(ulong)) == 1)) {
	...
	if (!lf || (note->n_type == NT_TASKSTRUCT) || (note->n_type == NT_DISKDUMP) || xen_core)
		netdump_print("\n");
		...
		if (store) {
			...
			if (nd->elf64->e_machine == EM_386) {
				u32ptr = (note->n_namesz == 5) ?
				    (uint *) (ptr + ((note->n_namesz + 3) & ~3)) : (uint *) (ptr + note->n_namesz);
				...
				uptr = (note->n_namesz == 5) ?
				    (ulonglong *) (ptr +
						   ((note->n_namesz + 3) & ~3)) : (ulonglong *) (ptr + note->n_namesz);
				nd->page_size = (uint) (1 << *uptr);
				uptr++;
				nd->task_struct = *uptr;
			}
 ...
 */
void get_netdump_regs(struct bt_info *bt, ulong * eip, ulong * esp)
{
	int e_machine;
	...
	default:
		error(FATAL, "support for ELF machine type %d not available\n", e_machine);
	}
}
...
void get_netdump_regs_x86_64(struct bt_info *bt, ulong * ripp, ulong * rspp)
{
	...
	if (((NETDUMP_DUMPFILE() || KDUMP_DUMPFILE()) &&
	    ...
	    (KDUMP_DUMPFILE() && (kt->flags & DWARF_UNWIND) && (bt->flags & BT_DUMPFILE_SEARCH))) {
		if (nd->num_prstatus_notes > 1)
			note = (Elf64_Nhdr *)
		...
		regs_size = VALID_STRUCT(user_regs_struct) ?
		    SIZE(user_regs_struct) : sizeof(struct x86_64_user_regs_struct);
		rsp_offset = VALID_MEMBER(user_regs_struct_rsp) ? OFFSET(user_regs_struct_rsp) : offsetof(struct
													  x86_64_user_regs_struct,
													  rsp);
		rip_offset = VALID_MEMBER(user_regs_struct_rip) ? OFFSET(user_regs_struct_rip) : offsetof(struct
													  x86_64_user_regs_struct,
													  rip);
		...
		if (CRASHDEBUG(1))
			netdump_print("ELF prstatus rsp: %lx rip: %lx\n",
				      ULONG(user_regs + rsp_offset), ULONG(user_regs + rip_offset));
	...
	if (ELF_NOTES_VALID() && (bt->flags & BT_DUMPFILE_SEARCH) && DISKDUMP_DUMPFILE() && (note = (Elf64_Nhdr *)
											     diskdump_get_prstatus_percpu
											     (bt->tc->processor))) {
...
void get_netdump_regs_x86(struct bt_info *bt, ulong * eip, ulong * esp)
{
	...
	search = ((bt->flags & BT_TEXT_SYMBOLS)
	...
	for (i = 0, up = (ulong *) bt->stackbuf; i < LONGS_PER_STACK; i++, up++) {
		...
		if (XEN_CORE_DUMPFILE()) {
			...
			if (STREQ(sym, "crash_kexec")) {
			   ...
			   (STREQ(sym, "crash_kexec") && !KVMDUMP_DUMPFILE()) || STREQ(sym, "disk_dump")) {
 crash_kexec:
			*eip = *up;
			*esp = search ? bt->stackbase + ((char *)(up + 1) - bt->stackbuf) : *(up - 1);
			return;
		}
 ...
 next_sysrq:
		...
		if (STREQ(sym, "smp_call_function_interrupt")) {
			if (ipintr_eip && IS_VMALLOC_ADDR(ipintr_func) && IS_KERNEL_STATIC_TEXT(*(up - 2)))
				continue;
			ipintr_eip = *up;
			ipintr_esp = search ?
		...
		if (XEN_CORE_DUMPFILE() && !panic_task && (bt->tc->pid == 0) && STREQ(sym, "safe_halt")) {
			halt_eip = *up;
			halt_esp = bt->stackbase + ((char *)(up + 1) - bt->stackbuf);
		}

		if (XEN_CORE_DUMPFILE() && !panic_task && (bt->tc->pid == 0) && !halt_eip && STREQ(sym, "xen_idle")) {
			halt_eip = *up;
			halt_esp = bt->stackbase + ((char *)(up + 1) - bt->stackbuf);
		}
	}
	...
	if (check_hardirq && (tt->hardirq_tasks[bt->tc->processor] == bt->tc->task)) {
	...
	if (check_softirq && (tt->softirq_tasks[bt->tc->processor] == bt->tc->task)) {
	...
	if (ELF_NOTES_VALID() && DISKDUMP_DUMPFILE() && (note = (Elf32_Nhdr *)
		...
		if (is_kernel_text(ip) &&
		    (((sp >= GET_STACKBASE(bt->task)) &&
		      (sp < GET_STACKTOP(bt->task))) || in_alternate_stack(bt->tc->processor, sp))) {
	...
	if (CRASHDEBUG(1))
		error(INFO, "get_netdump_regs_x86: cannot find anything useful (task: %lx)\n", bt->task);
...
static void get_netdump_regs_ppc(struct bt_info *bt, ulong * eip, ulong * esp)
{
	Elf32_Nhdr *note;
	size_t len;

	ppc_relocate_nt_prstatus_percpu(nd->nt_prstatus_percpu, &nd->num_prstatus_notes);

	if ((bt->task == tt->panic_task) || (is_task_active(bt->task) && nd->num_prstatus_notes > 1)) {
		/*
		...
		if (nd->num_prstatus_notes > 1) {
			if (!nd->nt_prstatus_percpu[bt->tc->processor])
				error(FATAL,
				      ...
				      "for %s task: %lx\n",
				      (bt->task == tt->panic_task) ? "panic" : "active", bt->task);
			note = (Elf32_Nhdr *)
			    nd->nt_prstatus_percpu[bt->tc->processor];
		} else
		...
		len = sizeof(Elf32_Nhdr);
		len = roundup(len + note->n_namesz, 4);
		bt->machdep = (void *)((char *)note + len + MEMBER_OFFSET("elf_prstatus", "pr_reg"));
	}
...
static void get_netdump_regs_ppc64(struct bt_info *bt, ulong * eip, ulong * esp)
{
	Elf64_Nhdr *note;
	size_t len;
...
static void get_netdump_regs_arm(struct bt_info *bt, ulong * eip, ulong * esp)
{
	machdep->get_stack_frame(bt, eip, esp);
}

static void get_netdump_regs_arm64(struct bt_info *bt, ulong * eip, ulong * esp)
{
	machdep->get_stack_frame(bt, eip, esp);
}
 ...
 */
int is_kdump(char *file, ulong source_query)
{
	return is_netdump(file, source_query);
}

int kdump_init(char *unused, FILE * fptr)
{
...
int read_kdump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	physaddr_t paddr_in = paddr;

	if ((nd->flags & QEMU_MEM_DUMP_KDUMP_BACKUP) &&
	    (paddr >= nd->backup_src_start) && (paddr < nd->backup_src_start + nd->backup_src_size)) {

		paddr += nd->backup_offset - nd->backup_src_start;

		if (CRASHDEBUG(1))
			error(INFO, "qemu_mem_dump: kdump backup region: %#llx => %#llx\n", paddr_in, paddr);
	}
	...
	if (XEN_CORE_DUMPFILE() && !XEN_HYPER_MODE()) {
		if (!(nd->xen_kdump_data->flags & KDUMP_P2M_INIT)) {
			...
			if ((nd->xen_kdump_data->page = (char *)malloc(PAGESIZE())) == NULL)
				error(FATAL, "cannot malloc xen kdump data page\n");

			if (!machdep->xen_kdump_p2m_create(nd->xen_kdump_data))
				error(FATAL, "cannot create xen kdump pfn-to-mfn mapping\n");
		...
		if ((paddr = xen_kdump_p2m(paddr)) == P2M_FAILURE) {
			if (CRASHDEBUG(8))
				fprintf(fp, "read_kdump: xen_kdump_p2m(%llx): " "P2M_FAILURE\n", (ulonglong) paddr_in);
			return READ_ERROR;
		}
		if (CRASHDEBUG(8))
...
int write_kdump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	return write_netdump(fd, bufptr, cnt, addr, paddr);
}

void get_kdump_regs(struct bt_info *bt, ulong * eip, ulong * esp)
{
 ...
 */
static physaddr_t xen_kdump_p2m(physaddr_t pseudo)
{
	ulong pfn, mfn_frame;
	ulong *mfnptr;
	...
	if (mfn_frame == xkd->last_mfn_read)
		...
		if (read_netdump(0, xkd->page, PAGESIZE(), 0, (physaddr_t) PTOB(mfn_frame)) != PAGESIZE())
			return P2M_FAILURE;
	}
	...
	if (CRASHDEBUG(7))
		fprintf(fp,
			...
			(ulonglong) pseudo, mfn_idx, frame_idx, mfn_frame, *mfnptr, (ulonglong) paddr);
 ...
 */
void xen_kdump_p2m_mfn(char *arg)
{
	ulong value;
	int errflag;
	...
	if (!errflag) {
		...
		if (CRASHDEBUG(1))
			error(INFO, "xen_kdump_data.p2m_mfn override: %lx\n", value);
	} else
...
void set_xen_phys_start(char *arg)
{
	ulong value;
	int errflag = 0;
...
int exist_regs_in_elf_notes(struct task_context *tc)
{
	if ((tc->task == tt->panic_task) ||
	    (is_task_active(tc->task) && (nd->num_prstatus_notes > 1) && (tc->processor < nd->num_prstatus_notes)))
		return TRUE;
	else
		return FALSE;
}
...
void *get_regs_from_elf_notes(struct task_context *tc)
{
	...
	case EM_ARM:
		break;
	case EM_AARCH64:
		error(FATAL, "get_regs_from_elf_notes: ARM64 support TBD\n");
	default:
		error(FATAL, "support for ELF machine type %d not available\n", e_machine);
	}
...
static void *get_x86_regs_from_elf_notes(struct task_context *tc)
{
	...
	pt_regs = (void *)((char *)note + len + MEMBER_OFFSET("elf_prstatus", "pr_reg"));
...
static void *get_ppc_regs_from_elf_notes(struct task_context *tc)
{
	Elf32_Nhdr *note;
	size_t len;
	void *pt_regs;
...
static void *get_arm_regs_from_elf_notes(struct task_context *tc)
{
	Elf32_Nhdr *note_32;
	Elf64_Nhdr *note_64;
	void *note;
 ...
 */
int read_proc_kcore(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	int i;
	size_t readcnt;
	ulong kvaddr;
	...
	case KCORE_ELF32:
		for (i = 0; i < pkd->segments; i++) {
			lp32 = pkd->load32 + i;
			if ((kvaddr >= lp32->p_vaddr) && (kvaddr < (lp32->p_vaddr + lp32->p_memsz))) {
				offset = (off_t) (kvaddr - lp32->p_vaddr) + (off_t) lp32->p_offset;
				break;
			}
		}
		/*
		...
		for (i = 0; i < pkd->segments; i++) {
			...
			if ((addr >= lp32->p_vaddr)
				...
				offset = (off_t) (addr - lp32->p_vaddr) + (off_t) lp32->p_offset;
				break;
			}
		}
	...
	case KCORE_ELF64:
		for (i = 0; i < pkd->segments; i++) {
			lp64 = pkd->load64 + i;
			if ((kvaddr >= lp64->p_vaddr) && (kvaddr < (lp64->p_vaddr + lp64->p_memsz))) {
				offset = (off_t) (kvaddr - lp64->p_vaddr) + (off_t) lp64->p_offset;
				break;
			}
		}
 ...
 */
int write_proc_kcore(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	error(FATAL, "cannot write to /proc/kcore\n");
	return FALSE;
}

int is_proc_kcore(char *file, ulong source_query)
{
	if (STREQ(file, "/proc/kcore") || same_file(file, "/proc/kcore")) {
		if (!is_netdump(file, source_query))
			error(FATAL, "cannot translate the ELF header of /proc/kcore\n");
...
int kcore_memory_dump(FILE * ofp)
{
	int i, others;
	Elf32_Phdr *lp32;
	Elf64_Phdr *lp64;
...
void dump_registers_for_qemu_mem_dump(void)
{
	...
	for (i = 0; i < nd->num_qemu_notes; i++) {
		...
		netdump_print("  rsp:%016llx  rbp:%016llx  ", ptr->rsp, ptr->rbp);
		...
		if (DUMPFILE_FORMAT(nd->flags) == KDUMP_ELF64) {
			...
			netdump_print("  r15:%016llx", ptr->r15);
		} else
			netdump_print("\n");

		netdump_print("  rip:%016llx  rflags:%08llx\n", ptr->rip, ptr->rflags);
		netdump_print("  cs:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->cs.selector, ptr->cs.limit, ptr->cs.flags, ptr->cs.pad, ptr->cs.base);
		netdump_print("  ds:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->ds.selector, ptr->ds.limit, ptr->ds.flags, ptr->ds.pad, ptr->ds.base);
		netdump_print("  es:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->es.selector, ptr->es.limit, ptr->es.flags, ptr->es.pad, ptr->es.base);
		netdump_print("  fs:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->fs.selector, ptr->fs.limit, ptr->fs.flags, ptr->fs.pad, ptr->fs.base);
		netdump_print("  gs:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->gs.selector, ptr->gs.limit, ptr->gs.flags, ptr->gs.pad, ptr->gs.base);
		netdump_print("  ss:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->ss.selector, ptr->ss.limit, ptr->ss.flags, ptr->ss.pad, ptr->ss.base);
		netdump_print("  ldt:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->ldt.selector, ptr->ldt.limit, ptr->ldt.flags, ptr->ldt.pad, ptr->ldt.base);
		netdump_print("  tr:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->tr.selector, ptr->tr.limit, ptr->tr.flags, ptr->tr.pad, ptr->tr.base);
		netdump_print("  gdt:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->gdt.selector, ptr->gdt.limit, ptr->gdt.flags, ptr->gdt.pad, ptr->gdt.base);
		netdump_print("  idt:\n    selector:%08lx  limit:%08lx  flags:%08lx\n\
		pad:%08lx  base:%016llx\n", ptr->idt.selector, ptr->idt.limit, ptr->idt.flags, ptr->idt.pad, ptr->idt.base);
...
void kdump_backup_region_init(void)
{
	...
	if (!readmem(symbol_value("kexec_crash_image"), KVADDR,
		     ...
		     "kexec backup region: kexec_crash_image", QUIET | RETURN_ON_ERROR))
		goto error;
	...
	kimage_segment_len = get_array_length("kimage.segment", NULL, STRUCT_SIZE("kexec_segment"));

	if (!readmem(kexec_crash_image_p + MEMBER_OFFSET("kimage", "segment"),
		     KVADDR, buf, MEMBER_SIZE("kimage", "segment"),
		     "kexec backup region: kexec_crash_image->segment", QUIET | RETURN_ON_ERROR))
		goto error;
	...
	for (i = 0; i < kimage_segment_len; ++i) {
		...
		mem = ULONG(buf + i * STRUCT_SIZE("kexec_segment") + MEMBER_OFFSET("kexec_segment", "mem"));
		if (!mem)
			continue;
	...
	if (!elfcorehdr_p) {
		if (CRASHDEBUG(1))
			error(INFO, "%s: elfcorehdr not found in segments of kexec_crash_image\n", typename);
		goto error;
	}

	if (is_32_bit) {
		if (!readmem
		    (elfcorehdr_p, PHYSADDR, buf, STRUCT_SIZE("elf32_hdr"), "elfcorehdr", QUIET | RETURN_ON_ERROR))
			goto error;
		...
		if (!readmem
		    (elfcorehdr_p, PHYSADDR, buf, STRUCT_SIZE("elf64_hdr"), "elfcorehdr", QUIET | RETURN_ON_ERROR))
			goto error;
	...
	for (i = 0; i < e_phnum; ++i) {
		...
		if (is_32_bit) {
			if (!readmem(elfcorehdr_p + e_phoff32 + i * e_phentsize,
				     PHYSADDR, buf, e_phentsize, "elfcorehdr: program header", QUIET | RETURN_ON_ERROR))
				goto error;
		 ...
		 */
		if (is_32_bit) {
			if (p_type == PT_LOAD && p_paddr32 <= KEXEC_BACKUP_SRC_END && p_paddr32 != p_offset32) {
			...
			if (p_type == PT_LOAD && p_paddr64 <= KEXEC_BACKUP_SRC_END && p_paddr64 != p_offset64) {
