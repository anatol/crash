...
void xen_hyper_init(void)
{
	...
	if (machine_type("X86_64") && symbol_exists("xen_phys_start") && !xen_phys_start())
		error(WARNING,
	...
	if (symbol_exists("crashing_cpu")) {
		get_symbol_data("crashing_cpu", sizeof(xht->crashing_cpu), &xht->crashing_cpu);
	} else {
	...
	if (symbol_exists("__per_cpu_offset")) {
		...
		if (!readmem(symbol_value("__per_cpu_offset"), KVADDR,
			     ...
			     sizeof(ulong) * XEN_HYPER_MAX_CPUS(), "__per_cpu_offset", RETURN_ON_ERROR)) {
	...
	strncpy(xht->utsname.machine, (char *)(buf + member_offset), sizeof(xht->utsname.machine) - 1);
	FREEBUF(buf);
#elif defined(IA64)
	...
	strncpy(xht->utsname.machine,
		(char *)(buf + XEN_HYPER_OFFSET(cpuinfo_ia64_vendor)), sizeof(xht->utsname.machine) - 1);
	FREEBUF(buf);
#endif
	...
	if (!readmem(symbol_value("idle_vcpu"), KVADDR, xht->idle_vcpu_array,
		     xht->idle_vcpu_size * sizeof(ulong), "idle_vcpu_array", RETURN_ON_ERROR)) {
...
void xen_hyper_domain_init(void)
{
	...
	XEN_HYPER_MEMBER_OFFSET_INIT
	    (arch_shared_info_pfn_to_mfn_frame_list_list, "arch_shared_info", "pfn_to_mfn_frame_list_list");
...
void xen_hyper_vcpu_init(void)
{
	...
	XEN_HYPER_ASSIGN_OFFSET(vcpu_thread_ksp) =
	    MEMBER_OFFSET("vcpu", "arch") + MEMBER_OFFSET("arch_vcpu",
							  "_thread") + MEMBER_OFFSET("thread_struct", "ksp");
#endif
...
static void xen_hyper_schedule_init(void)
{
	...
	if ((xhscht->scheduler_struct = malloc(XEN_HYPER_SIZE(scheduler))) == NULL) {
	...
	if (symbol_exists("ops")) {
		if (!readmem(symbol_value("ops") + scheduler_opt_name, KVADDR,
			     &opt_sched, sizeof(ulong), "ops.opt_name", RETURN_ON_ERROR)) {
			error(FATAL, "cannot read ops.opt_name.\n");
		}
	} else {
		opt_sched = symbol_value("opt_sched");
	}
	if (!readmem(opt_sched, KVADDR, xhscht->opt_sched, XEN_HYPER_OPT_SCHED_SIZE, "opt_sched,", RETURN_ON_ERROR)) {
	...
	while (xhscht->name == NULL) {
		if (!readmem(addr, KVADDR, schedulers_buf,
			     sizeof(long) * XEN_HYPER_SCHEDULERS_ARRAY_CNT, "schedulers", RETURN_ON_ERROR)) {
		...
		for (i = 0; i < XEN_HYPER_SCHEDULERS_ARRAY_CNT; i++) {
			...
			if (!readmem(schedulers_buf[i], KVADDR,
				     ...
				     XEN_HYPER_SIZE(scheduler), "scheduler", RETURN_ON_ERROR)) {
				error(FATAL, "cannot read scheduler.\n");
			}
			opt_name = ULONG(xhscht->scheduler_struct + scheduler_opt_name);
			if (!readmem
			    (opt_name, KVADDR, opt_name_buf, XEN_HYPER_OPT_SCHED_SIZE, "opt_name", RETURN_ON_ERROR)) {
				error(FATAL, "cannot read opt_name.\n");
			}
			if (strncmp(xhscht->opt_sched, opt_name_buf, XEN_HYPER_OPT_SCHED_SIZE))
				continue;
			xhscht->scheduler = schedulers_buf[i];
			xhscht->sched_id = INT(xhscht->scheduler_struct + XEN_HYPER_OFFSET(scheduler_sched_id));
			addr = ULONG(xhscht->scheduler_struct + XEN_HYPER_OFFSET(scheduler_name));
			if (!readmem(addr, KVADDR, buf, XEN_HYPER_SCHEDULER_NAME, "scheduler_name", RETURN_ON_ERROR)) {
	...
	BZERO(xhscht->sched_context_array, sizeof(struct xen_hyper_sched_context) * XEN_HYPER_MAX_CPUS());
	...
	for_cpu_indexes(i, cpuid) {
		...
		if (flag) {
			schc->schedule_data = xen_hyper_per_cpu(addr, i);
		} else {
			schc->schedule_data = addr + XEN_HYPER_SIZE(schedule_data) * i;
		}
		if (!readmem(schc->schedule_data,
			     KVADDR, buf, XEN_HYPER_SIZE(schedule_data), "schedule_data", RETURN_ON_ERROR)) {
		...
		schc->sched_priv = ULONG(buf + XEN_HYPER_OFFSET(schedule_data_sched_priv));
...
void xen_hyper_post_init(void)
{
	...
	for_cpu_indexes(i, cpuid) {
		pcc = &xhpct->context_array[cpuid];
		if (!pcc->current_vcpu) {
			pcc->current_vcpu = xen_hyper_get_active_vcpu_from_pcpuid(cpuid);
		}
	}

	/* set pcpu last */
	if (!(xhpct->last = xen_hyper_id_to_pcpu_context(XEN_HYPER_CRASHING_CPU()))) {
		xhpct->last = &xhpct->context_array[xht->cpu_idxs[0]];
	}

	/* set vcpu last */
	if (xhpct->last) {
		xhvct->last = xen_hyper_vcpu_to_vcpu_context(xhpct->last->current_vcpu);
		/* set crashing vcpu */
	...
	/* set domain last */
	if (xhvct->last) {
		xhdt->last = xen_hyper_domain_to_domain_context(xhvct->last->domain);
	}
	if (!xhdt->last) {
...
void xen_hyper_dumpinfo_init(void)
{
	...
	for (i = 0; i < machdep->get_smp_cpus(); i++) {
		...
		if (symbol_exists("per_cpu__crash_notes"))
			...
			addr_notes =
			    addr + i * STRUCT_SIZE("crash_note_range_t") + MEMBER_OFFSET("crash_note_range_t", "start");
		if (xhdit->note_ver == XEN_HYPER_ELF_NOTE_V4) {
			if (!readmem
			    (addr_notes, KVADDR,
			     &(xhdit->context_array[i].note), sizeof(ulong), "crash_notes", RETURN_ON_ERROR)) {
	...
	if (!xen_hyper_fill_elf_notes(xhdit->context_array[samp_cpuid].note, buf, XEN_HYPER_ELF_NOTE_FILL_T_NOTE)) {
	...
	case XEN_HYPER_ELF_NOTE_V4:
		...
		if (XEN_HYPER_ELF_NOTE_V4_NOTE_SIZE < xhdit->core_size + xhdit->xen_core_size + 32) {
		...
		if (XEN_HYPER_ELF_NOTE_V4_NOTE_SIZE < xhdit->core_size + xhdit->xen_core_size + xhdit->xen_info_size) {
	...
	if (xhdit->note_ver >= XEN_HYPER_ELF_NOTE_V3) {
		if ((xhdit->crash_note_xen_info_ptr = malloc(xhdit->xen_info_size)) == NULL) {
			error(FATAL, "cannot malloc dumpinfo table " "crash_note_xen_info_ptr space.\n");
		}
		memcpy(xhdit->crash_note_xen_info_ptr,
		       bp + xhdit->core_size + xhdit->xen_core_size, xhdit->xen_info_size);
		xhdit->context_xen_info.note =
		    xhdit->context_array[samp_cpuid].note + xhdit->core_size + xhdit->xen_core_size;
		xhdit->context_xen_info.pcpu_id = samp_cpuid;
		xhdit->context_xen_info.crash_xen_info_ptr = xhdit->crash_note_xen_info_ptr + xhdit->xen_info_offset;
	}
	...
	if (xhdit->note_ver >= XEN_HYPER_ELF_NOTE_V2) {
		...
		if (!(xhdit->crash_note_xen_core_array = malloc(size))) {
			error(FATAL, "cannot malloc dumpinfo table " "crash_note_xen_core_array space.\n");
		}
	...
	for_cpu_indexes(i, cpuid) {
		...
		if (!xen_hyper_fill_elf_notes(addr, nccp, XEN_HYPER_ELF_NOTE_FILL_T_CORE)) {
			error(FATAL, "cannot read elf note core.\n");
		}
		xhdit->context_array[cpuid].pcpu_id = cpuid;
		xhdit->context_array[cpuid].ELF_Prstatus_ptr = nccp + xhdit->core_offset;
		xhdit->context_array[cpuid].pr_reg_ptr =
		    nccp + xhdit->core_offset + XEN_HYPER_OFFSET(ELF_Prstatus_pr_reg);
		...
		if (xhdit->note_ver == XEN_HYPER_ELF_NOTE_V2 && cpuid != samp_cpuid) {
		...
		xhdit->context_xen_core_array[cpuid].note = xhdit->context_array[cpuid].note + xhdit->core_size;
		xhdit->context_xen_core_array[cpuid].pcpu_id = cpuid;
		xhdit->context_xen_core_array[cpuid].crash_xen_core_ptr = xccp + xhdit->xen_core_offset;
		if (!xen_hyper_fill_elf_notes
		    (xhdit->context_xen_core_array[cpuid].note, xccp, XEN_HYPER_ELF_NOTE_FILL_T_XEN_CORE)) {
...
char *xen_hyper_fill_elf_notes(ulong note, char *note_buf, int type)
{
	...
	if (!readmem(rp, KVADDR, note_buf, size, "note_buf_t or crash_note_t", RETURN_ON_ERROR)) {
		if (type == XEN_HYPER_ELF_NOTE_FILL_T_NOTE)
...
void xen_hyper_refresh_domain_context_space(void)
{
	...
	XEN_HYPER_RUNNING_DOMAINS() = XEN_HYPER_NR_DOMAINS() = xen_hyper_get_domains();
	...
	domain = next;
	dom0 = dc;
	while ((domain_struct = xen_hyper_get_domain_next(XEN_HYPER_DOMAIN_READ_NEXT, &next)) != NULL) {
		xen_hyper_store_domain_context(dc, domain, domain_struct);
		domain = next;
		dc++;
	}
...
int xen_hyper_get_domains(void)
{
	...
	while (domain != 0) {
		...
		if (!readmem(next_in_list, KVADDR, &domain, sizeof(void *), "domain.next_in_list", RETURN_ON_ERROR)) {
...
char *xen_hyper_get_domain_next(int mod, ulong * next)
{
	...
	if (idx) {
		if ((domain_struct = xen_hyper_read_domain(*next)) == NULL) {
...
struct xen_hyper_domain_context *xen_hyper_domain_to_domain_context(ulong domain)
{
	...
	if (xhdt->context_array == NULL || xhdt->context_array->domain == 0) {
		return NULL;
	}
	if (!domain) {
		return NULL;
	}
	for (i = 0, dc = xhdt->context_array; i < XEN_HYPER_NR_DOMAINS(); i++, dc++) {
		if (domain == dc->domain) {
			return dc;
		}
	}
								...
								*dc, ulong domain, char *dp)
{
	...
	dc->domain = domain;
	BCOPY((char *)(dp + XEN_HYPER_OFFSET(domain_domain_id)), &dc->domain_id, sizeof(domid_t));
	...
	if (XEN_HYPER_VALID_MEMBER(domain_domain_flags))
		...
		} else if (XEN_HYPER_VALID_MEMBER(domain_is_polling) && *(dp + XEN_HYPER_OFFSET(domain_is_polling))) {
	...
	if (!(dc->vcpu = malloc(sizeof(ulong) * max_vcpus))) {
		error(FATAL, "cannot malloc vcpu array (%d VCPUs).", max_vcpus);
	}
	if (MEMBER_TYPE("domain", "vcpu") == TYPE_CODE_ARRAY)
		...
		if (vcpu_array && max_vcpus) {
			if (!(vcpup = malloc(max_vcpus * sizeof(void *)))) {
				error(FATAL, "cannot malloc VCPU array for domain %lx.", domain);
			}
			if (!readmem(vcpu_array, KVADDR,
				     vcpup, max_vcpus * sizeof(void *), "VCPU array", RETURN_ON_ERROR)) {
				error(FATAL, "cannot read VCPU array for domain %lx.", domain);
			}
		} else {
			vcpup = NULL;
		}
...
void xen_hyper_alloc_domain_context_space(int domains)
{
	if (xhdt->context_array == NULL) {
		if (!(xhdt->context_array = malloc(domains * sizeof(struct xen_hyper_domain_context)))) {
			error(FATAL, "cannot malloc context array (%d domains).", domains);
		}
		xhdt->context_array_cnt = domains;
	} else if (domains > xhdt->context_array_cnt) {
		struct xen_hyper_domain_context *dc;
		int i;
		for (dc = xhdt->context_array, i = 0; i < xhdt->context_array_cnt; ++dc, ++i) {
			if (dc->vcpu)
				free(dc->vcpu);
		}
		if (!(xhdt->context_array =
		      realloc(xhdt->context_array, domains * sizeof(struct xen_hyper_domain_context)))) {
			error(FATAL, "cannot realloc context array (%d domains).", domains);
		}
		xhdt->context_array_cnt = domains;
	}
	BZERO(xhdt->context_array, domains * sizeof(struct xen_hyper_domain_context));
}
...
void xen_hyper_refresh_vcpu_context_space(void)
{
	...
	for (i = 0, xht->vcpus = 0, dc = xhdt->context_array,
	     vcca = xhvct->vcpu_context_arrays; i < XEN_HYPER_NR_DOMAINS(); i++, dc++, vcca++) {
		dc->vcpu_context_array = vcca;
		xen_hyper_alloc_vcpu_context_space(vcca, XEN_HYPER_NR_VCPUS_IN_DOM(dc));
		for (j = 0, vcc = vcca->context_array; j < XEN_HYPER_NR_VCPUS_IN_DOM(dc); j++, vcc++) {
			xen_hyper_read_vcpu(dc->vcpu[j]);
			xen_hyper_store_vcpu_context(vcc, dc->vcpu[j], xhvct->vcpu_struct);
		}
...
struct xen_hyper_vcpu_context *xen_hyper_vcpu_to_vcpu_context(ulong vcpu)
{
	...
	for (i = 0, vcca = xhvct->vcpu_context_arrays; i < xhvct->vcpu_context_arrays_cnt; i++, vcca++) {
		for (j = 0, vcc = vcca->context_array; j < vcca->context_array_cnt; j++, vcc++) {
			if (vcpu == vcc->vcpu) {
				return vcc;
			}
		}
...
struct xen_hyper_vcpu_context *xen_hyper_id_to_vcpu_context(ulong domain, domid_t did, int vcid)
{
	...
	for (i = 0, vcc = vcca->context_array; i < vcca->context_array_cnt; i++, vcc++) {
		if (vcid == vcc->vcpu_id) {
			return vcc;
		}
	}
							    ...
							    *vcc, ulong vcpu, char *vcp)
{
	...
	vcc->state = INT(vcp + XEN_HYPER_OFFSET(vcpu_runstate) + XEN_HYPER_OFFSET(vcpu_runstate_info_state));
...
char *xen_hyper_fill_vcpu_struct(ulong vcpu, char *vcpu_struct)
{
	if (!readmem(vcpu, KVADDR, vcpu_struct,
		     XEN_HYPER_SIZE(vcpu), "fill_vcpu_struct", ACTIVE()? (RETURN_ON_ERROR | QUIET) : RETURN_ON_ERROR)) {
...
void xen_hyper_alloc_vcpu_context_arrays_space(int domains)
{
	...
	if (xhvct->vcpu_context_arrays == NULL) {
		if (!(xhvct->vcpu_context_arrays = malloc(domains * sizeof(struct xen_hyper_vcpu_context_array)))) {
			error(FATAL, "cannot malloc context arrays (%d domains).", domains);
		}
		...
		if (!(xhvct->vcpu_context_arrays =
		      realloc(xhvct->vcpu_context_arrays, domains * sizeof(struct xen_hyper_vcpu_context_array)))) {
			error(FATAL, "cannot realloc context arrays (%d domains).", domains);
		}
		vcca = xhvct->vcpu_context_arrays + domains;
		BZERO(vcca, (domains - xhvct->vcpu_context_arrays_cnt) * sizeof(struct xen_hyper_vcpu_context_array));
		xhvct->vcpu_context_arrays_cnt = domains;
	}
}
					...
					*vcca, int vcpus)
{
	if (!vcpus) {
		...
		if (!(vcca->context_array = malloc(vcpus * sizeof(struct xen_hyper_vcpu_context)))) {
			error(FATAL, "cannot malloc context array (%d vcpus).", vcpus);
		}
		vcca->context_array_cnt = vcpus;
	} else if (vcpus > vcca->context_array_cnt) {
		if (!(vcca->context_array =
		      realloc(vcca->context_array, vcpus * sizeof(struct xen_hyper_vcpu_context_array)))) {
			error(FATAL, "cannot realloc context array (%d vcpus).", vcpus);
		}
		vcca->context_array_cnt = vcpus;
	}
...
{
	pcc->pcpu = pcpu;
	pcc->processor_id = UINT(pcp + XEN_HYPER_OFFSET(cpu_info_processor_id));
	pcc->guest_cpu_user_regs = (ulong) (pcpu + XEN_HYPER_OFFSET(cpu_info_guest_cpu_user_regs));
	pcc->current_vcpu = ULONG(pcp + XEN_HYPER_OFFSET(cpu_info_current_vcpu));
	return pcc;
}
...
{
	if (xhpct->context_array == NULL) {
		if (!(xhpct->context_array = malloc(pcpus * sizeof(struct xen_hyper_pcpu_context)))) {
			error(FATAL, "cannot malloc context array (%d pcpus).", pcpus);
		}
	}
...
char *xen_hyper_x86_fill_cpu_data(int idx, char *cpuinfo_x86)
{
	...
	if (!readmem(cpu_data, KVADDR, cpuinfo_x86, XEN_HYPER_SIZE(cpuinfo_x86), "cpu_data", RETURN_ON_ERROR)) {
...
char *xen_hyper_ia64_fill_cpu_data(int idx, char *cpuinfo_ia64)
{
	...
	if (!readmem(cpu_data, KVADDR, cpuinfo_ia64, XEN_HYPER_SIZE(cpuinfo_ia64), "cpu_data", RETURN_ON_ERROR)) {
		error(WARNING, "cannot read cpu_data.\n");
		return NULL;
	}
	return cpuinfo_ia64;
...
void xen_hyper_get_cpu_info(void)
{
	...
	if (!readmem(addr, KVADDR, xht->cpumask, XEN_HYPER_SIZE(cpumask_t), "cpu_present_map", RETURN_ON_ERROR)) {
...
uint64_t xen_hyper_x86_memory_size(void)
{
	...
	if (!readmem(vaddr, KVADDR, &xht->total_pages, sizeof(xht->total_pages), "total_pages", RETURN_ON_ERROR)) {
...
ulong xen_hyper_ia64_processor_speed(void)
{
	...
	if (!xht->cpu_data_address ||
	    !XEN_HYPER_VALID_STRUCT(cpuinfo_ia64) || XEN_HYPER_INVALID_MEMBER(cpuinfo_ia64_proc_freq))
...
ulong xen_hyper_get_active_vcpu_from_pcpuid(ulong pcpuid)
{
	struct xen_hyper_pcpu_context *pcc;
	struct xen_hyper_vcpu_context_array *vcca;
	struct xen_hyper_vcpu_context *vcc;
	...
	for (i = 0, vcca = xhvct->vcpu_context_arrays; i < xhvct->vcpu_context_arrays_cnt; i++, vcca++) {
		for (j = 0, vcc = vcca->context_array; j < vcca->context_array_cnt; j++, vcc++) {
			if (vcc->processor == pcpuid && vcc->state == XEN_HYPER_RUNSTATE_running) {
				return vcc->vcpu;
			}
		}
	}
...
void xen_hyper_print_bt_header(FILE * out, ulong vcpu, int newline)
{
	struct xen_hyper_vcpu_context *vcc;
