...
static void store_load_module_symbols(bfd *, int, void *, long, uint, ulong, char *);
...
void symtab_init(void)
{
	...
	if (pc->flags & SYSMAP) {
		bfd_map_over_sections(st->bfd, section_header_info, VERIFY_SECTIONS);
		if ((st->flags & (NO_SEC_LOAD | NO_SEC_CONTENTS)) == (NO_SEC_LOAD | NO_SEC_CONTENTS)) {
			error(INFO, "%s: no text and data contents\n", pc->namelist);
			error(FATAL, pc->flags & SYSMAP_ARG ? DEBUGINFO_ERROR_MESSAGE1 : DEBUGINFO_ERROR_MESSAGE2);
		}
	 ...
	 */
	if (!(LKCD_KERNTYPES()) && !(bfd_get_file_flags(st->bfd) & HAS_SYMS)) {
 ...
 */
static int check_gnu_debuglink(bfd * bfd)
{
	int i, exists, found;
	asection *sect;
	...
	bfd_get_section_contents(bfd, sect, contents, (file_ptr) 0, (bfd_size_type) debuglink_size);
 ...
 */
static int separate_debug_file_exists(const char *name, unsigned long crc, int *exists)
{
...
#else
#ifdef GDB_7_6
		file_crc = bfd_calc_gnu_debuglink_crc32(file_crc, (unsigned char *)buffer, count);
#else
		file_crc = gnu_debuglink_crc32(file_crc, (unsigned char *)buffer, count);
#endif
#endif
 ...
 */
char *check_specified_kernel_debug_file()
{
 ...
 *             is killed.
 */
void no_debugging_data(int error_type)
{
	switch (error_type) {
	case INFO:
...
void get_text_init_space(void)
{
	...
	kt->etext_init = kt->stext_init + (ulong) bfd_section_size(st->bfd, section);
 ...
 */
static char *strip_symbol_end(const char *name, char *buf)
{
	char *p;
 ...
 */
static void store_symbols(bfd * abfd, int dynamic, void *minisyms, long symcount, unsigned int size)
{
	asymbol *store;
	asymbol *sym;
	...
	if (!namespace_ctl(NAMESPACE_INIT, &st->kernel_namespace, (void *)symcount, NULL))
	...
	for (; from < fromend; from += size) {
		...
		if (machdep->verify_symbol(name, syminfo.value, syminfo.type)) {
			if (kt->flags & (RELOC_SET | RELOC_FORCE))
				sp->value = relocate(syminfo.value, (char *)syminfo.name, !(first++));
			else
				sp->value = syminfo.value;
			sp->type = syminfo.type;
			namespace_ctl(NAMESPACE_INSTALL, &st->kernel_namespace, sp, name);
			sp++;
			st->symcnt++;
		}
	}
	...
	namespace_ctl(NAMESPACE_COMPLETE, &st->kernel_namespace, st->symtable, st->symend);
}
...
static void store_sysmap_symbols(void)
{
	...
	while (fgets(buf, BUFSIZE, map)) {
		...
		if (machdep->verify_symbol(name, syment.value, syment.type)) {
			if (kt->flags & RELOC_SET)
				sp->value = relocate(syment.value, syment.name, !(first++));
			else
 ...
 */
static ulong relocate(ulong symval, char *symname, int first_symbol)
{
	if (XEN_HYPER_MODE()) {
 ...
 */
static int relocate_force(ulong symval, char *symname)
{
	int count, found;
	FILE *kp;
	char buf[BUFSIZE];
	...
	if (!ACTIVE() || !file_exists("/proc/kallsyms", NULL)) {
		if (CRASHDEBUG(1))
			fprintf(fp,
				...
				!ACTIVE()? "not a live system" : "/proc/kallsyms does not exist");
		return FALSE;
	}

	if ((kp = fopen("/proc/kallsyms", "r")) == NULL) {
		if (CRASHDEBUG(1))
			fprintf(fp, "cannot open /proc/kallsyms to determine relocation\n");
		return FALSE;
	}
	...
	while (!found && fgets(buf, BUFSIZE, kp) && (parse_line(buf, kallsyms) == 3)
		...
		if (CRASHDEBUG(1))
			fprintf(fp,
				"  %s @ %s %s\n",
				kallsyms[2], kallsyms[0], STREQ(kallsyms[2], symname) ? "(match!)" : "");
	}
	fclose(kp);
	...
	if (CRASHDEBUG(1))
		fprintf(fp, "cannot determine relocation value from" " %d symbols in /proc/kallsyms\n", count);
 ...
 */
static struct syment *symval_hash_search(ulong value)
{
	int index;
 ...
 *  Static kernel symbol value search
 */
static struct syment *symname_hash_search(char *name)
{
	struct syment *sp;
...
static void symbol_dump(ulong flags, char *module)
{
...
static void dump_percpu_symbols(struct load_module *lm)
{
	struct syment *sp, *sp_end;
 ...
 */
static asection *get_kernel_section(char *name)
{
	int i;
	asection **sec;
 ...
 */
static void check_for_dups(struct load_module *lm)
{
	struct syment *sp, *sp_end;
...
void store_module_symbols_v1(ulong total, int mods_installed)
{
	int i, m;
	...
	if (!namespace_ctl(NAMESPACE_INIT, &st->ext_module_namespace, (void *)total, NULL))
		error(FATAL, "module namespace malloc: %s\n", strerror(errno));

	if ((st->load_modules = (struct load_module *)calloc(st->mods_installed, sizeof(struct load_module))) == NULL)
	...
	for (mod = kt->module_list; mod != kt->kernel_module; mod = mod_next) {
		...
		readmem(mod, KVADDR, modbuf, SIZE(module), "module buffer", FAULT_ON_ERROR);
		...
		size_of_struct = ULONG(modbuf + OFFSET(module_size_of_struct));
		...
		namespace_ctl(NAMESPACE_INSTALL, &st->ext_module_namespace, &st->ext_module_symtable[mcnt], buf2);
		lm_mcnt = mcnt;
		mcnt++;
		...
		if (nsyms) {
			...
			readmem((ulong) syms, KVADDR, modsymbuf,
				nsyms * sizeof(struct module_symbol), "module symbols", FAULT_ON_ERROR);
		}
		...
		if (last > first) {
			...
			if ((first + strbuflen) >= (lm->mod_base + lm->mod_size)) {
				strbuflen = (lm->mod_base + lm->mod_size) - first;

			}
			strbuf = GETBUF(strbuflen);

			if (!readmem(first, KVADDR, strbuf, strbuflen, "module symbol strings", RETURN_ON_ERROR)) {
				FREEBUF(strbuf);
				strbuf = NULL;
			}
		} else
		...
		for (i = first = last = 0; i < nsyms; i++) {
			...
			if (strbuf)
				strcpy(buf1, &strbuf[(ulong) modsym->name - first]);
			else
				read_string((ulong) modsym->name, buf1, BUFSIZE - 1);

			if (strlen(buf1)) {
				st->ext_module_symtable[mcnt].value = modsym->value;
				st->ext_module_symtable[mcnt].type = '?';
				st->ext_module_symtable[mcnt].flags |= MODULE_SYMBOL;
				strip_module_symbol_end(buf1);
				strip_symbol_end(buf1, NULL);
				namespace_ctl(NAMESPACE_INSTALL,
					      &st->ext_module_namespace, &st->ext_module_symtable[mcnt], buf1);
	...
	namespace_ctl(NAMESPACE_COMPLETE, &st->ext_module_namespace, st->ext_module_symtable, st->ext_module_symend);

	qsort(st->ext_module_symtable, mcnt, sizeof(struct syment), compare_syms);
...
void store_module_symbols_v2(ulong total, int mods_installed)
{
	...
	for (mod = kt->module_list; mod != kt->kernel_module; mod = mod_next) {
		...
		if (THIS_KERNEL_VERSION >= LINUX(2, 6, 27)) {
			lm->mod_etext_guess = lm->mod_base + UINT(modbuf + OFFSET(module_core_text_size));
			lm->mod_init_size = UINT(modbuf + OFFSET(module_init_size));
			lm->mod_init_text_size = UINT(modbuf + OFFSET(module_init_text_size));
		} else {
			lm->mod_etext_guess = lm->mod_base + ULONG(modbuf + OFFSET(module_core_text_size));
			lm->mod_init_size = ULONG(modbuf + OFFSET(module_init_size));
			lm->mod_init_text_size = ULONG(modbuf + OFFSET(module_init_text_size));
		}
		...
		if (lm->mod_init_size > 0) {
			...
			namespace_ctl(NAMESPACE_INSTALL,
				      &st->ext_module_namespace, &st->ext_module_symtable[mcnt], buf3);
			lm_mcnt = mcnt;
			mcnt++;
		...
		if (ngplsyms) {
			modsymbuf = GETBUF(sizeof(struct kernel_symbol) * ngplsyms);
		...
		if (last > first) {
			...
			if (!readmem(first, KVADDR, strbuf, strbuflen, "module gpl symbol strings", RETURN_ON_ERROR)) {
				FREEBUF(strbuf);
				strbuf = NULL;
			}
		} else
		...
		if (lm->mod_init_size > 0) {
			...
			namespace_ctl(NAMESPACE_INSTALL,
				      &st->ext_module_namespace, &st->ext_module_symtable[mcnt], buf4);
			mcnt++;
		}
 ...
 */
static int store_module_kallsyms_v1(struct load_module *lm, int start, int curr, char *modbuf)
{
	int i, j;
	struct syment *sp;
	...
	if (!readmem(lm->mod_base, KVADDR, module_buf, lm->mod_size, "module (kallsyms)", RETURN_ON_ERROR | QUIET)) {
...
//      sections = UINT(header_buf + OFFSET(kallsyms_header_sections));
	...
	symptr = (ulong) (header_buf + ULONG(header_buf + OFFSET(kallsyms_header_symbol_off)));
	stringptr = (ulong) (header_buf + ULONG(header_buf + OFFSET(kallsyms_header_string_off)));
	sectionptr = (ulong) (header_buf + ULONG(header_buf + OFFSET(kallsyms_header_section_off)));
	...
	for (i = 0; i < symbols; i++, symptr += SIZE(kallsyms_symbol)) {
		...
		if (!IN_MODULE_BUF_V1(nameptr)) {
			if (CRASHDEBUG(7))
				error(INFO,
				      ...
				      lm->mod_name, nameptr, stringptr, name_off);
			continue;
		}
		if (!IN_MODULE_BUF_V1(secnameptr)) {
			if (CRASHDEBUG(7))
				error(INFO,
				      ...
				      lm->mod_name, secnameptr, stringptr, sec_name_off);
			continue;
		}
		...
		if (!STREQ(nameptr, secnameptr)) {
			...
			for (found = 0, j = start; j < curr; j++) {
				...
				if ((sp->value == symbol_addr) && STREQ(nameptr, &ns->address[(ulong) sp->name])) {
					if (CRASHDEBUG(7))
						fprintf(fp,
							"current symbol \"%s\" at %lx of type (%c)\n",
							&ns->address[(ulong) sp->name], sp->value, sp->type);
					if (sp->type == '?')
						sp->type = type;
					found++;
					break;
			...
			namespace_ctl(NAMESPACE_INSTALL,
				      &st->ext_module_namespace, &st->ext_module_symtable[mcnt_idx++], nameptr);
			mcnt++;
		}
	}
...
static int store_module_kallsyms_v2(struct load_module *lm, int start, int curr, char *modbuf)
{
	int i, j, found;
	...
	if (lm->mod_init_size > 0) {
		...
		if (!readmem
		    (lm->mod_init_module_ptr, KVADDR, module_buf_init,
		     lm->mod_init_size, "module init (kallsyms)", RETURN_ON_ERROR | QUIET)) {
	...
	if (!IN_MODULE(ksymtab, lm) && !IN_MODULE_INIT(ksymtab, lm)) {
		error(WARNING, "%s: module.symtab outside of module address space\n", lm->mod_name);
		FREEBUF(module_buf);
		if (module_buf_init)
			FREEBUF(module_buf_init);
		return 0;
	...
	if (!IN_MODULE(kstrtab, lm) && !IN_MODULE_INIT(kstrtab, lm)) {
		error(WARNING, "%s: module.strtab outside of module address space\n", lm->mod_name);
		FREEBUF(module_buf);
		if (module_buf_init)
			FREEBUF(module_buf_init);
		return 0;
	...
	for (i = 1; i < nksyms; i++) {	/* ELF starts real symbols at 1 */
		...
		if (!IN_MODULE(kstrtab + ec->st_name, lm)
			...
			if (CRASHDEBUG(3)) {
				error(WARNING,
				      ...
				      lm->mod_name,
				      ec->st_name, (kstrtab + ec->st_name), ec->st_value, ec->st_shndx, ec->st_info);
			}
			continue;
		}
		 ...
		 */
		if (machine_type("ARM") && !machdep->verify_symbol(nameptr, ec->st_value, ec->st_info))
			continue;

		if (CRASHDEBUG(7))
			fprintf(fp,
				"%s: st_name: %ld st_value: %lx st_shndx: %ld st_info: %c\n",
				nameptr, ec->st_name, ec->st_value, ec->st_shndx, ec->st_info);

		strip_symbol_end(nameptr, NULL);

		for (found = 0, j = start; j < curr; j++) {
			sp = &st->ext_module_symtable[j];
			if ((sp->value == ec->st_value) && STREQ(nameptr, &ns->address[(ulong) sp->name])) {
				if (CRASHDEBUG(7))
					fprintf(fp,
						"current symbol \"%s\" at %lx of type (%c)\n",
						&ns->address[(ulong) sp->name], sp->value, sp->type);
 ...
 */
static void strip_module_symbol_end(char *buf)
{
	char *p1, *lastR;
...
char *load_module_filter(char *s, int type)
{
	...
	case LM_P_FILTER:
		...
		clen = smp ? SMP_CLUTTER + CLUTTER_IDLEN : UP_CLUTTER + CLUTTER_IDLEN;
	...
	case LM_DIS_FILTER:
		...
		if (hexadecimal(arglist[0], VADDR_PRLEN + 2) &&
		    bracketed(arglist[1], &arglist[1][1], 0) && (colon = strstr(s, ":"))) {
			strcpy(buf2, colon + 2);

			vaddr = htol(arglist[0], FAULT_ON_ERROR, NULL);
			if ((sp = value_search(vaddr, &offset))) {
				if (offset)
					sprintf(s, "%s <%s+%ld>:\t%s", arglist[0], sp->name, offset, buf2);
				else
					sprintf(s, "%s <%s>:\t%s", arglist[0], sp->name, buf2);
			}
		}
		...
		if (bracketed(arglist[last], &arglist[last][1], 0) && hexadecimal(arglist[prev], VADDR_PRLEN + 2)) {
...
static int namespace_ctl(int cmd, struct symbol_namespace *ns, void *nsarg1, void *nsarg2)
{
	char *addr;
	struct syment *sp, *sp_end;
	char *name;
	...
	case NAMESPACE_FREE:
		if (!ns->address)
			error(FATAL, "attempt to free unallocated module namespace\n");
		free(ns->address);
		ns->address = 0;
		ns->index = 0;
		ns->size = 0;
...
static int compare_syms(const void *v1, const void *v2)
{
	struct syment *s1, *s2;
...
static int compare_mods(const void *v1, const void *v2)
{
	...
	return (lm1->mod_base < lm2->mod_base ? -1 : lm1->mod_base == lm2->mod_base ? 0 : 1);
}
 ...
 */
int is_kernel_text(ulong value)
{
	int i, s;
	...
	if (pc->flags & SYSMAP) {
		if ((sp = value_search(value, NULL)) && ((sp->type == 'T') || (sp->type == 't')))
			return TRUE;
 ...
 */
int is_kernel_text_offset(ulong value)
{
	struct syment *sp;
	ulong offset;
 ...
 */
int is_kernel_data(ulong value)
{
	return (IS_KVADDR(value) && !is_kernel_text(value) && !IS_MODULE_VADDR(value));
}

/*
 *  Check whether the closest symbol to a value is rodata.
 */
int is_rodata(ulong value, struct syment **spp)
{
	struct syment *sp;
...
void dump_symbol_table(void)
{
	...
	fprintf(fp, "               flags: %lx%s(", st->flags, count_bits_long(st->flags) > 3 ? "\n   " : "  ");
	others = 0;
	...
	if (st->flags & LOAD_MODULE_SYMS)
		fprintf(fp, "%sLOAD_MODULE_SYMS", others++ ? "|" : "");
	...
	if (st->first_ksymbol) {
		fprintf(fp, "%lx (%s)\n",
			st->first_ksymbol, st->flags & KERNEL_SYMS ? value_symbol(st->first_ksymbol) : "");
	} else
		fprintf(fp, "(unused)\n");
	...
	fprintf(fp, "    symval_hash[%d]: %lx\n", SYMVAL_HASH, (ulong) & st->symval_hash[0]);
	...
	fprintf(fp, " val_hash_iterations: %.0f  (avg: %.1f)\n",
		st->val_hash_iterations, st->val_hash_iterations / st->val_hash_searches);

	fprintf(fp, "   symname_hash[%d]: %lx\n", SYMNAME_HASH, (ulong) & st->symname_hash[0]);
	...
	fprintf(fp, "address: %lx  ", (ulong) st->ext_module_namespace.address);
	fprintf(fp, "index: %ld  ", st->ext_module_namespace.index);
	fprintf(fp, "size: %ld  ", (ulong) st->ext_module_namespace.size);
	...
	for (i = 0; i < st->mods_installed; i++) {
		...
		fprintf(fp, "address: %lx  ", (ulong) lm->mod_load_namespace.address);
		fprintf(fp, "index: %ld  ", lm->mod_load_namespace.index);
		fprintf(fp, "size: %ld  ", (ulong) lm->mod_load_namespace.size);
		...
		fprintf(fp, "        mod_text_start: %lx (%lx)\n",
			lm->mod_text_start, lm->mod_text_start ? lm->mod_text_start - lm->mod_base : 0);
		fprintf(fp, "       mod_etext_guess: %lx (%lx)\n",
			lm->mod_etext_guess, lm->mod_etext_guess ? lm->mod_etext_guess - lm->mod_base : 0);
		fprintf(fp, "      mod_rodata_start: %lx (%lx)\n",
			lm->mod_rodata_start, lm->mod_rodata_start ? lm->mod_rodata_start - lm->mod_base : 0);
		fprintf(fp, "        mod_data_start: %lx (%lx)\n",
			lm->mod_data_start, lm->mod_data_start ? lm->mod_data_start - lm->mod_base : 0);
		fprintf(fp, "         mod_bss_start: %lx (%lx)\n",
			lm->mod_bss_start, lm->mod_bss_start ? lm->mod_bss_start - lm->mod_base : 0);
		...
		if (lm->mod_percpu_size) {
			...
			fprintf(fp, "            mod_percpu: %lx - %lx\n",
				lm->mod_percpu, lm->mod_percpu + lm->mod_percpu_size);
		} else {
			if (lm->mod_percpu) {
				fprintf(fp, "       mod_percpu_size: (not loaded)\n");
				fprintf(fp, "            mod_percpu: %lx - (unknown)\n", lm->mod_percpu);
			} else {
				fprintf(fp, "       mod_percpu_size: (not used)\n");
		...
		fprintf(fp, "      mod_section_data: %lx %s\n",
			(ulong) lm->mod_section_data, lm->mod_section_data ? "" : "(not allocated)");
		...
		for (s = 0; s < lm->mod_sections; s++) {
			fprintf(fp,
				...
				lm->mod_section_data[s].offset, lm->mod_section_data[s].size);
		}
	...
	for (i = 0; sec && (i < st->bfd->section_count); i++, sec++) {
		...
		fprintf(fp, "%25s  vma: %.*lx  size: %ld\n",
			...
			(ulong) bfd_get_section_vma(st->bfd, section), (ulong) bfd_section_size(st->bfd, section));
	}
}
...
int is_kernel(char *file)
{
	...
	swap = (((eheader[EI_DATA] == ELFDATA2LSB) &&
		...
		((eheader[EI_DATA] == ELFDATA2MSB) && (__BYTE_ORDER == __LITTLE_ENDIAN)));

	if ((elf32->e_ident[EI_CLASS] == ELFCLASS32) &&
	    (swap16(elf32->e_type, swap) == ET_EXEC) && (swap32(elf32->e_version, swap) == EV_CURRENT)) {
		...
		if (endian_mismatch(file, elf32->e_ident[EI_DATA], 0))
		   ...
		   ((swap16(elf64->e_type, swap) == ET_EXEC) ||
		    (swap16(elf64->e_type, swap) == ET_DYN)) && (swap32(elf64->e_version, swap) == EV_CURRENT)) {
...
int is_compressed_kernel(char *file, char **tmp)
{
	int len, type, fd;
...
#define FNAME (1 << 3)
	...
	if ((header[0] == 0x1f) && (header[1] == 0x8b) && (header[2] == 8)) {
		if (!(header[3] & FNAME)) {
			if (!(st->flags & FORCE_DEBUGINFO)) {
				error(INFO, "%s: " "original filename unknown\n", file);
				error(CONT, "Use \"-f %s\" on command line to prevent this message.\n\n", file);
			}
		} else if (!STRNEQ((char *)&header[10], "vmlinux") && !(st->flags & FORCE_DEBUGINFO)) {
			error(INFO, "%s: compressed file name does not " "start with \"vmlinux\"\n", &header[10]);
			error(CONT, "Use \"-f %s\" on command line to override.\n\n", file);
			return FALSE;
		}
		type = GZIP;
	}
	...
	if ((header[0] == 'B') && (header[1] == 'Z') && (header[2] == 'h')) {
		if (!STRNEQ(basename(file), "vmlinux") && !(st->flags & FORCE_DEBUGINFO)) {
			error(INFO, "%s: compressed file name does not start " "with \"vmlinux\"\n", file);
			error(CONT, "Use \"-f %s\" on command line to override.\n\n", file);
			return FALSE;
		}
	...
	if (!(tmpdir = getenv("TMPDIR")))
		tmpdir = "/var/tmp";
	len = strlen(tmpdir) + strlen(basename(file)) + strlen("_XXXXXX") + 2;
	...
	case GZIP:
		sprintf(command, "%s -c %s > %s",
			file_exists("/bin/gunzip", NULL) ? "/bin/gunzip" : "/usr/bin/gunzip", file, tempname);
		break;
	case BZIP2:
		sprintf(command, "%s -c %s > %s",
			file_exists("/bin/bunzip2", NULL) ? "/bin/bunzip2" : "/usr/bin/bunzip2", file, tempname);
		break;
	}
	if (system(command) < 0) {
		please_wait_done();
...
int is_shared_object(char *file)
{
	int fd, swap;
	char eheader[BUFSIZE];
	Elf32_Ehdr *elf32;
	...
	if ((elf32->e_ident[EI_CLASS] == ELFCLASS32) && (swap16(elf32->e_type, swap) == ET_DYN)) {
		switch (swap16(elf32->e_machine, swap)) {
		case EM_386:
	...
	} else if ((elf64->e_ident[EI_CLASS] == ELFCLASS64) && (swap16(elf64->e_type, swap) == ET_DYN)) {
		switch (swap16(elf64->e_machine, swap)) {
		case EM_IA_64:
...
int select_namelist(char *new)
{
	...
	if (!file_exists(pc->namelist, &stat1) || !file_exists(new, &stat2)) {
		return FALSE;
	}
 ...
 */
static int is_bfd_format(char *filename)
{
#ifdef GDB_5_3
	struct _bfd *bfd;
#else
...
static int is_binary_stripped(char *filename)
{
#ifdef GDB_5_3
	struct _bfd *bfd;
#else
...
void cmd_sym(void)
{
	...
	while ((c = getopt(argcnt, args, "lLQ:q:npsMm:")) != EOF) {
		...
		case 'Q':
			fprintf(fp, "%d found ", symbol_query(optarg, NULL, &sp));
			if (sp)
	...
	if (args[optind]) {
		do {
			...
			if (clean_arg() && hexadecimal(args[optind], 0)) {
				errflag = 0;
				value = htol(args[optind], RETURN_ON_ERROR, &errflag);
				if (errflag || !in_ksymbol_range(value)) {
					...
					if (module_symbol(sp->value, NULL, NULL, NULL, 0))
						show_flags |= SHOW_MODULE;
					if (prev && (spp = prev_symbol(NULL, sp)))
						show_symbol(spp, 0, show_flags);

					show_symbol(sp, offset, show_flags);
				} else if (module_symbol(value, &sp, NULL, buf, *gdb_output_radix)) {
					name = buf;

					if (prev && sp && (spp = prev_symbol(NULL, sp)))
						show_symbol(spp, 0, show_flags);
 ...
 */
void show_symbol(struct syment *sp, ulong offset, ulong show_flags)
{
	char buf[BUFSIZE];
	char *p1;
	ulong radix;
	...
	if (MODULE_START(sp)) {
		...
		if (offset)
			fprintf(fp, (radix == 16) ? "+0x%lx" : "+%ld", offset);
		fprintf(fp, "\n");
		return;
	...
	if (offset)
		fprintf(fp, (radix == 16) ?
			"%lx (%c) %s+0x%lx" : "%lx (%c) %s+%ld", sp->value + offset, sp->type, sp->name, offset);
	else
	...
	if (is_kernel_text(sp->value + offset) && (show_flags & SHOW_LINENUM))
		fprintf(fp, " %s", get_line_number(sp->value + offset, buf, TRUE));
 ...
 */
char *get_line_number(ulong addr, char *buf, int reserved)
{
	char *p;
	...
	if ((lnh = machdep->line_number_hooks)) {
		...
		while (lnh->func) {
			if (STREQ(name, lnh->func)) {
				sprintf(buf, "%s/%s", get_build_directory(bldbuf) ? bldbuf : "..", *(lnh->file));
				break;
			}
			lnh++;
		}
	...
	if (module_symbol(vaddr, NULL, &lm, NULL, *gdb_output_radix)) {
		if (lm->mod_flags & MOD_LOAD_SYMS) {
			for (i = (lm->mod_sections - 1); i >= 0; i--) {
				start = lm->mod_base + lm->mod_section_data[i].offset;
				end = start + lm->mod_section_data[i].size;

				if ((vaddr >= start) && (vaddr < end)) {
					strcpy(buf, lm->mod_section_data[i].name);
					break;
				}
			}
		} else
		...
		for (i = 0; i < st->bfd->section_count; i++, sec++) {
			...
			if ((vaddr >= start) && (vaddr < end)) {
				strcpy(buf, bfd_get_section_name(st->bfd, section));
				break;
			}
		}
	}
 ...
 */
char *get_build_directory(char *buf)
{
	char *p;
 ...
 */
int symbol_query(char *s, char *print_pad, struct syment **spp)
{
	int i;
	struct syment *sp, *sp_end;
	struct load_module *lm;
 ...
 */
static int symbol_name_count(char *s)
{
	int i;
	struct syment *sp, *sp_end;
	struct load_module *lm;
 ...
 */
struct syment *symbol_search_next(char *s, struct syment *spstart)
{
	int i;
	struct syment *sp, *sp_end;
	struct load_module *lm;
 ...
 *  address space.
 */
int in_ksymbol_range(ulong value)
{
	if ((value >= st->symtable[0].value) && (value <= st->symtable[st->symcnt - 1].value)) {
 ...
 */
int module_symbol(ulong value, struct syment **spp, struct load_module **lmp, char *name, ulong radix)
{
	int i;
	struct load_module *lm;
	struct syment *sp;
	...
	for (i = 0; i < st->mods_installed; i++) {
		...
		if ((value >= base) && (value < end)) {
			...
			if (name) {
				...
				if ((sp = value_search(value, &offset))) {
					if (offset)
						sprintf(buf, radix == 16 ? "%s+0x%lx" : "%s+%ld", sp->name, offset);
					else
				...
				if (offs) {
					sprintf(buf, radix == 16 ? "+0x%lx" : "+%ld", offs);
					strcat(name, buf);
				}
			}
			return TRUE;
...
struct syment *value_search_module(ulong value, ulong * offset)
{
	int i;
 ...
 */
struct syment *value_search(ulong value, ulong * offset)
{
 ...
 */
struct syment *value_search_base_kernel(ulong value, ulong * offset)
{
	struct syment *sp;
 ...
 */
struct syment *generic_machdep_value_to_symbol(ulong value, ulong * offset)
{
	return NULL;
}
 ...
 */
char *value_to_symstr(ulong value, char *buf, ulong radix)
{
	struct syment *sp;
	ulong offset;
	...
	if (module_symbol(value, NULL, NULL, locbuf, *gdb_output_radix)) {
		if (sp) {
			if (STRNEQ(locbuf, "_MODULE_START_"))
				shift_string_left(locbuf, strlen("_MODULE_START_"));
			if ((p1 = strstr(locbuf, "+")))
				*p1 = NULLCHAR;

			if (offset) {
				if (is_module_name(locbuf, NULL, &lm) && (value < lm->mod_text_start))
					sprintf(buf, radix == 16 ?
						"(%s module)+0x%lx" : "(%s module)+%ld", locbuf, offset);
				else
					sprintf(buf, radix == 16 ? "%s+0x%lx" : "%s+%ld", locbuf, offset);
			} else {
				if (is_module_name(locbuf, NULL, &lm) && (value < lm->mod_text_start))
 ...
 */
char *closest_symbol(ulong value)
{
	struct syment *sp;
 ...
 */
ulong closest_symbol_value(ulong value)
{
	struct syment *sp;
 ...
 */
struct syment *next_symbol(char *symbol, struct syment *sp_in)
{
	int i;
 ...
 * Either a symbol name or syment pointer may be passed as an argument.
 */
struct syment *prev_symbol(char *symbol, struct syment *sp_in)
{
	int i, search_init;
...
void get_symbol_data(char *symbol, long size, void *local)
{
	...
	if ((sp = symbol_search(symbol)))
		readmem(sp->value, KVADDR, local, size, symbol, FAULT_ON_ERROR);
	else
 ...
 */
int try_get_symbol_data(char *symbol, long size, void *local)
{
	struct syment *sp;

	if ((sp = symbol_search(symbol)) && readmem(sp->value, KVADDR, local, size, symbol, RETURN_ON_ERROR | QUIET))
		return TRUE;
 ...
 */
ulong symbol_value_module(char *symbol, char *module)
{
	int i;
	struct syment *sp, *sp_end;
	struct load_module *lm;
 ...
 *  type.
 */
struct syment *per_cpu_symbol_search(char *symbol)
{
	struct syment *sp;
	char old[BUFSIZE];
	char *new;
 ...
 */
struct syment *kernel_symbol_search(char *symbol)
{
 ...
 */
int get_syment_array(char *symbol, struct syment **sp_array, int max)
{
	int i, cnt;
	struct syment *sp, *sp_end;
	struct load_module *lm;
	...
	for (sp = st->symtable; sp < st->symend; sp++) {
		if ((*symbol == *(sp->name)) && STREQ(symbol, sp->name)) {
			...
			if (max) {
				if (cnt == max) {
					error(INFO, "symbol count overflow (%s)\n", symbol);
					return cnt;
				} else
					sp_array[cnt] = sp;
			}
...
long datatype_info(char *name, char *member, struct datatype_member *dm)
{
	...
	if (CRASHDEBUG(2)) {
		if (req->typecode) {
			...
			console("typecode: %d%s ", req->typecode, req->is_typedef ? " (TYPEDEF)" : "");
	...
	if (dm && (dm != MEMBER_SIZE_REQUEST) && (dm != MEMBER_TYPE_REQUEST) && (dm != STRUCT_SIZE_REQUEST)) {
		dm->type = type_found;
		dm->size = size;
	...
	if (dm == MEMBER_SIZE_REQUEST)
		...
		if ((req->typecode == TYPE_CODE_STRUCT) || (req->typecode == TYPE_CODE_UNION) || req->is_typedef)
			return size;
		else
			return -1;
	} else if (member) {
		if ((req->typecode == TYPE_CODE_STRUCT) || (req->typecode == TYPE_CODE_UNION))
			return offset;
		else
			return -1;
	} else
 ...
 */
static long anon_member_offset(char *name, char *member)
{
	char buf[BUFSIZE];
	ulong value;
	int type;
 ...
 *  as well.
 */
int get_symbol_type(char *name, char *member, struct gnu_request *caller_req)
{
	struct gnu_request *req;
	int typecode;
...
int get_symbol_length(char *symbol)
{
	struct gnu_request *req;
	int len;
 ...
 */
void set_temporary_radix(unsigned int radix, unsigned int *restore_radix)
{
 ...
 *  by the caller.
 */
void restore_current_radix(unsigned int restore_radix)
{
 ...
 */
void dump_struct(char *s, ulong addr, unsigned radix)
{
 ...
 */
void dump_struct_member(char *s, ulong addr, unsigned radix)
{
 ...
 */
void dump_union(char *s, ulong addr, unsigned radix)
{
	unsigned restore_radix;
	long len;
...
static void print_struct_with_dereference(ulong addr, struct datatype_member *dm, ulong flags)
{
	int indent;
	char *p1;
	char buf1[BUFSIZE];
	...
	while (fgets(buf1, BUFSIZE, pc->tmpfile)) {
		...
		*p1 = NULLCHAR;
		...
		if ((arg_to_datatype(buf2, dm1, RETURN_ON_ERROR) == 2) && dereference_pointer(addr, dm1, flags))
			continue;
...
static int dereference_pointer(ulong addr, struct datatype_member *dm, ulong flags)
{
	char buf1[BUFSIZE];
	char buf2[BUFSIZE];
	...
	console("%s.%s typeptr: %s ", dm->name, dm->member, typeptr);
	if (charptr)
		console("[char *]");
	else if (voidptr)
		console("[void *]");
	...
	if (!readmem(addr + dm->member_offset, KVADDR,
		     &target, sizeof(void *), "target address", RETURN_ON_ERROR | QUIET)) {
		error(INFO, "cannot access %s.%s %lx\n", dm->name, dm->member, addr + dm->member_offset);
		return FALSE;
	}
	...
	if (charptr) {
		fprintf(pc->saved_fp, "  %s%s = 0x%lx\n  -> ", typeptr, dm->member, target);
		if (sym)
			fprintf(pc->saved_fp, "<%s> ", sym);
		if (!target)
			fprintf(pc->saved_fp, "NULL\n");
		else if (!accessible(target)
			 || !read_string(target, buf1, BUFSIZE - 1))
	...
	if (ptrptr) {
		...
		if (sym)
			fprintf(pc->saved_fp, "<%s> ", sym);
		if (!target ||
		    !readmem(target, KVADDR, &value, sizeof(void *), "target value", RETURN_ON_ERROR | QUIET))
			fprintf(pc->saved_fp, "\n");
		else
	...
	if (!datatype_exists(typeptr)) {
		fprintf(pc->saved_fp,
			"  %s %s%s = 0x%lx\n  -> (%s: no debuginfo data)\n",
			typeptr, typedef_is_ptr ? "" : "*", dm->member, target, typeptr);
		return TRUE;
	}
...
static void cmd_datatype_common(ulong flags)
{
	int i, c;
	ulong addr, aflag;
	struct syment *sp;
	...
	while ((c = getopt(argcnt, args, "pxdhfuc:rvol:")) != EOF) {
		...
		case 'l':
			if (IS_A_NUMBER(optarg))
				list_head_offset = stol(optarg, FAULT_ON_ERROR, NULL);
			else if (arg_to_datatype(optarg, dm, RETURN_ON_ERROR) > 1)
	...
	if ((count_chars(args[optind], '.') > 1) || (LASTCHAR(args[optind]) == ',') || (LASTCHAR(args[optind]) == '.'))
		...
		if (clean_arg() && IS_A_NUMBER(args[optind])) {
			if (aflag)
				...
				if (pc->curcmd_flags & MEMTYPE_FILEADDR)
					pc->curcmd_private = stoll(args[optind], FAULT_ON_ERROR, NULL);
				else if (pc->curcmd_flags & MEMTYPE_UVADDR) {
					addr = htol(args[optind], FAULT_ON_ERROR, NULL);
				} else if (!IS_KVADDR(addr = htol(args[optind], FAULT_ON_ERROR, NULL)))
	...
	if ((arg_to_datatype(structname, dm, DATATYPE_QUERY | ANON_MEMBER_QUERY | RETURN_ON_ERROR)
	     < 1))
	...
	if ((argc_members > 1) && aflag && (flags & SHOW_OFFSET))
		error(FATAL, "-o option not valid with multiple member format\n");
	...
	for (c = 0; c < abs(count); c++, addr += len, pc->curcmd_private += len) {
		...
		do {
			...
			switch (arg_to_datatype(structname, dm, ANON_MEMBER_QUERY | RETURN_ON_ERROR)) {
			case 0:
				error(FATAL, "invalid data structure reference: %s\n", structname);
				break;
			case 1:
			...
			if (rawdata)
				...
				if (dm->member) {
					if (!(pflag && dereference_pointer(addr, dm, flags)))
						parse_for_member(dm, PARSE_FOR_DATA);
					close_tmpfile();
				}
 ...
 */
static void do_datatype_declaration(struct datatype_member *dm, ulong flags)
{
	long len;
	char buf[BUFSIZE];
 ...
 */
int arg_to_datatype(char *s, struct datatype_member *dm, ulong flags)
{
	char *p1;
	int both;
	...
	if ((dm->size = DATATYPE_SIZE(dm)) < 0) {
		...
		error(FATAL, "cannot handle \"%s\": try \"gdb whatis\" or \"gdb ptype\"\n", s);
	}
	...
	if ((flags & ANON_MEMBER_QUERY) && ((dm->member_offset = ANON_MEMBER_OFFSET(dm->name, dm->member)) >= 0))
		return 2;
		...
		*p1 = '.';
		if (strstr(p1 + 1, "."))
			error(FATAL, "only one %s member allowed: %s\n",
			      ...
			      ((dm->type == UNION_REQUEST) ? "union" : "struct/union"), s);
	}
 ...
 */
static void dump_datatype_member(FILE * ofp, struct datatype_member *dm)
{
	int others;
...
void cmd_whatis(void)
{
	...
	if (STREQ(args[optind], "struct") || STREQ(args[optind], "union") || STREQ(args[optind], "enum"))
		optind++;
	else if ((sp = symbol_search(args[optind]))) {
		whatis_variable(sp);
		return;
		...
		if (dm->type == ENUM) {
			if (dm->tagname)
				fprintf(fp, "%senum%s%s = %ld\n",
					...
					strlen(dm->tagname) ? " " : "", dm->tagname, dm->value);
			else
 ...
 */
int dump_enumerator_list(char *e)
{
	struct gnu_request *req;
	struct datatype_member datatype_member, *dm;
	dm = &datatype_member;

	if (!arg_to_datatype(e, dm, RETURN_ON_ERROR) || (dm->size < 0) || (dm->type != ENUM) || dm->tagname)
		return FALSE;
...
int enumerator_value(char *e, long *value)
{
	...
	if (arg_to_datatype(e, dm, RETURN_ON_ERROR)) {
		if ((dm->size >= 0) && (dm->type == ENUM) && dm->tagname) {
			*value = dm->value;
			return TRUE;
		}
	}
...
void cmd_p(void)
{
	...
	if ((sp = symbol_search(args[optind])) && !args[optind + 1]) {
		if ((percpu_sp = per_cpu_symbol_search(args[optind])) && display_per_cpu_info(percpu_sp))
			return;
		sprintf(buf2, "%s = ", args[optind]);
		leader = strlen(buf2);
		if (module_symbol(sp->value, NULL, NULL, NULL, *gdb_output_radix))
			do_load_module_filter = TRUE;
	} else if ((percpu_sp = per_cpu_symbol_search(args[optind])) && display_per_cpu_info(percpu_sp))
		return;
	...
	success = gdb_pass_through(concat_args(buf1, 0, TRUE), NULL, GNU_RETURN_ON_ERROR);
			...
			if (firstline && (p1 = strstr(buf1, "{"))
				...
				*p1 = NULLCHAR;
				...
				print_verbatim(pc->saved_fp,
					       do_load_module_filter ? load_module_filter(buf1, LM_P_FILTER) : buf1);
 ...
 */
static int display_per_cpu_info(struct syment *sp)
{
	int c;
	ulong addr;
	char buf[BUFSIZE];
	...
	if (((kt->flags & (SMP | PER_CPU_OFF)) != (SMP | PER_CPU_OFF)) ||
	    (!is_percpu_symbol(sp)) || !((sp->type == 'd') || (sp->type == 'D') || (sp->type == 'V')))
		return FALSE;
...
static struct load_module *get_module_percpu_sym_owner(struct syment *sp)
{
	int i;
	struct load_module *lm;
...
static int is_percpu_symbol(struct syment *sp)
{
...
int is_datatype_command(void)
{
	...
	if ((sp = symbol_search(args[0])) && (argcnt == 1)) {
		if (is_gdb_command(FALSE, RETURN_ON_ERROR)) {
			...
			error(FATAL, "ambiguous command: %s (symbol and gdb command)\n", args[0]);
		}
		command = "p";
		...
		if (is_gdb_command(FALSE, RETURN_ON_ERROR)) {
			pc->curcmd = pc->program_name;
			error(FATAL, "ambiguous command: %s (symbol/data type and gdb command)\n", args[0]);
		}
 ...
 */
static void print_struct(char *s, ulong addr)
{
	char buf[BUFSIZE];
 ...
 */
static void print_union(char *s, ulong addr)
{
	char buf[BUFSIZE];
 ...
 */
static void whatis_datatype(char *st, ulong flags, FILE * ofp)
{
 ...
 */
static void whatis_variable(struct syment *sp)
{
	char *p1;
	char buf[BUFSIZE];
 ...
 */
static int is_typedef(char *name)
{
	struct datatype_member datatype_member, *dm;
...
static void dump_datatype_flags(ulong flags, FILE * ofp)
{
	int others;
 ...
 */
static void parse_for_member(struct datatype_member *dm, ulong flag)
{
	char *s;
	char buf[BUFSIZE];
	...
	case PARSE_FOR_DECLARATION:
		...
		while (fgets(buf, BUFSIZE, pc->tmpfile)) {
			...
			if (strstr(buf, lookfor1) ||
			    strstr(buf, lookfor2) ||
			    strstr(buf, lookfor3) || strstr(buf, lookfor4) || strstr(buf, lookfor5)) {
				if (strstr(buf, "}") && !on) {
					on = TRUE;
					fseek(pc->tmpfile, last_open_bracket, SEEK_SET);
				} else {
 ...
 */
static int show_member_offset(FILE * ofp, struct datatype_member *dm, char *inbuf)
{
	int i, c, len;
	long offset;
	char *t1, *target;
 ...
 */
int get_array_length(char *s, int *two_dim, long entry_size)
{
	char copy[BUFSIZE];
	char buf[BUFSIZE];
	char lookfor1[BUFSIZE];
	...
	if (!(LKCD_KERNTYPES()) && symbol_search(s)) {
		if (!two_dim) {
			...
			if ((get_symbol_type(copy, NULL, req) ==
			     TYPE_CODE_ARRAY) && req->target_typecode && req->target_length) {
		...
		if (arg_to_datatype(copy, dm, RETURN_ON_ERROR)) {
			...
			switch (dm->type) {
			case UNION_REQUEST:
				if (entry_size && dm->member_size && (dm->member_typecode == TYPE_CODE_ARRAY)) {
	...
	while (fgets(buf, BUFSIZE, pc->tmpfile2)) {
		if (STRNEQ(buf, "type = ") && (p1 = strstr(buf, "[")) && (p2 = strstr(buf, "]"))
		...
		if (STRNEQ(buf, "type = ") && (count_chars(buf, '[') == 2) && (count_chars(buf, ']') == 2) && two_dim) {
			p1 = strstr(buf, "[");
			p2 = strstr(buf, "]");
			*p2 = NULLCHAR;
			p1++;
 ...
 */
int get_array_length_alt(char *name, char *s, int *two_dim, long entry_size)
{
	int retval;
 ...
 */
int builtin_array_length(char *s, int len, int *two_dim)
{
	int *lenptr;
	int *dimptr;
 ...
 *  "help -o" output
 */
void dump_offset_table(char *spec, ulong makestruct)
{
	char buf[BUFSIZE], *p1;
	...
	fprintf(fp, "      neigh_table_hash_buckets: %d\n",
		get_array_length("neigh_table.hash_buckets", NULL, sizeof(void *)));
		...
		while (fgets(buf, BUFSIZE, pc->tmpfile)) {
			...
			if (strstr(buf, spec)) {
				if (in_size_table) {
					...
					if (!offsets)
						fprintf(pc->saved_fp, "                  offset_table:\n");
					offsets++;
				}
	...
	if (makestruct) {
		fprintf(pc->saved_fp, "static struct builtin_debug_table %s;\n\n", revname);
...
static struct numargs_cache {
	ulong function;
	int numargs;
} numargs_cache[NUMARGS_CACHE_ENTRIES] = { {
0}};

static int numargs_cache_index = 0;

int get_function_numargs(ulong callpc)
{
	int i;
...
void dump_numargs_cache(void)
{
	...
	for (i = 0; i < NUMARGS_CACHE_ENTRIES; i++) {
		...
		fprintf(fp, "%lx (%s): %d\n", na->function, value_to_symstr(na->function, buf, 0), na->numargs);
	}
}
 ...
 */
static void section_header_info(bfd * bfd, asection * section, void *reqptr)
{
	int i;
	struct load_module *lm;
	ulong request;
	...
	case (ulong) KERNEL_SECTIONS:
		...
		*sec = section;
		...
		if (STREQ(bfd_get_section_name(bfd, section), ".text.init") ||
			...
			kt->etext_init = kt->stext_init + (ulong) bfd_section_size(bfd, section);
		}
		if (STREQ(bfd_get_section_name(bfd, section), ".text") ||
			...
			if (!(bfd_get_section_flags(bfd, section) & SEC_HAS_CONTENTS))
...
static void store_section_data(struct load_module *lm, bfd * bfd, asection * section)
{
	int i;
	int prio;
	char *name;
	 ...
	 */
	if (lm->mod_percpu && (STREQ(name, ".data.percpu") || STREQ(name, ".data..percpu"))) {
...
static void calculate_load_order_v1(struct load_module *lm, bfd * bfd)
{
	int i;
	asection *section;
	ulong alignment;
	...
	qsort(&lm->mod_section_data[0], lm->mod_sections, sizeof(struct mod_section_data), compare_prios);
	...
	for (i = (lm->mod_sections - 1); i >= 0; i--) {
		...
		if (CRASHDEBUG(1))
			fprintf(fp, "%12s  prio: %x  flags: %x offset: %lx\n",
				...
				lm->mod_section_data[i].flags, lm->mod_section_data[i].offset);
	...
	while (s1 < s2) {
		...
		for (i = 0; i < lm->mod_sections; i++) {
			...
			if (sym_offset >= lm->mod_section_data[i].offset
			    && sym_offset < lm->mod_section_data[i].offset + lm->mod_section_data[i].size) {
				break;
			}
		}
		...
		if (i < lm->mod_sections) {
			if (CRASHDEBUG(2)) {
				fprintf(fp, "skip %lx %s %s\n", s1->value, s1->name, lm->mod_section_data[i].name);
			}
			s1++;
			continue;
		}
		...
		secname = NULL;
		for (; from < fromend; from += size) {
			if ((sym = bfd_minisymbol_to_symbol(bfd, dynamic, from, store)) == NULL)
				error(FATAL, "bfd_minisymbol_to_symbol() failed\n");

			bfd_get_symbol_info(bfd, sym, &syminfo);
			if (CRASHDEBUG(3)) {
				fprintf(fp,
					"matching sym %s %lx against bfd %s %lx\n",
					s1->name, (long)s1->value, syminfo.name, (long)syminfo.value);
			}
...
//          sec_end = sec_start + lm->mod_section_data[i].size;
		...
		if (CRASHDEBUG(2)) {
			fprintf(fp,
				"update sec offset sym %s @ %lx  val %lx  section %s\n",
...
static void check_insmod_builtin(struct load_module *lm, int index, ulong * offset)
{
	struct syment *sp;
	char buf[BUFSIZE];
	ulong offs;

	sprintf(buf, "__insmod_%s_S%s", lm->mod_name, lm->mod_section_data[index].name);
		...
		if (offs != *offset) {
			if (CRASHDEBUG(1))
				fprintf(fp,
					...
					lm->mod_name, lm->mod_section_data[index].name, offs, *offset);
			*offset = offs;
		}
	}
}
...
static int is_insmod_builtin(struct load_module *lm, struct syment *sp)
{
	char buf[BUFSIZE];
 ...
 */
int load_module_symbols(char *modref, char *namelist, ulong base_addr)
{
	static bfd *mbfd;
	char **matching;
	long symcount;
	...
	if (LKCD_KERNTYPES()
	    && (file_elf_version(namelist) == EV_DWARFEXTRACT))
	...
	store_load_module_symbols(mbfd, FALSE, minisyms, symcount, size, base_addr, namelist);
...
static int add_symbol_file(struct load_module *lm)
{
	...
	for (i = len = 0; i < lm->mod_sections; i++) {
		...
		if ((lm->mod_section_data[i].flags & SEC_FOUND) &&
		    (!STREQ(secname, ".text") && !STREQ(secname, ".data.percpu") && !STREQ(secname, ".data..percpu"))) {
			sprintf(buf, " -s %s 0x%lx", secname, lm->mod_section_data[i].offset + lm->mod_base);
			len += strlen(buf);
		}
	}
	...
	for (i = 0; i < lm->mod_sections; i++) {
		secname = lm->mod_section_data[i].name;
		if ((lm->mod_section_data[i].flags & SEC_FOUND) &&
		    (STREQ(secname, ".data.percpu") || STREQ(secname, ".data..percpu"))) {
...
static int add_symbol_file_percpu(struct load_module *lm, struct gnu_request *req, int buflen)
{
	char pbuf[BUFSIZE];
	int i, len;
	char *secname;
 ...
 */
static int add_symbol_file_kallsyms(struct load_module *lm, struct gnu_request *req)
{
	...
	if (!(st->flags & (MODSECT_VMASK | MODSECT_UNKNOWN))) {
		...
		MEMBER_OFFSET_INIT(module_sect_attrs, "module", "sect_attrs");
		MEMBER_OFFSET_INIT(module_sect_attrs_attrs, "module_sect_attrs", "attrs");
		MEMBER_OFFSET_INIT(module_sect_attrs_nsections, "module_sect_attrs", "nsections");
		MEMBER_OFFSET_INIT(module_sect_attr_mattr, "module_sect_attr", "mattr");
		MEMBER_OFFSET_INIT(module_sect_attr_name, "module_sect_attr", "name");
		MEMBER_OFFSET_INIT(module_sect_attr_address, "module_sect_attr", "address");
		MEMBER_OFFSET_INIT(module_attribute_attr, "module_attribute", "attr");
		MEMBER_OFFSET_INIT(module_sect_attr_attr, "module_sect_attr", "attr");
		MEMBER_OFFSET_INIT(module_sections_attrs, "module_sections", "attrs");
		MEMBER_OFFSET_INIT(attribute_owner, "attribute", "owner");

		if (VALID_MEMBER(module_sect_attrs_attrs) &&
		    VALID_MEMBER(module_sect_attr_mattr) &&
		    VALID_MEMBER(module_attribute_attr) && VALID_MEMBER(module_sect_attrs_nsections))
			st->flags |= MODSECT_V3;
		else if (VALID_MEMBER(module_sect_attrs_attrs) &&
			 VALID_MEMBER(module_sect_attr_mattr) && VALID_MEMBER(module_attribute_attr))
			st->flags |= MODSECT_V2;
		else if (VALID_MEMBER(module_sect_attr_attr) && VALID_MEMBER(module_sections_attrs))
			st->flags |= MODSECT_V1;
		else
		...
		if ((st->flags & MODSECT_UNKNOWN) ||
		    ...
		    INVALID_MEMBER(module_sect_attr_name) || INVALID_MEMBER(module_sect_attr_address)) {
			if (CRASHDEBUG(1))
				error(WARNING, "module section data structures " "unrecognized or changed\n");
	...
	if (!readmem(lm->module_struct + OFFSET(module_sect_attrs),
		     KVADDR, &vaddr, sizeof(void *), "module.sect_attrs", RETURN_ON_ERROR | QUIET))
		return FALSE;
	...
	case MODSECT_V3:
		...
		if (!readmem(vaddr + OFFSET(module_sect_attrs_nsections),
			     KVADDR, &nsections, sizeof(int), "module_sect_attrs.nsections", RETURN_ON_ERROR | QUIET))
			return FALSE;
		if (CRASHDEBUG(2))
	...
	for (done = FALSE; !done; array_entry += SIZE(module_sect_attr)) {
		...
		case TYPE_CODE_PTR:
			if (!readmem
			    (array_entry + OFFSET(module_sect_attr_name),
			     KVADDR, &name, sizeof(void *), "module_sect_attr.name", RETURN_ON_ERROR | QUIET)) {
				done = TRUE;
				retval = FALSE;
				continue;
			}
		...
		if (nsections == UNUSED) {
			if (!readmem(owner, KVADDR, &vaddr, sizeof(void *), "attribute.owner", RETURN_ON_ERROR | QUIET)) {
				done = TRUE;
				continue;
			}
		...
		if (STREQ(section_name, ".text")) {
			sprintf(buf, "add-symbol-file %s 0x%lx %s",
				lm->mod_namelist, section_vaddr, pc->curcmd_flags & MOD_READNOW ? "-readnow" : "");
 ...
 */
static int load_module_index(struct syment *sp)
{
	int i;
	ulong value;
	struct load_module *lm;
 ...
 */
static struct syment *kallsyms_module_symbol(struct load_module *lm, symbol_info * syminfo)
{
			  ...
			  long symcount, unsigned int size, ulong base_addr, char *namelist)
{
	...
	for (i = symalloc = 0; i < st->mods_installed; i++) {
		...
		if (lm->mod_base == base_addr) {
			...
			if (lm->mod_load_symtable && (lm->mod_symalloc < symalloc)) {
				free(lm->mod_load_symtable);
				namespace_ctl(NAMESPACE_FREE, &lm->mod_load_namespace, NULL, NULL);
	...
	if (!lm->mod_load_symtable) {
		...
		if (!namespace_ctl(NAMESPACE_INIT, &lm->mod_load_namespace, (void *)symalloc, NULL))
			error(FATAL, "module name space malloc: %s\n", strerror(errno));
	} else
		namespace_ctl(NAMESPACE_REUSE, &lm->mod_load_namespace, NULL, NULL);
	...
	if (!(lm->mod_section_data = (struct mod_section_data *)
	      malloc(sizeof(struct mod_section_data) * (bfd->section_count + 1))))
	...
	if (kt->flags & KMOD_V1)
		...
		calculate_load_order_v2(lm, bfd, dynamic, minisyms, symcount, size);
	...
	for (; from < fromend; from += size) {
			...
			case 'B':
				if (CRASHDEBUG(2))
					fprintf(fp, "%08lx (%c) [%s] %s\n",
						(ulong) syminfo.value, syminfo.type, secname, syminfo.name);
		...
		if (found) {
			...
			if (machdep->verify_symbol(name, syminfo.value, syminfo.type)) {
				...
				namespace_ctl(NAMESPACE_INSTALL, &lm->mod_load_namespace, sp, name);

				if (CRASHDEBUG(2))
					fprintf(fp, "installing %c %08lx %s\n", syminfo.type, sp->value, name);
	...
	for (spx = lm->mod_ext_symtable; spx <= lm->mod_ext_symend; spx++) {
				...
				if (spx->value == sp->value) {
					...
					if (CRASHDEBUG(2))
						fprintf(fp,
							...
							lm->mod_name, nameptr, sp->value, spx->value);
				}
				break;
			}
		}
		if (!found) {
			...
			/* append it here... */
			namespace_ctl(NAMESPACE_INSTALL, &lm->mod_load_namespace, lm->mod_load_symend, spx->name);
	 ...
	 */
	for (i = 0; (pc->curcmd_flags & MOD_SECTIONS) && (i < lm->mod_sections); i++) {
		if (!(lm->mod_section_data[i].flags & SEC_FOUND))
			continue;
		/* Section start */
		lm->mod_load_symend->value = lm->mod_base + lm->mod_section_data[i].offset;
		lm->mod_load_symend->type = 'S';
		lm->mod_load_symend->flags |= MODULE_SYMBOL;
		sprintf(name, "_MODULE_SECTION_START [%s]", lm->mod_section_data[i].name);
		namespace_ctl(NAMESPACE_INSTALL, &lm->mod_load_namespace, lm->mod_load_symend, name);
		lm->mod_load_symend++;
		lm->mod_load_symcnt++;

		/* Section end */
		lm->mod_load_symend->value = lm->mod_base +
		    lm->mod_section_data[i].offset + lm->mod_section_data[i].size;
		lm->mod_load_symend->type = 'S';
		lm->mod_load_symend->flags |= MODULE_SYMBOL;
		sprintf(name, "_MODULE_SECTION_END [%s]", lm->mod_section_data[i].name);
		namespace_ctl(NAMESPACE_INSTALL, &lm->mod_load_namespace, lm->mod_load_symend, name);
		lm->mod_load_symend++;
		lm->mod_load_symcnt++;
	}

	namespace_ctl(NAMESPACE_COMPLETE, &lm->mod_load_namespace, lm->mod_load_symtable, lm->mod_load_symend);

	qsort(lm->mod_load_symtable, lm->mod_load_symcnt, sizeof(struct syment), compare_syms);

	lm->mod_load_symend--;
	if (!MODULE_END(lm->mod_load_symend) && !IN_MODULE_PERCPU(lm->mod_load_symend->value, lm))
 ...
 */
void delete_load_module(ulong base_addr)
{
	int i;
	struct load_module *lm;
	struct gnu_request request, *req;
 ...
 */
int is_module_name(char *s, ulong * addr, struct load_module **lmp)
{
	int i;
	struct load_module *lm;
 ...
 */
int is_module_address(ulong check_addr, char *module_name)
{
	int i;
	struct load_module *lm;
...
static void find_mod_etext(struct load_module *lm)
{
	ulong start, end;
	char *modbuf;
	...
	while (start < end) {
		...
		readmem(start, KVADDR, modbuf, alloc, "module rodata search chunk", FAULT_ON_ERROR);
...
static long rodata_search(ulong * buf, ulong size)
{
	int i, acnt, words;
	long offset;
	ulong *wordptr;
...
static int non_numeric_forward(const void *P_x, const void *P_y)
{
	...
	return ((xn == NULL) ? ((yn == NULL) ? 0 : -1) : ((yn == NULL) ? 1 : strcmp(xn, yn)));
}

static int numeric_forward(const void *P_x, const void *P_y)
{
	asymbol *x, *y;
	asection *xs, *ys;
...
static void gnu_qsort(bfd * bfd, void *minisyms, long symcount, unsigned int size, asymbol * x, asymbol * y)
{
	gnu_sort_bfd = bfd;
	gnu_sort_x = x;
	gnu_sort_y = y;
 ...
 *  address.
 */
int text_value_cache(ulong vaddr, uint32_t value, uint32_t * valptr)
{
	int i;
	...
	if (value) {
		...
		if (tc->index == MAX_TEXT_CACHE) {
			...
			old_cache = tc->cache;
			if ((tc->cache = (struct text_cache_entry *)
			     realloc(old_cache, sizeof(struct text_cache_entry) * (TEXT_CACHE + tc->entries)))) {
				BZERO(&tc->cache[tc->index], sizeof(struct text_cache_entry) * TEXT_CACHE);
				tc->entries += TEXT_CACHE;
			} else {
				tc->cache = old_cache;
				tc->index = 0;
...
int text_value_cache_byte(ulong vaddr, unsigned char *valptr)
{
	int i;
	int shift;
	struct text_cache *tc;
	...
	for (i = 0; i < tc->entries; i++) {
		...
		if ((vaddr >= tc->cache[i].vaddr) && (vaddr < (tc->cache[i].vaddr + SIZEOF_32BIT))) {
...
void dump_text_value_cache(int verbose)
{
	int i;
	struct syment *sp;
	ulong offset;
	...
	if (!verbose) {
		...
		fprintf(stderr, "     text hit rate: %2ld%% (%ld of %ld)\n",
			(tc->hits * 100) / tc->refs, (ulong) tc->hits, (ulong) tc->refs);
		return;
	}
	...
	for (i = 0; tc->cache && (i < tc->entries); i++) {
		if (!tc->cache[i].vaddr)
			break;
		fprintf(fp, "[%2d]: %lx %08x ", i, tc->cache[i].vaddr, tc->cache[i].value);
	...
	fprintf(fp,
		...
		tc->entries, tc->index, (tc->hits * 100) / (tc->refs ? tc->refs : 1), tc->hits, tc->refs);
...
int patch_kernel_symbol(struct gnu_request *req)
{
	int i, c;
	...
	if (req->name == PATCH_KERNEL_SYMBOLS_START) {
		...
		fprintf(fp, (pc->flags & SILENT) || !(pc->flags & TTY) ? "" :
			"\nplease wait... (patching %ld gdb minimal_symbol values) ", st->symcnt);
		fflush(fp);
		req->count = 0;
		req->length = 0;
		req->last_sp = 0;
	...
	sp += sp ? 1 : 0;
	if (sp && (sp->cnt == 1) && !(sp->flags & SYMBOL_NAME_USED) && STREQ(sp->name, req->name)) {
 ...
 *  and act accordingly.
 */
long OFFSET_option(long offset1, long offset2, char *func, char *file, int line, char *item1, char *item2)
{
	char errmsg[BUFSIZE];
	...
	if (pc->flags & DATADEBUG) {
		...
		sprintf(errmsg, "invalid (optional) structure member offsets: %s or %s", item1, item2);
...
long SIZE_option(long size1, long size2, char *func, char *file, int line, char *item1, char *item2)
{
	char errmsg[BUFSIZE];
	...
	if (pc->flags & DATADEBUG) {
		void *retaddr[NUMBER_STACKFRAMES] = { 0 };
		SAVE_RETURN_ADDRESS(retaddr);
		sprintf(errmsg, "invalid (optional) structure sizes: %s or %s", item1, item2);
		datatype_error(retaddr, errmsg, func, file, line);
	}
 ...
 */
long OFFSET_verify(long offset, char *func, char *file, int line, char *item)
{
	char errmsg[BUFSIZE];
	...
	if (offset < 0) {
		void *retaddr[NUMBER_STACKFRAMES] = { 0 };
		SAVE_RETURN_ADDRESS(retaddr);
		sprintf(errmsg, "invalid structure member offset: %s", item);
		datatype_error(retaddr, errmsg, func, file, line);
	}
	return offset;
}

long SIZE_verify(long size, char *func, char *file, int line, char *item)
{
	char errmsg[BUFSIZE];
 ...
 */
static void datatype_error(void **retaddr, char *errmsg, char *func, char *file, int line)
{
	char buf[BUFSIZE];
	int fd;
...
void dump_trace(void **retaddr)
{
	...
	for (i = (NUMBER_STACKFRAMES - 1); i >= 0; i--) {
		if (retaddr[i])
			fprintf(stderr, "%s%lx%s", i == 3 ? "" : "=> ", (ulong) retaddr[i], i == 0 ? "\n" : " ");
	}
	fflush(stderr);
			...
			if (vaddr > lookfor) {
				if ((lookfor - last_vaddr) > last_size)
					...
					fprintf(stderr, "%s  %lx: %s+%ld\n",
						i == 0 ? "\n" : "", lookfor, symbol, lookfor - last_vaddr);
				break;
			}
...
static char *get_thisfile(void)
{
	...
	if (pc->program_path[0] == '.' || pc->program_path[0] == '/')
 ...
 */
struct load_module *init_module_function(ulong vaddr)
{
	int i;
	struct load_module *lm;

	if (((kt->flags & (KMOD_V1 | KMOD_V2)) == KMOD_V1) ||
	    INVALID_MEMBER(module_init_text_size) || INVALID_MEMBER(module_module_init))
		return NULL;
	...
	for (i = 0; i < st->mods_installed; i++) {
		...
		if ((vaddr >= lm->mod_init_module_ptr) && (vaddr < (lm->mod_init_module_ptr + lm->mod_init_text_size))
		    && accessible(vaddr))
			return lm;
	}
