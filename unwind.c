...
static int unw_init_from_blocked_task(struct unw_frame_info *, struct bt_info *);
static void unw_init_from_interruption(struct unw_frame_info *, struct bt_info *, ulong, ulong);
static int unw_switch_from_osinit_v1(struct unw_frame_info *, struct bt_info *);
static int unw_switch_from_osinit_v2(struct unw_frame_info *, struct bt_info *);
static int unw_switch_from_osinit_v3(struct unw_frame_info *, struct bt_info *, char *);
static unsigned long get_init_stack_ulong(unsigned long addr);
static void unw_init_frame_info(struct unw_frame_info *, struct bt_info *, ulong);
...
static struct unw_labeled_state *alloc_labeled_state(void)
{
	return ((struct unw_labeled_state *)
		GETBUF(sizeof(struct unw_labeled_state)));
}

static void free_labeled_state(struct unw_labeled_state *ls)
{
	FREEBUF(ls);
}
...
static inline unsigned long pt_regs_off_v2(unsigned long reg)
{
	short off = -1;
 ...
 */
static inline unsigned long pt_regs_off(unsigned long reg)
{
	unsigned long off = 0;
...
#ifdef UNWIND_V1
static inline struct pt_regs *get_scratch_regs(struct unw_frame_info *info)
{
...
static inline struct pt_regs *get_scratch_regs(struct unw_frame_info *info)
{
	if (!info->pt) {
		...
		error(INFO, "get_scratch_regs: bad unwind info: resetting info->pt\n");
...
#endif
{
	...
	if (regnum < 32) {
			...
			if (addr) {
				...
				case UNW_NAT_REGSTK:
					...
					if ((unsigned long)addr <
					    info->regstk.limit || (unsigned long)addr >= info->regstk.top) {
						error(INFO,
						      "unwind: %p outside of regstk "
						      "[0x%lx-0x%lx)\n",
						      (void *)addr, info->regstk.limit, info->regstk.top);
						return -1;
					}
		...
		nat_addr = ia64_rse_rnat_addr(addr);
		if ((unsigned long)addr < info->regstk.limit || (unsigned long)addr >= info->regstk.top) {
...
#ifdef UNWIND_V1
int unw_access_fr_v1(struct unw_frame_info *info, int regnum, struct ia64_fpreg *val, int write)
{
	...
	if (regnum <= 5) {
		...
		addr = (struct ia64_fpreg *)
		    (bt->task + OFFSET(task_struct_thread) +
		     OFFSET(thread_struct_fph) + ((regnum - 32) * sizeof(struct ia64_fpreg)));
#else
...
#ifdef UNWIND_V2
int unw_access_fr_v2(struct unw_frame_info *info, int regnum, struct ia64_fpreg *val, int write)
{
	struct ia64_fpreg *addr = 0;
	struct pt_regs *pt;
	struct bt_info *bt = (struct bt_info *)info->task;
...
#ifdef UNWIND_V3
int unw_access_fr_v3(struct unw_frame_info *info, int regnum, struct ia64_fpreg *val, int write)
{
	struct ia64_fpreg *addr = 0;
	struct pt_regs *pt;
	struct bt_info *bt = (struct bt_info *)info->task;
...
static inline void push(struct unw_state_record *sr)
{
...
static struct unw_reg_state *dup_state_stack(struct unw_reg_state *rs)
{
    ...
    decode_abreg(unsigned char abreg, int memory)
{
	...
	case 0x61:
		return UNW_REG_PSP;
	case 0x62:
		return memory ? UNW_REG_PRI_UNAT_MEM : UNW_REG_PRI_UNAT_GR;
	case 0x63:
		return UNW_REG_RP;
	case 0x64:
		return UNW_REG_BSP;
	case 0x65:
		return UNW_REG_BSPSTORE;
	case 0x66:
		return UNW_REG_RNAT;
	case 0x67:
		return UNW_REG_UNAT;
...
static void
alloc_spill_area(unsigned long *offp, unsigned long regsize, struct unw_reg_info *lo, struct unw_reg_info *hi)
{
...
static inline void spill_next_when(struct unw_reg_info **regp, struct unw_reg_info *lim, unw_word t)
{
	struct unw_reg_info *reg;
...
static inline void finish_prologue(struct unw_state_record *sr)
{
	struct unw_reg_info *reg;
	unsigned long off;
	int i;
	...
	if (sr->imask) {
		...
		for (t = 0; t < sr->region_len; ++t) {
			...
			kind = (mask >> 2 * (3 - (t & 3))) & 3;
			if (kind > 0)
				spill_next_when(&regs[kind - 1], sr->curr.reg + limit[kind - 1], sr->region_start + t);
		}
	}
	/*
...
static void
desc_prologue(int body, unw_word rlen, unsigned char mask, unsigned char grsave, struct unw_state_record *sr)
{
	int i;
		...
		for (i = 0; i < 4; ++i) {
			if (mask & 0x8)
				set_reg(sr->curr.reg + unw.save_order[i],
					UNW_WHERE_GR, sr->region_start + sr->region_len - 1, grsave++);
			mask <<= 1;
		}
...
static inline void desc_abi(unsigned char abi, unsigned char context, struct unw_state_record *sr)
{
...
static inline void desc_br_gr(unsigned char brmask, unsigned char gr, struct unw_state_record *sr)
{
	int i;
...
static inline void desc_br_mem(unsigned char brmask, struct unw_state_record *sr)
{
	int i;
	...
	for (i = 0; i < 5; ++i) {
		if (brmask & 1) {
			set_reg(sr->curr.reg + UNW_REG_B1 + i,
				UNW_WHERE_SPILL_HOME, sr->region_start + sr->region_len - 1, 0);
			sr->any_spills = 1;
		}
		brmask >>= 1;
	}
...
static inline void desc_frgr_mem(unsigned char grmask, unw_word frmask, struct unw_state_record *sr)
{
	int i;
...
static inline void desc_fr_mem(unsigned char frmask, struct unw_state_record *sr)
{
	int i;
...
static inline void desc_gr_gr(unsigned char grmask, unsigned char gr, struct unw_state_record *sr)
{
	int i;
...
static inline void desc_gr_mem(unsigned char grmask, struct unw_state_record *sr)
{
	int i;
...
static inline void desc_mem_stack_f(unw_word t, unw_word size, struct unw_state_record *sr)
{
	set_reg(sr->curr.reg + UNW_REG_PSP, UNW_WHERE_NONE,
		sr->region_start + MIN((int)t, sr->region_len - 1), 16 * size);
}

static inline void desc_mem_stack_v(unw_word t, struct unw_state_record *sr)
{
	sr->curr.reg[UNW_REG_PSP].when = sr->region_start + MIN((int)t, sr->region_len - 1);
}

static inline void desc_reg_gr(unsigned char reg, unsigned char dst, struct unw_state_record *sr)
{
	set_reg(sr->curr.reg + reg, UNW_WHERE_GR, sr->region_start + sr->region_len - 1, dst);
}

static inline void desc_reg_psprel(unsigned char reg, unw_word pspoff, struct unw_state_record *sr)
{
	set_reg(sr->curr.reg + reg, UNW_WHERE_PSPREL, sr->region_start + sr->region_len - 1, 0x10 - 4 * pspoff);
}

static inline void desc_reg_sprel(unsigned char reg, unw_word spoff, struct unw_state_record *sr)
{
	set_reg(sr->curr.reg + reg, UNW_WHERE_SPREL, sr->region_start + sr->region_len - 1, 4 * spoff);
}

static inline void desc_rp_br(unsigned char dst, struct unw_state_record *sr)
{
	sr->return_link_reg = dst;
}

static inline void desc_reg_when(unsigned char regnum, unw_word t, struct unw_state_record *sr)
{
...
static inline void desc_spill_base(unw_word pspoff, struct unw_state_record *sr)
{
	sr->spill_offset = 0x10 - 4 * pspoff;
}

static inline unsigned char *desc_spill_mask(unsigned char *imaskp, struct unw_state_record *sr)
{
	sr->imask = imaskp;
 ...
 * Body descriptors.
 */
static inline void desc_epilogue(unw_word t, unw_word ecount, struct unw_state_record *sr)
{
	sr->epilogue_start = sr->region_start + sr->region_len - 1 - t;
	sr->epilogue_count = ecount + 1;
}

static inline void desc_copy_state(unw_word label, struct unw_state_record *sr)
{
...
static inline void desc_label_state(unw_word label, struct unw_state_record *sr)
{
	struct unw_labeled_state *ls;
...
static inline int desc_is_active(unsigned char qp, unw_word t, struct unw_state_record *sr)
{
...
static inline void desc_restore_p(unsigned char qp, unw_word t, unsigned char abreg, struct unw_state_record *sr)
{
...
desc_spill_reg_p(unsigned char qp, unw_word t, unsigned char abreg,
		 unsigned char x, unsigned char ytreg, struct unw_state_record *sr)
{
...
static inline void
desc_spill_psprel_p(unsigned char qp, unw_word t, unsigned char abreg, unw_word pspoff, struct unw_state_record *sr)
{
	struct unw_reg_info *r;
...
static inline void
desc_spill_sprel_p(unsigned char qp, unw_word t, unsigned char abreg, unw_word spoff, struct unw_state_record *sr)
{
	struct unw_reg_info *r;
...
#endif
{
	...
	if (LKCD_KERNTYPES()) {
		if ((len = STRUCT_SIZE("unw")) == 0) {
			error(WARNING, "cannot determine unw.tables offset; no struct unw\n");
			machdep->flags |= UNW_OUT_OF_SYNC;
			return;
		}
		machdep->machspec->unw_tables_offset = MEMBER_OFFSET("unw", "tables");
		...
		if (MEMBER_EXISTS("unw", "pt_regs_offsets")) {
			machdep->machspec->unw_pt_regs_offsets =
			    MEMBER_OFFSET("unw", "pt_regs_offsets") - machdep->machspec->unw_tables_offset;
			machdep->machspec->unw_kernel_table_offset =
			    MEMBER_OFFSET("unw", "kernel_table") - machdep->machspec->unw_tables_offset;
	...
	if (!(machdep->flags & UNW_OUT_OF_SYNC)) {
		machdep->machspec->unw_tables_offset = req->member_offset / BITS_PER_BYTE;

		if (get_symbol_type("unw", "r0", req) != TYPE_CODE_UNDEF)
			machdep->flags |= UNW_R0;

		verify_unw_member("save_order", struct_offset(struct unw, save_order));
		verify_unw_member("sw_off", struct_offset(struct unw, sw_off));
		verify_unw_member("preg_index", struct_offset(struct unw, preg_index));

		if (get_symbol_type("unw", "pt_regs_offsets", req)
		    == TYPE_CODE_ARRAY) {
			machdep->machspec->unw_pt_regs_offsets =
			    req->member_offset / BITS_PER_BYTE - machdep->machspec->unw_tables_offset;
			get_symbol_type("unw", "kernel_table", req);
			machdep->machspec->unw_kernel_table_offset =
			    req->member_offset / BITS_PER_BYTE - machdep->machspec->unw_tables_offset;
			machdep->flags |= UNW_PTREGS;
		} else
			verify_unw_member("kernel_table", struct_offset(struct unw, kernel_table));
 ...
 verify:
	...
	verify_common_struct("unw_labeled_state", sizeof(struct unw_labeled_state));
 ...
 */
static void verify_unw_member(char *member, long loffs)
{
	struct gnu_request request, *req;
	long koffs;
	...
	if (get_symbol_type("unw", member, req) == TYPE_CODE_UNDEF) {
		...
		koffs = (req->member_offset / BITS_PER_BYTE) - machdep->machspec->unw_tables_offset;
		if (machdep->flags & UNW_R0)
			koffs -= sizeof(unsigned long);
		if (koffs != loffs) {
			error(WARNING, "unw.%s offset differs: %ld (local: %d)\n", member, koffs, loffs);
			machdep->flags |= UNW_OUT_OF_SYNC;
		} else if (CRASHDEBUG(3))
			error(INFO, "unw.%s offset OK: %ld (local: %d)\n", member, koffs, loffs);
	}
}
 ...
 */
static void verify_common_struct(char *structname, long loclen)
{
	long len;
 ...
 */
static int load_unw_table(int clear_cache)
{
	int i;
	size_t len;
	...
	if (clear_cache) {
		if (!ms->script_cache) {
			...
			if ((ms->script_cache = (struct unw_script *)malloc(len)) == NULL) {
				error(WARNING, "cannot malloc unw_script cache\n");
				return FALSE;
			}
		}

		for (i = 0; i < UNW_CACHE_SIZE; i++)
			BZERO((void *)&ms->script_cache[i], sizeof(struct unw_script));
		ms->script_index = 0;
	}
	...
	if (machdep->flags & UNW_R0) {
		...
		if (!readmem(symbol_value("unw") + ms->unw_tables_offset,
			     KVADDR, up,
			     sizeof(struct unw) + sizeof(struct unw_table *), "unw", RETURN_ON_ERROR | QUIET))
			return FALSE;
		 ...
		 */
		up = (struct unw *)(((unsigned long)unw_temp) + sizeof(struct unw_table *));
		...
		BCOPY(&up->kernel_table, &unw.kernel_table, sizeof(struct unw_table));
	...
	if (machdep->flags & UNW_PTREGS) {
		if (!readmem(symbol_value("unw") + ms->unw_kernel_table_offset +
			     ...
			     KVADDR, &unw.kernel_table,
			     sizeof(struct unw_table), "unw.kernel_table", RETURN_ON_ERROR | QUIET))
			return FALSE;
	...
	if ((kernel_unw_table_entry_array = (struct unw_table_entry *)malloc(len)) == NULL) {
		error(WARNING, "cannot malloc kernel unw.kernel_table array (len: %d)\n", len);
		return FALSE;
	}
 ...
 restart:
		...
		if (BT_REFERENCE_CHECK(bt)) {
			...
			fprintf(fp, "%s#%d [BSP:%lx] %s at %lx",
				frame >= 10 ? "" : " ", frame, bsp, name_plus_offset ? name_plus_offset : name, ip);
		...
		if (STREQ(name, "start_kernel") || STREQ(name, "start_secondary") || STREQ(name, "start_kernel_thread"))
			break;
		...
		if (STREQ(name, "ia64_mca_handler") && symbol_exists("ia64_mca_modify_original_stack"))
...
#endif
{
	...
	for (i = 0; i < UNW_CACHE_SIZE; i++) {
		if (ms->script_cache[i].ip)
			fprintf(fp, "              [%3d]: %lx %s\n",
				i, ms->script_cache[i].ip, value_to_symstr(ms->script_cache[i].ip, buf, 0));
	}
}
...
#endif
{
	...
	do {
		if (!readmem((ulong) table, KVADDR, &unw_table_buf,
			     sizeof(struct unw_table), "module unw_table", RETURN_ON_ERROR))
			break;
...
static void dump_unwind_table(struct unw_table *table)
{
	struct unw_table unw_table_buf, *tbl;

	readmem((ulong) table, KVADDR, &unw_table_buf, sizeof(struct unw_table), "module unw_table", RETURN_ON_ERROR);
	tbl = &unw_table_buf;
...
static unsigned long get_init_stack_ulong(unsigned long addr)
{
	...
	readmem(addr, KVADDR, &tmp, sizeof(unsigned long), "get_init_stack_ulong", FAULT_ON_ERROR);

	return tmp;
}

static int unw_init_from_blocked_task(struct unw_frame_info *info, struct bt_info *bt)
{
	ulong sw;
 ...
 */
static void unw_init_from_interruption(struct unw_frame_info *info, struct bt_info *bt, ulong pt, ulong sw)
{
 ...
 */
static int unw_switch_from_osinit_v1(struct unw_frame_info *info, struct bt_info *bt)
{
	unsigned long pt, sw;
	char is_nat;
...
static int unw_switch_from_osinit_v2(struct unw_frame_info *info, struct bt_info *bt)
{
	unsigned long pt, sw;
	char is_nat;
...
static int user_mode(struct bt_info *bt, unsigned long pt)
{
...
static int unw_switch_from_osinit_v3(struct unw_frame_info *info, struct bt_info *bt, char *type)
{
	...
	if (STREQ(bt->tc->comm, type)) {
		...
		pid = 0;
		offset_kr = MEMBER_OFFSET("ia64_sal_os_state", "prev_IA64_KR_CURRENT");
		readmem(sos + offset_kr, KVADDR, &kr_current, sizeof(ulong),
			"ia64_sal_os_state prev_IA64_KR_CURRENT", FAULT_ON_ERROR);
		...
		if (pid)
			...
			tc = pid_to_context(0);
			while (tc) {
				if (tc != bt->tc && tc->processor == bt->tc->processor)
					break;
				tc = tc->tc_next;
			}
		}
...
static void unw_init_frame_info(struct unw_frame_info *info, struct bt_info *bt, ulong sw)
{
	...
	readmem(sw + OFFSET(switch_stack_ar_bspstore), KVADDR,
		&rbstop, sizeof(ulong), "switch_stack ar_bspstore", FAULT_ON_ERROR);
	if (rbstop - (unsigned long)t >= IA64_STK_OFFSET)
		rbstop = rbslimit;
...
static void rse_function_params(struct bt_info *bt, struct unw_frame_info *info, char *name)
{
	int i;
	int numargs;
	...
	for (i = 0; i < numargs; i++) {
		...
		if (is_nat[i])
			...
			if (bt->flags & BT_FULL_SYM_SLAB)
				sprintf(buf2, "%s", format_stack_entry(bt, buf3, arglist[i], kt->end));
			else
...
static int find_save_locs(struct unw_frame_info *info)
{
	struct unw_script *scr;

	if ((info->ip & (machdep->machspec->unimpl_va_mask | 0xf)) || IS_UVADDR(info->ip, NULL)) {
		info->rp_loc = 0;
		return -1;
	}
	...
	if (!scr) {
		...
		if (!scr) {
			error(INFO, "failed to build unwind script for ip %lx\n", info->ip);
			return -1;
		}
	}
...
static int unw_unwind(struct unw_frame_info *info)
{
	...
	/* restore the ip */
	if (!info->rp_loc) {
		error(INFO, "unwind: failed to locate return link (ip=0x%lx)!\n", info->ip);
		return -1;
	}
	...
	if ((info->flags & UNW_FLAG_INTERRUPT_FRAME)) {
		...
		if ((pr & (1UL << pNonSys)) != 0)
			num_regs = IA64_GET_STACK_ULONG(info->cfm_loc) & 0x7f;	/* size of frame */
		info->pfs_loc = (unsigned long *)(info->pt + offsetof(struct pt_regs, ar_pfs));
#endif
#ifdef UNWIND_V3
		info->pt = info->sp + 16;
		if ((pr & (1UL << pNonSys)) != 0)
	...
	if (info->ip == prev_ip && info->sp == prev_sp && info->bsp == prev_bsp) {
		error(INFO, "unwind: ip, sp, bsp remain unchanged; stopping here (ip=0x%lx)\n", ip);
		return -1;
	}
 ...
 * unwinder represents.
 */
static void run_script(struct unw_script *script, struct unw_frame_info *state)
{
 ...
 redo:
		...
		case UNW_INSN_LOAD:
#if UNW_DEBUG
			if ((s[val] & (local_cpu_data->unimpl_va_mask | 0x7)) != 0 || s[val] < TASK_SIZE) {
 ...
 *  about lookup speed.
 */
static struct unw_script *script_lookup(struct unw_frame_info *info)
{
	int i;
...
static struct unw_script *script_new(unsigned long ip)
{
	struct unw_script *script;
	struct machine_specific *ms;
...
static void script_finalize(struct unw_script *script, struct unw_state_record *sr)
{
	script->pr_mask = sr->pr_mask;
	script->pr_val = sr->pr_val;
}

static void script_emit(struct unw_script *script, struct unw_insn insn)
{
...
static void emit_nat_info(struct unw_state_record *sr, int i, struct unw_script *script)
{
	...
	default:
		error(INFO, "unwind: don't know how to emit nat info for where = %u\n", r->where);
		return;
	}
	insn.opc = opc;
...
static struct unw_script *build_script(struct unw_frame_info *info)
{
	...
	sr.when_target = (3 * ((ip & ~0xfUL) - (table->segment_base + e->start_offset)) / 16 + (ip & 0xfUL));
#ifdef REDHAT
	readmem((ulong) (table->segment_base + e->info_offset), KVADDR,
		unwind_info_buf, UNWIND_INFO_BUFSIZE, "unwind info", FAULT_ON_ERROR);
	 ...
	 */
	if (sr.when_target > sr.curr.reg[UNW_REG_PSP].when && (sr.curr.reg[UNW_REG_PSP].where == UNW_WHERE_NONE)
	    && sr.curr.reg[UNW_REG_PSP].val != 0) {
...
static struct unw_table_entry *lookup(struct unw_table *table, unsigned long rel_ip)
{
	...
	if (table == &unw.kernel_table) {
		...
		if (!readmem((ulong) table->array, KVADDR, loc_array,
			...
			if (IS_MODULE_VADDR(table->segment_base + rel_ip))
				error(WARNING, "cannot read module unw_table_entry array\n");
			return 0;
		}
		array = loc_array;
	}
...
static void compile_reg(struct unw_state_record *sr, int i, struct unw_script *script)
{
	struct unw_reg_info *r = sr->curr.reg + i;
	enum unw_insn_opcode opc;
	...
	case UNW_WHERE_FR:
		if (rval <= 5)
			...
			if (rval <= 9)
				...
				error(INFO, "unwind: kernel may not touch f%lu\n", rval);
		}
		break;
	...
	default:
		error(INFO, "unwind: register %u has unexpected `where' value of %u\n", i, r->where);
		break;
	}
	insn.opc = opc;
	insn.dst = unw.preg_index[i];
...
static void compile_reg_v2(struct unw_state_record *sr, int i, struct unw_script *script)
{
	struct unw_reg_info *r = sr->curr.reg + i;
	enum unw_insn_opcode opc;
	unsigned long val, rval;
	...
	case UNW_WHERE_FR:
		if (rval <= 5)
			...
			if (rval <= 11)
				...
				error(INFO, "compile_reg: kernel may not touch f%lu\n", rval);
		}
		break;
	...
	default:
		error(INFO, "compile_reg: register %u has unexpected `where' value of %u\n", i, r->where);
		break;
	}
	insn.opc = opc;
	insn.dst = unw.preg_index[i];
