 ...
 */
static int xc_core_verify(char *file, char *buf)
{
	...
	if (!xcp->xch_nr_vcpus) {
		error(INFO, "faulty xc_core dump file header: xch_nr_vcpus is 0\n\n");
	...
	if (!xd->page_size)
		error(FATAL, "unknown page size: use -p <pagesize> command line option\n");

	if (!(xd->page = (char *)malloc(xd->page_size)))
		error(FATAL, "cannot malloc page space.");

	if (!(xd->poc = (struct pfn_offset_cache *)calloc
	      (PFN_TO_OFFSET_CACHE_ENTRIES, sizeof(struct pfn_offset_cache))))
 ...
 */
static int xc_core_read(void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	...
	if ((page_index = xc_core_pfn_to_page_index(pfn)) == PFN_NOT_FOUND)
		return READ_ERROR;

	offset = (off_t) xd->xc_core.header.xch_pages_offset + ((off_t) (page_index) * (off_t) xd->page_size);
...
static int xc_save_verify(char *buf)
{
	...
	if (CRASHDEBUG(1)) {
		...
		fprintf(stderr, "endian: %d %s\n", __BYTE_ORDER,
			...
			(__BYTE_ORDER == __LITTLE_ENDIAN ? "__LITTLE_ENDIAN" : "???"));
	}
	...
	if (!(xd->xc_save.vmconfig_buf = (char *)malloc(xd->xc_save.vmconfig_size)))
	...
	if (!(xd->xc_save.region_pfn_type = (ulong *) calloc(MAX_BATCH_SIZE, sizeof(ulong))))
		error(FATAL, "cannot malloc region_pfn_type\n");

	if (read(xd->xfd, xd->xc_save.vmconfig_buf, xd->xc_save.vmconfig_size) != xd->xc_save.vmconfig_size)
	...
	if (!(xd->xc_save.batch_offsets = (off_t *) calloc((size_t) P2M_FL_ENTRIES, sizeof(off_t))))
		error(FATAL, "cannot allocate batch_offsets array");

	xd->xc_save.batch_count = P2M_FL_ENTRIES;

	if (read(xd->xfd, xd->xc_save.p2m_frame_list, P2M_FL_SIZE) != P2M_FL_SIZE)
		goto xc_save_bailout;
	...
	while (!done_batch) {
		...
		if (CRASHDEBUG(1))
			fprintf(stderr, "batch[%ld]: %d ", batch_index, batch_count);
		...
		default:
			if (batch_count > MAX_BATCH_SIZE) {
				if (CRASHDEBUG(1))
					fprintf(stderr, ": Max batch size exceeded. Giving up.\n");
				done_batch = TRUE;
				continue;
			}
			if (CRASHDEBUG(1))
		...
		if (read
		    (xd->xfd, xd->xc_save.region_pfn_type, batch_count * sizeof(ulong)) != batch_count * sizeof(ulong))
			goto xc_save_bailout;
		...
		for (i = 0; i < batch_count; i++) {
			...
			if (pfn > xd->xc_save.nr_pfns) {
				if (CRASHDEBUG(1))
					fprintf(stderr, "batch_count: %d pfn %ld out of range", batch_count, pfn);
			}
	 ...
	 */
	if (read(xd->xfd, &xd->xc_save.pfns_not, sizeof(xd->xc_save.pfns_not)) != sizeof(xd->xc_save.pfns_not))
		goto xc_save_bailout;

	if (CRASHDEBUG(1))
		fprintf(stderr, "PFNs not in pseudo-phys map: %d\n", xd->xc_save.pfns_not);

	if ((total_pages_read + xd->xc_save.pfns_not) != xd->xc_save.nr_pfns)
		error(WARNING,
		      "nr_pfns: %ld != (total pages: %ld + pages not saved: %d)\n",
		      xd->xc_save.nr_pfns, total_pages_read, xd->xc_save.pfns_not);
 ...
 xc_save_ia64:
	...
	if (!(xd->xc_save.ia64_page_offsets = (ulong *) calloc(xd->xc_save.nr_pfns, sizeof(off_t))))
	 ...
	 */
	if (read(xd->xfd, buf, sizeof(xen_domctl_arch_setup_t)) != sizeof(xen_domctl_arch_setup_t))
		goto xc_save_bailout;

	if (CRASHDEBUG(1)) {
		xen_domctl_arch_setup_t *setup = (xen_domctl_arch_setup_t *) buf;
	...
	for (i = N = 0; i < xd->xc_save.nr_pfns; i++) {
		...
		if (N < xd->xc_save.nr_pfns)
			xd->xc_save.ia64_page_offsets[N] = lseek(xd->xfd, 0, SEEK_CUR);
		else
			error(WARNING,
 ...
 xc_save_bailout:
	...
	error(INFO, "xc_save_verify: \"LinuxGuestRecord\" file handling/format error\n");
 ...
 */
static int xc_save_read(void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	int b, i, redundant;
	ulong reqpfn;
	int batch_count;
	...
	if (xd->flags & XC_SAVE_IA64) {
		...
		if (!file_offset) {
			if (CRASHDEBUG(1))
				fprintf(xd->ofp, "xc_save_read: pfn %lx not stored in xendump\n", reqpfn);
			return SEEK_ERROR;
		}
	...
	for (b = 0; b < xd->xc_save.batch_count; b++) {
		...
		case 0:
			if (CRASHDEBUG(1) && !STREQ(pc->curcmd, "search")) {
				fprintf(xd->ofp, "batch[%d]: has count of zero -- bailing out on pfn %ld\n", b, reqpfn);
			}
			return READ_ERROR;
		...
		default:
			if (CRASHDEBUG(8))
				fprintf(xd->ofp,
					...
					b, (ulonglong) xd->xc_save.batch_offsets[b], batch_count);
			break;
		}
 ...
 */
static void poc_store(ulong pfn, off_t file_offset)
{
	int i;
 ...
 */
static off_t poc_get(ulong pfn, int *redundant)
{
	int i;
 ...
 */
int xendump_init(char *unused, FILE * fptr)
{
	if (!XENDUMP_VALID())
		return FALSE;
...
int read_xendump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
...
int read_xendump_hyper(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	ulong pfn, page_index;
	off_t offset;
...
int write_xendump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	return WRITE_ERROR;
}
 ...
 */
int xendump_memory_dump(FILE * fp)
{
	...
	for (i = 0; i < PFN_TO_OFFSET_CACHE_ENTRIES; i++) {
		...
		if (CRASHDEBUG(2))
			fprintf(fp,
				...
				i, xd->poc[i].pfn, xd->poc[i].pfn, xd->poc[i].cnt, (ulonglong) xd->poc[i].file_offset);
	}
	if (!xd->poc)
		fprintf(fp, "\n");
	...
	fprintf(fp, "            batch_offsets: %lx %s\n",
		(ulong) xd->xc_save.batch_offsets, xd->xc_save.batch_offsets ? "" : "(none)");
	for (i = linefeed = 0; i < xd->xc_save.batch_count; i++) {
		fprintf(fp, "[%d]: %llx ", i, (ulonglong) xd->xc_save.batch_offsets[i]);
		if (((i + 1) % 4) == 0) {
			fprintf(fp, "\n");
			linefeed = FALSE;
		} else
	...
	fprintf(fp, "      xc_core:\n");
	fprintf(fp, "                   header:\n");
	fprintf(fp, "                xch_magic: %x ", xd->xc_core.header.xch_magic);
	...
	fprintf(fp, "             xch_nr_pages: %d (0x%x)\n",
		xd->xc_core.header.xch_nr_pages, xd->xc_core.header.xch_nr_pages);
	fprintf(fp, "          xch_ctxt_offset: %ld (0x%lx)\n",
		xd->xc_core.header.xch_ctxt_offset, xd->xc_core.header.xch_ctxt_offset);
	fprintf(fp, "         xch_index_offset: %ld (0x%lx)\n",
		xd->xc_core.header.xch_index_offset, xd->xc_core.header.xch_index_offset);
	fprintf(fp, "         xch_pages_offset: %ld (0x%lx)\n",
		xd->xc_core.header.xch_pages_offset, xd->xc_core.header.xch_pages_offset);

	fprintf(fp, "                elf_class: %s\n",
		xd->xc_core.elf_class ==
		ELFCLASS64 ? "ELFCLASS64" : xd->xc_core.elf_class == ELFCLASS32 ? "ELFCLASS32" : "n/a");
	fprintf(fp, "        elf_strtab_offset: %lld (0x%llx)\n",
		(ulonglong) xd->xc_core.elf_strtab_offset, (ulonglong) xd->xc_core.elf_strtab_offset);
	fprintf(fp, "           format_version: %016llx\n", (ulonglong) xd->xc_core.format_version);
	fprintf(fp, "       shared_info_offset: %lld (0x%llx)\n",
		(ulonglong) xd->xc_core.shared_info_offset, (ulonglong) xd->xc_core.shared_info_offset);
	if (machine_type("IA64"))
		fprintf(fp, "  ia64_mapped_regs_offset: %lld (0x%llx)\n",
			(ulonglong) xd->xc_core.ia64_mapped_regs_offset,
			(ulonglong) xd->xc_core.ia64_mapped_regs_offset);
	fprintf(fp, "       elf_index_pfn[%d]: %s", INDEX_PFN_COUNT, xd->xc_core.elf_class ? "\n" : "(none used)\n");
	if (xd->xc_core.elf_class) {
		for (i = 0; i < INDEX_PFN_COUNT; i++) {
			fprintf(fp, "%ld:%ld ", xd->xc_core.elf_index_pfn[i].index, xd->xc_core.elf_index_pfn[i].pfn);
		}
		fprintf(fp, "\n");
	}
	fprintf(fp, "               last_batch:\n");
	fprintf(fp, "                    index: %ld (%ld - %ld)\n",
		xd->xc_core.last_batch.index, xd->xc_core.last_batch.start, xd->xc_core.last_batch.end);
	fprintf(fp, "                 accesses: %ld\n", xd->xc_core.last_batch.accesses);
	fprintf(fp, "               duplicates: %ld ", xd->xc_core.last_batch.duplicates);
	if (xd->xc_core.last_batch.accesses)
		fprintf(fp, "(%ld%%)\n", xd->xc_core.last_batch.duplicates * 100 / xd->xc_core.last_batch.accesses);
	else
		fprintf(fp, "\n");
	...
	for (i = 0; i < xd->xc_core.p2m_frames; i++) {
		fprintf(fp, "%ld ", xd->xc_core.p2m_frame_index_list[i]);
	}
...
static void xen_dump_vmconfig(FILE * fp)
{
	int i, opens, closes;
	char *p;
...
void get_xendump_regs(struct bt_info *bt, ulong * pc, ulong * sp)
{
	...
	if ((tt->panic_task == bt->task) && (xd->panic_pc && xd->panic_sp)) {
		*pc = xd->panic_pc;
		*sp = xd->panic_sp;
		return;
	}
	...
	case XC_SAVE:
		...
		for (i = 0, up = (ulong *) bt->stackbuf; i < LONGS_PER_STACK; i++, up++) {
			if (is_kernel_text(*up)
			    && (STREQ(closest_symbol(*up), "__do_suspend"))) {
				*pc = *up;
				*sp = tt->flags & THREAD_INFO ?
				    bt->tc->thread_info + (i * sizeof(long)) : bt->task + (i * sizeof(long));
				xd->panic_pc = *pc;
				xd->panic_sp = *sp;
				return;
			}
...
static void xc_core_create_pfn_tables(void)
{
	if (xd->flags & XC_CORE_P2M_CREATE) {
		if (!machdep->xendump_p2m_create)
			error(FATAL, "xen xc_core dumpfiles not supported on this architecture");

		if (!machdep->xendump_p2m_create((void *)xd))
			error(FATAL, "cannot create xen pfn-to-mfn mapping\n");
	}
 ...
 */
char *xc_core_mfn_to_page(ulong mfn, char *pgbuf)
{
	int i, b, idx, done;
	...
	if (lseek(xd->xfd, (off_t) xd->xc_core.header.xch_index_offset, SEEK_SET) == -1) {
	...
	for (b = 0, idx = -1, done = FALSE; !done && (b < nr_pages); b += MAX_BATCH_SIZE) {
		...
		for (i = 0; i < MAX_BATCH_SIZE; i++) {
			...
			if (tmp[i] == mfn) {
				idx = i + b;
				if (CRASHDEBUG(4))
					fprintf(xd->ofp, "page: found mfn 0x%lx (%ld) at index %d\n", mfn, mfn, idx);
				done = TRUE;
			}
		}
	}
	...
	if (lseek(xd->xfd, (off_t) xd->xc_core.header.xch_pages_offset, SEEK_SET) == -1) {
 ...
 *  machine page into the buffer.
 */
static char *xc_core_elf_mfn_to_page(ulong mfn, char *pgbuf)
{
	int i, b, idx, done;
	off_t offset;
	size_t size;
	...
	for (b = 0, idx = -1, done = FALSE; !done && (b < nr_pages); b += MAX_BATCH_SIZE) {
		size = sizeof(struct xen_dumpcore_p2m) * MIN(MAX_BATCH_SIZE, nr_pages - b);
 ...
 */
int xc_core_mfn_to_page_index(ulong mfn)
{
	int i, b;
	ulong tmp[MAX_BATCH_SIZE];
	uint nr_pages;
 ...
 *  Find and return the page index containing the mfn.
 */
static int xc_core_elf_mfn_to_page_index(ulong mfn)
{
	int i, b;
	off_t offset;
	size_t size;
 ...
 */
static int xc_core_mfns(ulong arg, FILE * ofp)
{
	int i, b;
	uint nr_pages;
	ulong tmp[MAX_BATCH_SIZE];
 ...
 check_next_4:
		...
		if ((tmp[0] == 0xffffffff) || (tmp[1] == 0xffffffff) ||
		    (tmp[2] == 0xffffffff) || (tmp[3] == 0xffffffff) || (!tmp[0] && !tmp[1]) || (!tmp[2] && !tmp[3]))
			goto check_next_4;
 ...
 show_64bit_mfns:
		...
		for (b = 0; b < nr_pages; b += MAX_BATCH_SIZE) {
			size = sizeof(ulonglong) * MIN(MAX_BATCH_SIZE, nr_pages - b);
...
static ulong xc_core_pfn_to_page_index(ulong pfn)
{
 ...
 */
static ulong xc_core_elf_pfn_to_page_index(ulong pfn)
{
	int i, b, start_index;
	off_t offset;
	size_t size;
	...
	if ((pfn >= xd->xc_core.last_batch.start) && (pfn <= xd->xc_core.last_batch.end)) {
 ...
 */
static int xc_core_pfn_valid(ulong pfn)
{
	ulong mfn;
	off_t offset;
	 ...
	 */
	if (lseek(xd->xfd, offset, SEEK_SET) == -1) {
		error(INFO, "xendump: cannot lseek to page index for pfn %lx\n", pfn);
		return FALSE;
	}

	if (read(xd->xfd, &mfn, sizeof(ulong)) != sizeof(ulong)) {
		error(INFO, "xendump: cannot read index page for pfn %lx\n", pfn);
		return FALSE;
	}
	...
	if (mfn == INVALID_MFN) {
		if (CRASHDEBUG(1) && !STREQ(pc->curcmd, "search"))
			error(INFO, "xendump: pfn %lx contains INVALID_MFN\n", pfn);
		return FALSE;
	}
 ...
 */
static ulong xc_core_elf_pfn_valid(ulong pfn)
{
	int i, b, start_index;
	off_t offset;
	size_t size;
 ...
 */
void xendump_panic_hook(char *stack)
{
	int i, err, argc;
	...
	if ((value = htol(strip_ending_char(arglist[0], ':'), RETURN_ON_ERROR, &err)) == BADADDR)
		return;
	for (sp = (ulong *) value, i = 1; i < argc; i++, sp++) {
		if (strstr(arglist[i], "xen_panic_event")) {
			if (!readmem((ulong) sp, KVADDR, &value,
				     sizeof(ulong), "xen_panic_event address", RETURN_ON_ERROR))
				return;
 ...
 */
static int xc_core_elf_verify(char *file, char *buf)
{
	int i;
	Elf32_Ehdr *elf32;
	Elf64_Ehdr *elf64;
	...
	if ((elf32->e_ident[EI_CLASS] == ELFCLASS32) &&
	    ...
	    (swap32(elf32->e_version, swap) == EV_CURRENT) && (swap16(elf32->e_shnum, swap) > 0)) {
		...
		BCOPY(buf, xd->xc_core.elf32, sizeof(Elf32_Ehdr));
		   ...
		   (swap32(elf64->e_version, swap) == EV_CURRENT) && (swap16(elf64->e_shnum, swap) > 0)) {
 ...
 */
static void xc_core_dump_Elf32_Ehdr(Elf32_Ehdr * elf)
{
	char buf[BUFSIZE];
	Elf32_Off offset32;
	Elf32_Shdr shdr;
	...
	xendump_print("    e_ident[EI_VERSION]: %d ", elf->e_ident[EI_VERSION]);
	...
	offset32 = elf->e_shoff + (elf->e_shstrndx * elf->e_shentsize);

	if (lseek(xd->xfd, offset32, SEEK_SET) != offset32)
		error(FATAL, "xc_core_dump_Elf32_Ehdr: cannot seek to strtab Elf32_Shdr\n");
	if (read(xd->xfd, &shdr, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr))
		error(FATAL, "xc_core_dump_Elf32_Ehdr: cannot read strtab Elf32_Shdr\n");
 ...
 */
static void xc_core_dump_Elf64_Ehdr(Elf64_Ehdr * elf)
{
	char buf[BUFSIZE];
	Elf64_Off offset64;
	Elf64_Shdr shdr;
	...
	offset64 = elf->e_shoff + (elf->e_shstrndx * elf->e_shentsize);

	if (lseek(xd->xfd, offset64, SEEK_SET) != offset64)
		error(FATAL, "xc_core_dump_Elf64_Ehdr: cannot seek to strtab Elf32_Shdr\n");
	if (read(xd->xfd, &shdr, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr))
		error(FATAL, "xc_core_dump_Elf64_Ehdr:  cannot read strtab Elf32_Shdr\n");
 ...
 */
static void xc_core_dump_Elf32_Shdr(Elf32_Off offset, int store)
{
	Elf32_Shdr shdr;
	char name[BUFSIZE];
	int i;
	...
	if (lseek(xd->xfd, offset, SEEK_SET) != offset)
		error(FATAL, "xc_core_dump_Elf32_Shdr: cannot seek to Elf32_Shdr\n");
	if (read(xd->xfd, &shdr, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr))
		error(FATAL, "xc_core_dump_Elf32_Shdr: cannot read Elf32_Shdr\n");
	...
	if (STREQ(name, ".shstrtab")) {
		if (lseek(xd->xfd, xd->xc_core.elf_strtab_offset, SEEK_SET) != xd->xc_core.elf_strtab_offset)
			error(FATAL, "xc_core_dump_Elf32_Shdr: cannot seek to strtab data\n");

		xendump_print("                         ");
		for (i = 0; i < shdr.sh_size; i++) {
			if (read(xd->xfd, &c, sizeof(char)) != sizeof(char))
				error(FATAL, "xc_core_dump_Elf32_Shdr: cannot read strtab data\n");
			if (i && !c)
				xendump_print("\n                         ");
			else
				xendump_print("%c", c);
	...
	if (STREQ(name, ".note.Xen"))
		xc_core_dump_elfnote((off_t) shdr.sh_offset, (size_t) shdr.sh_size, store);

	if (!store)
		return;

	if (STREQ(name, ".xen_prstatus"))
		xd->xc_core.header.xch_ctxt_offset = (unsigned long)shdr.sh_offset;
	...
	if (STREQ(name, ".xen_pages"))
		xd->xc_core.header.xch_pages_offset = (unsigned long)shdr.sh_offset;

	if (STREQ(name, ".xen_ia64_mapped_regs"))
		xd->xc_core.ia64_mapped_regs_offset = (off_t) shdr.sh_offset;
}

/*
 *  Dump each 64-bit section header and the data that they reference.
 */
static void xc_core_dump_Elf64_Shdr(Elf64_Off offset, int store)
{
	Elf64_Shdr shdr;
	char name[BUFSIZE];
	int i;
	...
	if (lseek(xd->xfd, offset, SEEK_SET) != offset)
		error(FATAL, "xc_core_dump_Elf64_Shdr: cannot seek to Elf64_Shdr\n");
	if (read(xd->xfd, &shdr, sizeof(Elf64_Shdr)) != sizeof(Elf64_Shdr))
		error(FATAL, "xc_core_dump_Elf64_Shdr: cannot read Elf64_Shdr\n");
	...
	if (STREQ(name, ".shstrtab")) {
		if (lseek(xd->xfd, xd->xc_core.elf_strtab_offset, SEEK_SET) != xd->xc_core.elf_strtab_offset)
			error(FATAL, "xc_core_dump_Elf64_Shdr: cannot seek to strtab data\n");

		xendump_print("                         ");
		for (i = 0; i < shdr.sh_size; i++) {
			if (read(xd->xfd, &c, sizeof(char)) != sizeof(char))
				error(FATAL, "xc_core_dump_Elf64_Shdr: cannot read strtab data\n");
			if (i && !c)
				xendump_print("\n                         ");
			else
				xendump_print("%c", c);
 ...
 */
static char *xc_core_strtab(uint32_t index, char *buf)
{
	off_t offset;
	int i;
	...
	if (lseek(xd->xfd, offset, SEEK_SET) != offset)
		error(FATAL, "xc_core_strtab: cannot seek to Elf64_Shdr\n");
 ...
 */
static void xc_core_dump_elfnote(off_t sh_offset, size_t sh_size, int store)
{
	int i, lf, index;
	char *notes_buffer;
	...
	if (lseek(xd->xfd, sh_offset, SEEK_SET) != sh_offset)
		error(FATAL, "xc_core_dump_elfnote: cannot seek to sh_offset\n");

	if (read(xd->xfd, notes_buffer, sh_size) != sh_size)
		error(FATAL, "xc_core_dump_elfnote: cannot read elfnote data\n");
	...
	if (format_version) {
		...
		case FORMAT_VERSION_0000000000000001:
			break;
		default:
			error(WARNING, "unsupported xen dump-core format version: %016llx\n", format_version->version);
		}
...
static void xc_core_elf_pfn_init(void)
{
	...
	case (XC_CORE_ELF | XC_CORE_NO_P2M):
		...
		for (i = c = 0; i < INDEX_PFN_COUNT; i++, c += chunk) {
			offset = (off_t) xd->xc_core.header.xch_index_offset + (off_t) (c * sizeof(uint64_t));

			if (lseek(xd->xfd, offset, SEEK_SET) == -1)
				error(FATAL, "cannot lseek to page index %d\n", c);
			if (read(xd->xfd, &pfn, sizeof(uint64_t)) != sizeof(uint64_t))
				error(FATAL, "cannot read page index %d\n", c);
