...
struct qemu_device *device_alloc(struct qemu_device_list *dl, size_t sz,
...
struct qemu_device *device_find(struct qemu_device_list *dl, uint32_t section_id)
{
...
struct qemu_device *device_find_instance(struct qemu_device_list *dl, const char *name, uint32_t instance_id)
{
	struct qemu_device *d;
	d = dl->head;
	while (d && (strcmp(d->vtbl->name, name)
		     || d->instance_id != instance_id))
		d = d->next;
...
void device_free(struct qemu_device *d)
{
...
void device_list_free(struct qemu_device_list *l)
{
	if (!l)
		return;
...
static inline uint16_t get_be16(FILE * fp)
{
	uint8_t a = getc(fp);
	uint8_t b = getc(fp);
	return (a << 8) | b;
...
static inline uint16_t get_le16(FILE * fp)
{
	uint8_t b = getc(fp);
	uint8_t a = getc(fp);
	return (a << 8) | b;
...
static inline uint32_t get_be32(FILE * fp)
{
...
static inline uint32_t get_le32(FILE * fp)
{
...
static inline uint64_t get_be64(FILE * fp)
{
...
static inline uint64_t get_le64(FILE * fp)
{
...
static inline void get_qemu128(FILE * fp, union qemu_uint128_t *result)
{
...
static void ram_alloc(struct qemu_device_ram *dram, uint64_t size)
{
...
static int get_string(FILE * fp, char *name)
{
...
static void ram_read_blocks(FILE * fp, uint64_t size)
{
	char name[257];
...
static uint32_t ram_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	char name[257];
...
static void ram_free(struct qemu_device *d, struct qemu_device_list *dl)
{
	struct qemu_device_ram *dram = (struct qemu_device_ram *)d;
	free(dram->offsets);
}

int ram_read_phys_page(struct qemu_device_ram *dram, void *buf, uint64_t addr)
{
	off_t ofs;
...
static struct qemu_device *ram_init_load(struct qemu_device_list *dl,
					 uint32_t section_id,
					 uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	...
	return device_alloc(dl, sizeof(struct qemu_device_ram), &ram, section_id, instance_id, version_id);
}

#define BLK_MIG_FLAG_EOS 2

static uint32_t block_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	uint64_t header;
...
static struct qemu_device *block_init_load(struct qemu_device_list *dl,
					   uint32_t section_id,
					   uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	...
	return device_alloc(dl, sizeof(struct qemu_device), &block, section_id, instance_id, version_id);
}

/* RHEL5 marker.  */

static uint32_t rhel5_marker_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	return 0;
}

static struct qemu_device *rhel5_marker_init_load(struct qemu_device_list
						  *dl, uint32_t section_id,
						  uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	...
	assert(!live);
	return device_alloc(dl, sizeof(struct qemu_device), &rhel5_marker, section_id, instance_id, version_id);
}
...
static uint32_t cpu_common_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
...
static struct qemu_device *cpu_common_init_load(struct qemu_device_list
						*dl, uint32_t section_id,
						uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
...
static inline uint64_t get_be_long(FILE * fp, int size)
{
...
static inline void get_be_fp80(FILE * fp, union qemu_fpu_reg *result)
{
...
static void cpu_load_seg(FILE * fp, struct qemu_x86_seg *seg, int size)
{
...
static uint32_t cpu_load(struct qemu_device *d, FILE * fp, int size)
{
...
//              device_free ((struct qemu_device *) dcpu);
	...
	if (version_id >= 9) {
		...
		if (pending_irq >= 0 && pending_irq <= 255)
			dx86->kvm.int_bitmap[pending_irq / 64] |= (uint64_t) 1 << (pending_irq & 63);
...
static uint32_t cpu_load_32(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	return cpu_load(d, fp, 32);
}

static struct qemu_device *cpu_init_load_32(struct qemu_device_list *dl,
					    uint32_t section_id,
					    uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
...
//      assert (version_id >= 4 && version_id <= 9);
	...
	dx86 = (struct qemu_device_x86 *)
	    device_alloc(dl, sizeof(struct qemu_device_x86), &cpu, section_id, instance_id, version_id);
	return (struct qemu_device *)dx86;
}

static uint32_t cpu_load_64(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	return cpu_load(d, fp, 64);
}

static struct qemu_device *cpu_init_load_64(struct qemu_device_list *dl,
					    uint32_t section_id,
					    uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	struct qemu_device_x86 *dx86;
...
static uint32_t apic_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	switch (d->version_id) {
	case 1:
		fseek(fp, 173, SEEK_CUR);
		break;
	case 2:
	case 3:
		fseek(fp, 181, SEEK_CUR);
		break;
	}
...
static struct qemu_device *apic_init_load(struct qemu_device_list *dl,
					  uint32_t section_id,
					  uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	...
	assert(!live);
	return device_alloc(dl, sizeof(struct qemu_device), &apic, section_id, instance_id, version_id);
}

/* timer loader.  */

static uint32_t timer_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	fseek(fp, 24, SEEK_CUR);
	return QEMU_FEATURE_TIMER;
}

static struct qemu_device *timer_init_load(struct qemu_device_list *dl,
					   uint32_t section_id,
					   uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	...
	assert(!live);
	return device_alloc(dl, sizeof(struct qemu_device), &timer, section_id, instance_id, version_id);
}

/* kvmclock loader.  */

static uint32_t kvmclock_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	fseek(fp, 8, SEEK_CUR);
	return QEMU_FEATURE_KVM;
}

static struct qemu_device *kvmclock_init_load(struct qemu_device_list *dl,
					      uint32_t section_id,
					      uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	...
	assert(!live);
	return device_alloc(dl, sizeof(struct qemu_device), &kvmclock, section_id, instance_id, version_id);
}

/* kvm-tpr-opt loader.  */

static uint32_t kvm_tpr_opt_load(struct qemu_device *d, FILE * fp, enum qemu_save_section sec)
{
	fseek(fp, 144, SEEK_CUR);
	return QEMU_FEATURE_KVM;
}

static struct qemu_device *kvm_tpr_opt_init_load(struct qemu_device_list
						 *dl, uint32_t section_id,
						 uint32_t instance_id, uint32_t version_id, bool live, FILE * fp)
{
	...
	assert(!live);
	return device_alloc(dl, sizeof(struct qemu_device), &kvm_tpr_opt, section_id, instance_id, version_id);
}
...
static struct qemu_device *device_get(const struct qemu_device_loader
				      *devices, struct qemu_device_list *dl, enum qemu_save_section sec, FILE * fp)
{
	char name[257];
...
//      bool live;
 ...
 next_device:
	...
	return devp->init_load(dl, section_id, instance_id, version_id, sec == QEMU_VM_SECTION_START, fp);
}

struct qemu_device_list *qemu_load(const struct qemu_device_loader
				   *devices, uint32_t required_features, FILE * fp)
{
	...
	if (ferror(fp) || (result->features & required_features) != required_features)
		goto fail;
...
int is_qemu_vm_file(char *filename)
{
	struct libvirt_header header;
	FILE *vmp;
	int retval;
...
void dump_qemu_header(FILE * out)
{
	int i;
	struct libvirt_header header;
	char magic[4];
...
static long device_search(const struct qemu_device_loader *devices, FILE * fp)
{
	uint sz;
	char *p1, *p2;
	long next_device_offset;
	...
	while (devices->name) {
		for (p1 = buf, remaining = 4096;
		     (p2 = memchr(p1, devices->name[0], remaining)); p1 = p2 + 1, remaining = 4096 - (p1 - buf)) {
