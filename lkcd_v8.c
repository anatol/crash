...
int get_lkcd_regs_for_cpu_v8(struct bt_info *bt, ulong * eip, ulong * esp)
{
	...
	if (!bt || !bt->tc) {
		fprintf(stderr, "get_lkcd_regs_for_cpu_v8: invalid tc " "(CPU=%d)\n", cpu);
		return -EINVAL;
	}
...
#ifndef HAVE_NO_DUMP_HEADER_ASM
int lkcd_dump_init_v8_arch(dump_header_t * dh)
{
	off_t ret_of;
	ssize_t ret_sz;
	...
	ret_of = lseek(lkcd->fd, dh->dh_header_size + offsetof(dump_header_asm_t, dha_header_size), SEEK_SET);
	if (ret_of < 0) {
	...
	/* registers */
	memcpy(&arch_hdr.dha_smp_regs, (void *)&hdr_buf[offset], nr_cpus * sizeof(struct pt_regs));
	...
	/* stack */
	memcpy(&arch_hdr.dha_stack, (void *)&hdr_buf[offset], nr_cpus * sizeof(&arch_hdr.dha_stack[0]));
	offset += nr_cpus * sizeof(&arch_hdr.dha_stack[0]);

	/* stack_ptr */
	memcpy(&arch_hdr.dha_stack_ptr, (void *)&hdr_buf[offset], nr_cpus * sizeof(&arch_hdr.dha_stack_ptr[0]));
 ...
 */
int lkcd_dump_init_v8(FILE * fp, int fd, char *dumpfile)
{
	int i;
	int eof;
	uint32_t pgcnt;
	...
	if (read(lkcd->fd, dh, sizeof(dump_header_t)) != sizeof(dump_header_t))
		return FALSE;
	if ((dh->dh_version & LKCD_DUMP_VERSION_NUMBER_MASK) == LKCD_DUMP_V9) {
		if (read(lkcd->fd, &dh_dump_buffer_size, sizeof(dh_dump_buffer_size)) != sizeof(dh_dump_buffer_size))
			return FALSE;
	...
	if (lkcd_dump_init_v8_arch(dh) != 0) {
		fprintf(stderr, "Warning: Failed to initialise " "arch specific dump code\n");
	}
#ifdef IA64
	...
	if (ifd >= 0) {
		...
		lkcd->page_offsets =
		    mmap((void *)0, dump_index_size, (PROT_READ | PROT_WRITE), MAP_SHARED, ifd, (off_t) 0);
	...
	for (pgcnt = 0, eof = FALSE; !eof; pgcnt++) {
		...
		if (dp->dp_flags & ~(DUMP_DH_COMPRESSED | DUMP_DH_RAW | DUMP_DH_END | LKCD_DUMP_MCLX_V0)) {
	 ...
	 */
	if ((lkcd->page_cache_buf = (char *)malloc(dh->dh_page_size * (LKCD_CACHED_PAGES))) == NULL)
		return FALSE;
	...
	for (i = 0; i < LKCD_CACHED_PAGES; i++) {
		lkcd->page_cache_hdr[i].pg_bufptr = &lkcd->page_cache_buf[i * dh->dh_page_size];
	}
	...
	lkcd->total_pages = eof || (pgcnt > dh->dh_num_pages) ? pgcnt : dh->dh_num_pages;
 ...
 */
void dump_lkcd_environment_v8(ulong arg)
{
	int others;
	dump_header_t *dh;
	dump_page_t *dp;
...
void dump_dump_page_v8(char *s, void *dpp)
{
	dump_page_t *dp;
	uint32_t flags;
	int others;
	...
	console(BITS32()? "dp_address: %llx " : "dp_address: %lx ", dp->dp_address);
...
static void mclx_cache_page_headers_v8(void)
{
	...
	if ((((dump_header_t *) lkcd->dump_header)->dh_version & LKCD_DUMP_VERSION_NUMBER_MASK) == LKCD_DUMP_V9)
	 ...
	 */
	if (lseek(lkcd->fd, page_headers[0] + lkcd_offset_to_first_page, SEEK_SET) == -1)
		return;
	if (read(lkcd->fd, dp, lkcd->page_header_size) != lkcd->page_header_size)
		return;
	physaddr1 = (dp->dp_address - lkcd->kvbase) << lkcd->page_shift;

	if (lseek(lkcd->fd, page_headers[1] + lkcd_offset_to_first_page, SEEK_SET) == -1)
		return;
	...
	if ((physaddr1 % MEGABYTES(1)) || (physaddr2 % MEGABYTES(1)) || (physaddr2 < physaddr1))
		return;
