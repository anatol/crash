...
static int
processCFI(const u8 * start, const u8 * end, unsigned long targetLoc, signed ptrType, struct unwind_state *state)
{
	union {
		const u8 *p8;
		const u16 *p16;
	...
	return result && ptr.p8 == end && (targetLoc == 0 || (	/*todo While in theory this should apply, gcc in practice omits
...
int unwind(struct unwind_frame_info *frame, int is_ehframe)
{
...
//              unsigned long tableSize = unwind_table_size;
		...
		for (fde = unwind_table; tableSize > sizeof(*fde)
			...
			endLoc =
			    startLoc + read_pointer(&ptr,
						    ...
						    *fde,
						    ...
						    ? ptrType : ptrType & (DW_EH_PE_FORM | DW_EH_PE_signed));
...
#undef CASE
		...
		case Memory:{
				...
				if ((state.regs[i].value * state.dataAlign)
				    ...
				    || addr < startLoc
				    || addr + sizeof(unsigned long) < addr || addr + sizeof(unsigned long) > endLoc) {
					return -EIO;
				}
				switch (reg_info[i].width) {
#define CASE(n)     case sizeof(u##n): \
...
void init_unwind_table(void)
{
	...
	if (symbol_exists("__start_unwind") && symbol_exists("__end_unwind")) {
		...
		if (!readmem
		    (symbol_value("__start_unwind"), KVADDR, unwind_table,
 ...
 try_eh_frame:
	...
	if (st->dwarf_eh_frame_size || st->dwarf_debug_frame_size) {
		...
		unwind_table_size = is_ehframe ? st->dwarf_eh_frame_size : st->dwarf_debug_frame_size;
		...
		if (read(fd, unwind_table, unwind_table_size) != unwind_table_size) {
			if (CRASHDEBUG(1))
...
static int gather_in_memory_unwind_tables(void)
{
	...
	if (INVALID_SIZE(unwind_table) ||
	    ...
	    INVALID_MEMBER(unwind_table_link) || INVALID_MEMBER(unwind_table_name)) {
		if (CRASHDEBUG(1))
			error(NOTE, "unwind_table structure has changed, or does not exist in this kernel\n");
		return 0;
	}
	...
	for (i = found = 0; i < cnt; i++) {
		...
		if (!readmem(sp->value, KVADDR, root_table_buf,
			     SIZE(unwind_table), "root unwind_table", RETURN_ON_ERROR | QUIET))
			goto gather_failed;

		name = ULONG(root_table_buf + OFFSET(unwind_table_name));
		if (read_string(name, buf, strlen("kernel") + 1) && STREQ("kernel", buf)) {
			found++;
			if (CRASHDEBUG(1))
 ...
 */
static int populate_local_tables(ulong root, char *buf)
{
	...
	if (!(local_unwind_tables = malloc(sizeof(struct local_unwind_table) * cnt))) {
	...
	for (i = 0; i < cnt; i++, tp++) {
		...
		if (!readmem(table_list[i], KVADDR, buf, SIZE(unwind_table), "unwind_table", RETURN_ON_ERROR | QUIET)) {
		 ...
		 */
		BCOPY(buf + OFFSET(unwind_table_core), (char *)&tp->core.pc, sizeof(ulong) * 2);
		BCOPY(buf + OFFSET(unwind_table_init), (char *)&tp->init.pc, sizeof(ulong) * 2);
		BCOPY(buf + OFFSET(unwind_table_size), (char *)&tp->size, sizeof(ulong));
		...
		if (!readmem(vaddr, KVADDR, tp->address, tp->size, "DWARF CFI data", RETURN_ON_ERROR | QUIET)) {
 ...
 */
static struct local_unwind_table *find_table(unsigned long pc)
{
	int i;
	...
	for (i = 0; i < unwind_tables_cnt; i++, tp++) {
		tp = &local_unwind_tables[i];
		if ((pc >= tp->core.pc && pc < tp->core.pc + tp->core.range)
		    || (pc >= tp->init.pc && pc < tp->init.pc + tp->init.range)) {
			table = tp;
			break;
		}
	}
...
int dwarf_backtrace(struct bt_info *bt, int level, ulong stacktop)
{
	...
	frame = (struct unwind_frame_info *)
	    GETBUF(sizeof(struct unwind_frame_info));
...
//              readmem(frame->regs.rsp, KVADDR, &bp,
		readmem(UNW_SP(frame), KVADDR, &bp, sizeof(unsigned long), "reading bp", FAULT_ON_ERROR);
	...
	if (CRASHDEBUG(2))
		fprintf(fp, "    < SP: %lx PC: %lx FP: %lx >\n", UNW_SP(frame), UNW_PC(frame), frame->regs.rbp);
	...
	while ((UNW_SP(frame) < stacktop)
		...
		fprintf(fp, "%s#%d [%016lx] %s at %016lx \n",
			level < 10 ? " " : "", level, UNW_SP(frame), name, UNW_PC(frame));
...
void dwarf_debug(struct bt_info *bt)
{
	struct unwind_frame_info *frame;
	ulong bp;
	int is_ehframe = (!st->dwarf_debug_frame_size && st->dwarf_eh_frame_size);
