...
void cmd_extend(void)
{
	...
	while ((c = getopt(argcnt, args, "lu")) != EOF) {
		...
		case 'l':
			if (flag & UNLOAD_EXTENSION) {
				error(INFO, "-l and -u are mutually exclusive\n");
				argerrs++;
			} else
				flag |= LOAD_EXTENSION;
			break;
		...
		case 'u':
			if (flag & LOAD_EXTENSION) {
				error(INFO, "-u and -l are mutually exclusive\n");
				argerrs++;
			} else
	...
	case LOAD_EXTENSION:
		if (!args[optind]) {
			error(INFO, "-l requires one or more extension library arguments\n");
			cmd_usage(pc->curcmd, SYNOPSIS);
			break;
		}
 ...
 */
void dump_extension_table(int verbose)
{
	int i;
	...
	do {
		fprintf(fp, "%s  ", mkstring(buf, longest, LJUST, ext->filename));
...
void load_extension(char *lib)
{
	...
	size = sizeof(struct extension_table) + strlen(lib) + MAX(env_len, strlen("/usr/lib64/crash/extensions/")) + 1;
	...
	if (!ext->handle) {
		...
		if (strstr(buf, "undefined symbol: register_extension")) {
			error(INFO, "%s may be statically linked: ", pc->program_name);
	...
	if (!(ext->flags & REGISTERED)) {
		...
		if (ext->flags & (DUPLICATE_COMMAND_NAME | NO_MINIMAL_COMMANDS))
			error(INFO, "%s: shared object unloaded\n", ext->filename);
		else
			error(INFO, "%s: no commands registered: shared object unloaded\n", ext->filename);
		free(ext);
		return;
	}
 ...
 */
static int in_extensions_library(char *lib, char *buf)
{
	char *env;

	if ((env = getenv("CRASH_EXTENSIONS"))) {
		sprintf(buf, "%s%s%s", env, LASTCHAR(env) == '/' ? "" : "/", lib);
 ...
 */
static char *get_extensions_directory(char *dirbuf)
{
	char *env;
...
void preload_extensions(void)
{
	...
	for (found = 0, dp = readdir(dirp); dp != NULL; dp = readdir(dirp)) {
		sprintf(filename, "%s%s%s", dirbuf, LASTCHAR(dirbuf) == '/' ? "" : "/", dp->d_name);
	...
	if (found)
		...
		error(NOTE, "%s: no extension modules found in directory\n\n", dirbuf);
}

/*
 *  Unload all, or as specified, extension libraries.
 */
void unload_extension(char *lib)
{
	struct extension_table *ext;
	int found;
	char buf[BUFSIZE];
	...
	if (!lib) {
		while (extension_table) {
			...
			if (dlclose(ext->handle))
				error(FATAL, "dlclose: %s: shared object not open\n", ext->filename);
	...
	if ((*lib != '.') && (*lib != '/')) {
		if (!file_exists(lib, NULL) && in_extensions_library(lib, buf))
			lib = buf;
	}
	...
	for (ext = extension_table, found = FALSE; ext; ext = ext->next) {
		if (same_file(lib, ext->filename)) {
			found = TRUE;
			if (dlclose(ext->handle))
				error(INFO, "dlclose: %s: shared object not open\n", ext->filename);
			else {
				fprintf(fp, "%s: shared object unloaded\n", ext->filename);
 ...
 */
void register_extension(struct command_table_entry *command_table)
{
	struct command_table_entry *cp;
	...
	if ((pc->flags & MINIMAL_MODE)
	    && (pc->curext->flags & NO_MINIMAL_COMMANDS)) {
		error(INFO, "%s: does not contain any commands which support minimal mode\n", pc->curext->filename);
		return;
	}
