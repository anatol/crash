...
static void ppc_print_stack_entry(int, struct gnu_request *, ulong, ulong, struct bt_info *);
...
static int probe_ppc44x_platform(char *name)
{
...
static int fsl_booke_vtop(ulong vaddr, physaddr_t * paddr, int verbose)
{
	...
	for (i = 0, found = FALSE;; i++, fsl_mmu++) {
		if (vaddr >= fsl_mmu->tlbcamrange.start && vaddr < fsl_mmu->tlbcamrange.limit) {
			*paddr = fsl_mmu->tlbcamrange.phys + (vaddr - fsl_mmu->tlbcamrange.start);
			found = TRUE;
			break;
		}
	...
	if (found && verbose) {
		...
		fprintf(fp, "             %-8x %-8x %-8lx %-8x %-8x\n",
			...
			fsl_mmu->tlbcam.MAS2, fsl_mmu->tlbcam.MAS3, fsl_mmu->tlbcam.MAS7);
		/* TLBCAM range */
		...
		fprintf(fp, "             PHYSICAL RANGE: %llx - %llx\n",
			...
			fsl_mmu->tlbcamrange.phys + (fsl_mmu->tlbcamrange.limit - fsl_mmu->tlbcamrange.start));
		/* translated addr and its tlbcam's offset. */
		fprintf(fp, "  => VIRTUAL  PHYSICAL TLBCAM-OFFSET\n");
		fprintf(fp, "     %-8lx %-8llx %lu\n", vaddr, *paddr, vaddr - fsl_mmu->tlbcamrange.start);
	}
	if (CRASHDEBUG(1))
...
static void fsl_booke_mmu_setup(void)
{
	struct fsl_booke_tlbcam *fsl_mmu;
	uint i, tlbcam_index;
	ulong tlbcam_addrs, TLBCAM;

	readmem(symbol_value("tlbcam_index"), KVADDR, &tlbcam_index, sizeof(uint), "tlbcam_index", FAULT_ON_ERROR);
	if (tlbcam_index != 0 && tlbcam_index < NUM_TLBCAMS) {
		...
		for (i = 0; i < tlbcam_index; i++) {
			...
			readmem(tlbcam_addrs +
				...
				sizeof(fsl_mmu[i].tlbcamrange), "tlbcam_addrs", FAULT_ON_ERROR);
			readmem(TLBCAM + i * sizeof(fsl_mmu[i].tlbcam), KVADDR,
				&fsl_mmu[i].tlbcam, sizeof(fsl_mmu[i].tlbcam), "TLBCAM", FAULT_ON_ERROR);
		}
...
static int probe_ppce500_platform(char *name)
{
...
static int probe_default_platform(char *name)
{
	if (IS_PAE()) {
 ...
 */
void ppc_probe_base_platform(void)
{
	probe_func_t probe;
...
void ppc_init(int when)
{
	...
	case POST_GDB:
		...
		if (PTE_SIZE < 0)
			error(FATAL, "gdb could not handle \"pte_t\" size request\n");
		...
		if (symbol_exists("irq_desc"))
			...
			get_symbol_data("nr_irqs", sizeof(int), &machdep->nr_irqs);
		else
		...
		if (symbol_exists("cur_cpu_spec")) {
			...
			readmem(cur_cpu_spec +
				MEMBER_OFFSET("cpu_spec", "cpu_user_features"),
				KVADDR, &cpu_features, sizeof(uint), "cpu user features", FAULT_ON_ERROR);
...
void ppc_dump_machdep_table(ulong arg)
{
	int others;
...
static ulonglong ppc_pte_physaddr(ulonglong pte)
{
...
static int ppc_pgd_vtop(ulong * pgd, ulong vaddr, physaddr_t * paddr, int verbose)
{
	ulong *page_dir;
...
static int ppc_uvtop(struct task_context *tc, ulong vaddr, physaddr_t * paddr, int verbose)
{
	ulong mm, active_mm;
	ulong *pgd;
 ...
 * other callers quietly accept the translation.
 */
static int ppc_kvtop(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	ulong *pgd;
 ...
 */
static int ppc_is_task_addr(ulong task)
{
	int i;
 ...
 */
static int ppc_verify_symbol(const char *name, ulong value, char type)
{
 ...
 */
static ulong ppc_get_task_pgd(ulong task)
{
	long offset;
	ulong pg_tables;
 ...
 */
static int ppc_translate_pte(ulong pte32, void *physaddr, ulonglong pte64)
{
	...
	if (pte64) {
		if (_PAGE_PRESENT && (pte64 & _PAGE_PRESENT) == _PAGE_PRESENT)
			fprintf(fp, "%sPRESENT", others++ ? "|" : "");
		if (_PAGE_USER && (pte64 & _PAGE_USER) == _PAGE_USER)
			fprintf(fp, "%sUSER", others++ ? "|" : "");
		if (_PAGE_RW && (pte64 & _PAGE_RW) == _PAGE_RW)
			fprintf(fp, "%sRW", others++ ? "|" : "");
		if (_PAGE_K_RW && ((pte64 & _PAGE_K_RW) == _PAGE_K_RW))
			fprintf(fp, "%sK-RW", others++ ? "|" : "");
		if (_PAGE_GUARDED && (pte64 & _PAGE_GUARDED) == _PAGE_GUARDED)
			fprintf(fp, "%sGUARDED", others++ ? "|" : "");
		if (_PAGE_COHERENT && (pte64 & _PAGE_COHERENT) == _PAGE_COHERENT)
			fprintf(fp, "%sCOHERENT", others++ ? "|" : "");
		if (_PAGE_NO_CACHE && (pte64 & _PAGE_NO_CACHE) == _PAGE_NO_CACHE)
			fprintf(fp, "%sNO_CACHE", others++ ? "|" : "");
		if (_PAGE_WRITETHRU && (pte64 & _PAGE_WRITETHRU) == _PAGE_WRITETHRU)
			fprintf(fp, "%sWRITETHRU", others++ ? "|" : "");
		if (_PAGE_DIRTY && (pte64 & _PAGE_DIRTY) == _PAGE_DIRTY)
			fprintf(fp, "%sDIRTY", others++ ? "|" : "");
		if (_PAGE_ACCESSED && (pte64 & _PAGE_ACCESSED) == _PAGE_ACCESSED)
			fprintf(fp, "%sACCESSED", others++ ? "|" : "");
		if (_PAGE_HWWRITE && (pte64 & _PAGE_HWWRITE) == _PAGE_HWWRITE)
...
static int ppc_eframe_search(struct bt_info *bt)
{
	return (error(FATAL, "ppc_eframe_search: function not written yet!\n"));
}

static ulong ppc_in_irqstack(ulong addr)
{
	int c;
 ...
 */
static void ppc_back_trace_cmd(struct bt_info *bt)
{
	char buf[BUFSIZE];
 ...
 */
static void ppc_back_trace(struct gnu_request *req, struct bt_info *bt)
{
	int frame = 0;
	ulong lr = 0;
	...
	while (INSTACK(req->sp, bt)) {
		...
		if (IS_KVADDR(newsp) && INSTACK(newsp, bt))
			newpc = *(ulong *) & bt->stackbuf[newsp + STACK_FRAME_LR_SAVE - bt->stackbase];
		if ((req->name = closest_symbol(req->pc)) == NULL) {
			error(FATAL, "ppc_back_trace hit unknown symbol (%lx).\n", req->pc);
			break;
		}
		 ...
		 */
		if (THIS_KERNEL_VERSION < LINUX(2, 6, 0)) {
			if (frame && (newsp - req->sp - STACK_FRAME_OVERHEAD >= sizeof(struct ppc_pt_regs)))
			...
			/* also possible ones here... */
				...
				eframe_found = TRUE;
		} else if ((newsp - req->sp - STACK_FRAME_OVERHEAD) >= sizeof(struct ppc_pt_regs)) {
		...
		if (eframe_found) {
			...
			readmem(req->sp + STACK_FRAME_OVERHEAD, KVADDR, &regs,
				sizeof(struct ppc_pt_regs), "exception frame", FAULT_ON_ERROR);
...
static void ppc_display_full_frame(struct bt_info *bt, ulong nextsp, FILE * ofp)
{
	int i, u_idx;
	ulong *nip;
	ulong words, addr;
 ...
 */
static void ppc_print_stack_entry(int frame, struct gnu_request *req, ulong newsp, ulong lr, struct bt_info *bt)
{
 ...
 */
static char *ppc_check_eframe(struct ppc_pt_regs *regs)
{
...
static void ppc_print_regs(struct ppc_pt_regs *regs)
{
	int i;
	...
	for (i = 0; i < 32; i++) {
		...
		fprintf(fp, "R%d:%s %08lx   ", i, ((i < 10) ? " " : ""), regs->gpr[i]);
		/*
 ...
 */
static void ppc_print_eframe(char *efrm_str, struct ppc_pt_regs *regs, struct bt_info *bt)
{
	if (BT_REFERENCE_CHECK(bt))
		return;
...
static void ppc_kdump_stack_frame(struct bt_info *bt, ulong * nip, ulong * ksp)
{
...
static void ppc_dumpfile_stack_frame(struct bt_info *bt, ulong * getpc, ulong * getsp)
{
	struct syment *sp;
 ...
 */
static void ppc_get_stack_frame(struct bt_info *bt, ulong * pcp, ulong * spp)
{
 ...
 */
static void get_ppc_frame(struct bt_info *bt, ulong * getpc, ulong * getsp)
{
	ulong ip;
	ulong sp;
	ulong *stack;
	...
	readmem(sp + STACK_FRAME_OVERHEAD, KVADDR, &regs, sizeof(struct ppc_pt_regs), "PPC pt_regs", FAULT_ON_ERROR);
	ip = regs.nip;
 ...
 */
static int ppc_dis_filter(ulong vaddr, char *inbuf, unsigned int output_radix)
{
	char buf1[BUFSIZE];
	char buf2[BUFSIZE];
	char *colon, *p1;
...
static void ppc_dump_line_number(ulong callpc)
{
	int retries;
	char buf[BUFSIZE], *p;
 ...
 */
static int verify_crash_note_in_kernel(int cpu)
{
	int ret;
	Elf32_Nhdr *note32;
	...
	if (!readmem(symbol_value("crash_notes"), KVADDR, &crash_notes_ptr,
		     sizeof(ulong), "crash_notes", QUIET | RETURN_ON_ERROR) || !crash_notes_ptr)
		goto out;

	buf = GETBUF(SIZE(note_buf));
	if ((kt->flags & SMP) && (kt->flags & PER_CPU_OFF))
		crash_notes_ptr += kt->__per_cpu_offset[cpu];
	if (!readmem(crash_notes_ptr, KVADDR, buf, SIZE(note_buf), "cpu crash_notes", QUIET | RETURN_ON_ERROR))
		goto freebuf;
	...
	if (note32->n_type != NT_PRSTATUS ||
	    ...
	    strncmp(name, "CORE", note32->n_namesz) || note32->n_descsz != SIZE(elf_prstatus))
		ret = FALSE;
 freebuf:
	FREEBUF(buf);
 out:
...
void ppc_relocate_nt_prstatus_percpu(void **nt_prstatus_percpu, uint * num_prstatus_notes)
{
	...
	if (relocated == TRUE)
		return;
	relocated = TRUE;
	if (!symbol_exists("crash_notes") || !VALID_STRUCT(note_buf) || !VALID_STRUCT(elf_prstatus))
		return;
