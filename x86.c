...
static void db_nextframe(struct i386_frame **, db_addr_t *, struct eframe *, struct bt_info *);
...
static int mach_CRASHDEBUG(ulong dval)
{
	if (CRASHDEBUG(dval))
		return TRUE;
...
static void db_print_stack_entry(char *, int, char **, int *,
				 db_addr_t, struct bt_info *, struct eframe *, int, struct i386_frame *);
#else
...
#ifdef MCLX
static int eframe_numargs(ulong eip, struct bt_info *bt)
{
	int inst;
	int args;
...
#endif
{
	...
	if (!name) {
		if (IS_MODULE_VADDR(callpc) && module_symbol(callpc, NULL, NULL, buf1, *gdb_output_radix)) {
			sprintf(buf2, "(%s)", buf1);
			name = buf2;
		} else
			name = "(unknown module)";
	...
	if (BT_REFERENCE_CHECK(bt)) {
		...
		case BT_REF_SYMBOL:
			if (ep->eframe_found && ep->jmp_error_code_eip) {
				if (STREQ(closest_symbol(ep->jmp_error_code_eip), bt->ref->str)
		...
		case BT_REF_HEXVAL:
			if (ep->eframe_found && ep->jmp_error_code_eip && (bt->ref->hexval == ep->jmp_error_code_eip))
				bt->ref->cmdflags |= BT_REF_FOUND;
		...
		fprintf(fp, "%s#%d [%08lx] ", fnum < 10 ? " " : "", fnum, (ulong) frame);

		if (ep->eframe_found && ep->jmp_error_code_eip)
			fprintf(fp, "%s (via %s)", closest_symbol(callpc), closest_symbol(ep->jmp_error_code_eip));
		else
			fprintf(fp, "%s", name);
	...
	while (narg) {
		...
		if ((sp = value_symbol(arg)))
			fprintf(fp, "%s", sp);
		else if ((bt->flags & BT_SYMBOLIC_ARGS) && strlen(value_to_symstr(arg, buf1, 0)))
			fprintf(fp, "%s", buf1);
		else
			fprintf(fp, "%x", arg);
...
#ifdef MCLX
static db_sym_t db_search_symbol(db_addr_t val, db_strategy_t strategy, db_expr_t * offp)
{
	struct syment *sp;
	ulong offset;
 ...
 */
static void db_symbol_values(db_sym_t sym, char **namep, db_expr_t * valuep)
{
	struct syment *sp;
...
static db_expr_t db_get_value(addr, size, is_signed, bt)
db_addr_t addr;
int size;
boolean_t is_signed;
struct bt_info *bt;
{
	...
	if (INSTACK(addr, bt)) {
		...
		if ((size == sizeof(int)) && text_value_cache(addr, 0, (uint32_t *) & value))
			return value;

		if (!readmem(addr, KVADDR, &value, size, "db_get_value", RETURN_ON_ERROR))
...
static int db_sym_numargs(db_sym_t sym, int *nargp, char **argnames)
{
	return FALSE;
}
 ...
 */
#ifdef MCLX
static void db_nextframe(fp, ip, ep, bt)
struct i386_frame **fp;		/* in/out */
db_addr_t *ip;			/* out */
struct eframe *ep;
struct bt_info *bt;
#else
static void db_nextframe(fp, ip)
struct i386_frame **fp;		/* in/out */
db_addr_t *ip;			/* out */
#endif
{
	...
	case SIGNAL_RETURN:
		...
		ep->eframe_addr = x86_next_eframe(last_ebp + sizeof(ulong) * 2, bt);
		break;

	case IDT_JMP_ERROR_CODE:
		ep->eframe_found = TRUE;
		ep->eframe_addr = x86_next_eframe(last_ebp + sizeof(ulong) * 4, bt);
	...
	*fp = (struct i386_frame *)ebp;
	...
	case TRAP:
		if (INKERNEL((int)tf)) {
			eip = tf->tf_eip;
			ebp = tf->tf_ebp;
			db_printf("--- trap %#r, eip = %#r, esp = %#r, ebp = %#r ---\n", tf->tf_trapno, eip, esp, ebp);
		}
		break;
	case SYSCALL:
		if (INKERNEL((int)tf)) {
			eip = tf->tf_eip;
			ebp = tf->tf_ebp;
			db_printf("--- syscall %#r, eip = %#r, esp = %#r, ebp = %#r ---\n", tf->tf_eax, eip, esp, ebp);
		}
		break;
	case INTERRUPT:
		...
		if (INKERNEL((int)tf)) {
			eip = tf->tf_eip;
			ebp = tf->tf_ebp;
			db_printf("--- interrupt, eip = %#r, esp = %#r, ebp = %#r ---\n", eip, esp, ebp);
		}
		break;
	default:
		break;
...
#ifdef MCLX
void x86_back_trace_cmd(struct bt_info *bt)
#else
ulong db_stack_trace_cmd(addr, have_addr, count, modif, task, flags)
db_expr_t addr;
boolean_t have_addr;
db_expr_t count;
char *modif;
...
#endif				/* MCLX */
{
	...
	if (!(bt->flags & BT_USER_SPACE) && (!bt->stkptr || !accessible(bt->stkptr))) {
	...
	if ((machdep->flags & OMIT_FRAME_PTR) || bt->debug || (bt->flags & BT_FRAMESIZE_DEBUG)
	...
	if (!have_addr) {
		...
		callpc = (db_addr_t) db_get_value((int)&frame->f_retaddr, 4, FALSE, bt);
		if (modif) {
			frame_number = 0;
			forced = TRUE;
			...
			if (!is_kernel_text(callpc))
				error(INFO, "callpc from stack is not a text address\n");
		}
	}
...
#define MAXNARG	16
		...
		if (is_kernel_text(callpc) || IS_MODULE_VADDR(callpc)) {
			if (mach_CRASHDEBUG(2))
				fprintf(fp, "--> (1) lastframe: %lx => frame: %lx\n", lastframe, (ulong) frame);

			db_print_stack_entry(name, narg, argnp, argp, callpc, bt, ep, frame_number++, frame);
			...
			if (INSTACK(frame, bt) && ((ulong) frame > lastframe))
				lastframe = (ulong) frame;
			...
			if (!(forced && frame_number == 1)) {
				if (is_kernel_data(callpc)) {
					if (mach_CRASHDEBUG(2))
						fprintf(fp, "--> break(1): callpc %lx is data?\n", callpc);
					if (!is_rodata_text(callpc))
						break;
				}

				if (mach_CRASHDEBUG(2))
					fprintf(fp, "--> (2) lastframe: %lx => frame: %lx\n", lastframe, (ulong) frame);
		...
		if (!INSTACK(frame, bt)) {
			if (mach_CRASHDEBUG(2))
				fprintf(fp, "--> break: !INSTACK(frame: %lx, task: %lx)\n", (ulong) frame, bt->task);
			break;
		}
#else
		db_print_stack_entry(name, narg, argnp, argp, callpc);
		...
		if (actframe != frame) {
			...
			callpc = (db_addr_t)
			    db_get_value((int)&actframe->f_retaddr, 4, FALSE, bt);
			continue;
		}
 ...
 skip_frame:
		...
		if (mach_CRASHDEBUG(2)) {
			fprintf(fp,
				...
				(ulong) frame, callpc, value_to_symstr(callpc, dbuf, 0));
			if (callpc == last_callpc)
				fprintf(fp, "last callpc == callpc!\n");
		}

		if ((callpc == last_callpc) && STREQ(closest_symbol(callpc), "smp_stop_cpu_interrupt"))
			goto skip_frame;

		if (INSTACK(frame, bt) && ((ulong) frame < lastframe))
			if (mach_CRASHDEBUG(2))
				fprintf(fp, "--> frame pointer reversion?\n");
		...
		if (INKERNEL((int)callpc) && !INKERNEL((int)frame)) {
			...
			if (is_kernel_data(callpc)) {
				if (mach_CRASHDEBUG(2))
					fprintf(fp, "--> break(2): callpc %lx is data?\n", callpc);
				if (!is_rodata_text(callpc))
					break;
			}

			if (mach_CRASHDEBUG(2))
				fprintf(fp, "--> (3) lastframe: %lx => frame: %lx\n", lastframe, (ulong) frame);

			db_print_stack_entry(name, 0, 0, 0, callpc, bt, ep, frame_number++, frame);
			...
			if (mach_CRASHDEBUG(2))
				fprintf(fp,
					...
					callpc, value_to_symstr(callpc, dbuf, 0), (ulong) frame);
			break;
		}
		if (!INKERNEL((int)frame)) {
			if (mach_CRASHDEBUG(2))
				fprintf(fp, "--> break: !INKERNEL(frame: %lx)\n", (ulong) frame);
			break;
		}
	}
...
static int dump_eframe(struct eframe *ep, int frame_number, struct bt_info *bt)
{
	int i;
	char buf[BUFSIZE], *sp;
	...
	GET_STACK_DATA(ep->eframe_addr, (char *)int_eframe, SIZE(pt_regs));

	if (int_eframe[INT_EFRAME_CS] & DPL_BITS) {
		if (!INSTACK(ep->eframe_addr + SIZE(pt_regs) - 1, bt))
			return (frame_number);
		/* error(FATAL, "read of exception frame would go beyond stack\n"); */
		eframe_type = EFRAME_USER;
	} else {
		if (!INSTACK(ep->eframe_addr + (KERNEL_EFRAME_SIZE * sizeof(ulong)) - 1, bt))
			return (frame_number);
		/* error(FATAL, "read of exception frame would go beyond stack\n"); */
	...
	fprintf(fp, "%s#%d [%08lx] %s at %08lx\n",
		...
		value_to_symstr(int_eframe[INT_EFRAME_EIP], buf, 0), int_eframe[INT_EFRAME_EIP]);
	...
	args = is_system_call(NULL, value) ? 4 : eframe_numargs(int_eframe[INT_EFRAME_EIP], bt);
	...
	for (i = 0; i < args; i++, argp++) {
		if (INSTACK(argp, bt))
			...
			readmem((ulong) argp, KVADDR, &value, sizeof(ulong), "syscall arg", FAULT_ON_ERROR);
		...
		if ((sp = value_symbol(value)))
			fprintf(fp, "%s", sp);
		else if ((bt->flags & BT_SYMBOLIC_ARGS) && strlen(value_to_symstr(value, buf, 0)))
			fprintf(fp, "%s", buf);
		else
			fprintf(fp, "%lx", value);
	}
 ...
 */
void x86_dump_eframe_common(struct bt_info *bt, ulong * int_eframe, int kernel)
{
	struct syment *sp;
	ulong offset;
	...
	if (bt && BT_REFERENCE_CHECK(bt)) {
		...
		if (!kernel) {
			if ((int_eframe[INT_EFRAME_ESP] == bt->ref->hexval) ||
			    ((short)int_eframe[INT_EFRAME_SS] == (short)bt->ref->hexval))
				bt->ref->cmdflags |= BT_REF_FOUND;
		}
	...
	if (kernel) {
		if (bt && (bt->flags & BT_EFRAME_SEARCH)) {
			...
			if ((sp = value_search(int_eframe[INT_EFRAME_EIP], &offset))) {
		...
		fprintf(fp,
			...
			int_eframe[INT_EFRAME_ECX], int_eframe[INT_EFRAME_EDX], int_eframe[INT_EFRAME_EBP]);
	} else
		fprintf(fp,
			"    EAX: %08lx  EBX: %08lx  ECX: %08lx  EDX: %08lx \n",
			int_eframe[INT_EFRAME_EAX],
			int_eframe[INT_EFRAME_EBX], int_eframe[INT_EFRAME_ECX], int_eframe[INT_EFRAME_EDX]);

	fprintf(fp,
		"    DS:  %04x      ESI: %08lx  ES:  %04x      EDI: %08lx",
		(short)int_eframe[INT_EFRAME_DS],
		int_eframe[INT_EFRAME_ESI], (short)int_eframe[INT_EFRAME_ES], int_eframe[INT_EFRAME_EDI]);
	...
	if (!kernel) {
		fprintf(fp, "    SS:  %04x      ESP: %08lx  EBP: %08lx",
			(short)int_eframe[INT_EFRAME_SS], int_eframe[INT_EFRAME_ESP], int_eframe[INT_EFRAME_EBP]);
		if (INT_EFRAME_GS != -1)
			fprintf(fp, "  GS:  %04x", (short)int_eframe[INT_EFRAME_GS]);
		fprintf(fp, "\n");
	}
	...
	fprintf(fp,
		...
		(short)int_eframe[INT_EFRAME_CS],
		int_eframe[INT_EFRAME_EIP], int_eframe[INT_EFRAME_ERR], int_eframe[INT_EFRAME_EFLAGS]);
}
 ...
 *  functions.
 */
int is_rodata_text(ulong callpc)
{
	struct syment *sp;
...
static int check_for_eframe(char *name, struct bt_info *bt)
{
	int i;
	ulong *ip;
	char buf[BUFSIZE];
	...
	if (STREQ(name, "ret_from_intr") ||
	    ...
	    STREQ(name, "call_reschedule_interrupt") || STREQ(name, "call_invalidate_interrupt"))
 ...
 */
struct syment *x86_jmp_error_code(ulong callpc)
{
	struct syment *sp;
...
static void x86_dump_line_number(ulong callpc)
{
	int retries;
	char buf[BUFSIZE], *p;
...
static ulong x86_next_eframe(ulong addr, struct bt_info *bt)
{
	...
	for (; first <= last; first++) {
		...
		if (((short)pt->reg_value[INT_EFRAME_CS] == 0x10) &&
		    ...
		    ((short)pt->reg_value[INT_EFRAME_ES] == 0x18) && IS_KVADDR(pt->reg_value[INT_EFRAME_EIP])) {
			if (!(machdep->flags & OMIT_FRAME_PTR) && !INSTACK(pt->reg_value[INT_EFRAME_EBP], bt))
				continue;
		...
		if (((short)pt->reg_value[INT_EFRAME_CS] == 0x60) &&
		    ...
		    ((short)pt->reg_value[INT_EFRAME_ES] == 0x68) && IS_KVADDR(pt->reg_value[INT_EFRAME_EIP])) {
			if (!(machdep->flags & OMIT_FRAME_PTR) && !INSTACK(pt->reg_value[INT_EFRAME_EBP], bt))
				continue;
			rv = bt->stackbase + sizeof(ulong) * (first - stack);
			break;
		...
		if (((short)pt->reg_value[INT_EFRAME_CS] == 0x60) &&
		    ...
		    ((short)pt->reg_value[INT_EFRAME_ES] == 0x7b) && IS_KVADDR(pt->reg_value[INT_EFRAME_EIP])) {
			if (!(machdep->flags & OMIT_FRAME_PTR) && !INSTACK(pt->reg_value[INT_EFRAME_EBP], bt))
				continue;
			rv = bt->stackbase + sizeof(ulong) * (first - stack);
			break;
		...
		if (((short)pt->reg_value[INT_EFRAME_CS] == 0x73) &&
		    ...
		    ((short)pt->reg_value[INT_EFRAME_ES] == 0x7b) &&
		    ((short)pt->reg_value[INT_EFRAME_SS] == 0x7b) &&
		    (pt->reg_value[INT_EFRAME_EFLAGS] == 0x246) && IS_UVADDR(pt->reg_value[INT_EFRAME_ESP], bt->tc)) {
			rv = bt->stackbase + sizeof(ulong) * (first - stack);
			break;
		}
	}
...
static int x86_eframe_search(struct bt_info *bt_in)
{
	ulong addr;
	struct x86_pt_regs *pt;
	struct eframe eframe, *ep;
	...
	if (bt->hp && bt->hp->esp) {
		...
		addr = bt->stackbase + roundup(SIZE(thread_info), sizeof(ulong));
	else
		addr = bt->stackbase + roundup(SIZE(task_struct), sizeof(ulong));
	...
	while ((addr = x86_next_eframe(addr, bt)) != 0) {
		...
		pt = (struct x86_pt_regs *)(bt->stackbuf + (addr - bt->stackbase));
		ep->eframe_addr = addr;
...
static ulong x86_in_irqstack(ulong addr)
{
	int c;
	...
	for (c = 0; c < NR_CPUS; c++) {
		if (tt->hardirq_ctx[c]) {
			if ((addr >= tt->hardirq_ctx[c]) && (addr < (tt->hardirq_ctx[c] + SIZE(irq_ctx))))
				return (tt->hardirq_ctx[c]);

		}
		if (tt->softirq_ctx[c]) {
			if ((addr >= tt->softirq_ctx[c]) && (addr < (tt->softirq_ctx[c] + SIZE(irq_ctx))))
 ...
 */
static void x86_user_eframe(struct bt_info *bt)
{
	struct eframe eframe, *ep;
	...
	readmem(pt_regs_addr, KVADDR, &x86_pt_regs, sizeof(struct x86_pt_regs), "x86 pt_regs", FAULT_ON_ERROR);
	...
	if (((short)pt->reg_value[INT_EFRAME_CS] == 0x23) &&
	    ...
	    ((short)pt->reg_value[INT_EFRAME_SS] == 0x2b) &&
	    IS_UVADDR(pt->reg_value[INT_EFRAME_EIP], bt->tc) &&
	    IS_UVADDR(pt->reg_value[INT_EFRAME_ESP], bt->tc) && IS_UVADDR(pt->reg_value[INT_EFRAME_EBP], bt->tc)) {
		ep = &eframe;
		BZERO(ep, sizeof(struct eframe));
...
void x86_init(int when)
{
	...
	case PRE_GDB:
		if (symbol_exists("pae_pgd_cachep") ||
		    ((sp = symbol_search("pkmap_count")) &&
		     (spn = next_symbol(NULL, sp)) && (((spn->value - sp->value) / sizeof(int)) == 512))) {
			machdep->flags |= PAE;
	...
	case POST_GDB:
		...
		if (MEMBER_EXISTS("user_regs_struct", "ebp"))
			MEMBER_OFFSET_INIT(user_regs_struct_ebp, "user_regs_struct", "ebp");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_ebp, "user_regs_struct", "bp");
		if (MEMBER_EXISTS("user_regs_struct", "esp"))
			MEMBER_OFFSET_INIT(user_regs_struct_esp, "user_regs_struct", "esp");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_esp, "user_regs_struct", "sp");
		if (MEMBER_EXISTS("user_regs_struct", "eip"))
			MEMBER_OFFSET_INIT(user_regs_struct_eip, "user_regs_struct", "eip");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_eip, "user_regs_struct", "ip");
		if (MEMBER_EXISTS("user_regs_struct", "eax"))
			MEMBER_OFFSET_INIT(user_regs_struct_eax, "user_regs_struct", "eax");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_eax, "user_regs_struct", "ax");
		if (MEMBER_EXISTS("user_regs_struct", "ebx"))
			MEMBER_OFFSET_INIT(user_regs_struct_ebx, "user_regs_struct", "ebx");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_ebx, "user_regs_struct", "bx");
		if (MEMBER_EXISTS("user_regs_struct", "ecx"))
			MEMBER_OFFSET_INIT(user_regs_struct_ecx, "user_regs_struct", "ecx");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_ecx, "user_regs_struct", "cx");
		if (MEMBER_EXISTS("user_regs_struct", "edx"))
			MEMBER_OFFSET_INIT(user_regs_struct_edx, "user_regs_struct", "edx");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_edx, "user_regs_struct", "dx");
		if (MEMBER_EXISTS("user_regs_struct", "esi"))
			MEMBER_OFFSET_INIT(user_regs_struct_esi, "user_regs_struct", "esi");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_esi, "user_regs_struct", "si");
		if (MEMBER_EXISTS("user_regs_struct", "edi"))
			MEMBER_OFFSET_INIT(user_regs_struct_edi, "user_regs_struct", "edi");
		else
			MEMBER_OFFSET_INIT(user_regs_struct_edi, "user_regs_struct", "di");
		...
		MEMBER_OFFSET_INIT(user_regs_struct_ds, "user_regs_struct", "ds");
		MEMBER_OFFSET_INIT(user_regs_struct_es, "user_regs_struct", "es");
		MEMBER_OFFSET_INIT(user_regs_struct_fs, "user_regs_struct", "fs");
		MEMBER_OFFSET_INIT(user_regs_struct_gs, "user_regs_struct", "gs");
		...
		if (!VALID_STRUCT(user_regs_struct)) {
			...
			ASSIGN_SIZE(user_regs_struct) = sizeof(struct x86_user_regs_struct);
			ASSIGN_OFFSET(user_regs_struct_ebp) = offsetof(struct x86_user_regs_struct, ebp);
			ASSIGN_OFFSET(user_regs_struct_esp) = offsetof(struct x86_user_regs_struct, esp);
			ASSIGN_OFFSET(user_regs_struct_eip) = offsetof(struct x86_user_regs_struct, eip);
			ASSIGN_OFFSET(user_regs_struct_eax) = offsetof(struct x86_user_regs_struct, eax);
			ASSIGN_OFFSET(user_regs_struct_ebx) = offsetof(struct x86_user_regs_struct, ebx);
			ASSIGN_OFFSET(user_regs_struct_ecx) = offsetof(struct x86_user_regs_struct, ecx);
			ASSIGN_OFFSET(user_regs_struct_edx) = offsetof(struct x86_user_regs_struct, edx);
			ASSIGN_OFFSET(user_regs_struct_esi) = offsetof(struct x86_user_regs_struct, esi);
			ASSIGN_OFFSET(user_regs_struct_edi) = offsetof(struct x86_user_regs_struct, edi);
			ASSIGN_OFFSET(user_regs_struct_eflags) = offsetof(struct x86_user_regs_struct, eflags);
			ASSIGN_OFFSET(user_regs_struct_cs) = offsetof(struct x86_user_regs_struct, cs);
			ASSIGN_OFFSET(user_regs_struct_ds) = offsetof(struct x86_user_regs_struct, ds);
			ASSIGN_OFFSET(user_regs_struct_es) = offsetof(struct x86_user_regs_struct, es);
			ASSIGN_OFFSET(user_regs_struct_fs) = offsetof(struct x86_user_regs_struct, fs);
			ASSIGN_OFFSET(user_regs_struct_gs) = offsetof(struct x86_user_regs_struct, gs);
			ASSIGN_OFFSET(user_regs_struct_ss) = offsetof(struct x86_user_regs_struct, ss);
		}
		...
		if (machdep->flags & PAE) {
			if (THIS_KERNEL_VERSION < LINUX(2, 6, 26))
				machdep->section_size_bits = _SECTION_SIZE_BITS_PAE_ORIG;
			else
				machdep->section_size_bits = _SECTION_SIZE_BITS_PAE_2_6_26;
		...
		if (XEN()) {
			...
			MEMBER_OFFSET_INIT(cpu_user_regs_esp, "cpu_user_regs", "esp");
			MEMBER_OFFSET_INIT(cpu_user_regs_eip, "cpu_user_regs", "eip");
		}
...
static int remap_init(void)
{
	...
	max_numnodes = get_array_length("node_remap_start_pfn", NULL, sizeof(ulong));
	if (max_numnodes < 1)
		max_numnodes = 1;
	...
	readmem(start_vaddr, KVADDR, ms->remap_start_vaddr,
		max_numnodes * sizeof(ulong), "node_remap_start_vaddr", FAULT_ON_ERROR);
	readmem(end_vaddr, KVADDR, ms->remap_end_vaddr,
		max_numnodes * sizeof(ulong), "node_remap_end_vaddr", FAULT_ON_ERROR);
...
static int x86_kvtop_remap(ulong kvaddr, physaddr_t * paddr)
{
	struct machine_specific *ms;
	int i;
	...
	for (i = 0; i < ms->max_numnodes; ++i) {
		if (kvaddr >= ms->remap_start_vaddr[i]
			...
			*paddr = PTOB(ms->remap_start_pfn[i]) + kvaddr - ms->remap_start_vaddr[i];
			return TRUE;
		}
	}
	return FALSE;
 ...
 */
static int x86_is_uvaddr(ulong vaddr, struct task_context *tc)
{
...
static int x86_uvtop(struct task_context *tc, ulong vaddr, physaddr_t * paddr, int verbose)
{
	ulong mm, active_mm;
	ulong *pgd;
	ulong *page_dir;
	...
	*paddr = 0;
	...
	if (pgd_pte & _PAGE_4M) {
		if (verbose) {
			fprintf(fp, " PAGE: %s  (4MB)\n\n",
				mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX, MKSTR(NONPAE_PAGEBASE(pgd_pte))));
	...
	page_table = (ulong *) (PTOV(NONPAE_PAGEBASE(pmd_pte)) + ((vaddr >> 10) & ((PTRS_PER_PTE - 1) << 2)));

	FILL_PTBL(NONPAE_PAGEBASE(page_table), KVADDR, PAGESIZE());
	pte = ULONG(machdep->ptbl + PAGEOFFSET(page_table));
#else
	page_table = (ulong *) ((NONPAE_PAGEBASE(pmd_pte)) + ((vaddr >> 10) & ((PTRS_PER_PTE - 1) << 2)));
	...
	*paddr = NONPAE_PAGEBASE(pte) + PAGEOFFSET(vaddr);
	...
	if (verbose) {
		fprintf(fp, " PAGE: %s\n\n", mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX, MKSTR(NONPAE_PAGEBASE(pte))));
		x86_translate_pte(pte, 0, 0);
	}
...
static int x86_uvtop_xen_wpt(struct task_context *tc, ulong vaddr, physaddr_t * paddr, int verbose)
{
	ulong mm, active_mm;
	ulong *pgd;
	ulong *page_dir;
	...
	*paddr = 0;
	...
	if (pgd_pte & _PAGE_4M) {
		...
		if (verbose) {
			fprintf(fp, " PAGE: %s  (4MB)\n\n",
				mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX, MKSTR(NONPAE_PAGEBASE(pseudo_pgd_pte))));

			x86_translate_pte(pseudo_pgd_pte, 0, 0);
		}

		*paddr = NONPAE_PAGEBASE(pseudo_pgd_pte) + (vaddr & ~_4MB_PAGE_MASK);
	...
	machine_page_table = (ulong *) ((NONPAE_PAGEBASE(pmd_pte)) + ((vaddr >> 10) & ((PTRS_PER_PTE - 1) << 2)));
	...
	if (verbose) {
		fprintf(fp, "  PTE: %s [machine]\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX, MKSTR((ulong) machine_page_table)));

		fprintf(fp, "  PTE: %s => %lx\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX,
				 MKSTR((ulong) pseudo_page_table + PAGEOFFSET(machine_page_table))), machine_pte);
	}
	...
	*paddr = NONPAE_PAGEBASE(pseudo_pte) + PAGEOFFSET(vaddr);
	...
	if (verbose) {
		fprintf(fp, " PAGE: %s [machine]\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX, MKSTR(NONPAE_PAGEBASE(machine_pte))));

		fprintf(fp, " PAGE: %s\n\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX, MKSTR(NONPAE_PAGEBASE(pseudo_pte))));
...
static int x86_uvtop_PAE(struct task_context *tc, ulong vaddr, physaddr_t * paddr, int verbose)
{
	ulong mm, active_mm;
	ulonglong *pgd;
	...
	*paddr = 0;
	...
	if (is_kernel_thread(tc->task) && IS_KVADDR(vaddr)) {
		...
		if ((mm = task_mm(tc->task, TRUE)))
			pgd = (ulonglong *) (ULONG_PTR(tt->mm_struct + OFFSET(mm_struct_pgd)));
		else
			readmem(tc->mm_struct + OFFSET(mm_struct_pgd),
				KVADDR, &pgd, sizeof(long), "mm_struct pgd", FAULT_ON_ERROR);
	}
	...
	offset = ((vaddr >> PGDIR_SHIFT) & (PTRS_PER_PGD - 1)) * sizeof(ulonglong);

	page_dir_entry = *((ulonglong *) & machdep->pgd[offset]);

	if (verbose)
		fprintf(fp, "  PGD: %s => %llx\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONG_HEX, MKSTR((ulong) pgd + offset)), page_dir_entry);
	...
	if (verbose) {
		...
		fprintf(fp, "  PMD: %s => %llx\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONGLONG_HEX, MKSTR(&ull)), page_middle_entry);
	}
	...
	if (page_middle_entry & _PAGE_PSE) {
		if (verbose) {
			...
			fprintf(fp, " PAGE: %s  (2MB)\n\n",
				mkstring(buf, VADDR_PRLEN, RJUST | LONGLONG_HEX, MKSTR(&ull)));
			x86_translate_pte(0, 0, page_middle_entry);
		}

		physpage = PAE_PAGEBASE(page_middle_entry) + (vaddr & ~_2MB_PAGE_MASK);
		*paddr = physpage;
	...
	offset = ((vaddr >> PAGESHIFT()) & (PTRS_PER_PTE - 1)) * sizeof(ulonglong);
	...
	if (verbose) {
		...
		fprintf(fp, "  PTE: %s => %llx\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONGLONG_HEX, MKSTR(&ull)), page_table_entry);
	}
...
static int x86_uvtop_xen_wpt_PAE(struct task_context *tc, ulong vaddr, physaddr_t * paddr, int verbose)
{
	ulong mm, active_mm;
	ulonglong *pgd;
	ulonglong page_dir_entry;
	...
	*paddr = 0;
	...
	if (verbose)
		fprintf(fp, "  PGD: %s => %llx\n",
			...
			pseudo_page_middle | PAGEOFFSET(page_dir_entry) | (page_dir_entry & _PAGE_NX));
	...
	if (verbose) {
		...
		fprintf(fp, "  PMD: %s => %llx\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONGLONG_HEX,
				 MKSTR(&ull)),
			pseudo_page_table | PAGEOFFSET(page_middle_entry) | (page_middle_entry & _PAGE_NX));
	}
	...
	if (verbose) {
		...
		fprintf(fp, "  PTE: %s => %llx\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONGLONG_HEX,
				 MKSTR(&ull)),
			pseudo_physpage | PAGEOFFSET(page_table_entry) | (page_table_entry & _PAGE_NX));
	}
	...
	*paddr = pseudo_physpage + PAGEOFFSET(vaddr);
	...
	if (verbose) {
		...
		fprintf(fp, " PAGE: %s [machine]\n",
			mkstring(buf, VADDR_PRLEN, RJUST | LONGLONG_HEX, MKSTR(&physpage)));

		fprintf(fp, " PAGE: %s\n\n", mkstring(buf, VADDR_PRLEN, RJUST | LONGLONG_HEX, MKSTR(&pseudo_physpage)));

		pte = pseudo_physpage | PAGEOFFSET(page_table_entry) | (page_table_entry & _PAGE_NX);
...
static int x86_kvtop(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	ulong *pgd;
	ulong *page_dir;
	ulong *page_middle;
...
#ifdef PTES_IN_LOWMEM
	page_table = (ulong *) (PTOV(NONPAE_PAGEBASE(pmd_pte)) + ((kvaddr >> 10) & ((PTRS_PER_PTE - 1) << 2)));

	FILL_PTBL(NONPAE_PAGEBASE(page_table), KVADDR, PAGESIZE());
	pte = ULONG(machdep->ptbl + PAGEOFFSET(page_table));
#else
	page_table = (ulong *) ((NONPAE_PAGEBASE(pmd_pte)) + ((kvaddr >> 10) & ((PTRS_PER_PTE - 1) << 2)));
...
static int x86_kvtop_xen_wpt(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	ulong *pgd;
	ulong *page_dir;
	ulong *page_middle;
	...
	if (pgd_pte & _PAGE_4M) {
		...
		*paddr = NONPAE_PAGEBASE(pseudo_pgd_pte) + (kvaddr & ~_4MB_PAGE_MASK);
	...
	machine_page_table = (ulong *) ((NONPAE_PAGEBASE(pmd_pte)) + ((kvaddr >> 10) & ((PTRS_PER_PTE - 1) << 2)));
...
static int x86_kvtop_PAE(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	ulonglong *pgd;
	ulonglong page_dir_entry;
	ulonglong page_middle;
	...
	offset = ((kvaddr >> PGDIR_SHIFT) & (PTRS_PER_PGD - 1)) * sizeof(ulonglong);
	...
	if (page_middle_entry & _PAGE_PSE) {
		...
		physpage = PAE_PAGEBASE(page_middle_entry) + (kvaddr & ~_2MB_PAGE_MASK);
		*paddr = physpage;
	...
	offset = ((kvaddr >> PAGESHIFT()) & (PTRS_PER_PTE - 1)) * sizeof(ulonglong);
...
static int x86_kvtop_xen_wpt_PAE(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	ulonglong *pgd;
	ulonglong page_dir_entry;
	ulonglong page_middle, pseudo_page_middle;
...
void x86_clear_machdep_cache(void)
{
 ...
 */
static ulong x86_get_task_pgd(ulong task)
{
	long offset;
	ulong cr3;
	...
	if (INVALID_MEMBER(thread_struct_cr3))
		error(FATAL, "cr3 does not exist in this kernel's thread_struct\n");

	offset += OFFSET(thread_struct_cr3);

	readmem(task + offset, KVADDR, &cr3, sizeof(ulong), "task thread cr3", FAULT_ON_ERROR);
 ...
 */
ulong x86_processor_speed(void)
{
...
void x86_dump_machdep_table(ulong arg)
{
	int others;
	ulong xen_wpt;
	char buf[BUFSIZE];
	...
	fprintf(fp, "          is_uvaddr: %s\n", COMMON_VADDR_SPACE()? "x86_is_uvaddr()" : "generic_is_uvaddr()");
	fprintf(fp, "       verify_paddr: generic_verify_paddr()\n");
	fprintf(fp, "    init_kernel_pgd: x86_init_kernel_pgd()\n");
	fprintf(fp, "    value_to_symbol: %s\n",
		machdep->value_to_symbol == generic_machdep_value_to_symbol ?
		"generic_machdep_value_to_symbol()" : "x86_is_entry_tramp_address()");
	fprintf(fp, "  line_number_hooks: %s\n", machdep->line_number_hooks ? "x86_line_number_hooks" : "(not used)");
 ...
 */
static void x86_get_stack_frame(struct bt_info *bt, ulong * pcp, ulong * spp)
{
	if (pcp)
		*pcp = x86_get_pc(bt);
	if (spp)
 ...
 */
static ulong x86_get_pc(struct bt_info *bt)
{
	ulong offset;
	ulong eip;
	...
	if (tt->flags & THREAD_INFO) {
		readmem(bt->task + OFFSET(task_struct_thread_eip), KVADDR,
			&eip, sizeof(void *), "thread_struct eip", FAULT_ON_ERROR);
		return eip;
	}
 ...
 */
static ulong x86_get_sp(struct bt_info *bt)
{
	ulong offset, ksp;
	...
	if (tt->flags & THREAD_INFO) {
		readmem(bt->task + OFFSET(task_struct_thread_esp), KVADDR,
			&ksp, sizeof(void *), "thread_struct esp", FAULT_ON_ERROR);
		return ksp;
	}
 ...
 */
static int x86_translate_pte(ulong pte, void *physaddr, ulonglong pae_pte)
{
...
static uint64_t x86_memory_size(void)
{
	...
	if (!(machdep->flags & PAE)) {
		readmem(vt->kernel_pgd[0], KVADDR, kpgd,
			sizeof(ulong) * PTRS_PER_PGD, "kernel page directory", FAULT_ON_ERROR);

		for (i = j = 0, pp = &kpgd[0]; i < PTRS_PER_PGD; i++, pp++) {
			if ((*pp & (_PAGE_PRESENT | _PAGE_4M)) == (_PAGE_PRESENT | _PAGE_4M)) {
				j++;
			}
		}
 ...
 */
static char *extract_idt_function(ulong * ip, char *buf, ulong * retaddr)
{
	ulong i1, i2, addr;
	char locbuf[BUFSIZE];
	physaddr_t phys;
 ...
 */
static ulong *read_idt_table(int flag)
{
	...
	case READ_IDT_INIT:
		...
		if (!readmem(symbol_value("idt_table"), KVADDR, idt, desc_struct_size, "idt_table", RETURN_ON_ERROR)) {
		...
		if (addr) {
			if (symbol_exists("__entry_tramp_start") &&
			    symbol_exists("__entry_tramp_end") && symbol_exists("__start___entry_text")) {
				ms->entry_tramp_start = symbol_value("__start___entry_text");
				ms->entry_tramp_end =
				    ms->entry_tramp_start +
				    (symbol_value("__entry_tramp_end") - symbol_value("__entry_tramp_start"));
				ms->entry_tramp_start_phys = 0;
				machdep->value_to_symbol = x86_is_entry_tramp_address;
			} else if (!(sp = value_search(addr, &offset))) {
				addr = VIRTPAGEBASE(addr);
				if (kvtop(NULL, addr, &phys, 0) &&
				    (sp = value_search(PTOV(phys), &offset)) && STREQ(sp->name, "entry_tramp_start")) {
					ms->entry_tramp_start = addr;
					ms->entry_tramp_start_phys = phys;
					ms->entry_tramp_end = addr +
					    (symbol_value("entry_tramp_end") - symbol_value("entry_tramp_start"));
					machdep->value_to_symbol = x86_is_entry_tramp_address;
				}
			}
		}
	...
	case READ_IDT_RUNTIME:
		if (!symbol_exists("idt_table"))
			error(FATAL, "idt_table does not exist on this architecture\n");

		idt = (ulong *) GETBUF(desc_struct_size);
		readmem(symbol_value("idt_table"), KVADDR, idt, desc_struct_size, "idt_table", FAULT_ON_ERROR);
		break;
	}
 ...
 *  associated with it.
 */
struct syment *x86_is_entry_tramp_address(ulong vaddr, ulong * retoffset)
{
	struct syment *sp;
	struct machine_specific *ms;
	ulong addr, offset;
	...
	if (!ms->entry_tramp_start || !((vaddr >= ms->entry_tramp_start)
	...
	if (ms->entry_tramp_start_phys)	/* old */
		addr = machdep->kvbase + (ulong) ms->entry_tramp_start_phys + PAGEOFFSET(vaddr);
	else			/* new */
		addr = vaddr;
 ...
 */
static int x86_is_task_addr(ulong task)
{
	if (tt->flags & THREAD_INFO)
 ...
 */
static int x86_verify_symbol(const char *name, ulong value, char type)
{
 ...
 */
static int x86_dis_filter(ulong vaddr, char *inbuf, unsigned int output_radix)
{
	char buf1[BUFSIZE];
	char buf2[BUFSIZE];
	char *colon, *p1;
...
int x86_get_smp_cpus(void)
{
	...
	if (XEN() && (count == 1) && symbol_exists("cpu_present_map")) {
		...
		get_symbol_data("cpu_present_map", sizeof(ulong), &cpu_present_map);
...
static void x86_display_cpu_data(unsigned int radix)
{
	int cpu;
	ulong cpu_data = 0;
...
static int x86_omit_frame_pointer(void)
{
	...
	for (i = 0; i < 2; i++) {
		if (!readmem(symbol_value(checkfuncs[i]), KVADDR,
			     &push_bp_mov_esp_bp, sizeof(ulonglong), "x86_omit_frame_pointer", RETURN_ON_ERROR))
			return TRUE;
		if (!(((push_bp_mov_esp_bp & 0x0000ffffffULL) ==
		       PUSH_BP_MOV_ESP_BP) || ((push_bp_mov_esp_bp & 0xffffffffffULL) == PUSH_BP_CLR_EAX_MOV_ESP_BP)))
			return TRUE;
	}
 ...
 */
char *x86_function_called_by(ulong eip)
{
	struct syment *sp;
	...
	if (!readmem(eip, KVADDR, &byte, sizeof(unsigned char), "call byte", RETURN_ON_ERROR))
		return funcname;
	if (byte != 0xe8)
		return funcname;
		...
		while (fgets(buf, BUFSIZE, pc->tmpfile2)) {
			if ((p1 = strstr(buf, "call   "))) {
				...
				if ((p2 = strstr(p1, " <"))) {
					...
					if (!extract_hex(strip_linefeeds(p2), &value, NULLCHAR, TRUE))
						continue;
...
struct syment *x86_text_lock_jmp(ulong eip, ulong * offset)
{
	int i, c;
	char buf1[BUFSIZE];
	char buf2[BUFSIZE];
...
static ulong xen_m2p_nonPAE(ulong machine)
{
	ulonglong pseudo;
...
static int x86_xen_kdump_p2m_create(struct xen_kdump_data *xkd)
{
	int i, j;
	ulong kvaddr;
	ulong *up;
	...
	if (CRASHDEBUG(1)) {
		...
		for (i = 0; i < 4; i++) {
			fprintf(fp, "%08lx: %08lx %08lx %08lx %08lx\n",
				(ulong) ((i * 4) * sizeof(ulong)), *up, *(up + 1), *(up + 2), *(up + 3));
			up += 4;
		}
		fprintf(fp, "\n");
	}
	...
	for (i = 0; i < MAX_X86_FRAMES; i++) {
		...
		if (CRASHDEBUG(7)) {
			...
			for (j = 0; j < 256; j++) {
				fprintf(fp, "%08lx: %08lx %08lx %08lx %08lx\n",
					(ulong) ((j * 4) * sizeof(ulong)), *up, *(up + 1), *(up + 2), *(up + 3));
				up += 4;
			}
		}
	}
 ...
 use_cr3:
	...
	if (!readmem(PTOB(xkd->cr3), PHYSADDR, machdep->pgd, PAGESIZE(), "xen kdump cr3 page", RETURN_ON_ERROR))
	...
	if (CRASHDEBUG(7)) {
		...
		if (!x86_xen_kdump_load_page(kvaddr, xkd->page))
			error(INFO, "cannot read first phys_to_machine_mapping page\n");
 ...
 */
static char *x86_xen_kdump_load_page(ulong kvaddr, char *pgbuf)
{
	ulong *entry;
	ulong *up;
	ulong mfn;
	...
	if (!readmem(PTOB(mfn), PHYSADDR, pgbuf, PAGESIZE(), "xen kdump pgd entry", RETURN_ON_ERROR)) {
	...
	if (!readmem(PTOB(mfn), PHYSADDR, pgbuf, PAGESIZE(), "xen page table page", RETURN_ON_ERROR)) {
...
static char *x86_xen_kdump_load_page_PAE(ulong kvaddr, char *pgbuf)
{
	ulonglong *entry;
	ulonglong *up;
	ulong mfn;
	...
	if (!readmem(PTOB(mfn), PHYSADDR, pgbuf, PAGESIZE(), "xen kdump pmd entry", RETURN_ON_ERROR)) {
 ...
 */
static ulong x86_xen_kdump_page_mfn(ulong kvaddr)
{
	ulong *entry;
	ulong *up;
	ulong mfn;
	...
	if ((mfn != machdep->last_ptbl_read) &&
	    !readmem(PTOB(mfn), PHYSADDR, machdep->ptbl, PAGESIZE(), "xen kdump pgd entry", RETURN_ON_ERROR))
		error(FATAL, "cannot read/find pgd entry from cr3 page (mfn: %lx)\n", mfn);
...
static ulong x86_xen_kdump_page_mfn_PAE(ulong kvaddr)
{
	ulonglong *entry;
	ulonglong *up;
	ulong mfn;
	...
	if ((mfn != machdep->last_pmd_read) &&
	    !readmem(PTOB(mfn), PHYSADDR, machdep->pmd, PAGESIZE(), "xen kdump pgd entry", RETURN_ON_ERROR))
		error(FATAL, "cannot read/find pgd entry from cr3 page (mfn: %lx)\n", mfn);
	...
	if ((mfn != machdep->last_ptbl_read) &&
	    !readmem(PTOB(mfn), PHYSADDR, machdep->ptbl, PAGESIZE(), "xen kdump pmd entry", RETURN_ON_ERROR))
		error(FATAL, "cannot read/find pmd entry from pgd (mfn: %lx)\n", mfn);
	machdep->last_ptbl_read = mfn;
 ...
 */
static int x86_xendump_p2m_create(struct xendump_data *xd)
{
	int i, idx;
	...
	if (CRASHDEBUG(1)) {
		...
		if (machdep->flags & PAE) {
			ulp = (ulonglong *) machdep->pgd;
			fprintf(xd->ofp, "%016llx %016llx %016llx %016llx\n", *ulp, *(ulp + 1), *(ulp + 2), *(ulp + 3));
		} else {
			up = (ulong *) machdep->pgd;
			for (i = 0; i < 256; i++) {
				fprintf(xd->ofp,
...
static int x86_pvops_xendump_p2m_create(struct xendump_data *xd)
{
	int i;
	ulong mfn, kvaddr, ctrlreg[8], ctrlreg_offset;
	ulong *up;
...
static int x86_pvops_xendump_p2m_l2_create(struct xendump_data *xd)
{
	...
	for (p = 0; p < xd->xc_core.p2m_frames; p += XEN_PFNS_PER_PAGE) {
		...
		if (CRASHDEBUG(7))
			x86_debug_dump_page(xd->ofp, xd->page, "contents of page:");
...
static int x86_pvops_xendump_p2m_l3_create(struct xendump_data *xd)
{
	...
	if (CRASHDEBUG(7))
		x86_debug_dump_page(xd->ofp, xd->page, "contents of p2m_top page:");
	...
	for (i = 0; i < XEN_P2M_TOP_PER_PAGE; ++i) {
		...
		if (CRASHDEBUG(7))
			x86_debug_dump_page(xd->ofp, xd->page, "contents of p2m_mid page:");
...
static void x86_debug_dump_page(FILE * ofp, char *page, char *name)
{
	int i;
	ulong *up;
 ...
 *  into the passed-in buffer.
 */
static char *x86_xendump_load_page(ulong kvaddr, char *pgbuf)
{
	ulong *entry;
	ulong *up;
	ulong mfn;
...
static char *x86_xendump_load_page_PAE(ulong kvaddr, char *pgbuf)
{
	ulonglong *entry;
	ulonglong *up;
	ulong mfn;
 ...
 */
static int x86_xendump_page_index(ulong kvaddr)
{
	int idx;
	ulong *entry;
	ulong *up;
...
static int x86_xendump_page_index_PAE(ulong kvaddr)
{
	int idx;
	ulonglong *entry;
	ulonglong *up;
 ...
 */
static ulong x86_xendump_panic_task(struct xendump_data *xd)
{
	return NO_TASK;
	...
	offset = (off_t) xd->xc_core.header.xch_ctxt_offset +
	    (off_t) OFFSET(vcpu_guest_context_user_regs) + (off_t) OFFSET(cpu_user_regs_esp);
	...
	if (IS_KVADDR(esp) && (task = stkptr_to_task(esp))) {
		...
		for (i = 0; i < NR_CPUS; i++) {
			if (task == tt->active_set[i]) {
				if (CRASHDEBUG(0))
					error(INFO, "x86_xendump_panic_task: esp: %lx -> task: %lx\n", esp, task);
				return task;
			}
		}

		error(WARNING, "x86_xendump_panic_task: esp: %lx -> task: %lx (not active)\n", esp);
	}
 ...
 */
static void x86_get_xendump_regs(struct xendump_data *xd, struct bt_info *bt, ulong * eip, ulong * esp)
{
	ulong task, xeip, xesp;
	off_t offset;

	if (INVALID_MEMBER(vcpu_guest_context_user_regs) ||
	    INVALID_MEMBER(cpu_user_regs_eip) || INVALID_MEMBER(cpu_user_regs_esp))
		goto generic;
	...
	offset = (off_t) xd->xc_core.header.xch_ctxt_offset +
	    (off_t) OFFSET(vcpu_guest_context_user_regs) + (off_t) OFFSET(cpu_user_regs_eip);
	if (lseek(xd->xfd, offset, SEEK_SET) == -1)
		goto generic;
	if (read(xd->xfd, &xeip, sizeof(ulong)) != sizeof(ulong))
		goto generic;

	if (IS_KVADDR(xesp) && (task = stkptr_to_task(xesp)) && (task == bt->task)) {
		if (CRASHDEBUG(1))
			fprintf(xd->ofp, "hooks from vcpu_guest_context: eip: %lx esp: %lx\n", xeip, xesp);
		*eip = xeip;
		*esp = xesp;
		return;
	}
...
static int x86_xenhyper_is_kvaddr(ulong addr)
{
	if (machdep->flags & PAE) {
...
static ulong x86_get_stackbase_hyper(ulong task)
{
	...
	if (!readmem(init_tss, KVADDR, buf, XEN_HYPER_SIZE(tss_struct), "init_tss", RETURN_ON_ERROR)) {
...
static ulong x86_get_stacktop_hyper(ulong task)
{
	return x86_get_stackbase_hyper(task) + STACKSIZE();
}

static void x86_get_stack_frame_hyper(struct bt_info *bt, ulong * pcp, ulong * spp)
{
	struct xen_hyper_vcpu_context *vcc;
	int pcpu;
	ulong *regs;
	...
	if (spp) {
		if (esp < x86_get_stackbase_hyper(bt->task) || esp >= x86_get_stacktop_hyper(bt->task))
			*spp = x86_get_stackbase_hyper(bt->task);
		else
			*spp = esp;
	}
...
static void x86_init_hyper(int when)
{
	switch (when) {
	case PRE_SYMTAB:
		machdep->verify_symbol = x86_verify_symbol;
	...
	case PRE_GDB:
		if (symbol_exists("create_pae_xen_mappings") || symbol_exists("idle_pg_table_l3")) {
			machdep->flags |= PAE;
			PGDIR_SHIFT = PGDIR_SHIFT_3LEVEL;
			PTRS_PER_PTE = PTRS_PER_PTE_3LEVEL;
			PTRS_PER_PGD = PTRS_PER_PGD_3LEVEL;
