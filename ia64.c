...
void ia64_init(int when)
{
	...
	case PRE_GDB:
		...
		if ((sp = symbol_search("empty_zero_page")) &&
		    (spn = next_symbol(NULL, sp)) && ((spn->value - sp->value) != PAGESIZE()))
			error(FATAL,
		...
		if ((sp = symbol_search("_stext"))) {
			machdep->machspec->kernel_region = VADDR_REGION(sp->value);
		...
		if (machdep->machspec->kernel_region == KERNEL_VMALLOC_REGION) {
			machdep->machspec->vmalloc_start = machdep->machspec->kernel_start + GIGABYTES((ulong) (4));
	...
	case POST_GDB:
		...
		MEMBER_OFFSET_INIT(switch_stack_ar_bspstore, "switch_stack", "ar_bspstore");
		MEMBER_OFFSET_INIT(switch_stack_ar_pfs, "switch_stack", "ar_pfs");
		MEMBER_OFFSET_INIT(switch_stack_ar_rnat, "switch_stack", "ar_rnat");
		MEMBER_OFFSET_INIT(switch_stack_pr, "switch_stack", "pr");
		MEMBER_OFFSET_INIT(cpuinfo_ia64_proc_freq, "cpuinfo_ia64", "proc_freq");
		MEMBER_OFFSET_INIT(cpuinfo_ia64_unimpl_va_mask, "cpuinfo_ia64", "unimpl_va_mask");
		MEMBER_OFFSET_INIT(cpuinfo_ia64_unimpl_pa_mask, "cpuinfo_ia64", "unimpl_pa_mask");
		if (kernel_symbol_exists("nr_irqs"))
			...
			ARRAY_LENGTH_INIT(machdep->nr_irqs, irq_desc, "_irq_desc", NULL, 0);
		if (!machdep->hz)
			machdep->hz = 1024;
	...
	case LOG_ONLY:
		...
		machdep->machspec->kernel_region = VADDR_REGION(kt->vmcoreinfo._stext_SYMBOL);
		if (machdep->machspec->kernel_region == KERNEL_VMALLOC_REGION) {
			machdep->machspec->vmalloc_start = machdep->machspec->kernel_start + GIGABYTES((ulong) (4));
			ia64_calc_phys_start();
		}
...
void parse_cmdline_args(void)
{
	...
	for (index = 0; index < MAX_MACHDEP_ARGS; index++) {
		...
		if (!strstr(machdep->cmdline_args[index], "=")) {
			errflag = 0;
			value = htol(machdep->cmdline_args[index], RETURN_ON_ERROR | QUIET, &errflag);
			if (!errflag) {
				ms->phys_start = value;
				...
				if (strlen(p)) {
					...
					if (!errflag) {
						...
						error(NOTE,
						      ...
						      ms->ia64_init_stack_size, ms->ia64_init_stack_size);
						continue;
					}
				}
...
int ia64_in_init_stack(ulong addr)
{
	...
	addr = ia64_VTOP(addr);
	if ((addr < init_stack_addr) || (addr >= (init_stack_addr + machdep->machspec->ia64_init_stack_size)))
		return FALSE;
...
static ulong ia64_in_per_cpu_mca_stack(void)
{
	...
	if (!symbol_exists("__per_cpu_mca") ||
	    !(plen = get_array_length("__per_cpu_mca", NULL, 0)) || (plen < kt->cpus))
		return 0;
...
void ia64_dump_machdep_table(ulong arg)
{
	...
	if (arg) {
		...
		case 2:
			if (machdep->flags & NEW_UNWIND) {
				machdep->flags &= ~(NEW_UNWIND | NEW_UNW_V1 | NEW_UNW_V2 | NEW_UNW_V3);
	...
	fprintf(fp, "       verify_paddr: %s()\n",
		(machdep->verify_paddr == ia64_verify_paddr) ? "ia64_verify_paddr" : "generic_verify_paddr");
	...
	fprintf(fp, "                unw_pt_regs_offsets: %ld %s\n",
		machdep->machspec->unw_pt_regs_offsets, machdep->machspec->unw_pt_regs_offsets ? "" : "(unused)");
	fprintf(fp, "                       script_index: %d\n", machdep->machspec->script_index);
	fprintf(fp, "                       script_cache: %lx%s",
		(ulong) machdep->machspec->script_cache, machdep->flags & OLD_UNWIND ? "\n" : " ");
	if (machdep->flags & NEW_UNWIND)
	...
	fprintf(fp, "                         phys_start: %lx (%lx)\n",
		machdep->machspec->phys_start, machdep->machspec->phys_start & KERNEL_TR_PAGE_MASK);
 ...
 */
static int ia64_verify_symbol(const char *name, ulong value, char type)
{
	ulong region;
 ...
 */
static int ia64_eframe_search(struct bt_info *bt)
{
	return (error(FATAL, "ia64_eframe_search: not available for this architecture\n"));
}
...
static void ia64_back_trace_cmd(struct bt_info *bt)
{
	...
	if (machdep->flags & UNW_OUT_OF_SYNC)
		error(FATAL, "kernel and %s unwind data structures are out of sync\n", pc->program_name);
 ...
 *  Dump the IRQ table.
 */
static void ia64_dump_irq(int irq)
{
	if (symbol_exists("irq_desc") || symbol_exists("_irq_desc") || kernel_symbol_exists("irq_desc_ptrs")) {
		machdep->dump_irq = generic_dump_irq;
		return (generic_dump_irq(irq));
	}

	error(FATAL, "ia64_dump_irq: neither irq_desc or _irq_desc exist\n");
}
...
static ulong ia64_processor_speed(void)
{
	...
	if (!machdep->machspec->cpu_data_address || !VALID_STRUCT(cpuinfo_ia64)
	    || !VALID_MEMBER(cpuinfo_ia64_proc_freq))
		return (machdep->mhz = mhz);

	if (symbol_exists("bootstrap_processor"))
		get_symbol_data("bootstrap_processor", sizeof(int), &bootstrap_processor);
	if (bootstrap_processor == -1)
		bootstrap_processor = 0;

	readmem(machdep->machspec->cpu_data_address +
		OFFSET(cpuinfo_ia64_proc_freq),
		KVADDR, &proc_freq, sizeof(ulong), "cpuinfo_ia64 proc_freq", FAULT_ON_ERROR);
 ...
 */
static int ia64_uvtop(struct task_context *tc, ulong uvaddr, physaddr_t * paddr, int verbose)
{
	ulong mm;
	ulong *pgd;
 ...
 *  other callers quietly accept the translation.
 */
static int ia64_kvtop(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	ulong *pgd;
	...
	case KERNEL_VMALLOC_REGION:
		...
		if ((kvaddr < machdep->machspec->kernel_start) &&
		    (machdep->machspec->kernel_region == KERNEL_VMALLOC_REGION)) {
 ...
 */
static ulong ia64_get_stackbase(ulong task)
{
	return (task);
}

static ulong ia64_get_stacktop(ulong task)
{
 ...
 */
static ulong ia64_get_task_pgd(ulong task)
{
	return (error(FATAL, "ia64_get_task_pgd: N/A\n"));
}

static void ia64_get_stack_frame(struct bt_info *bt, ulong * pcp, ulong * spp)
{
	if (pcp)
		*pcp = ia64_get_pc(bt);
	if (spp)
 ...
 */
static ulong ia64_get_pc(struct bt_info *bt)
{
	ulong b0;
 ...
 */
static ulong ia64_get_sp(struct bt_info *bt)
{
	ulong bspstore;

	readmem(SWITCH_STACK_ADDR(bt->task) + OFFSET(switch_stack_ar_bspstore),
		KVADDR, &bspstore, sizeof(void *), "switch_stack ar_bspstore", FAULT_ON_ERROR);
 ...
 */
static ulong ia64_get_thread_ksp(ulong task)
{
	ulong ksp;

	if (XEN_HYPER_MODE()) {
		readmem(task + XEN_HYPER_OFFSET(vcpu_thread_ksp), KVADDR,
			&ksp, sizeof(void *), "vcpu thread ksp", FAULT_ON_ERROR);
	} else {
		readmem(task + OFFSET(task_struct_thread_ksp), KVADDR,
			&ksp, sizeof(void *), "thread_struct ksp", FAULT_ON_ERROR);
	}
 ...
 */
ulong ia64_get_switch_stack(ulong task)
{
	ulong sw;
	 ...
	 */
	if (CRASHDEBUG(3) && NETDUMP_DUMPFILE() && (sw = get_netdump_switch_stack(task)))
		return sw;
 ...
 */
static int ia64_translate_pte(ulong pte, void *physaddr, ulonglong unused)
{
 ...
 */
static int ia64_is_task_addr(ulong task)
{
	int i;
 ...
 */
static int ia64_dis_filter(ulong vaddr, char *inbuf, unsigned int output_radix)
{
	char buf1[BUFSIZE];
	char buf2[BUFSIZE];
	char *colon, *p1, *p2;
 ...
 */
	...
	if ((FIRSTCHAR(argv[argc - 1]) == '<') && (LASTCHAR(argv[argc - 1]) == '>')) {
		revise_bracket = TRUE;
		stop_bit = FALSE;
	} else if ((FIRSTCHAR(argv[argc - 1]) == '<') && strstr(argv[argc - 1], ">;;")) {
		revise_bracket = TRUE;
		stop_bit = TRUE;
	}
	...
	if (revise_bracket) {
		...
		sprintf(buf1, "0x%lx <%s>%s\n", value,
			value_to_symstr(value, buf2, output_radix), stop_bit ? ";;" : "");

		sprintf(p1, "%s", buf1);

	} else if (STRNEQ(argv[argc - 2], "br.call.") && STRNEQ(argv[argc - 1], "b0=0x")) {
		/*
...
void ia64_exception_frame(ulong addr, struct bt_info *bt)
{
	...
	if (bt->flags & BT_INCOMPLETE_USER_EFRAME) {
		fprintf(fp, "  [exception frame incomplete -- check salinfo for complete context]\n");
...
**/
	...
	fprintf(fp, "      F6: %05lx%016lx  ", eframe[P_f6_hi], eframe[P_f6_lo]);
	fprintf(fp, "   F7: %05lx%016lx\n", eframe[P_f7_hi], eframe[P_f7_lo]);
	fprintf(fp, "      F8: %05lx%016lx  ", eframe[P_f8_hi], eframe[P_f8_lo]);
	fprintf(fp, "   F9: %05lx%016lx\n", eframe[P_f9_hi], eframe[P_f9_lo]);

	if (machdep->flags & NEW_UNW_V3) {
		fprintf(fp, "     F10: %05lx%016lx  ", eframe[P_f10_hi], eframe[P_f10_lo]);
 ...
 */
static void ia64_dump_switch_stack(ulong task, ulong flag)
{
	ulong addr;
	char buf[BUFSIZE], *p;
	int fval;
...
static void ia64_display_cpu_data(unsigned int radix)
{
	int cpu;
	ulong cpu_data;
	...
	for (cpu = 0; cpu < kt->cpus; cpu++) {
		...
		if ((sp = per_cpu_symbol_search("per_cpu__cpu_info"))) {
			if ((kt->flags & SMP) && (kt->flags & PER_CPU_OFF))
				cpu_data = sp->value + kt->__per_cpu_offset[cpu + 1];
			else
...
static void ia64_display_memmap(void)
{
	...
	for (i = 0; i < ms->efi_memmap_size / ms->efi_memdesc_size; i++) {
		...
		fprintf(fp, "%016lx - %016lx  ",
			desc->phys_addr, desc->phys_addr + (desc->num_pages * (1 << EFI_PAGE_SHIFT)));

		switch (desc->type) {
		case EFI_RESERVED_TYPE:
			fprintf(fp, "%s", "RESERVED_TYPE");
			break;
		case EFI_LOADER_CODE:
			fprintf(fp, "%s", "LOADER_CODE");
			break;
		case EFI_LOADER_DATA:
			fprintf(fp, "%s", "LOADER_DATA");
			break;
		case EFI_BOOT_SERVICES_CODE:
			fprintf(fp, "%s", "BOOT_SERVICES_CODE");
			break;
		case EFI_BOOT_SERVICES_DATA:
			fprintf(fp, "%s", "BOOT_SERVICES_DATA");
			break;
		case EFI_RUNTIME_SERVICES_CODE:
			fprintf(fp, "%s", "RUNTIME_SERVICES_CODE");
			break;
		case EFI_RUNTIME_SERVICES_DATA:
			fprintf(fp, "%s", "RUNTIME_SERVICES_DATA");
			break;
		case EFI_CONVENTIONAL_MEMORY:
			fprintf(fp, "%s", "CONVENTIONAL_MEMORY");
			break;
		case EFI_UNUSABLE_MEMORY:
			fprintf(fp, "%s", "UNUSABLE_MEMORY");
			break;
		case EFI_ACPI_RECLAIM_MEMORY:
			fprintf(fp, "%s", "ACPI_RECLAIM_MEMORY");
			break;
		case EFI_ACPI_MEMORY_NVS:
			fprintf(fp, "%s", "ACPI_MEMORY_NVS");
			break;
		case EFI_MEMORY_MAPPED_IO:
			fprintf(fp, "%s", "MEMORY_MAPPED_IO");
			break;
		case EFI_MEMORY_MAPPED_IO_PORT_SPACE:
			fprintf(fp, "%s", "MEMORY_MAPPED_IO_PORT_SPACE");
			break;
		case EFI_PAL_CODE:
			fprintf(fp, "%s", "PAL_CODE");
			break;
		default:
		...
		fprintf(fp, " %s", ia64_available_memory(desc) ? "[available]" : "");
		...
		fprintf(fp,
			...
			desc->phys_addr, (1 << EFI_PAGE_SHIFT) / 1024, desc->num_pages, desc->virt_addr);

		fprintf(fp, "type: ");
		switch (desc->type) {
		case EFI_RESERVED_TYPE:
			fprintf(fp, "%-27s", "RESERVED_TYPE");
			break;
		case EFI_LOADER_CODE:
			fprintf(fp, "%-27s", "LOADER_CODE");
			break;
		case EFI_LOADER_DATA:
			fprintf(fp, "%-27s", "LOADER_DATA");
			break;
		case EFI_BOOT_SERVICES_CODE:
			fprintf(fp, "%-27s", "BOOT_SERVICES_CODE");
			break;
		case EFI_BOOT_SERVICES_DATA:
			fprintf(fp, "%-27s", "BOOT_SERVICES_DATA");
			break;
		case EFI_RUNTIME_SERVICES_CODE:
			fprintf(fp, "%-27s", "RUNTIME_SERVICES_CODE");
			break;
		case EFI_RUNTIME_SERVICES_DATA:
			fprintf(fp, "%-27s", "RUNTIME_SERVICES_DATA");
			break;
		case EFI_CONVENTIONAL_MEMORY:
			fprintf(fp, "%-27s", "CONVENTIONAL_MEMORY");
			break;
		case EFI_UNUSABLE_MEMORY:
			fprintf(fp, "%-27s", "UNUSABLE_MEMORY");
			break;
		case EFI_ACPI_RECLAIM_MEMORY:
			fprintf(fp, "%-27s", "ACPI_RECLAIM_MEMORY");
			break;
		case EFI_ACPI_MEMORY_NVS:
			fprintf(fp, "%-27s", "ACPI_MEMORY_NVS");
			break;
		case EFI_MEMORY_MAPPED_IO:
			fprintf(fp, "%-27s", "MEMORY_MAPPED_IO");
			break;
		case EFI_MEMORY_MAPPED_IO_PORT_SPACE:
			fprintf(fp, "%-27s", "MEMORY_MAPPED_IO_PORT_SPACE");
			break;
		case EFI_PAL_CODE:
			fprintf(fp, "%-27s", "PAL_CODE");
			break;
		default:
			fprintf(fp, "%-27s", "(unknown type)");
			break;
		}
		...
		if (desc->attribute & EFI_MEMORY_RUNTIME)
			fprintf(fp, "%sRUNTIME", others++ ? "|" : "");
		fprintf(fp, ") %s\n", ia64_available_memory(desc) ? "[available]" : "");
...
static int ia64_available_memory(struct efi_memory_desc_t *desc)
{
	if (desc->attribute & EFI_MEMORY_WB) {
		switch (desc->type) {
		case EFI_LOADER_CODE:
...
static void ia64_create_memmap(void)
{
	...
	if ((ms->mem_limit && (ia64_VTOP(ia64_boot_param) >= ms->mem_limit)) ||
	    !readmem(ia64_boot_param +
		     ...
		     KVADDR, &efi_memmap, sizeof(uint64_t), "efi_memmap", QUIET | RETURN_ON_ERROR)) {
		if (!XEN() || CRASHDEBUG(1))
			error(WARNING, "cannot read ia64_boot_param: " "memory verification will not be performed\n\n");
		return;
	}

	readmem(ia64_boot_param + MEMBER_OFFSET("ia64_boot_param",
						"efi_memmap_size"), KVADDR,
		&ms->efi_memmap_size, sizeof(uint64_t), "efi_memmap_size", FAULT_ON_ERROR);
	readmem(ia64_boot_param +
		MEMBER_OFFSET("ia64_boot_param", "efi_memdesc_size"), KVADDR,
		&ms->efi_memdesc_size, sizeof(uint64_t), "efi_memdesc_size", FAULT_ON_ERROR);
	...
	if ((ms->mem_limit && (efi_memmap >= ms->mem_limit)) ||
	    !readmem(PTOV(efi_memmap), KVADDR, memmap,
		     ms->efi_memmap_size, "efi_mmap contents", QUIET | RETURN_ON_ERROR)) {
		if (!XEN() || (XEN() && CRASHDEBUG(1)))
			error(WARNING, "cannot read efi_mmap: " "EFI memory verification will not be performed\n\n");
		free(memmap);
		return;
	}
 ...
 */
static int ia64_verify_paddr(uint64_t paddr)
{
	...
	for (i = cnt = 0; i < efi_pages; i++, paddr += efi_pagesize) {
		...
		for (j = found = 0; j < desc_count; j++) {
			...
			if (ia64_available_memory(desc)) {
				phys_end = desc->phys_addr + (desc->num_pages * efi_pagesize);
...
static ulong check_mem_limit(void)
{
	...
	*p2 = NULLCHAR;
	...
	error(pc->flags & RUNTIME ? INFO : WARNING, "boot command line argument: %s\n", p1);
	return mem_limit;

 no_command_line:
	error(pc->flags & RUNTIME ? INFO : WARNING, "boot command line memory limit: %lx\n", mem_limit);
	return mem_limit;
}
...
static void ia64_post_init(void)
{
	...
	if (!VALID_STRUCT(cpuinfo_ia64))
		...
		if (symbol_exists("_cpu_data"))
			...
			get_symbol_data("boot_cpu_data", sizeof(ulong), &ms->cpu_data_address);
			...
			if ((kt->flags & SMP) && (kt->flags & PER_CPU_OFF))
				ms->cpu_data_address = sp->value + kt->__per_cpu_offset[0];
			else
		...
		if (ms->cpu_data_address) {
			if (VALID_MEMBER(cpuinfo_ia64_unimpl_va_mask))
				readmem(ms->cpu_data_address +
					...
					KVADDR, &ms->unimpl_va_mask, sizeof(ulong), "unimpl_va_mask", FAULT_ON_ERROR);
			if (VALID_MEMBER(cpuinfo_ia64_unimpl_pa_mask))
				readmem(ms->cpu_data_address +
					OFFSET(cpuinfo_ia64_unimpl_pa_mask),
					KVADDR, &ms->unimpl_pa_mask, sizeof(ulong), "unimpl_pa_mask", FAULT_ON_ERROR);
		}
	}
	...
	if (DUMPFILE()
	    && ia64_in_init_stack(SWITCH_STACK_ADDR(CURRENT_TASK())))
		machdep->flags |= INIT;
 ...
 *  same size.
 */
static void try_old_unwind(struct bt_info *bt)
{
	if ((machdep->flags & NEW_UNWIND) && (STRUCT_SIZE("unw_frame_info") == sizeof(struct unw_frame_info))) {
...
static void ia64_old_unwind(struct bt_info *bt)
{
	...
	do {
		...
		if (BT_REFERENCE_CHECK(bt)) {
			...
			fprintf(fp, "%s#%d [BSP:%lx] %s at %lx\n",
				frame >= 10 ? "" : " ", frame, info->bsp, name, info->ip);
 ...
 */
static unsigned long *ia64_rse_skip_regs(unsigned long *addr, long num_regs)
{
	...
	if (CRASHDEBUG(1)) {
		fprintf(fp,
			...
			space(unw_debug), (ulong) addr, ia64_rse_slot_num(addr), num_regs);
	}
	...
	if (CRASHDEBUG(1)) {
		fprintf(fp, "%sia64_rse_skip_regs: delta: %ld return(%lx)",
			space(unw_debug), delta, (ulong) (addr + num_regs + delta / 0x3f));
 ...
 * address SLOT_ADDR.
 */
static unsigned long *ia64_rse_rnat_addr(unsigned long *slot_addr)
{
 ...
 */
static void unw_init_from_blocked_task(struct unw_frame_info *info, ulong task)
{
	ulong sw;
	ulong sol, limit, top;
	...
	readmem(sw + OFFSET(switch_stack_b0), KVADDR, &b0, sizeof(ulong), "switch_stack b0", FAULT_ON_ERROR);
	...
	readmem(sw + OFFSET(switch_stack_ar_bspstore), KVADDR,
		&ar_bspstore, sizeof(ulong), "switch_stack ar_bspstore", FAULT_ON_ERROR);
	...
	if (CRASHDEBUG(1)) {
		unw_debug++;
		fprintf(fp, "unw_init_from_blocked_task: stack top: %lx sol: %ld\n", top, sol);
	}
...
static int old_unw_unwind(struct unw_frame_info *info)
{
	unsigned long sol, cfm;
	int is_nat;

	if (!readmem((ulong) info->cfm, KVADDR, &cfm, sizeof(long), "info->cfm", QUIET | RETURN_ON_ERROR))
		return -1;
 ...
 */
static ulong rse_read_reg(struct unw_frame_info *info, int regnum, int *is_nat)
{
	...
	if (((ulong) addr < info->regstk.limit) || ((ulong) addr >= info->regstk.top) || (((long)addr & 0x7) != 0)) {
		*is_nat = 1;
	...
	if ((unsigned long)rnat_addr >= info->regstk.top)
		readmem((ulong) (info->sw) + OFFSET(switch_stack_ar_rnat),
			KVADDR, &rnat, sizeof(long), "info->sw->ar_rnat", FAULT_ON_ERROR);
	else
		readmem((ulong) rnat_addr, KVADDR, &rnat, sizeof(long), "rnat_addr", FAULT_ON_ERROR);
	...
	*is_nat = (rnat & (1UL << ia64_rse_slot_num(addr))) != 0;
	...
	readmem((ulong) addr, KVADDR, &regcontent, sizeof(long), "rse_read_reg addr", FAULT_ON_ERROR);

	if (CRASHDEBUG(1)) {
		char buf[BUFSIZE];

		fprintf(fp, "%srse_read_reg: addr: %lx => %lx ", space(unw_debug), (ulong) addr, regcontent);
		if (is_kernel_text(regcontent))
			fprintf(fp, "(%s)", value_to_symstr(regcontent, buf, pc->output_radix));
		fprintf(fp, "\n");
	}
...
static void rse_function_params(struct unw_frame_info *info, char *name)
{
	int i;
...
static void dump_unw_frame_info(struct unw_frame_info *info)
{
	unw_debug++;
...
void ia64_dump_line_number(ulong ip)
{
	int retries;
	char buf[BUFSIZE], *p;
...
ulong ia64_VTOP(ulong vaddr)
{
	...
	case KERNEL_VMALLOC_REGION:
		 ...
		 */
		if (ia64_IS_VMALLOC_ADDR(vaddr) || (ms->kernel_region != KERNEL_VMALLOC_REGION))
			return (error(FATAL, "ia64_VTOP(%lx): unexpected region 5 address\n", vaddr));
		/*
		 *  If it's a region 5 kernel address, subtract the starting
		 *  kernel virtual address, and then add the base physical page.
		 */
		paddr = vaddr - ms->kernel_start + (ms->phys_start & KERNEL_TR_PAGE_MASK);
		break;

	default:
		return (error(FATAL, "ia64_VTOP(%lx): invalid kernel address\n", vaddr));
	}
 ...
 */
int ia64_IS_VMALLOC_ADDR(ulong vaddr)
{
	return ((vaddr >= machdep->machspec->vmalloc_start) && (vaddr < (ulong) KERNEL_UNCACHED_BASE));
}
...
static int ia64_get_kvaddr_ranges(struct vaddr_range *vrp)
{
	int cnt;
 ...
 * addresses to physical using a 4 level page table.
 */
static int ia64_vtop_4l_xen_wpt(ulong vaddr, physaddr_t * paddr, ulong * pgd, int verbose, int usr)
{
 ...
 * addresses to physical using a 3 level page table.
 */
static int ia64_vtop_xen_wpt(ulong vaddr, physaddr_t * paddr, ulong * pgd, int verbose, int usr)
{
...
static void ia64_calc_phys_start(void)
{
	...
	if (DISKDUMP_DUMPFILE()) {
		if (diskdump_phys_base(&phys_start)) {
			...
			if (CRASHDEBUG(1))
				fprintf(fp, "compressed kdump: phys_start: %lx\n", phys_start);
		}
		return;
		...
		if (lkcd_get_kernel_start(&phys_start)) {
			machdep->machspec->phys_start = phys_start;
			if (CRASHDEBUG(1))
				fprintf(fp, "LKCD dump: phys_start: %lx\n", phys_start);
		}
	}
 ...
 */
static int ia64_xen_kdump_p2m_create(struct xen_kdump_data *xkd)
{
	/*
	...
	if (!readmem
	    (PTOB(xkd->p2m_mfn), PHYSADDR, xkd->p2m_mfn_frame_list, PAGESIZE(),
...
physaddr_t ia64_xen_kdump_p2m(struct xen_kdump_data * xkd, physaddr_t pseudo)
{
	...
	if (pmd != xkd->last_pmd_read) {
		if (!readmem(pmd, PHYSADDR, &pte, sizeof(ulong), "ia64_xen_kdump_p2m pmd", RETURN_ON_ERROR)) {
	...
	if (pte != xkd->last_mfn_read) {
		if (!readmem(pte, PHYSADDR, xkd->page, PAGESIZE(), "ia64_xen_kdump_p2m pte page", RETURN_ON_ERROR)) {
			xkd->last_pmd_read = BADADDR;
			xkd->last_mfn_read = BADADDR;
			paddr = P2M_FAILURE;
			goto out;
 ...
 */
static int ia64_xendump_p2m_create(struct xendump_data *xd)
{
...
static void ia64_debug_dump_page(FILE * ofp, char *page, char *name)
{
	int i;
	ulong *up;
 ...
 */
static char *ia64_xendump_load_page(ulong kvaddr, struct xendump_data *xd)
{
 ...
 */
static int ia64_xendump_page_index(ulong kvaddr, struct xendump_data *xd)
{
...
static ulong ia64_xendump_panic_task(struct xendump_data *xd)
{
	if (CRASHDEBUG(1))
...
static void ia64_get_xendump_regs(struct xendump_data *xd, struct bt_info *bt, ulong * rip, ulong * rsp)
{
	machdep->get_stack_frame(bt, rip, rsp);

	if (is_task_active(bt->task) &&
	    !(bt->flags & (BT_TEXT_SYMBOLS_ALL | BT_TEXT_SYMBOLS)) && STREQ(closest_symbol(*rip), "schedule"))
		error(INFO, "xendump: switch_stack possibly not saved -- try \"bt -t\"\n");
}

/* for XEN Hypervisor analysis */

static int ia64_is_kvaddr_hyper(ulong addr)
{
	return (addr >= HYPERVISOR_VIRT_START && addr < HYPERVISOR_VIRT_END);
}

static int ia64_kvtop_hyper(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	...
	readmem(dirp, KVADDR, &entry, sizeof(ulong), "frametable_pg_dir", FAULT_ON_ERROR);

	dirp = entry & _PFN_MASK;
	if (!dirp)
		return FALSE;
	dirp += ((addr >> PMD_SHIFT) & (PTRS_PER_PMD - 1)) * sizeof(ulong);
	readmem(dirp, PHYSADDR, &entry, sizeof(ulong), "frametable pmd", FAULT_ON_ERROR);

	dirp = entry & _PFN_MASK;
	if (!dirp)
		return FALSE;
	dirp += ((addr >> PAGESHIFT()) & (PTRS_PER_PTE - 1)) * sizeof(ulong);
	readmem(dirp, PHYSADDR, &entry, sizeof(ulong), "frametable pte", FAULT_ON_ERROR);
...
static void ia64_post_init_hyper(void)
{
	struct machine_specific *ms;
	ulong frame_table;
	...
	if (symbol_exists("frame_table")) {
		...
		readmem(frame_table, KVADDR, &xhmachdep->frame_table,
			sizeof(ulong), "frame_table virtual address", FAULT_ON_ERROR);
	} else {
...
int ia64_in_mca_stack_hyper(ulong addr, struct bt_info *bt)
{
	...
	if (!symbol_exists("__per_cpu_mca") ||
	    !(plen = get_array_length("__per_cpu_mca", NULL, 0)) || (plen < xht->pcpus))
		return 0;
...
static void ia64_init_hyper(int when)
{
	struct syment *sp;
