...
static inline unsigned long KL_ULONG(void *ptr, char *struct_name, char *member_name)
{
...
typedef int (debug_format_proc_t) (debug_info_t * id, debug_view_t * view, char *out_buf, const char *in_buf);
typedef int (debug_prolog_proc_t) (debug_info_t * id, debug_view_t * view, char *out_buf);
 ...
 */
static int dflt_header_fn(debug_info_t * id, debug_view_t * view, int area, debug_entry_t * entry, char *out_buf)
{
	...
	if (KL_ARCH == KL_ARCH_S390X) {
		rc += sprintf(out_buf,
			      ...
			      except_str, entry->id.fields.cpuid, caller_name, offset);
	} else {
		rc += sprintf(out_buf,
			      "%02i %011lu:%06lu %1u %1s %02i <%-20s+%04i>  ",
			      area, time_val.tv_sec, time_val.tv_usec, level,
			      except_str, lentry.id.fields.cpuid, caller_name, offset);
	}
	return rc;
}
 ...
 */
static int raw_header_fn(debug_info_t * id, debug_view_t * view, int area, debug_entry_t * entry, char *out_buf)
{
	int rc;
 ...
 */
static int raw_format_fn(debug_info_t * id, debug_view_t * view, char *out_buf, const char *in_buf)
{
	int rc;
 ...
 */
static int hex_ascii_format_fn(debug_info_t * id, debug_view_t * view, char *out_buf, const char *in_buf)
{
	int i, rc = 0;
 ...
 */
static int sprintf_format_fn(debug_info_t * id, debug_view_t * view, char *out_buf, const char *in_buf)
{
#define _BUFSIZE 1024
	...
	for (i = 0; buf[i] && (buf[i] != '\n'); i++) {
		...
		if (buf[i + 1] != 's') {
			...
			addr = KL_GET_PTR((void *)in_buf + ((k + 1) * KL_NBPW));
			GET_BLOCK(addr, _BUFSIZE, (void *)(uaddr_t) (inbuf_cpy[k]));
		}
		k++;
	}
	...
	rc = sprintf(out_buf + rc, buf, (uaddr_t) (inbuf_cpy[0]),
		     ...
		     (uaddr_t) (inbuf_cpy[7]), (uaddr_t) (inbuf_cpy[8]), (uaddr_t) (inbuf_cpy[9]));
 out:
	while (num_strings--) {
 ...
 */
static int prolog_level_fn(debug_info_t * id, debug_view_t * view, char *out_buf)
{
	int rc = 0;
 ...
 */
static int prolog_pages_fn(debug_info_t * id, debug_view_t * view, char *out_buf)
{
	int rc = 0;
 ...
 * prints out prolog
 */
static int prolog_fn(debug_info_t * id, debug_view_t * view, char *out_buf)
{
	int rc = 0;
 ...
 */
static int hex_format_fn(debug_info_t * id, debug_view_t * view, char *out_buf, const char *in_buf)
{
	int i, rc = 0;
 ...
 */
static int ascii_format_fn(debug_info_t * id, debug_view_t * view, char *out_buf, const char *in_buf)
{
	int i, rc = 0;
 ...
 */
static int ebcdic_format_fn(debug_info_t * id, debug_view_t * view, char *out_buf, const char *in_buf)
{
	int i, rc = 0;
...
static debug_entry_t *debug_find_oldest_entry(debug_entry_t * entries, int num, int entry_size)
{
 ...
 */
static int debug_format_output_v1(debug_info_t * debug_area, debug_view_t * view, FILE * ofp)
{
	int i, j, len;
	int nr_of_entries;
	...
	for (i = 0; i < debug_area->nr_areas; i++) {
		act_entry = debug_find_oldest_entry(debug_area->areas[i], nr_of_entries, debug_area->entry_size);
		last_entry = (debug_entry_t *) ((char *)debug_area->areas[i] +
						(PAGE_SIZE << debug_area->page_order) - debug_area->entry_size);
		for (j = 0; j < nr_of_entries; j++) {
			act_entry_data = (char *)act_entry + dbe_size;
			if (act_entry->id.stck == 0)
				break;	/* empty entry */
			if (view->header_proc) {
				len = view->header_proc(debug_area, view, i, act_entry, buf);
				items = fwrite(buf, len, 1, ofp);
				memset(buf, 0, 2048);
			}
			if (view->format_proc) {
				len = view->format_proc(debug_area, view, buf, act_entry_data);
				items = fwrite(buf, len, 1, ofp);
				memset(buf, 0, 2048);
			}
			act_entry = (debug_entry_t *) (((char *)act_entry) + debug_area->entry_size);
 ...
 * - calls prolog, header and format functions of view to format output
 */
static int debug_format_output_v2(debug_info_t * debug_area, debug_view_t * view, FILE * ofp)
{
	int i, j, k, len;
...
static debug_view_t *get_debug_view(kaddr_t addr)
{
	void *k_debug_view;
...
static void free_debug_view(debug_view_t * view)
{
	if (view)
		free(view);
}

static void debug_get_areas_v1(debug_info_t * db_info, void *k_dbi)
{
	kaddr_t mem_pos;
	kaddr_t dbe_addr;
	int area_size, i;
...
static void debug_get_areas_v2(debug_info_t * db_info, void *k_dbi)
{
	kaddr_t area_ptr;
	...
	for (i = 0; i < db_info->nr_areas; i++) {
		db_info->areas_v2[i] = (void **)malloc(db_info->pages_per_area_v2 * sizeof(void *));
...
static debug_info_t *get_debug_info(kaddr_t addr, int get_areas)
{
	void *k_dbi;
	kaddr_t mem_pos;
	kaddr_t view_addr;
	...
	if (!(dbi_size = kl_struct_len("debug_info"))) {
		fprintf(KL_ERRORFP, "Could not determine sizeof(struct debug_info)\n");
		return (NULL);
	}
	if (!(dbe_size = kl_struct_len("__debug_entry"))) {
		fprintf(KL_ERRORFP, "Could not determine sizeof(struct __debug_entry)\n");
		return (NULL);
	}
	...
	db_info->addr = addr;
	strncpy(db_info->name, K_PTR(k_dbi, "debug_info", "name"), DEBUG_MAX_PROCF_LEN);
...
static void free_debug_info_v1(debug_info_t * db_info)
{
	int i;
	if (db_info->areas) {
		for (i = 0; i < db_info->nr_areas; i++) {
...
static void free_debug_info_v2(debug_info_t * db_info)
{
	int i, j;
	if (db_info->areas) {
		for (i = 0; i < db_info->nr_areas; i++) {
...
static void debug_write_output(debug_info_t * db_info, debug_view_t * db_view, FILE * fp)
{
	if (dbf_version == DBF_VERSION_V1) {
...
static debug_view_t *find_lcrash_debug_view(const char *name)
{
	int i;
...
static int add_lcrash_debug_view(debug_view_t * view)
{
	int i;
	for (i = 0; i < LCRASH_DB_VIEWS; i++) {
		if (debug_views[i] == NULL) {
...
static int save_one_view(const char *dbf_dir_name, const char *area_name, const char *view_name, command_t * cmd)
{
...
static int save_one_area(const char *dbf_dir_name, const char *area_name, command_t * cmd)
{
	...
	for (i = 0; i < DEBUG_MAX_VIEWS; i++) {
		...
		if (!find_lcrash_debug_view(db_info->views[i]->name))
			continue;
		save_one_view(dbf_dir_name, area_name, db_info->views[i]->name, cmd);
	}
	return 0;
}

static void save_dbf(const char *dbf_dir_name, command_t * cmd)
{
	debug_info_t *act_debug_info = debug_area_first;
	FILE *ofp = cmd->ofp;
 ...
 */
static int s390dbf_cmd(command_t * cmd)
{
	...
	if ((dbf_version != DBF_VERSION_V1) && (dbf_version != DBF_VERSION_V2)) {
		fprintf(cmd->efp, "lcrash does not support the" " debug feature version of the dump kernel:\n");
