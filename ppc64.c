...
static void ppc64_print_stack_entry(int, struct gnu_request *, ulong, ulong, struct bt_info *);
...
static void ppc64_print_eframe(char *, struct ppc64_pt_regs *, struct bt_info *);
...
void ppc64_init(int when)
{
	...
	case POST_GDB:
		...
		if (machdep->flags & VM_ORIG) {
			...
			if (machdep->pagesize == 65536) {
				...
				if (!(machdep->flags & BOOK3E))
					m->pte_shift =
					    symbol_exists("demote_segment_4k") ?
				...
				m->pte_shift = (machdep->flags & BOOK3E) ? PTE_SHIFT_L4_BOOK3E_4K : PTE_SHIFT_L4_4K;
		...
		MEMBER_OFFSET_INIT(thread_struct_pg_tables, "thread_struct", "pg_tables");
		...
		MEMBER_OFFSET_INIT(device_node_allnext, "device_node", "allnext");
		MEMBER_OFFSET_INIT(device_node_properties, "device_node", "properties");
		...
		MEMBER_OFFSET_INIT(machdep_calls_setup_residual, "machdep_calls", "setup_residual");
		MEMBER_OFFSET_INIT(RESIDUAL_VitalProductData, "RESIDUAL", "VitalProductData");
		...
		if (symbol_exists("paca") && MEMBER_EXISTS("paca_struct", "xHrdIntStack")) {
			...
			for (cpu = 0; cpu < kt->cpus; cpu++) {
				readmem(paca_sym + (paca_size * cpu) + offset,
					...
					sizeof(ulong), "PPC64 HW_intr_stack", FAULT_ON_ERROR);
			}
			 ...
			 */
			BZERO(&machdep->machspec->hwintrstack, NR_CPUS * sizeof(ulong));
		if (!machdep->hz) {
			machdep->hz = HZ;
		 ...
		 */
		if ((THIS_KERNEL_VERSION >= LINUX(2, 6, 0)) && symbol_exists("hardirq_ctx"))
...
static ulong ppc64_task_to_stackbase(ulong task)
{
...
static ulong ppc64_get_stackbase(ulong task)
{
	return ppc64_task_to_stackbase(task);
}

static ulong ppc64_get_stacktop(ulong task)
{
	return ppc64_task_to_stackbase(task) + STACKSIZE();
}

void ppc64_dump_machdep_table(ulong arg)
{
	int i, c, others;
	...
	fprintf(fp, "          is_kvaddr: %s\n",
		machdep->is_kvaddr == book3e_is_kvaddr ? "book3e_is_kvaddr()" : "generic_is_kvaddr()");
	...
	fprintf(fp, "            is_kvaddr: %s\n",
		machdep->machspec->is_kvaddr == book3e_is_kvaddr ? "book3e_is_kvaddr()" : "generic_is_kvaddr()");
	...
	if (machdep->machspec->vmemmap_cnt) {
		...
		for (i = 0; i < machdep->machspec->vmemmap_cnt; i++) {
			fprintf(fp,
				...
				i, machdep->machspec->vmemmap_list[i].virt, machdep->machspec->vmemmap_list[i].phys);
		}
	} else {
 ...
 */
static int ppc64_vtop(ulong vaddr, ulong * pgd, physaddr_t * paddr, int verbose)
{
	ulong *page_dir;
	ulong *page_middle;
	ulong *page_table;
	...
	if (verbose)
		fprintf(fp, "  PMD: %lx => %lx\n", (ulong) page_middle, (ulong) page_table);
 ...
 * by both ppc64_kvtop and ppc64_uvtop.
 */
static int ppc64_vtop_level4(ulong vaddr, ulong * level4, physaddr_t * paddr, int verbose)
{
	ulong *level4_dir;
	ulong *page_dir;
	ulong *page_middle;
...
static int ppc64_uvtop(struct task_context *tc, ulong vaddr, physaddr_t * paddr, int verbose)
{
	ulong mm, active_mm;
	ulong *pgd;
 ...
 *  other callers quietly accept the translation.
 */
static int ppc64_kvtop(struct task_context *tc, ulong kvaddr, physaddr_t * paddr, int verbose)
{
	if (!IS_KVADDR(kvaddr))
		return FALSE;

	if ((machdep->flags & VMEMMAP) && (kvaddr >= machdep->machspec->vmemmap_base))
...
static void ppc64_vmemmap_init(void)
{
	...
	if (!(kernel_symbol_exists("vmemmap_list")) ||
	    ...
	    !MEMBER_EXISTS("vmemmap_backing", "virt_addr") || !MEMBER_EXISTS("vmemmap_backing", "list"))
		return;
	...
	if (!readmem(symbol_value("mmu_vmemmap_psize"),
		     KVADDR, &psize, sizeof(int), "mmu_vmemmap_psize", RETURN_ON_ERROR))
		return;
	if (!readmem(symbol_value("mmu_psize_defs") +
		     (STRUCT_SIZE("mmu_psize_def") * psize) +
		     MEMBER_OFFSET("mmu_psize_def", "shift"),
		     KVADDR, &shift, sizeof(int), "mmu_psize_def shift", RETURN_ON_ERROR))
		return;
	...
	if (!readmem(symbol_value("vmemmap_list"), KVADDR, &ld->start, sizeof(void *), "vmemmap_list", RETURN_ON_ERROR))
		return;
	...
	if ((ms->vmemmap_list = (struct ppc64_vmemmap *)malloc(cnt * sizeof(struct ppc64_vmemmap)))
	    == NULL)
		error(FATAL, "cannot malloc vmemmap list space");

	vmemmap_buf = GETBUF(backing_size);
	for (i = 0; i < cnt; i++) {
		if (!readmem(vmemmap_list[i], KVADDR, vmemmap_buf, backing_size, "vmemmap_backing", RETURN_ON_ERROR)) {
 ...
 */
static int ppc64_vmemmap_to_phys(ulong kvaddr, physaddr_t * paddr, int verbose)
{
	int i;
	ulong offset;
	struct machine_specific *ms;
	...
	for (i = 0; i < ms->vmemmap_cnt; i++) {
		if ((kvaddr >= ms->vmemmap_list[i].virt) && (kvaddr < (ms->vmemmap_list[i].virt + ms->vmemmap_psize))) {
 ...
 *
 */
static int ppc64_is_task_addr(ulong task)
{
	int i;
...
static ulong ppc64_processor_speed(void)
{
	...
	if (symbol_exists("allnodes")) {
		...
		while (node) {
			readmem(node + OFFSET(device_node_type),
				KVADDR, &type, sizeof(ulong), "node type", FAULT_ON_ERROR);
			if (type != 0) {
				len = read_string(type, str_buf, sizeof(str_buf));
			...
			readmem(node + OFFSET(device_node_allnext),
				KVADDR, &node, sizeof(ulong), "node allnext", FAULT_ON_ERROR);
		}

		/* now, if we found a CPU node, get the speed property */
		if (node) {
			readmem(node + OFFSET(device_node_properties),
				KVADDR, &properties, sizeof(ulong), "node properties", FAULT_ON_ERROR);

			while (properties) {
				readmem(properties + OFFSET(property_name),
					KVADDR, &name, sizeof(ulong), "property name", FAULT_ON_ERROR);

				len = read_string(name, str_buf, sizeof(str_buf));

				if (len && (strcasecmp(str_buf, "clock-frequency")
					    == 0)) {
					...
					readmem(properties +
						...
						"clock freqency pointer", FAULT_ON_ERROR);
					readmem(value, KVADDR, &mhz,
						sizeof(int), "clock frequency value", FAULT_ON_ERROR);
					mhz /= 1000000;
					break;
				} else if (len && (strcasecmp(str_buf, "ibm,extended-clock-frequency")
						   == 0)) {
					/* found the right cpu property */
					...
					readmem(properties +
						...
						"clock freqency pointer", FAULT_ON_ERROR);
					readmem(value, KVADDR, &mhz,
						sizeof(ulong), "clock frequency value", FAULT_ON_ERROR);
					mhz /= 1000000;
					break;
				}
				...
				readmem(properties +
					...
					KVADDR, &properties, sizeof(ulong), "property next", FAULT_ON_ERROR);
			}
			if (!properties) {
	...
	if (symbol_exists("res") && !mhz) {
		...
		if (symbol_exists("prep_setup_residual")) {
			get_symbol_data("prep_setup_residual", sizeof(void *), &prep_setup_res);
			get_symbol_data("ppc_md", sizeof(void *), &ppc_md);
			readmem(ppc_md +
				OFFSET(machdep_calls_setup_residual),
				KVADDR, &md_setup_res, sizeof(ulong), "ppc_md setup_residual", FAULT_ON_ERROR);
			...
			if (prep_setup_res == md_setup_res) {
				...
				readmem(res +
					...
					KVADDR, &mhz, sizeof(ulong), "res VitalProductData", FAULT_ON_ERROR);
		...
		if (!mhz) {
			...
			readmem(res +
				...
				KVADDR, &mhz, sizeof(ulong), "bd_info bi_intfreq", FAULT_ON_ERROR);
 ...
 */
static int ppc64_verify_symbol(const char *name, ulong value, char type)
{
 ...
 */
static ulong ppc64_get_task_pgd(ulong task)
{
	long offset;
	ulong pg_tables;

	offset = VALID_MEMBER(task_struct_thread) ? OFFSET(task_struct_thread) : OFFSET(task_struct_tss);
	...
	readmem(task + offset, KVADDR, &pg_tables, sizeof(ulong), "task thread pg_tables", FAULT_ON_ERROR);
 ...
 */
static int ppc64_translate_pte(ulong pte, void *physaddr, ulonglong pte_shift)
{
 ...
 */
static ulong ppc64_check_sp_in_HWintrstack(ulong sp, struct bt_info *bt)
{
	/*
	...
	if (is_task_active(bt->task) && (tt->panic_task != bt->task) &&
		...
		readmem(machdep->machspec->hwintrstack[bt->tc->processor],
			KVADDR, &newsp, sizeof(ulong), "stack pointer", FAULT_ON_ERROR);
		if (INSTACK(newsp, bt))
			sp = newsp;
	}
 ...
 */
static int ppc64_eframe_search(struct bt_info *bt_in)
{
	ulong addr;
	...
	for (; first <= last; first++) {
		...
		if (THIS_KERNEL_VERSION < LINUX(2, 6, 0)) {
			...
			if (!IS_KVADDR(regs->gpr[1]) || !IS_KVADDR(regs->nip)
				...
				if (!IS_UVADDR(regs->gpr[1], bt->tc) || !IS_UVADDR(regs->nip, bt->tc))
					continue;
		} else {
			/*
...
static ulong ppc64_in_irqstack(ulong addr)
{
	int c;
 ...
 */
static void ppc64_back_trace_cmd(struct bt_info *bt)
{
	char buf[BUFSIZE];
 ...
 */
  ...
  */
static void ppc64_back_trace(struct gnu_request *req, struct bt_info *bt)
{
	int frame = 0;
	...
	if (!INSTACK(req->sp, bt)) {
		...
		if ((irqstack = ppc64_in_irqstack(req->sp))) {
			...
			bt->stacktop = ms->hwintrstack[bt->tc->processor] + sizeof(ulong);
			bt->stackbase = ms->hwintrstack[bt->tc->processor] - ms->hwstacksize + STACK_FRAME_OVERHEAD;
	...
	while (INSTACK(req->sp, bt)) {
		...
		if ((req->name = closest_symbol(req->pc)) == NULL) {
			if (CRASHDEBUG(1)) {
				error(FATAL, "ppc64_back_trace hit unknown symbol (%lx).\n", req->pc);
			}
		}
		...
		if (IS_KVADDR(newsp)) {
			...
			if (IS_KVADDR(newsp) && INSTACK(newsp, bt))
				newpc = *(ulong *) & bt->stackbuf[newsp + 16 - bt->stackbase];
		}
		 ...
		 */
		if (THIS_KERNEL_VERSION < LINUX(2, 6, 0)) {
			if (frame && (newsp - req->sp - STACK_FRAME_OVERHEAD) >= sizeof(struct ppc64_pt_regs))
				eframe_found = TRUE;
			else if (STREQ(req->name, ".ret_from_except"))
				eframe_found = TRUE;
		} else if ((newsp - req->sp - STACK_FRAME_OVERHEAD) >= sizeof(struct ppc64_pt_regs)) {
			readmem(req->sp + 0x60, KVADDR, &marker, sizeof(ulong), "stack frame", FAULT_ON_ERROR);
		...
		if (eframe_found) {
			...
			readmem(req->sp + STACK_FRAME_OVERHEAD, KVADDR, &regs,
				sizeof(struct ppc64_pt_regs), "exception frame", FAULT_ON_ERROR);
...
static void ppc64_display_full_frame(struct bt_info *bt, ulong nextsp, FILE * ofp)
{
	int i, u_idx;
	ulong *nip;
	ulong words, addr;
 ...
 */
static void ppc64_print_stack_entry(int frame, struct gnu_request *req, ulong newsp, ulong lr, struct bt_info *bt)
{
	...
	if (BT_REFERENCE_CHECK(bt)) {
		...
		if (lr) {
			...
			if (req->pc != lr) {
				fprintf(fp, "\n [Link Register ] ");
				fprintf(fp, " [%lx] %s at %lx", req->sp, lrname, lr);
			}
			req->ra = lr;
		}
 ...
 */
static char *ppc64_check_eframe(struct ppc64_pt_regs *regs)
{
...
static void ppc64_print_regs(struct ppc64_pt_regs *regs)
{
	int i;
	...
	for (i = 0; i < 32; i++) {
		...
		fprintf(fp, " R%d:%s %016lx   ", i, ((i < 10) ? " " : ""), regs->gpr[i]);
		/*
 ...
 */
static void ppc64_print_eframe(char *efrm_str, struct ppc64_pt_regs *regs, struct bt_info *bt)
{
	if (BT_REFERENCE_CHECK(bt))
		return;
 ...
 */
static int ppc64_kdump_stack_frame(struct bt_info *bt_in, ulong * nip, ulong * ksp)
{
	...
	*ksp = pt_regs->gpr[1];
	if (IS_KVADDR(*ksp)) {
		readmem(*ksp + 16, KVADDR, &unip, sizeof(ulong), "Regs NIP value", FAULT_ON_ERROR);
		*nip = unip;
	} else {
	...
	if (bt_in->flags && ((BT_TEXT_SYMBOLS | BT_TEXT_SYMBOLS_PRINT | BT_TEXT_SYMBOLS_NOPRINT)))
		return TRUE;
	...
	fprintf(fp, " NIP [%016lx] %s\n", pt_regs->nip, closest_symbol(pt_regs->nip));
	if (unip != pt_regs->link)
		fprintf(fp, " LR  [%016lx] %s\n", pt_regs->link, closest_symbol(pt_regs->link));
...
static int ppc64_get_dumpfile_stack_frame(struct bt_info *bt_in, ulong * nip, ulong * ksp)
{
	...
	if (panic_task && bt->machdep) {
		...
		if (symbol_exists("dump_header")) {	/* Diskdump */
			 ...
			 */
			ulong offset = roundup(STRUCT_SIZE("timespec") + STRUCT_SIZE("new_utsname") + 52,
					       8);
			offset += sizeof(ulong) * bt->tc->processor;
			readmem(symbol_value("dump_header") + offset, KVADDR,
				&task_addr, sizeof(ulong), "Task Address", FAULT_ON_ERROR);
			if (task_addr)
				cpu_frozen = TRUE;
		}
		if (!cpu_frozen && symbol_exists("cpus_frozen")) {	/* Netdump */
			readmem(symbol_value("cpus_frozen") +
				...
				&cpu_frozen, sizeof(char), "CPU Frozen Value", FAULT_ON_ERROR);
		}
		...
		if (IS_KVADDR(ur_ksp)) {
			 ...
			 */
			if (cpu_frozen)
				readmem(ur_ksp, KVADDR, &ur_ksp, sizeof(ulong), "Stack Pointer", FAULT_ON_ERROR);
			else
				fprintf(fp, "%0lx: GPR1 register value (SP) was not saved\n", bt->task);
			if (IS_KVADDR(ur_ksp))
				/*
				 ...
				 */
				readmem(ur_ksp + 16, KVADDR, &ur_nip, sizeof(ulong), "Regs NIP value", FAULT_ON_ERROR);
			*ksp = ur_ksp;
			*nip = ur_nip;
		} else {
			*ksp = ur_ksp;
 ...
 retry:
	...
	for (i = 0, up = (ulong *) bt->stackbuf; i < (bt->stacktop - bt->stackbase) / sizeof(ulong); i++, up++) {
		sym = closest_symbol(*up);

		if (STREQ(sym, ".netconsole_netdump") ||
		    STREQ(sym, ".netpoll_start_netdump") ||
		    STREQ(sym, ".start_disk_dump") ||
		    STREQ(sym, ".crash_kexec") || STREQ(sym, ".crash_fadump") || STREQ(sym, ".disk_dump")) {
			*nip = *up;
			*ksp = bt->stackbase + ((char *)(up) - 16 - bt->stackbuf);
			return TRUE;
		}
	}
 ...
 */
static void ppc64_get_stack_frame(struct bt_info *bt, ulong * pcp, ulong * spp)
{
	ulong ksp, nip;
...
static ulong ppc64_get_sp(ulong task)
{
	ulong sp;

	if (tt->flags & THREAD_INFO)
		readmem(task + OFFSET(task_struct_thread_ksp), KVADDR,
			&sp, sizeof(void *), "thread_struct ksp", FAULT_ON_ERROR);
	else {
		ulong offset;
		offset = OFFSET_OPTION(task_struct_thread_ksp, task_struct_tss_ksp);
		readmem(task + offset, KVADDR, &sp, sizeof(void *), "task_struct ksp", FAULT_ON_ERROR);
	}
	return sp;
}
 ...
 */
static void get_ppc64_frame(struct bt_info *bt, ulong * getpc, ulong * getsp)
{
	ulong ip;
	ulong sp;
	ulong *stack;
	...
	readmem(sp + STACK_FRAME_OVERHEAD, KVADDR, &regs,
		sizeof(struct ppc64_pt_regs), "PPC64 pt_regs", FAULT_ON_ERROR);
	ip = regs.nip;
 ...
 */
static void ppc64_dump_irq(int irq)
{
	...
	readmem(irq_desc_addr + OFFSET(irqdesc_level), KVADDR, &level, sizeof(int), "irq_desc entry", FAULT_ON_ERROR);
	readmem(irq_desc_addr + OFFSET(irqdesc_action), KVADDR, &action,
		sizeof(long), "irq_desc entry", FAULT_ON_ERROR);
	readmem(irq_desc_addr + OFFSET(irqdesc_ctl), KVADDR, &ctl, sizeof(long), "irq_desc entry", FAULT_ON_ERROR);
	...
	if (ctl) {
		...
		if (VALID_MEMBER(hw_interrupt_type_handle)) {
			...
			readmem(ctl + OFFSET(hw_interrupt_type_handle),
				KVADDR, &addr, sizeof(ulong), "interrupt handle", FAULT_ON_ERROR);
			fprintf(fp, "           handle: ");
			if (value_symbol(addr)) {
				fprintf(fp, "%08lx  <%s>\n", addr, value_symbol(addr));
			} else
	...
	while (action) {
		...
		if (value) {
			...
			if (value & SA_INTERRUPT)
				fprintf(fp, "%sSA_INTERRUPT", others++ ? "|" : "");
			if (value & SA_PROBE)
				fprintf(fp, "%sSA_PROBE", others++ ? "|" : "");
			if (value & SA_SAMPLE_RANDOM)
				fprintf(fp, "%sSA_SAMPLE_RANDOM", others++ ? "|" : "");
			if (value & SA_SHIRQ)
				fprintf(fp, "%sSA_SHIRQ", others++ ? "|" : "");
			fprintf(fp, ")");
			if (value & ~ACTION_FLAGS) {
				fprintf(fp, "  (bits %lx not translated)", value & ~ACTION_FLAGS);
			}
		}
		...
		/* mask */
		readmem(action + OFFSET(irqaction_mask), KVADDR, &value, sizeof(ulong), "action mask", FAULT_ON_ERROR);
		fprintf(fp, "             mask: %lx\n", value);

		/* name */
		readmem(action + OFFSET(irqaction_name), KVADDR, &addr, sizeof(ulong), "action name", FAULT_ON_ERROR);
		...
		/* next */
		readmem(action + OFFSET(irqaction_next), KVADDR, &value, sizeof(ulong), "action next", FAULT_ON_ERROR);
		fprintf(fp, "             next: %lx\n", value);
 ...
 */
static int ppc64_dis_filter(ulong vaddr, char *inbuf, unsigned int output_radix)
{
	char buf1[BUFSIZE];
	char buf2[BUFSIZE];
	char *colon, *p1;
...
static void ppc64_dump_line_number(ulong callpc)
{
	int retries;
	char buf[BUFSIZE], *p;
...
static int ppc64_paca_init(int map)
{
	...
	if (get_symbol_type("paca", NULL, NULL) == TYPE_CODE_PTR)
		readmem(symbol_value("paca"), KVADDR, &paca, sizeof(ulong), "paca", FAULT_ON_ERROR);
	else
	...
	if (nr_paca > NR_CPUS) {
		error(WARNING, "PPC64: Number of paca entries (%d) greater than NR_CPUS (%d)\n", nr_paca, NR_CPUS);
	...
	for (i = cpus = 0; i < nr_paca; i++) {
		...
		readmem(paca + (i * SIZE(ppc64_paca)),
			KVADDR, cpu_paca_buf, SIZE(ppc64_paca), "paca entry", FAULT_ON_ERROR);
...
static int ppc64_get_cpu_map(void)
{
	...
	if (cpu_map_addr("possible"))
		...
		map = 0;
		error(FATAL, "PPC64: cannot find 'cpu_possible_map' or\
...
void ppc64_clear_machdep_cache(void)
{
	if (machdep->machspec->last_level4_read != vt->kernel_pgd[0])
		machdep->machspec->last_level4_read = 0;
}

static int ppc64_get_kvaddr_ranges(struct vaddr_range *vrp)
{
	int cnt;
	...
	if (machdep->flags & VMEMMAP) {
		...
		if (phys_to_page(phys1, &pp1) && phys_to_page(phys2, &pp2)) {
