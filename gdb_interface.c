 ...
 */
void gdb_main_loop(int argc, char **argv)
{
	argc = 1;
	...
	if (pc->flags & SILENT) {
		...
		argv[argc++] = pc->namelist_debug ?
		    pc->namelist_debug :
		    (pc->debuginfo_file && (st->flags & CRC_MATCHES) ? pc->debuginfo_file : pc->namelist);
	} else {
		if (pc->flags & READNOW)
			argv[argc++] = "--readnow";
		argv[argc++] = pc->namelist_debug ?
...
void update_gdb_hooks(void)
{
	...
	command_loop_hook = pc->flags & VERSION_QUERY ? exit_after_gdb_info : main_loop;
	target_new_objfile_hook = NULL;
#endif
#if defined(GDB_7_0) || defined(GDB_7_3_1) || defined(GDB_7_6)
	deprecated_command_loop_hook = pc->flags & VERSION_QUERY ? exit_after_gdb_info : main_loop;
#endif
}

void gdb_readnow_warning(void)
{
	if ((THIS_GCC_VERSION >= GCC(3, 4, 0))
	    && (THIS_GCC_VERSION < GCC(4, 0, 0))
	    && !(pc->flags & READNOW)) {
		fprintf(stderr,
		...
		if (!debug_data_pulled_in) {
			if (CRASHDEBUG(1))
				error(INFO,
				      ...
				      symbol_exists("sysfs_mount") ? "and syfs_mount" : "");
 ...
 */
int gdb_pass_through(char *cmd, FILE * fptr, ulong flags)
{
	struct gnu_request *req;
	int retval;
	...
	if ((req->flags & (GNU_RETURN_ON_ERROR | GNU_COMMAND_FAILED)) == (GNU_RETURN_ON_ERROR | GNU_COMMAND_FAILED))
		retval = FALSE;
	else
		retval = TRUE;
 ...
 */
void gdb_interface(struct gnu_request *req)
{
	...
	if (!req->fp) {
		req->fp = pc->flags & RUNTIME ? fp : CRASHDEBUG(1) ? fp : pc->nullfp;
	}
	...
	if (interruptible()) {
		SIGACTION(SIGINT, pc->gdb_sigaction.sa_handler, &pc->gdb_sigaction, NULL);
	} else {
...
void dump_gnu_request(struct gnu_request *req, int in_gdb)
{
	int others;
	char buf[BUFSIZE];
...
char *gdb_command_string(int cmd, char *buf, int live)
{
	switch (cmd) {
	...
	case GNU_GET_DATATYPE:
		if (live)
			sprintf(buf, "GNU_GET_DATATYPE[%s]", pc->cur_req->name ? pc->cur_req->name : "?");
		else
 ...
 */
int is_gdb_command(int merge_orig_args, ulong flags)
{
	int retval;
	struct gnu_request *req;
...
static int is_restricted_command(char *cmd, ulong flags)
{
	int i;
	char *newline;
	...
	for (i = 0; restricted_list[i]; i++) {
		if (STREQ(restricted_list[i], cmd)) {
			...
			error(FATAL, RESTRICTED_GDB_COMMAND,
			      cmd, newline, cmd, newline, pc->program_name, newline, newline, pc->program_name);
		}
	}
 ...
 */
static void strip_redirection(char *buf)
{
	char *p1, *p2;
 ...
 */
int gdb_readmem_callback(ulong addr, void *buf, int len, int write)
{
	...
	if (pc->curcmd_flags & MEMTYPE_UVADDR)
		...
		if (STREQ(pc->curcmd, "gdb") && STRNEQ(pc->cur_req->buf, "x/")) {
			memtype = UVADDR;
		} else {
			if (CRASHDEBUG(1))
	...
	if (memtype == FILEADDR)
		return (readmem(pc->curcmd_private, memtype, buf, len, "gdb_readmem_callback", RETURN_ON_ERROR));

	switch (len) {
	case SIZEOF_8BIT:
		p1 = (char *)buf;
		if ((memtype == KVADDR) && text_value_cache_byte(addr, (unsigned char *)p1))
			return TRUE;

		if (!readmem(addr, memtype, locbuf, SIZEOF_32BIT, "gdb_readmem_callback", RETURN_ON_ERROR))
			return FALSE;
	...
	case SIZEOF_32BIT:
		...
		if (!readmem(addr, memtype, buf, SIZEOF_32BIT, "gdb_readmem callback", RETURN_ON_ERROR))
			return FALSE;

		if (memtype == KVADDR)
			text_value_cache(addr, (uint32_t) * ((uint32_t *) buf), NULL);
		return TRUE;
	}

	return (readmem(addr, memtype, buf, len, "gdb_readmem_callback", RETURN_ON_ERROR));
 ...
 */
int gdb_line_number_callback(ulong pc, ulong low, ulong high)
{
...
void gdb_error_hook(void)
{
	...
	if (CRASHDEBUG(2)) {
		...
		console("%s: returned via gdb_error_hook %s", gdb_command_string(pc->cur_gdb_cmd, buf1, TRUE), buf2);
	}
#ifdef GDB_7_6
...
static ulong gdb_user_print_option_address(char *name)
{
	struct gnu_request request;
 ...
 */
int gdb_set_crash_scope(ulong vaddr, char *arg)
{
	...
	if (CRASHDEBUG(1))
		fprintf(fp,
			...
			req->flags & GNU_COMMAND_FAILED ? "FAILED" : "OK", req->addr, req->addr2);

	if (req->flags & GNU_COMMAND_FAILED) {
		error(INFO, "gdb cannot find text block for address: %s\n", arg);
		return FALSE;
	}
 ...
 */
int get_frame_offset(ulong pc)
{
	return (error(FATAL, "get_frame_offset: invalid request for non-alpha systems!\n"));
}
#endif				/* !ALPHA */
