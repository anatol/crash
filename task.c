...
void task_init(void)
{
	...
	if (DUMPFILE() && !(tt->panic_threads = (ulong *) calloc(NR_CPUS, sizeof(ulong))))
	...
	if (kernel_symbol_exists("nr_tasks")) {
		...
		tss_offset = MEMBER_OFFSET_INIT(task_struct_tss, "task_struct", "tss");
		eip_offset = MEMBER_OFFSET_INIT(thread_struct_eip, "thread_struct", "eip");
		esp_offset = MEMBER_OFFSET_INIT(thread_struct_esp, "thread_struct", "esp");
		ksp_offset = MEMBER_OFFSET_INIT(thread_struct_ksp, "thread_struct", "ksp");
		ASSIGN_OFFSET(task_struct_tss_eip) =
		    (eip_offset == INVALID_OFFSET) ? INVALID_OFFSET : tss_offset + eip_offset;
		ASSIGN_OFFSET(task_struct_tss_esp) =
		    (esp_offset == INVALID_OFFSET) ? INVALID_OFFSET : tss_offset + esp_offset;
		ASSIGN_OFFSET(task_struct_tss_ksp) =
		    (ksp_offset == INVALID_OFFSET) ? INVALID_OFFSET : tss_offset + ksp_offset;

		tt->flags |= TASK_REFRESH;
		tt->refresh_task_table = refresh_fixed_task_table;

		readmem(tt->task_start, KVADDR, &tt->idle_threads[0],
			kt->cpus * sizeof(void *), "idle threads", FAULT_ON_ERROR);
	} else {
		/*
		...
		thread_offset = MEMBER_OFFSET_INIT(task_struct_thread, "task_struct", "thread");
		eip_offset = MEMBER_OFFSET_INIT(thread_struct_eip, "thread_struct", "eip");
		esp_offset = MEMBER_OFFSET_INIT(thread_struct_esp, "thread_struct", "esp");
		/*
		...
		if (eip_offset == INVALID_OFFSET)
			eip_offset = MEMBER_OFFSET_INIT(thread_struct_eip, "thread_struct", "ip");
		...
		ASSIGN_OFFSET(task_struct_thread_eip) =
		    (eip_offset == INVALID_OFFSET) ? INVALID_OFFSET : thread_offset + eip_offset;
		ASSIGN_OFFSET(task_struct_thread_esp) =
		    (esp_offset == INVALID_OFFSET) ? INVALID_OFFSET : thread_offset + esp_offset;
		ASSIGN_OFFSET(task_struct_thread_ksp) =
		    (ksp_offset == INVALID_OFFSET) ? INVALID_OFFSET : thread_offset + ksp_offset;
	...
	if (VALID_MEMBER(task_struct_thread_info)) {
		...
		MEMBER_OFFSET_INIT(thread_info_previous_esp, "thread_info", "previous_esp");
	...
	if (INVALID_MEMBER(task_struct_parent))
		MEMBER_OFFSET_INIT(task_struct_parent, "task_struct", "real_parent");
	MEMBER_OFFSET_INIT(task_struct_has_cpu, "task_struct", "has_cpu");
	MEMBER_OFFSET_INIT(task_struct_cpus_runnable, "task_struct", "cpus_runnable");
	...
	MEMBER_OFFSET_INIT(task_struct_pidhash_next, "task_struct", "pidhash_next");
	...
	if (VALID_MEMBER(task_struct_sched_info))
		MEMBER_OFFSET_INIT(sched_info_last_arrival, "sched_info", "last_arrival");
	if (VALID_MEMBER(task_struct_last_run) ||
	    VALID_MEMBER(task_struct_timestamp) || VALID_MEMBER(sched_info_last_arrival)) {
		char buf[BUFSIZE];
	...
	MEMBER_OFFSET_INIT(signal_struct_shared_pending, "signal_struct", "shared_pending");
	...
	if (VALID_STRUCT(sighand_struct))
		MEMBER_OFFSET_INIT(sighand_struct_action, "sighand_struct", "action");
	...
	if (symbol_exists("cfq_slice_async")) {
		...
		get_symbol_data("cfq_slice_async", sizeof(int), &cfq_slice_async);

		if (cfq_slice_async) {
			machdep->hz = cfq_slice_async * 25;

			if (CRASHDEBUG(2))
				fprintf(fp, "cfq_slice_async exists: setting hz to %d\n", machdep->hz);
		}
	}

	if (VALID_MEMBER(runqueue_arrays))
		MEMBER_OFFSET_INIT(task_struct_run_list, "task_struct", "run_list");
	...
	if ((tt->flags & THREAD_INFO) && ((tt->thread_info = (char *)malloc(SIZE(thread_info))) == NULL))
	...
	} else if (VALID_SIZE(thread_union) && ((len = SIZE(thread_union)) != STACKSIZE()))
		machdep->stacksize = len;

	if (symbol_exists("pidhash") && symbol_exists("pid_hash") && !symbol_exists("pidhash_shift"))
	...
	if (symbol_exists("pid_hash") && symbol_exists("pidhash_shift")) {
		...
		if (get_symbol_type("PIDTYPE_PID", NULL, &req) != TYPE_CODE_ENUM)
			error(FATAL, "cannot determine PIDTYPE_PID pid_hash dimension\n");
		...
		if (VALID_MEMBER(pid_link_pid) && VALID_MEMBER(pid_hash_chain)) {
			...
			if (LKCD_KERNTYPES()) {
				if (VALID_STRUCT(pid_link)) {
					if (VALID_STRUCT(upid)
					    && VALID_MEMBER(pid_numbers))
						tt->refresh_task_table = refresh_hlist_task_table_v3;
					else
						tt->refresh_task_table = refresh_hlist_task_table_v2;
				} else
					tt->refresh_task_table = refresh_hlist_task_table;
				builtin_array_length("pid_hash", tt->pidhash_len, NULL);
			} else {
		...
		if (get_symbol_type("PIDTYPE_PGID", NULL, &req) != TYPE_CODE_ENUM)
			error(FATAL, "cannot determine PIDTYPE_PID pid_hash dimension\n");
		if (!(tt->pidhash_len = get_array_length("pid_hash", &dim, SIZE(list_head))))
		...
		if (tt->pidhash_len == 0) {
			if (!(nsp = next_symbol("pidhash", NULL)))
				error(FATAL, "cannot determine pidhash length\n");
			tt->pidhash_len = (nsp->value - tt->pidhash_addr) / sizeof(void *);
		}
		if (ACTIVE())
...
static void irqstacks_init(void)
{
		...
		if ((kt->flags & SMP) && (kt->flags & PER_CPU_OFF)) {
			for (i = 0; i < NR_CPUS; i++) {
				if (!kt->__per_cpu_offset[i])
					continue;
				tt->hardirq_ctx[i] = hard_sp->value + kt->__per_cpu_offset[i];
			}
		} else
			tt->hardirq_ctx[0] = hard_sp->value;
	} else if (symbol_exists("hardirq_ctx")) {
		i = get_array_length("hardirq_ctx", NULL, 0);
		get_symbol_data("hardirq_ctx", sizeof(long) * (i <= NR_CPUS ? i : NR_CPUS), &tt->hardirq_ctx[0]);
	} else
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		if (!readmem(tt->hardirq_ctx[i], KVADDR, thread_info_buf,
			     SIZE(irq_ctx), "hardirq thread_union", RETURN_ON_ERROR)) {
			error(INFO, "cannot read hardirq_ctx[%d] at %lx\n", i, tt->hardirq_ctx[i]);
			continue;
		}

		tt->hardirq_tasks[i] = ULONG(thread_info_buf + OFFSET(thread_info_task));
	}
	...
	if ((soft_sp = per_cpu_symbol_search("per_cpu__softirq_ctx"))) {
		if ((kt->flags & SMP) && (kt->flags & PER_CPU_OFF)) {
			for (i = 0; i < NR_CPUS; i++) {
				if (!kt->__per_cpu_offset[i])
					continue;
				tt->softirq_ctx[i] = soft_sp->value + kt->__per_cpu_offset[i];
			}
		} else
			tt->softirq_ctx[0] = soft_sp->value;
	} else if (symbol_exists("softirq_ctx")) {
		i = get_array_length("softirq_ctx", NULL, 0);
		get_symbol_data("softirq_ctx", sizeof(long) * (i <= NR_CPUS ? i : NR_CPUS), &tt->softirq_ctx[0]);
	} else
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		if (!readmem(tt->softirq_ctx[i], KVADDR, thread_info_buf,
			     SIZE(irq_ctx), "softirq thread_union", RETURN_ON_ERROR)) {
			error(INFO, "cannot read softirq_ctx[%d] at %lx\n", i, tt->hardirq_ctx[i]);
			continue;
		}

		tt->softirq_tasks[i] = ULONG(thread_info_buf + OFFSET(thread_info_task));
	}
...
int in_irq_ctx(ulonglong type, int cpu, ulong addr)
{
	...
	case BT_SOFTIRQ:
		if (tt->softirq_ctx[cpu] &&
		    (addr >= tt->softirq_ctx[cpu]) && (addr < (tt->softirq_ctx[cpu] + STACKSIZE())))
			return TRUE;
		break;

	case BT_HARDIRQ:
		if (tt->hardirq_ctx[cpu] &&
		    (addr >= tt->hardirq_ctx[cpu]) && (addr < (tt->hardirq_ctx[cpu] + STACKSIZE())))
			return TRUE;
		break;
	}
 ...
 */
static void allocate_task_space(int cnt)
{
	if (tt->context_array == NULL) {
		...
		if (!(tt->task_local = (void *)
			...
			error(FATAL,
			      "%scannot realloc kernel task array (%d tasks)", (pc->flags & RUNTIME) ? "" : "\n", cnt);

		if (!(tt->context_array = (struct task_context *)
		      realloc(tt->context_array, cnt * sizeof(struct task_context))))
			error(FATAL,
			      "%scannot realloc context array (%d tasks)", (pc->flags & RUNTIME) ? "" : "\n", cnt);
	}
}
...
#define TASK_IN_USE(x) (!TASK_FREE(x))
	...
	if (DUMPFILE()) {
		fprintf(fp, (pc->flags & SILENT) || !(pc->flags & TTY) ?
			"" : "%splease wait... (gathering task table data)", GDB_PATCHED()? "" : "\n");
		fflush(fp);
 ...
 retry:
	if (!readmem(tt->task_start, KVADDR, tt->task_local,
		     tt->max_tasks * sizeof(void *), "kernel task array", RETURN_ON_ERROR))
		error(FATAL, "cannot read kernel task array");

	clear_task_cache();

	for (i = 0, tlp = (ulong *) tt->task_local,
	     tt->running_tasks = 0, tc = tt->context_array; i < tt->max_tasks; i++, tlp++) {
		if (TASK_IN_USE(*tlp)) {
 ...
 */
static int verify_task(struct task_context *tc, int level)
{
	int i;
	ulong next_task;
	ulong readflag;
	...
	case 2:
		...
		if ((tc->processor < 0) || (tc->processor >= NR_CPUS)) {
			...
			if (CRASHDEBUG(1))
				error(INFO, "verify_task: task: %lx invalid processor: %u", tc->task, tc->processor);
			return FALSE;
		}
...
void refresh_unlimited_task_table(void)
{
	int i;
	ulong *tlp;
	struct task_context *tc;
 ...
 retry:
	...
	if ((retries == MAX_UNLIMITED_TASK_RETRIES) && !(tt->flags & TASK_INIT_DONE))
...
static void refresh_pidhash_task_table(void)
{
 ...
 retry_pidhash:
	...
	if (!readmem(tt->pidhash_addr, KVADDR, pidhash, len * sizeof(ulong), "pidhash contents", RETURN_ON_ERROR))
	...
	for (i = 0, pp = (ulong *) pidhash; i < len; i++, pp++) {
		...
		while (next) {
			if (!IS_TASK_ADDR(next)) {
				error(INFO, "%sinvalid task address in pidhash: %lx\n", DUMPFILE()? "\n" : "", next);
				if (DUMPFILE())
					break;
				hq_close();
				retries++;
			...
			if (!readmem(next + OFFSET(task_struct_pidhash_next),
				     KVADDR, &pnext, sizeof(void *), "pidhash_next entry", QUIET | RETURN_ON_ERROR)) {
			...
			if (!hq_enter(next)) {
				error(INFO, "%sduplicate task in pidhash: %lx\n", DUMPFILE()? "\n" : "", next);
				if (DUMPFILE())
					break;
				hq_close();
				retries++;
	...
	for (i = 0, tlp = (ulong *) tt->task_local,
		...
		if (!IS_TASK_ADDR(*tlp)) {
			error(WARNING, "%sinvalid task address found in task list: %lx\n", DUMPFILE()? "\n" : "", *tlp);
			if (DUMPFILE())
				continue;
			retries++;
			goto retry_pidhash;
...
static void refresh_pid_hash_task_table(void)
{
 ...
 retry_pid_hash:
	...
	if (!readmem(tt->pidhash_addr, KVADDR, pid_hash, len * SIZE(list_head), "pid_hash contents", RETURN_ON_ERROR))
	...
	for (i = 0; i < len; i++) {
		pp = &pid_hash[i];
		kpp = (struct kernel_list_head *)(tt->pidhash_addr + i * SIZE(list_head));
		if (pp->next == kpp)
			continue;

		if (CRASHDEBUG(7))
			console("%lx: pid_hash[%d]: %lx (%lx) %lx (%lx)\n", kpp,
				i, pp->next, HASH_TO_TASK(pp->next), pp->prev, HASH_TO_TASK(pp->prev));
		...
		while (next) {
			...
			if (!readmem(TASK_TO_HASH(next),
				     KVADDR, &pnext, sizeof(void *), "pid_hash entry", QUIET | RETURN_ON_ERROR)) {
				error(INFO, "%scannot read from task: %lx\n", DUMPFILE()? "\n" : "", next);
				if (DUMPFILE())
					break;
				hq_close();
...
static void refresh_hlist_task_table(void)
{
	...
	if (!(plen = get_array_length("pid_hash", NULL, sizeof(void *)))) {
		...
		if (enumerator_value("PIDTYPE_MAX", &value)) {
			...
			error(WARNING, "cannot determine pid_hash array dimensions\n");
			plen = 1;
		}
	}
	...
	if (CRASHDEBUG(1)) {
		if (!enumerator_value("PIDTYPE_PID", &value))
			error(WARNING, "possible pid_hash array mis-handling: PIDTYPE_PID: (unknown)\n");
		else if (value != 0)
			error(WARNING, "possible pid_hash array mis-handling: PIDTYPE_PID: %d \n", value);
	}
 ...
 retry_pid_hash:
	...
	if (!readmem(pidhash_array, KVADDR, pid_hash, len * SIZE(hlist_head), "pid_hash[0] contents", RETURN_ON_ERROR))
	...
	for (i = 0; i < len; i++) {
		...
		if (!readmem(pid_hash[i], KVADDR, nodebuf, SIZE(hlist_node), "pid_hash node", RETURN_ON_ERROR | QUIET)) {
		...
		if (CRASHDEBUG(1))
			console
			    ("pid_hash[%d]: %lx task: %lx (node: %lx) next: %lx pprev: %lx\n",
...
static void refresh_hlist_task_table_v2(void)
{
 ...
 retry_pid_hash:
	...
	if (!readmem(pidhash_array, KVADDR, pid_hash, len * SIZE(hlist_head), "pid_hash contents", RETURN_ON_ERROR))
		error(FATAL, "\ncannot read pid_hash array\n");
...
static void refresh_hlist_task_table_v3(void)
{
 ...
 retry_pid_hash:
	...
	for (i = 0; i < len; i++) {
		...
		chained = 0;
 do_chained:
		if (!readmem(upid, KVADDR, nodebuf, SIZE(upid), "pid_hash upid", RETURN_ON_ERROR | QUIET)) {
 ...
 */
static struct task_context *store_context(struct task_context *tc, ulong task, char *tp)
{
	pid_t *pid_addr;
	char *comm_addr;
	int *processor_addr;
	...
	if (tt->flags & THREAD_INFO) {
		...
		processor_addr = (int *)(tt->thread_info + OFFSET(thread_info_cpu));
 ...
 */
static void refresh_context(ulong curtask, ulong curpid)
{
...
void sort_context_array(void)
{
	...
	curtask = CURRENT_TASK();
	qsort((void *)tt->context_array, (size_t) tt->running_tasks, sizeof(struct task_context), sort_by_pid);
	set_context(curtask, NO_PID);
}

static int sort_by_pid(const void *arg1, const void *arg2)
{
	...
	if ((t1->pid == 0) && (t2->pid == 0))
		return (t1->processor < t2->processor ? -1 : t1->processor == t2->processor ? 0 : 1);
	else
		return (t1->pid < t2->pid ? -1 : t1->pid == t2->pid ? 0 : 1);
}
...
static void sort_context_array_by_last_run(void)
{
	...
	curtask = CURRENT_TASK();
	qsort((void *)tt->context_array, (size_t) tt->running_tasks, sizeof(struct task_context), sort_by_last_run);
	set_context(curtask, NO_PID);
}
...
char *fill_task_struct(ulong task)
{
	if (XEN_HYPER_MODE())
		return NULL;
...
char *fill_thread_info(ulong thread_info)
{
 ...
 */
void fill_stackbuf(struct bt_info *bt)
{
	if (!bt->stackbuf) {
 ...
 *  and stacktop values.
 */
void alter_stackbuf(struct bt_info *bt)
{
	if (!readmem(bt->stackbase, KVADDR, bt->stackbuf,
		     bt->stacktop - bt->stackbase, "stack contents", RETURN_ON_ERROR))
		error(FATAL, "read of stack at %lx failed\n", bt->stackbase);
...
void cmd_task(void)
{
	...
	while (args[optind]) {
		...
		} else if (strstr(args[optind], ",") || MEMBER_EXISTS("task_struct", args[optind])) {
			if (strlen(ref->str))
				strcat(ref->str, ",");
			strcat(ref->str, args[optind]);
		} else
			error(INFO, "invalid task, pid, or task_struct member: %s\n\n", args[optind]);
		optind++;
	}
 ...
 */
void do_task(ulong task, ulong flags, struct reference *ref, unsigned int radix)
{
	struct task_context *tc;
 ...
 */
static void task_struct_member(struct task_context *tc, unsigned int radix, struct reference *ref)
{
	int i;
	int argcnt;
	...
	if ((count_chars(ref->str, ',') + 1) > MAXARGS) {
		error(INFO, "too many -R arguments in comma-separated list!\n");
		return;
	}
	...
	for (i = 0; i < argcnt; i++)
		if (!MEMBER_EXISTS("task_struct", arglist[i]) && !MEMBER_EXISTS("thread_info", arglist[i]))
...
static char *ps_exclusive = "-a, -t, -c, -p, -g, -l and -r flags are all mutually-exclusive\n";
...
void cmd_ps(void)
{
	...
	while ((c = getopt(argcnt, args, "gstcpkuGlar")) != EOF) {
		...
		case 'k':
			if (flag & PS_USER)
				error(FATAL, "-u and -k are mutually exclusive\n");
			flag |= PS_KERNEL;
			break;
		...
		case 'l':
			if (INVALID_MEMBER(task_struct_last_run) &&
			    INVALID_MEMBER(task_struct_timestamp) && INVALID_MEMBER(sched_info_last_arrival)) {
				error(INFO, "last-run timestamps do not exist in this kernel\n");
				argerrs++;
				break;
			}
			if (flag & PS_EXCLUSIVE)
	...
	while (args[optind]) {
		if (IS_A_NUMBER(args[optind])) {
			...
			if (psinfo.regexs == MAX_PS_ARGS)
				...
				if (regcomp(&psinfo.regex_data[psinfo.regexs].regex, p, REG_EXTENDED | REG_NOSUB)) {
					error(INFO, "invalid regular expression: %s\n", p);
					free(p);
					goto bailout;
				}
			...
			psinfo.pid[ac] = NO_PID;
			psinfo.task[ac] = NO_TASK;
			p = args[optind][0] == '\\' ? &args[optind][1] : args[optind];
 ...
 */
static void ps_cleanup(void *arg)
{
	int i;
	struct psinfo *ps;
 ...
 */
static void show_ps_data(ulong flag, struct task_context *tc)
{
	...
	fprintf(fp, "%5ld  %5ld  %2s  %s %3s",
		...
		task_pointer_string(tc, flag & PS_KSTACKP, buf3), task_state_string(tc->task, buf1, !VERBOSE));
...
static void show_ps(ulong flag, struct psinfo *psi)
{
	int i, ac;
	struct task_context *tc;
	int print;
	...
	for (i = 0; i < RUNNING_TASKS(); i++, tc++) {
		for (ac = 0; ac < psi->argc; ac++) {
			...
			case PS_BY_REGEX:
				if (regexec(&psi->regex_data[ac].regex, tc->comm, 0, NULL, 0) == 0) {
					if (flag & (PS_TGID_LIST | PS_GROUP)) {
						if (tc->pid == task_tgid(tc->task))
							print = TRUE;
						else
 ...
 *  current state.
 */
static void show_last_run(struct task_context *tc)
{
	int i, c;
	...
	tcp = FIRST_CONTEXT();
	sprintf(buf, pc->output_radix == 10 ? "%lld" : "%llx", task_last_run(tcp->task));
	c = strlen(buf);
	sprintf(format, "[%c%dll%c] ", '%', c, pc->output_radix == 10 ? 'u' : 'x');
 ...
 */
static void show_task_args(struct task_context *tc)
{
	...
	while (size > 0) {
		...
		if (!readmem(paddr, PHYSADDR, bufptr, cnt, "user stack contents", RETURN_ON_ERROR | QUIET)) {
			error(INFO, "cannot access user stack address: %lx\n\n", uvaddr);
			goto bailout;
		}
 ...
 */
static void show_task_rlimit(struct task_context *tc)
{
	...
	if (in_task_struct) {
		BCOPY(tt->task_struct + OFFSET(task_struct_rlim), rlimit_buffer, rlimit_index * SIZE(rlimit));
	...
	fprintf(fp, "  %s   %s   %s\n",
		...
		mkstring(buf2, len2, CENTER | RJUST, "CURRENT"), mkstring(buf3, len2, CENTER | RJUST, "MAXIMUM"));

	for (p1 = (ulong *) rlimit_buffer, i = 0; i < rlimit_index; i++) {
		fprintf(fp, "  %s   ", mkstring(buf1, len1, RJUST, rlim_names[i] ? rlim_names[i] : "(unknown)"));
		if (*p1 == (ulong) RLIM_INFINITY)
			fprintf(fp, "(unlimited)   ");
		else
			fprintf(fp, "%s   ", mkstring(buf1, len2, CENTER | LJUST | LONG_DEC, MKSTR(*p1)));
		p1++;
		if (*p1 == (ulong) RLIM_INFINITY)
			fprintf(fp, "(unlimited)\n");
		else
			fprintf(fp, "%s\n", mkstring(buf1, len2, CENTER | LJUST | LONG_DEC, MKSTR(*p1)));
		p1++;
	}
 ...
 */
static char *task_pointer_string(struct task_context *tc, ulong do_kstackp, char *buf)
{
	...
	if (do_kstackp) {
		...
		if (is_task_active(tc->task)) {
			...
			readmem(tc->task + OFFSET(task_struct_thread_esp),
				KVADDR, &bt->stkptr, sizeof(void *), "thread_struct esp", FAULT_ON_ERROR);
		} else if (VALID_MEMBER(task_struct_thread_ksp)) {
			readmem(tc->task + OFFSET(task_struct_thread_ksp),
				KVADDR, &bt->stkptr, sizeof(void *), "thread_struct ksp", FAULT_ON_ERROR);
		} else {
			bt->task = tc->task;
			bt->tc = tc;
		...
		if (bt->stkptr)
			sprintf(buf, mkstring(buf1, VADDR_PRLEN, CENTER | RJUST | LONG_HEX, MKSTR(bt->stkptr)));
		else
			sprintf(buf, "%s", mkstring(buf1, VADDR_PRLEN, CENTER | RJUST, "--"));
	} else
		sprintf(buf, mkstring(buf1, VADDR_PRLEN, CENTER | RJUST | LONG_HEX, MKSTR(tc->task)));
...
static void show_task_times(struct task_context *tcp, ulong flags)
{
	...
	if (VALID_MEMBER(task_struct_utime) &&
	    (SIZE(task_struct_utime) == (BITS32()? sizeof(uint32_t) : sizeof(uint64_t))))
	...
	for (i = tasks = 0; i < RUNNING_TASKS(); i++, tc++) {
		...
		if (BITS32() && (SIZE(task_struct_start_time) == 8)) {
			if (start_time_timespec())
				tsp->start_time = ULONG(tt->task_struct + OFFSET(task_struct_start_time));
			else
				tsp->start_time = ULONGLONG(tt->task_struct + OFFSET(task_struct_start_time));
		} else {
			start_time_timespec();
			tsp->start_time = ULONG(tt->task_struct + OFFSET(task_struct_start_time));
		}
		...
		if (VALID_MEMBER(task_struct_times)) {
			tsp->tms_utime = ULONG(tt->task_struct + OFFSET(task_struct_times) + OFFSET(tms_tms_utime));
			tsp->tms_stime = ULONG(tt->task_struct + OFFSET(task_struct_times) + OFFSET(tms_tms_stime));
		} else if (VALID_MEMBER(task_struct_utime)) {
			if (use_utime_stime) {
				tsp->utime = ULONG(tt->task_struct + OFFSET(task_struct_utime));
				tsp->stime = ULONG(tt->task_struct + OFFSET(task_struct_stime));
			} else if (use_kernel_timeval) {
				BCOPY(tt->task_struct +
				      OFFSET(task_struct_utime), &tsp->kutime, sizeof(struct kernel_timeval));
				BCOPY(tt->task_struct +
				      OFFSET(task_struct_stime), &tsp->kstime, sizeof(struct kernel_timeval));
			} else if (VALID_STRUCT(cputime_t)) {
				/* since linux 2.6.11 */
				if (SIZE(cputime_t) == 8) {
					uint64_t utime_64, stime_64;
					BCOPY(tt->task_struct + OFFSET(task_struct_utime), &utime_64, 8);
					BCOPY(tt->task_struct + OFFSET(task_struct_stime), &stime_64, 8);
					...
					BCOPY(tt->task_struct + OFFSET(task_struct_utime), &utime_32, 4);
					BCOPY(tt->task_struct + OFFSET(task_struct_stime), &stime_32, 4);
					tsp->old_utime.tv_sec = utime_32;
					tsp->old_stime.tv_sec = stime_32;
				}
			} else {
				BCOPY(tt->task_struct + OFFSET(task_struct_utime), &tsp->utime, sizeof(struct timeval));
				BCOPY(tt->task_struct + OFFSET(task_struct_stime), &tsp->stime, sizeof(struct timeval));
			}
		}
	...
	qsort((void *)task_start_times, (size_t) tasks, sizeof(struct task_start_time), compare_start_time);

	for (i = 0, tsp = task_start_times; i < tasks; i++, tsp++) {
		print_task_header(fp, tsp->tc, 0);
		fprintf(fp, "    RUN TIME: %s\n", symbol_exists("jiffies_64") ?
			convert_time(convert_start_time
				     (tsp->start_time, jiffies_64),
				     buf1) : convert_time(jiffies - tsp->start_time, buf1));
...
static ulonglong convert_start_time(ulonglong start_time, ulonglong current)
{
	ulong tmp1, tmp2;
	ulonglong wrapped;
...
static int compare_start_time(const void *v1, const void *v2)
{
	...
	return (t1->start_time < t2->start_time ? -1 : t1->start_time == t2->start_time ? 0 : 1);
}

/*
 *  Dump the parental hierarchy of a task.
 */
static void parent_list(ulong task)
{
	int i, j;
	int cnt, errflag;
	struct list_data list_data, *ld;
 ...
 *  Dump the children of a task.
 */
static void show_tgid_list(ulong task)
{
	int i;
	int cnt;
	struct task_context *tc;
 ...
 */
struct task_context *tgid_to_context(ulong parent_tgid)
{
	int i;
	struct task_context *tc;
	ulong tgid;
 ...
 */
ulong task_to_thread_info(ulong task)
{
	int i;
	struct task_context *tc;

	if (!(tt->flags & THREAD_INFO))
		error(FATAL, "task_to_thread_info: thread_info struct does not exist!\n");
 ...
 */
ulong task_to_stackbase(ulong task)
{
	if (tt->flags & THREAD_INFO)
 ...
 */
int str_to_context(char *string, ulong * value, struct task_context **tcp)
{
	ulong dvalue, hvalue;
	int found, type;
	char *s;
 ...
 */
ulong vaddr_in_task_struct(ulong vaddr)
{
	int i;
	struct task_context *tc;
 ...
 */
int set_context(ulong task, ulong pid)
{
	int i;
	struct task_context *tc;
	int found;
...
static int panic_context_adjusted(struct task_context *tc)
{
	...
	pgrp = tt->last_task_read ? UINT(tt->task_struct + OFFSET(task_struct_pgrp)) : 0;
	tgid = tt->last_task_read ? UINT(tt->task_struct + OFFSET(task_struct_tgid)) : 0;
...
void show_context(struct task_context *tc)
{
	...
	INDENT(indent);
	fprintf(fp, "  STATE: %s ", task_state_string(tc->task, buf, VERBOSE));
	if (is_task_active(tc->task)) {
		if (machdep->flags & HWRESET)
			...
			fprintf(fp, "(MCA)");
		else if ((tc->processor >= 0) && (tc->processor < NR_CPUS) && (kt->cpu_flags[tc->processor] & NMI))
			fprintf(fp, "(NMI)");
		else if ((tc->task == tt->panic_task) && XENDUMP_DUMPFILE() && (kt->xen_flags & XEN_SUSPEND))
			fprintf(fp, "(SUSPEND)");
	...
	if (!(pc->flags & RUNTIME) && !ACTIVE() && (tt->flags & PANIC_TASK_NOT_FOUND) && !SYSRQ_TASK(tc->task)) {
		fprintf(fp, "\n");
		INDENT(indent);
		if (machine_type("S390") || machine_type("S390X"))
			...
			fprintf(fp,
				"WARNING: reported panic task %lx not found", tt->panic_threads[tt->panic_processor]);
		else
...
static void initialize_task_state(void)
{
	...
	if (!symbol_exists("task_state_array") ||
	    !readmem(task_state_array = symbol_value("task_state_array"),
		     KVADDR, &str, sizeof(void *), "task_state_array", RETURN_ON_ERROR)) {
 old_defaults:
		_RUNNING_ = 0;
		_INTERRUPTIBLE_ = 1;
		_UNINTERRUPTIBLE_ = 2;
	...
	for (i = 0; i < len; i++) {
		...
		if (!readmem(task_state_array, KVADDR, &str, sizeof(void *), "task_state_array", RETURN_ON_ERROR))
			break;
	}

	if ((THIS_KERNEL_VERSION >= LINUX(2, 6, 16)) && (THIS_KERNEL_VERSION < LINUX(2, 6, 24))) {
		_NONINTERACTIVE_ = 64;
	}
	...
	if (!valid_task_state(_RUNNING_) ||
	    ...
	    !valid_task_state(_UNINTERRUPTIBLE_) || !valid_task_state(_ZOMBIE_) || !valid_task_state(_STOPPED_)) {
		if (CRASHDEBUG(3))
			fprintf(fp, "initialize_task_state: using old defaults\n");
		goto old_defaults;
	}
}
 ...
 */
static char *task_state_string_verbose(ulong task, char *buf)
{
	long state, both;
	int count;
	...
	if (state & _INTERRUPTIBLE_)
		sprintf(&buf[strlen(buf)], "%sTASK_INTERRUPTIBLE", count++ ? "|" : "");

	if (state & _UNINTERRUPTIBLE_)
		sprintf(&buf[strlen(buf)], "%sTASK_UNINTERRUPTIBLE", count++ ? "|" : "");

	if (state & _STOPPED_)
		sprintf(&buf[strlen(buf)], "%sTASK_STOPPED", count++ ? "|" : "");

	if (state & _TRACING_STOPPED_)
		sprintf(&buf[strlen(buf)], "%sTASK_TRACED", count++ ? "|" : "");

	if ((both = (state & _DEAD_))) {
		if (count_bits_long(both) > 1)
			sprintf(&buf[strlen(buf)], "%sEXIT_DEAD|TASK_DEAD", count++ ? "|" : "");
		else
			sprintf(&buf[strlen(buf)], "%sEXIT_DEAD", count++ ? "|" : "");
	}

	if (state & _ZOMBIE_)
		sprintf(&buf[strlen(buf)], "%sEXIT_ZOMBIE", count++ ? "|" : "");

	if (valid_task_state(_WAKING_) && (state & _WAKING_))
		sprintf(&buf[strlen(buf)], "%sTASK_WAKING", count++ ? "|" : "");

	if (valid_task_state(_WAKEKILL_) && (state & _WAKEKILL_))
		sprintf(&buf[strlen(buf)], "%sTASK_WAKEKILL", count++ ? "|" : "");

	if (valid_task_state(_NONINTERACTIVE_) && (state & _NONINTERACTIVE_))
		sprintf(&buf[strlen(buf)], "%sTASK_NONINTERACTIVE", count++ ? "|" : "");
...
char *task_state_string(ulong task, char *buf, int verbose)
{
	...
	if (valid_task_state(_TRACING_STOPPED_) && (state & _TRACING_STOPPED_)) {
		sprintf(buf, "TR");
		valid++;
		set++;
	}
...
ulong task_state(ulong task)
{
	...
	exit_state = VALID_MEMBER(task_struct_exit_state) ? ULONG(tt->task_struct + OFFSET(task_struct_exit_state)) : 0;
 ...
 */
ulong task_flags(ulong task)
{
	ulong flags;

	fill_task_struct(task);

	flags = tt->last_task_read ? ULONG(tt->task_struct + OFFSET(task_struct_flags)) : 0;
...
ulonglong task_last_run(ulong task)
{
	ulong last_run;
	ulonglong timestamp;
	...
	if (VALID_MEMBER(task_struct_last_run)) {
		last_run = tt->last_task_read ? ULONG(tt->task_struct + OFFSET(task_struct_last_run))
		    : 0;
		timestamp = (ulonglong) last_run;
	} else if (VALID_MEMBER(task_struct_timestamp))
		timestamp = tt->last_task_read ? ULONGLONG(tt->task_struct + OFFSET(task_struct_timestamp))
		    : 0;
	else if (VALID_MEMBER(sched_info_last_arrival))
		timestamp = tt->last_task_read ? ULONGLONG(tt->task_struct + OFFSET(task_struct_sched_info)
 ...
 *  cache is loaded.
 */
ulong task_mm(ulong task, int fill)
{
	ulong mm_struct;
 ...
 */
char *task_cpu(int processor, char *buf, int verbose)
{
...
int is_task_active(ulong task)
{
	...
	has_cpu = tt->last_task_read ? task_has_cpu(task, tt->task_struct) : 0;
 ...
 *  panic_threads[] array.
 */
int is_panic_thread(ulong task)
{
	int i;
 ...
 */
static int task_has_cpu(ulong task, char *local_task)
{
	int i, has_cpu;
	ulong cpus_runnable;
	...
	if (VALID_MEMBER(task_struct_has_cpu)) {
		if (local_task)
			has_cpu = INT(local_task + OFFSET(task_struct_has_cpu));
				  ...
				  KVADDR, &has_cpu, sizeof(int), "task_struct has_cpu", RETURN_ON_ERROR))
			has_cpu = FALSE;
	} else if (VALID_MEMBER(task_struct_cpus_runnable)) {
		if (local_task)
			cpus_runnable = ULONG(local_task + OFFSET(task_struct_cpus_runnable));
		else if (!readmem
			 ((ulong) (task + OFFSET(task_struct_cpus_runnable)),
			  KVADDR, &cpus_runnable, sizeof(ulong), "task_struct cpus_runnable", RETURN_ON_ERROR))
			cpus_runnable = ~0UL;
		...
		error(FATAL, "task_struct has no has_cpu, or cpus_runnable; runqueues[] not defined?\n");
 ...
 */
int get_panic_ksp(struct bt_info *bt, ulong * ksp)
{
	int i;

	if (tt->flags & PANIC_KSP) {
		for (i = 0; i < NR_CPUS; i++) {
			if ((tt->panic_threads[i] == bt->task) && tt->panic_ksp[i] && INSTACK(tt->panic_ksp[i], bt)) {
				*ksp = tt->panic_ksp[i];
				return TRUE;
			}
		}
...
static ulong get_panic_context(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		if (!task_exists(task)) {
			error(WARNING, "active task %lx on cpu %d not found in PID hash\n\n", task, i);
	...
	if (symbol_exists("panic_threads") && symbol_exists("panicmsg") && symbol_exists("panic_processor")) {
		panic_threads_addr = symbol_value("panic_threads");
		get_symbol_data("panic_processor", sizeof(int), &tt->panic_processor);
		get_symbol_data("panicmsg", sizeof(char *), &tt->panicmsg);

		if (!readmem(panic_threads_addr, KVADDR, tt->panic_threads,
			     sizeof(void *) * NR_CPUS, "panic_processor array", RETURN_ON_ERROR))
			goto use_task_0;
		...
		if (symbol_exists("panic_ksp")) {
			if (!(tt->panic_ksp = (ulong *)
				...
				error(FATAL, "cannot malloc panic_ksp array.\n");
			readmem(symbol_value("panic_ksp"), KVADDR,
				tt->panic_ksp, sizeof(void *) * NR_CPUS, "panic_ksp array", RETURN_ON_ERROR);
			tt->flags |= PANIC_KSP;
		}
...
char *get_panicmsg(char *buf)
{
	...
	rewind(pc->tmpfile);
	while (fgets(buf, BUFSIZE, pc->tmpfile)) {
		if (strstr(buf, "SysRq : Crash") || strstr(buf, "SysRq : Trigger a crashdump")) {
			pc->flags |= SYSRQ;
			break;
		}
	}
	...
	rewind(pc->tmpfile);
	while (!msg_found && fgets(buf, BUFSIZE, pc->tmpfile)) {
		if (strstr(buf, "Oops: ") || strstr(buf, "kernel BUG at"))
			msg_found = TRUE;
	}
	rewind(pc->tmpfile);
	while (!msg_found && fgets(buf, BUFSIZE, pc->tmpfile)) {
		if (strstr(buf, "SysRq : Netdump") ||
		    strstr(buf, "SysRq : Trigger a crashdump") || strstr(buf, "SysRq : Crash")) {
			pc->flags |= SYSRQ;
			msg_found = TRUE;
		}
	}
	...
	while (args[optind]) {
		...
		if (IS_A_NUMBER(args[optind])) {
			...
			case STR_PID:
				if (p == MAX_FOREACH_PIDS)
					error(INFO, "too many pids specified!\n");
				else {
			...
			case STR_TASK:
				if (t == MAX_FOREACH_TASKS)
					error(INFO, "too many tasks specified!\n");
				else {
		...
		if (STREQ(args[optind], "kernel")) {
			if (fd->flags & FOREACH_USER)
				error(FATAL, "user and kernel are mutually exclusive!\n");
		...
		if (STREQ(args[optind], "RU") ||
		    ...
		    STREQ(args[optind], "ZO") || STREQ(args[optind], "DE") || STREQ(args[optind], "SW")) {
		...
		if (STREQ(args[optind], "active")) {
			if (!DUMPFILE())
				error(FATAL, "active option not allowed on live systems\n");
		...
		if (SINGLE_QUOTED_STRING(args[optind])) {
			if (fd->regexs == MAX_REGEX_ARGS)
				...
				if (regcomp(&fd->regex_info[fd->regexs].regex, p1, REG_EXTENDED | REG_NOSUB)) {
					error(INFO, "invalid regular expression: %s\n", p1);
					free(p1);
					goto bailout;
				}
		 ...
		 */
		p1 = (args[optind][0] == '\\') ? &args[optind][1] : args[optind];
 ...
 */
void foreach(struct foreach_data *fd)
{
	int i, j, k, a;
	...
	if (CRASHDEBUG(1)) {
		...
		for (j = 0; j < fd->regexs; j++) {
			fprintf(fp, "%s[%d] pattern: [%s] ", j ? "               " : "", j, fd->regex_info[j].pattern);
		...
		for (a = 0; a < fd->args; a++)
			fprintf(fp, "[%lx (%s)] ", fd->arg_array[a], (char *)fd->arg_array[a]);
		fprintf(fp, "\n");
	...
	for (k = 0; k < fd->keys; k++) {
		...
		case FOREACH_NET:
			...
			case (FOREACH_s_FLAG | FOREACH_S_FLAG):
				error(WARNING, "net -s and -S options are mutually exclusive!\n");
				fd->flags = FOREACH_s_FLAG;
				break;

			case 0:
				error(WARNING, "net command requires -s or -S option\n\n");
				fd->flags |= FOREACH_s_FLAG;
				break;
			}
			if ((fd->flags & (FOREACH_x_FLAG | FOREACH_d_FLAG)) == (FOREACH_x_FLAG | FOREACH_d_FLAG))
				error(FATAL, "net: -x and -d options are mutually exclusive\n");
			break;

		case FOREACH_VTOP:
			if (!fd->args)
				error(FATAL, "foreach command requires address argument\n");
			if (fd->reference)
				error(FATAL, "vtop command does not support -R option\n");
			if ((fd->flags & (FOREACH_u_FLAG | FOREACH_k_FLAG)) == (FOREACH_u_FLAG | FOREACH_k_FLAG))
				error(FATAL, "vtop: -u and -k options are mutually exclusive\n");
			break;

		case FOREACH_VM:
			if ((fd->flags & (FOREACH_x_FLAG | FOREACH_d_FLAG)) == (FOREACH_x_FLAG | FOREACH_d_FLAG))
				error(FATAL, "vm: -x and -d options are mutually exclusive\n");
			if (count_bits_long
			    (fd->flags & (FOREACH_i_FLAG | FOREACH_p_FLAG | FOREACH_m_FLAG | FOREACH_v_FLAG)) > 1)
				error(FATAL, "vm command accepts only one of -p, -m or -v flags\n");
			if (fd->reference) {
				if (fd->flags & FOREACH_i_FLAG)
					error(FATAL, "vm: -i is not applicable to the -R option\n");
				if (fd->flags & FOREACH_m_FLAG)
					error(FATAL, "vm: -m is not applicable to the -R option\n");
				if (fd->flags & FOREACH_v_FLAG)
					error(FATAL, "vm: -v is not applicable to the -R option\n");
			}
			break;

		case FOREACH_BT:
			if ((fd->flags & (FOREACH_x_FLAG | FOREACH_d_FLAG)) == (FOREACH_x_FLAG | FOREACH_d_FLAG))
				error(FATAL, "bt: -x and -d options are mutually exclusive\n");
			...
			if ((fd->flags & FOREACH_g_FLAG))
				error(FATAL, "bt -g option is not supported when issued from foreach\n");
#endif
			bt = &bt_info;
			break;
		...
		case FOREACH_TASK:
			if ((fd->flags & (FOREACH_x_FLAG | FOREACH_d_FLAG)) == (FOREACH_x_FLAG | FOREACH_d_FLAG))
				error(FATAL, "task: -x and -d options are mutually exclusive\n");
			if (count_bits_long(fd->flags & (FOREACH_x_FLAG | FOREACH_d_FLAG)) > 1)
				error(FATAL, "task command accepts -R member[,member]," " and either -x or -d flags\n");
			break;

		case FOREACH_SET:
			if (fd->reference)
				error(FATAL, "set command does not support -R option\n");
			break;

		case FOREACH_SIG:
			if (fd->flags & (FOREACH_l_FLAG | FOREACH_s_FLAG))
				error(FATAL, "sig: -l and -s options are not applicable\n");
			if (fd->flags & FOREACH_g_FLAG) {
				if (!hq_open()) {
					error(INFO, "cannot hash thread group tasks\n");
					fd->flags &= ~FOREACH_g_FLAG;
	...
	subsequent = FALSE;
	specified = (fd->tasks || fd->pids || fd->comms || fd->regexs || (fd->flags & FOREACH_SPECIFIED));
	ref = &reference;
	...
	for (i = 0; i < RUNNING_TASKS(); i++, tc++) {
		...
		if (specified) {
			...
			for (j = 0; !doit && (j < fd->regexs); j++) {
				if (regexec(&fd->regex_info[j].regex, tc->comm, 0, NULL, 0)
				    == 0) {
					doit = TRUE;
					break;
				}
		...
		for (k = 0; k < fd->keys; k++) {
			...
			case FOREACH_VM:
				...
				if (fd->flags & FOREACH_i_FLAG)
					vm_area_dump(tc->task, PRINT_INODES, 0, NULL);
				else if (fd->flags & FOREACH_p_FLAG)
					vm_area_dump(tc->task, PHYSADDR, 0, fd->reference ? ref : NULL);
				else if (fd->flags & FOREACH_m_FLAG)
					vm_area_dump(tc->task, PRINT_MM_STRUCT | cmdflags, 0, NULL);
				else if (fd->flags & FOREACH_v_FLAG)
					vm_area_dump(tc->task, PRINT_VMA_STRUCTS | cmdflags, 0, NULL);
				else
					vm_area_dump(tc->task, 0, 0, fd->reference ? ref : NULL);
				break;
			...
			case FOREACH_TASK:
				...
				do_task(tc->task, FOREACH_TASK, fd->reference ? ref : NULL, radix);
				break;
			...
			case FOREACH_SIG:
				...
				if (fd->flags & FOREACH_g_FLAG) {
					...
					do_sig(tc->task, FOREACH_SIG, fd->reference ? ref : NULL);
				break;
			...
			case FOREACH_FILES:
				...
				open_files_dump(tc->task,
						...
						PRINT_INODES : 0, fd->reference ? ref : NULL);
				break;

			case FOREACH_NET:
				pc->curcmd = "net";
				if (fd->flags & (FOREACH_s_FLAG | FOREACH_S_FLAG))
					dump_sockets_workhorse(tc->task, fd->flags, fd->reference ? ref : NULL);
				break;
			...
			case FOREACH_VTOP:
				...
				for (a = 0; a < fd->args; a++) {
					do_vtop(htol((char *)fd->arg_array[a], FAULT_ON_ERROR, NULL), tc, cmdflags);
				}
				break;
 ...
 *  Clean up regex buffers and pattern strings.
 */
static void foreach_cleanup(void *arg)
{
	int i;
	struct foreach_data *fd;
 ...
 */
static int is_foreach_keyword(char *s, int *key)
{
...
static ulong get_dumpfile_panic_task(void)
{
	...
	if (NETDUMP_DUMPFILE()) {
		task = pc->flags & REM_NETDUMP ? tt->panic_task : get_netdump_panic_task();
		if (task)
			return task;
...
static void populate_panic_threads(void)
{
	...
	tc = FIRST_CONTEXT();
	for (i = 0; i < RUNNING_TASKS(); i++, tc++) {
		if (task_has_cpu(tc->task, NULL) && (tc->processor >= 0) && (tc->processor < NR_CPUS)) {
			tt->panic_threads[tc->processor] = tc->task;
			found++;
		}
	}
	...
	if (!found && !(kt->flags & SMP) &&
	    (LKCD_DUMPFILE() || NETDUMP_DUMPFILE() || KDUMP_DUMPFILE() || DISKDUMP_DUMPFILE() || KVMDUMP_DUMPFILE()))
 ...
 */
void print_task_header(FILE * out, struct task_context *tc, int newline)
{
	char buf[BUFSIZE];
	char buf1[BUFSIZE];
 ...
 *  "help -t" output
 */
void dump_task_table(int verbose)
{
	...
	fprintf(fp, "           current: %lx [%ld]\n", (ulong) tt->current, (ulong) (tt->current - tt->context_array));
	if (tt->current) {
		fprintf(fp, "              .pid: %ld\n", tc->pid);
		fprintf(fp, "             .comm: \"%s\"\n", tc->comm);
		fprintf(fp, "             .task: %lx\n", tc->task);
	...
	sprintf(buf, "(");
	if (tt->flags & TASK_INIT_DONE)
		sprintf(&buf[strlen(buf)], "%sTASK_INIT_DONE", others++ ? "|" : "");
	if (tt->flags & TASK_ARRAY_EXISTS)
		sprintf(&buf[strlen(buf)], "%sTASK_ARRAY_EXISTS", others++ ? "|" : "");
	if (tt->flags & PANIC_TASK_NOT_FOUND)
		sprintf(&buf[strlen(buf)], "%sPANIC_TASK_NOT_FOUND", others++ ? "|" : "");
	if (tt->flags & TASK_REFRESH)
		sprintf(&buf[strlen(buf)], "%sTASK_REFRESH", others++ ? "|" : "");
	if (tt->flags & TASK_REFRESH_OFF)
		sprintf(&buf[strlen(buf)], "%sTASK_REFRESH_OFF", others++ ? "|" : "");
	if (tt->flags & PANIC_KSP)
		sprintf(&buf[strlen(buf)], "%sPANIC_KSP", others++ ? "|" : "");
	if (tt->flags & POPULATE_PANIC)
		sprintf(&buf[strlen(buf)], "%sPOPULATE_PANIC", others++ ? "|" : "");
	if (tt->flags & ACTIVE_SET)
		sprintf(&buf[strlen(buf)], "%sACTIVE_SET", others++ ? "|" : "");
	if (tt->flags & PIDHASH)
		sprintf(&buf[strlen(buf)], "%sPIDHASH", others++ ? "|" : "");
	if (tt->flags & PID_HASH)
		sprintf(&buf[strlen(buf)], "%sPID_HASH", others++ ? "|" : "");
	if (tt->flags & THREAD_INFO)
		sprintf(&buf[strlen(buf)], "%sTHREAD_INFO", others++ ? "|" : "");
	if (tt->flags & IRQSTACKS)
		sprintf(&buf[strlen(buf)], "%sIRQSTACKS", others++ ? "|" : "");
	if (tt->flags & TIMESPEC)
		sprintf(&buf[strlen(buf)], "%sTIMESPEC", others++ ? "|" : "");
	if (tt->flags & NO_TIMESPEC)
		sprintf(&buf[strlen(buf)], "%sNO_TIMESPEC", others++ ? "|" : "");
	if (tt->flags & ACTIVE_ONLY)
		sprintf(&buf[strlen(buf)], "%sACTIVE_ONLY", others++ ? "|" : "");
	...
	for (i = 0; i < RUNNING_TASKS(); i++, tc++) {
		if (tt->flags & THREAD_INFO)
			fprintf(fp,
				...
				tc->processor, tc->ptask, (ulong) tc->mm_struct, tc->comm);
		else
			fprintf(fp, "[%3d] %08lx %5ld %d %08lx %08lx %s\n",
				i, tc->task, tc->pid, tc->processor, tc->ptask, (ulong) tc->mm_struct, tc->comm);
	}
}
 ...
 */
int is_kernel_thread(ulong task)
{
	struct task_context *tc;
	ulong mm;
 ...
 */
void get_idle_threads(ulong * tasklist, int nr_cpus)
{
	int i, cnt;
	ulong runq, runqaddr;
	char *runqbuf;
	...
	if ((rq_sp = per_cpu_symbol_search("per_cpu__runqueues")) && VALID_MEMBER(runqueue_idle)) {
		...
		for (i = 0; i < nr_cpus; i++) {
			...
			readmem(runq, KVADDR, runqbuf, SIZE(runqueue), "runqueues entry (per_cpu)", FAULT_ON_ERROR);
		...
		for (i = 0; i < nr_cpus; i++, runq += SIZE(runqueue)) {
			readmem(runq, KVADDR, runqbuf, SIZE(runqueue), "runqueues entry (old)", FAULT_ON_ERROR);
			tasklist[i] = ULONG(runqbuf + OFFSET(runqueue_idle));
			if (IS_KVADDR(tasklist[i]))
				cnt++;
		}
		...
		for (i = 0; i < nr_cpus; i++) {
			...
			readmem(runqaddr, KVADDR, runqbuf, SIZE(runqueue), "runqueues entry", FAULT_ON_ERROR);
			if ((tasklist[i] = get_idle_task(i, runqbuf)))
				cnt++;
		}
	} else if (symbol_exists("init_tasks")) {
		readmem(symbol_value("init_tasks"), KVADDR, tasklist,
			sizeof(void *) * nr_cpus, "init_tasks array", FAULT_ON_ERROR);
		...
		for (i = 0; i < nr_cpus; i++, runq += SIZE(pcpu_info)) {
			readmem(runq, KVADDR, runqbuf, SIZE(pcpu_info), "pcpu info", FAULT_ON_ERROR);
	...
	if (!cnt) {
		error(INFO, "cannot determine idle task addresses from init_tasks[] or runqueues[]\n");
 ...
 */
static ulong get_idle_task(int cpu, char *runqbuf)
{
	ulong idle_task;

	idle_task = ULONG(runqbuf + OFFSET(runqueue_cpu) + (SIZE(cpu_s) * cpu_idx(cpu)) + OFFSET(cpu_s_idle));
 ...
 */
static ulong get_curr_task(int cpu, char *runqbuf)
{
	ulong curr_task;

	curr_task = ULONG(runqbuf + OFFSET(runqueue_cpu) + (SIZE(cpu_s) * cpu_idx(cpu)) + OFFSET(cpu_s_curr));
...
int get_active_set(void)
{
	...
	if (!tt->active_set && !(tt->active_set = (ulong *) calloc(NR_CPUS, sizeof(ulong))))
	...
	if (OPENVZ()) {
		...
		for (i = 0; i < kt->cpus; i++, runq += SIZE(pcpu_info)) {
			readmem(runq, KVADDR, pcpu_info_buf, SIZE(pcpu_info), "pcpu_info", FAULT_ON_ERROR);
			vcpu_struct = ULONG(pcpu_info_buf + OFFSET(pcpu_info_vcpu));
			readmem(vcpu_struct, KVADDR, vcpu_struct_buf,
				SIZE(vcpu_struct), "pcpu_info->vcpu", FAULT_ON_ERROR);
			tt->active_set[i] = ULONG(vcpu_struct_buf + OFFSET(vcpu_struct_rq) + OFFSET(runqueue_curr));
		...
		for (i = 0; i < kt->cpus; i++) {
			...
			readmem(runq, KVADDR, runqbuf, SIZE(runqueue),
				"active runqueues entry (per_cpu)", FAULT_ON_ERROR);

			tt->active_set[i] = ULONG(runqbuf + OFFSET(runqueue_curr));
			if (IS_KVADDR(tt->active_set[i]))
				cnt++;
		}
	} else if (VALID_MEMBER(runqueue_curr)) {
		for (i = 0; i < MAX(kt->cpus, kt->kernel_NR_CPUS); i++, runq += SIZE(runqueue)) {
			readmem(runq, KVADDR, runqbuf, SIZE(runqueue), "(old) runqueues curr", FAULT_ON_ERROR);
			tt->active_set[i] = ULONG(runqbuf + OFFSET(runqueue_curr));
			if (IS_KVADDR(tt->active_set[i]))
				cnt++;
		}
		...
		for (i = 0; i < kt->cpus; i++) {
			runqaddr = runq + (SIZE(runqueue) * rq_idx(i));
			readmem(runqaddr, KVADDR, runqbuf, SIZE(runqueue), "runqueues curr", FAULT_ON_ERROR);
 ...
 */
static ulong get_active_set_panic_task()
{
	int i, j, found;
	ulong task;
	char buf[BUFSIZE];
	...
	if (crash_kexec_task) {
		if (CRASHDEBUG(1))
			error(INFO, "get_active_set_panic_task: %lx (crash_kexec)\n", crash_kexec_task);
		return crash_kexec_task;
	}
	if (crash_fadump_task) {
		if (CRASHDEBUG(1))
			error(INFO, "get_active_set_panic_task: %lx (crash_fadump)\n", crash_fadump_task);
		return crash_fadump_task;
	}

	if (xen_sysrq_task) {
		if (CRASHDEBUG(1))
			error(INFO, "get_active_set_panic_task: %lx (sysrq_handle_crashdump)\n", xen_sysrq_task);
 ...
 no_panic_task_found:
	...
	if (CRASHDEBUG(1))
		error(INFO, "get_active_set_panic_task: failed\n");
	...
	while ((c = getopt(argcnt, args, "dtg")) != EOF) {
		...
		case 'g':
			if (INVALID_MEMBER(task_group_cfs_rq) ||
			    INVALID_MEMBER(task_group_rt_rq) || INVALID_MEMBER(task_group_parent))
...
static void dump_on_rq_timestamp(void)
{
	...
	for (cpu = 0; cpu < kt->cpus; cpu++) {
		...
		readmem(runq + OFFSET(rq_timestamp), KVADDR, &timestamp,
			sizeof(ulonglong), "per-cpu rq timestamp", FAULT_ON_ERROR);

		sprintf(buf, pc->output_radix == 10 ? "%llu" : "%llx", timestamp);
		fprintf(fp, "%sCPU %d: %s\n", cpu < 10 ? " " : "", cpu, buf);
		len = strlen(buf);
		...
		if ((tc = task_to_context(tt->active_set[cpu]))) {
			...
			sprintf(format, "%c0%dll%c", '%', len, pc->output_radix == 10 ? 'u' : 'x');
			sprintf(buf, format, timestamp);
			fprintf(fp,
				"%s%s  PID: %-5ld  TASK: %lx  COMMAND: \"%s\"\n",
...
static void dump_runq(void)
{
 ...
 start_again:
	...
	do {
		...
		readmem(next + offs, KVADDR, &next, sizeof(void *), "run queue entry", FAULT_ON_ERROR);
	...
	for (i = 0; i < cnt; i++) {
		...
		if (!(tc = task_to_context(VIRTPAGEBASE(tlist[i])))) {
			fprintf(fp, "PID: ?      TASK: %lx  CPU: ?   COMMAND: ?\n", tlist[i]);
			continue;
		}
...
static void dump_runqueues(void)
{
	...
	for (cpu = 0; cpu < kt->cpus; cpu++, runq += SIZE(runqueue)) {
		...
		readmem(runq, KVADDR, runqbuf, SIZE(runqueue), "runqueues array entry", FAULT_ON_ERROR);
		...
		offset = active == arrays ? OFFSET(runqueue_arrays) : OFFSET(runqueue_arrays) + SIZE(prio_array);
		offset = active - runq;
		dump_prio_array(RUNQ_ACTIVE, active, &runqbuf[offset]);

		offset = expired == arrays ? OFFSET(runqueue_arrays) : OFFSET(runqueue_arrays) + SIZE(prio_array);
		offset = expired - runq;
		dump_prio_array(RUNQ_EXPIRED, expired, &runqbuf[offset]);
	}
}

static void dump_prio_array(int which, ulong k_prio_array, char *u_prio_array)
{
	...
	qheads = (i = ARRAY_LENGTH(prio_array_queue)) ? i : get_array_length("prio_array.queue", NULL, SIZE(list_head));
	...
	for (i = tot = 0; i < 140; i++) {
		...
		if (CRASHDEBUG(1))
			fprintf(fp, "prio_array[%d] @ %lx => %lx/%lx %s\n",
				...
				(list_head[0] == list_head[1]) && (list_head[0] == kvaddr) ? "(empty)" : "");

		if ((list_head[0] == kvaddr) && (list_head[1] == kvaddr))
			continue;

		console("[%d] %lx => %lx-%lx ", i, kvaddr, list_head[0], list_head[1]);
...
static void print_task_group_info_array(void)
{
	...
	for (i = 0; i < tgi_p; i++) {
		fprintf(fp, "%d : use=%d, depth=%d, group=%lx, ", i,
			tgi_array[i]->use, tgi_array[i]->depth, tgi_array[i]->task_group);
		fprintf(fp, "name=%s, ", tgi_array[i]->name ? tgi_array[i]->name : "NULL");
		if (tgi_array[i]->parent)
			fprintf(fp, "parent=%lx", tgi_array[i]->parent->task_group);
		fprintf(fp, "\n");
	}
}
...
static void dump_task_runq_entry(struct task_context *tc)
{
	int prio;

	readmem(tc->task + OFFSET(task_struct_prio), KVADDR, &prio, sizeof(int), "task prio", FAULT_ON_ERROR);
	fprintf(fp, "[%3d] ", prio);
	fprintf(fp, "PID: %-5ld  TASK: %lx  COMMAND: \"%s\"\n", tc->pid, tc->task, tc->comm);
}

static void print_group_header_fair(int depth, ulong cfs_rq, void *t)
{
	int throttled;
	struct rb_root *root;
	...
	if (VALID_MEMBER(task_group_cfs_bandwidth)) {
		readmem(cfs_rq + OFFSET(cfs_rq_throttled), KVADDR,
			&throttled, sizeof(int), "cfs_rq throttled", FAULT_ON_ERROR);
		if (throttled)
...
static void print_parent_task_group_fair(void *t, int cpu)
{
	...
	readmem(tgi->task_group + OFFSET(task_group_cfs_rq),
		KVADDR, &cfs_rq_c, sizeof(ulong), "task_group cfs_rq", FAULT_ON_ERROR);
	readmem(cfs_rq_c + cpu * sizeof(ulong), KVADDR, &cfs_rq_p, sizeof(ulong), "task_group cfs_rq", FAULT_ON_ERROR);

	print_group_header_fair(tgi->depth, cfs_rq_p, tgi);
	tgi->use = 0;
}

static int dump_tasks_in_lower_dequeued_cfs_rq(int depth, ulong cfs_rq, int cpu)
{
	...
	for (i = 0; i < tgi_p; i++) {
		...
		readmem(cfs_rq + OFFSET(cfs_rq_tg), KVADDR, &group, sizeof(ulong), "cfs_rq tg", FAULT_ON_ERROR);
		...
		readmem(cfs_rq_p + OFFSET(cfs_rq_nr_running), KVADDR,
			&nr_running, sizeof(int), "cfs_rq nr_running", FAULT_ON_ERROR);
		if (nr_running == 0) {
			total += dump_tasks_in_lower_dequeued_cfs_rq(depth + 1, cfs_rq_p, cpu);
			continue;
		}
...
static int dump_tasks_in_cfs_rq(ulong cfs_rq)
{
	struct task_context *tc;
	struct rb_root *root;
	struct rb_node *node;
	...
	if (VALID_MEMBER(sched_entity_my_q)) {
		readmem(cfs_rq + OFFSET(cfs_rq_curr), KVADDR, &curr, sizeof(ulong), "curr", FAULT_ON_ERROR);
		if (curr) {
			readmem(curr + OFFSET(sched_entity_my_q), KVADDR,
				&curr_my_q, sizeof(ulong), "curr->my_q", FAULT_ON_ERROR);
			if (curr_my_q)
				total += dump_tasks_in_cfs_rq(curr_my_q);
		}
	}

	readmem(cfs_rq + OFFSET(cfs_rq_rb_leftmost), KVADDR, &leftmost, sizeof(ulong), "rb_leftmost", FAULT_ON_ERROR);
	root = (struct rb_root *)(cfs_rq + OFFSET(cfs_rq_tasks_timeline));

	for (node = rb_first(root); leftmost && node; node = rb_next(node)) {
		if (VALID_MEMBER(sched_entity_my_q)) {
			readmem((ulong) node - OFFSET(sched_entity_run_node)
				+ OFFSET(sched_entity_my_q), KVADDR, &my_q, sizeof(ulong), "my_q", FAULT_ON_ERROR);
			if (my_q) {
		...
		tc = task_to_context((ulong) node - OFFSET(task_struct_se) - OFFSET(sched_entity_run_node));
		if (!tc)
			continue;
...
static int dump_tasks_in_task_group_cfs_rq(int depth, ulong cfs_rq, int cpu)
{
	struct task_context *tc;
	struct rb_root *root;
	struct rb_node *node;
	...
	if (depth) {
		readmem(cfs_rq + OFFSET(cfs_rq_tg), KVADDR, &tg, sizeof(ulong), "cfs_rq tg", FAULT_ON_ERROR);
		for (i = 0; i < tgi_p; i++) {
	...
	if (VALID_MEMBER(sched_entity_my_q)) {
		...
		if (curr) {
			...
			if (curr_my_q) {
				total++;
				total += dump_tasks_in_task_group_cfs_rq(depth + 1, curr_my_q, cpu);
			}
		}
	}
	...
	for (node = rb_first(root); leftmost && node; node = rb_next(node)) {
		if (VALID_MEMBER(sched_entity_my_q)) {
			...
			if (my_q) {
				total++;
				total += dump_tasks_in_task_group_cfs_rq(depth + 1, my_q, cpu);
				continue;
			}
		}
		...
		for (i = 0; i < RUNNING_TASKS(); i++, tc++) {
			...
			if (VALID_MEMBER(task_struct_on_rq)) {
				readmem(tc->task + OFFSET(task_struct_on_rq),
					KVADDR, &on_rq, sizeof(int), "task on_rq", FAULT_ON_ERROR);
			} else {
				readmem(tc->task + OFFSET(task_struct_se),
					KVADDR, buf, SIZE(sched_entity), "task se", FAULT_ON_ERROR);
...
static void cfs_rq_offset_init(void)
{
	if (!VALID_STRUCT(cfs_rq)) {
		...
		MEMBER_OFFSET_INIT(task_struct_se, "task_struct", "se");
		STRUCT_SIZE_INIT(sched_entity, "sched_entity");
		MEMBER_OFFSET_INIT(sched_entity_run_node, "sched_entity", "run_node");
		MEMBER_OFFSET_INIT(sched_entity_cfs_rq, "sched_entity", "cfs_rq");
		MEMBER_OFFSET_INIT(sched_entity_my_q, "sched_entity", "my_q");
		MEMBER_OFFSET_INIT(sched_rt_entity_my_q, "sched_rt_entity", "my_q");
		MEMBER_OFFSET_INIT(sched_entity_on_rq, "sched_entity", "on_rq");
		MEMBER_OFFSET_INIT(cfs_rq_rb_leftmost, "cfs_rq", "rb_leftmost");
		MEMBER_OFFSET_INIT(cfs_rq_nr_running, "cfs_rq", "nr_running");
		MEMBER_OFFSET_INIT(cfs_rq_tasks_timeline, "cfs_rq", "tasks_timeline");
		...
		MEMBER_OFFSET_INIT(sched_rt_entity_run_list, "sched_rt_entity", "run_list");
...
static void task_group_offset_init(void)
{
	if (!VALID_STRUCT(task_group)) {
		...
		MEMBER_OFFSET_INIT(task_group_cfs_bandwidth, "task_group", "cfs_bandwidth");
		MEMBER_OFFSET_INIT(cfs_rq_throttled, "cfs_rq", "throttled");

		MEMBER_OFFSET_INIT(task_group_rt_bandwidth, "task_group", "rt_bandwidth");
...
static void dump_CFS_runqueues(void)
{
	...
	for (cpu = 0; cpu < kt->cpus; cpu++) {
		...
		readmem(runq, KVADDR, runqbuf, SIZE(runqueue), "per-cpu rq", FAULT_ON_ERROR);
		...
		if (cfs_rq_buf) {
			...
			readmem(cfs_rq, KVADDR, cfs_rq_buf, SIZE(cfs_rq), "per-cpu cfs_rq", FAULT_ON_ERROR);
			root = (struct rb_root *)(cfs_rq + OFFSET(cfs_rq_tasks_timeline));
		} else {
		...
		tot = dump_RT_prio_array(prio_array, &runqbuf[OFFSET(rq_rt) + OFFSET(rt_rq_active)]);
		if (!tot) {
			INDENT(5);
			fprintf(fp, "[no tasks queued]\n");
		}
...
static void print_group_header_rt(ulong rt_rq, void *t)
{
	int throttled;
	struct task_group_info *tgi = (struct task_group_info *)t;
	...
	if (VALID_MEMBER(task_group_rt_bandwidth)) {
		readmem(rt_rq + OFFSET(rt_rq_rt_throttled), KVADDR,
			&throttled, sizeof(int), "rt_rq rt_throttled", FAULT_ON_ERROR);
		if (throttled)
			fprintf(fp, " (THROTTLED)");
	}
	fprintf(fp, "\n");
...
static void print_parent_task_group_rt(void *t, int cpu)
{
	int prio;
	struct task_group_info *tgi;
	...
	readmem(tgi->task_group + OFFSET(task_group_rt_rq),
		KVADDR, &rt_rq_c, sizeof(ulong), "task_group rt_rq", FAULT_ON_ERROR);
	readmem(rt_rq_c + cpu * sizeof(ulong), KVADDR, &rt_rq_p, sizeof(ulong), "task_group rt_rq", FAULT_ON_ERROR);

	readmem(rt_rq_p + OFFSET(rt_rq_highest_prio), KVADDR, &prio, sizeof(int), "rt_rq highest prio", FAULT_ON_ERROR);
...
static int dump_tasks_in_lower_dequeued_rt_rq(int depth, ulong rt_rq, int cpu)
{
	...
	for (i = 0; i < tgi_p; i++) {
		...
		readmem(rt_rq + OFFSET(rt_rq_tg), KVADDR, &group, sizeof(ulong), "rt_rq tg", FAULT_ON_ERROR);
		if (group != tgi_array[i]->parent->task_group)
			continue;
		...
		readmem(rt_rq_p + OFFSET(rt_rq_rt_nr_running), KVADDR,
			&nr_running, sizeof(int), "rt_rq rt_nr_running", FAULT_ON_ERROR);
		if (nr_running == 0) {
			tot += dump_tasks_in_lower_dequeued_rt_rq(depth + 1, rt_rq_p, cpu);
			continue;
		}
		...
		readmem(rt_rq_p + OFFSET(rt_rq_highest_prio), KVADDR,
			&prio, sizeof(int), "rt_rq highest_prio", FAULT_ON_ERROR);
		INDENT(5 + 6 * depth);
		fprintf(fp, "[%3d] ", prio);
		tot++;
...
static int dump_RT_prio_array(ulong k_prio_array, char *u_prio_array)
{
	...
	for (i = tot = 0; i < qheads; i++) {
		...
		BZERO(ld, sizeof(struct list_data));
		ld->start = list_head[0];
		if (VALID_MEMBER(task_struct_rt) && VALID_MEMBER(sched_rt_entity_run_list))
		...
		for (c = 0; c < cnt; c++) {
			...
			if (VALID_MEMBER(sched_rt_entity_my_q)) {
				readmem(tlist[c] + OFFSET(sched_rt_entity_my_q),
					KVADDR, &my_q, sizeof(ulong), "my_q", FAULT_ON_ERROR);
				if (my_q) {
					rt_rq_buf = GETBUF(SIZE(rt_rq));
					readmem(my_q, KVADDR, rt_rq_buf, SIZE(rt_rq), "rt_rq", FAULT_ON_ERROR);

					tot +=
					    dump_RT_prio_array(my_q +
							       OFFSET(rt_rq_active), &rt_rq_buf[OFFSET(rt_rq_active)]);
					FREEBUF(rt_rq_buf);
					continue;
				} else
...
static void dump_tasks_in_task_group_rt_rq(int depth, ulong rt_rq, int cpu)
{
	int i, c, tot, cnt, qheads;
	ulong offset, kvaddr, uvaddr;
	ulong list_head[2];
	...
	if (depth) {
		readmem(rt_rq + OFFSET(rt_rq_tg), KVADDR, &tg, sizeof(ulong), "rt_rq tg", FAULT_ON_ERROR);
		for (i = 0; i < tgi_p; i++) {
			if (tgi_array[i]->task_group == tg) {
...
static char *get_task_group_name(ulong group)
{
	...
	tmp = NULL;
	readmem(group + OFFSET(task_group_css) +
		OFFSET(cgroup_subsys_state_cgroup), KVADDR, &cgroup,
		sizeof(ulong), "task_group css cgroup", FAULT_ON_ERROR);
	if (cgroup == 0)
		return NULL;

	readmem(cgroup + OFFSET(cgroup_dentry), KVADDR, &dentry, sizeof(ulong), "cgroup dentry", FAULT_ON_ERROR);
	if (dentry == 0)
		return NULL;

	dentry_buf = GETBUF(SIZE(dentry));
	readmem(dentry, KVADDR, dentry_buf, SIZE(dentry), "dentry", FAULT_ON_ERROR);
...
static void fill_task_group_info_array(int depth, ulong group, char *group_buf, int i)
{
	int d;
	...
	while (next != kvaddr) {
		...
		readmem(group, KVADDR, group_buf, SIZE(task_group), "task_group", FAULT_ON_ERROR);
		next = ULONG(group_buf + OFFSET(task_group_siblings) + OFFSET(list_head_next));
...
static void dump_tasks_by_task_group(void)
{
	...
	readmem(root_task_group, KVADDR, buf, SIZE(task_group), "task_group", FAULT_ON_ERROR);
	...
	for (cpu = 0; cpu < kt->cpus; cpu++) {
		...
		fprintf(fp, "  TASK_GROUP RT_RQ: %lx\n", rt_rq_p);
		...
		fprintf(fp, "  TASK_GROUP CFS_RQ: %lx\n", cfs_rq_p);
		fprintf(fp, "  CFS RB_ROOT: %lx\n", (ulong) root);
		reuse_task_group_info_array();
...
static int sigrt_minmax(int *min, int *max)
{
	int sigrtmax, j;

	sigrtmax = THIS_KERNEL_VERSION < LINUX(2, 5, 0) ? _NSIG - 1 : _NSIG;
...
static void signame_list(void)
{
	...
	for (i = 1; i <= sigrtmax; i++) {
		if ((i == SIGRTMIN) || (i == sigrtmax)) {
			...
			fprintf(fp, "%s[%d] %s", i < 10 ? " " : "", i, signame[i].name);
 ...
 */
static void translate_sigset(ulonglong sigset)
{
	...
	for (i = 1, c = 0; i <= sigrtmax; i++) {
		if (sigset & (ulonglong) 1) {
			if (i == SIGRTMIN || i == sigrtmax)
				sprintf(buf, "%s%s", c++ ? " " : "", (i == SIGRTMIN) ? "SIGRTMIN" : "SIGRTMAX");
			else if (i > SIGRTMIN) {
				if (j <= min)
				...
				sprintf(buf, "%s%s", c++ ? " " : "", signame[i].name);
 ...
 */
static void do_sig_thread_group(ulong task)
{
	int i;
	int cnt;
	struct task_context *tc;
 ...
 */
void do_sig(ulong task, ulong flags, struct reference *ref)
{
	struct task_context *tc;
 ...
 */
static void signal_reference(struct task_context *tc, ulong flags, struct reference *ref)
{
	if (flags & FOREACH_SIG)
 ...
 */
static void dump_signal_data(struct task_context *tc, ulong flags)
{
	...
	if (VALID_MEMBER(task_struct_sig))
		...
		signal_struct = ULONG(tt->task_struct + OFFSET(task_struct_signal));

	size = MAX(SIZE(signal_struct), VALID_SIZE(signal_queue) ? SIZE(signal_queue) : SIZE(sigqueue));
	if (VALID_SIZE(sighand_struct))
		size = MAX(size, SIZE(sighand_struct));
	signal_buf = GETBUF(size);

	if (signal_struct)
		readmem(signal_struct, KVADDR, signal_buf, SIZE(signal_struct), "signal_struct buffer", FAULT_ON_ERROR);
	...
	if (flags & THREAD_GROUP_LEVEL) {
		...
		fprintf(fp, " SIG %s %s %s %s\n",
			...
			mkstring(buf3, 16, CENTER, "MASK"), mkstring(buf4, VADDR_PRLEN, LJUST, "FLAGS"));

		if (VALID_MEMBER(task_struct_sighand)) {
			sighand_struct = ULONG(tt->task_struct + OFFSET(task_struct_sighand));
		...
		for (i = 1; i <= sigrtmax; i++) {
			...
			if (use_sighand) {
				kaddr = sighand_struct + OFFSET(sighand_struct_action) + ((i - 1) * SIZE(k_sigaction));
				uaddr = signal_buf + OFFSET(sighand_struct_action) + ((i - 1) * SIZE(k_sigaction));
			} else {
				kaddr = signal_struct + OFFSET(signal_struct_action) + ((i - 1) * SIZE(k_sigaction));
				uaddr = signal_buf + OFFSET(signal_struct_action) + ((i - 1) * SIZE(k_sigaction));
			}
			...
			case 1:
				...
				break;
			default:
				mkstring(buf1, UVADDR_PRLEN, RJUST | LONG_HEX, MKSTR(handler));
				break;
			}
			...
			fprintf(fp, "%s%s %s %016llx %lx ",
				space(MINSPACE - 1),
				mkstring(buf2, UVADDR_PRLEN, LJUST | LONG_HEX, MKSTR(kaddr)), buf1, mask, sa_flags);
			...
			if (sa_flags) {
				...
				if (sa_flags & SA_RESTORER)
					fprintf(fp, "%s%sSA_RESTORER", translate-- > 0 ? "(" : "", others++ ? "|" : "");
#endif
#ifdef SA_NOCLDWAIT
				...
				if (sa_flags & SA_SIGINFO)
					fprintf(fp, "%s%sSA_SIGINFO", translate-- > 0 ? "(" : "", others++ ? "|" : "");
				if (sa_flags & SA_ONSTACK)
					fprintf(fp, "%s%sSA_ONSTACK", translate-- > 0 ? "(" : "", others++ ? "|" : "");
				if (sa_flags & SA_RESTART)
					fprintf(fp, "%s%sSA_RESTART", translate-- > 0 ? "(" : "", others++ ? "|" : "");
				if (sa_flags & SA_NODEFER)
					fprintf(fp, "%s%sSA_NODEFER", translate-- > 0 ? "(" : "", others++ ? "|" : "");
	...
	if (flags & TASK_LEVEL) {
		...
		if (VALID_MEMBER(task_struct_sigqueue))
			sigqueue = ULONG(tt->task_struct + OFFSET(task_struct_sigqueue));

		else if (VALID_MEMBER(task_struct_pending))
			sigqueue = ULONG(tt->task_struct +
					 OFFSET(task_struct_pending) + OFFSET_OPTION(sigpending_head, sigpending_list));
	 ...
	 */
	if ((flags & THREAD_GROUP_LEVEL) && VALID_MEMBER(signal_struct_shared_pending)) {
		...
		readmem(signal, KVADDR, signal_buf, SIZE(sigpending_signal), "signal", FAULT_ON_ERROR);
		sigset = task_signal(0, (ulong *) signal_buf);
		if (flags & TASK_INDENT)
			INDENT(2);
		fprintf(fp, "    SIGNAL: %016llx\n", sigset);
		sigqueue = (shared_pending + OFFSET_OPTION(sigpending_head, sigpending_list) + OFFSET(list_head_next));
		readmem(sigqueue, KVADDR, signal_buf, SIZE(sigqueue), "sigqueue", FAULT_ON_ERROR);
...
static void sigqueue_list(ulong sigqueue)
{
	...
	while (sigqueue) {
		readmem(sigqueue, KVADDR, signal_buf,
			SIZE_OPTION(signal_queue, sigqueue), "signal_queue/sigqueue", FAULT_ON_ERROR);

		if (VALID_MEMBER(signal_queue_next)
		    && VALID_MEMBER(signal_queue_info)) {
			next = ULONG(signal_buf + OFFSET(signal_queue_next));
			sig = INT(signal_buf + OFFSET(signal_queue_info) + OFFSET(siginfo_si_signo));
		} else {
			next = ULONG(signal_buf + OFFSET_OPTION(sigqueue_next, sigqueue_list));
			sig = INT(signal_buf + OFFSET(sigqueue_info) + OFFSET(siginfo_si_signo));
		}

		if (sigqueue_save == next)
			break;

		fprintf(fp, "             %3d  %lx\n", sig, sigqueue + OFFSET_OPTION(signal_queue_info, sigqueue_info));
	...
	if (task) {
		...
		if (VALID_MEMBER(sigpending_signal)) {
			sigset_ptr = (ulong *) (tt->task_struct +
						OFFSET(task_struct_pending) + OFFSET(sigpending_signal));
		} else if (VALID_MEMBER(task_struct_signal)) {
			sigset_ptr = (ulong *) (tt->task_struct + OFFSET(task_struct_signal));
		} else
			return 0;
	} else if (signal) {
		sigset_ptr = signal;
...
static ulonglong task_blocked(ulong task)
{
	ulonglong sigset;
	ulong *sigset_ptr;
...
static ulonglong sigaction_mask(ulong sigaction)
{
	ulonglong sigset;
	ulong *sigset_ptr;
 ...
 */
ulong generic_get_stackbase(ulong task)
{
	return task_to_stackbase(task);
}

ulong generic_get_stacktop(ulong task)
{
