...
ulonglong fix_lkcd_address(ulonglong addr)
{
	int i;
	ulong offset;

	for (i = 0; i < lkcd->fix_addr_num; i++) {
		if ((addr >= lkcd->fix_addr[i].task) && (addr < lkcd->fix_addr[i].task + STACKSIZE())) {
 ...
 */
int lkcd_dump_init(FILE * fp, int fd, char *dumpfile)
{
	switch (lkcd->version) {
	case LKCD_DUMP_V1:
 ...
 */
unsigned long get_lkcd_panic_task(void)
{
	return (lkcd->flags & (LKCD_VALID | LKCD_REMOTE) ? lkcd->panic_task : 0);
}

void get_lkcd_panicmsg(char *buf)
{
 ...
 *  two routines above.
 */
void set_remote_lkcd_panic_data(ulong task, char *buf)
{
	if (buf) {
		if (!(lkcd->panic_string = (char *)malloc(strlen(buf) + 1))) {
			fprintf(stderr, "cannot malloc space for panic message!\n");
			clean_exit(1);
		}
 ...
 */
int is_lkcd_compressed_dump(char *s)
{
	int tmpfd;
	uint64_t magic;
	uint32_t version;
 ...
 */
static void dump_dump_page(char *s, void *dp)
{
	switch (lkcd->version) {
	case LKCD_DUMP_V1:
 ...
 */
void dump_lkcd_environment(ulong arg)
{
	int others;
 ...
 */
void set_lkcd_debug(ulong debug)
{
	lkcd->debug = debug;
}
...
int lkcd_memory_dump(FILE * fp)
{
	...
	pct_compressed = (lkcd->compressed * 100) / (lkcd->hashed ? lkcd->hashed : 1);
	pct_raw = (lkcd->raw * 100) / (lkcd->hashed ? lkcd->hashed : 1);
	lkcd_print("          hashed: %ld\n", lkcd->hashed);
	lkcd_print("      compressed: %ld (%ld%%)\n", lkcd->compressed, pct_compressed);
	lkcd_print("             raw: %ld (%ld%%)\n", lkcd->raw, pct_raw);
	pct_cached = (lkcd->cached_reads * 100) / (lkcd->total_reads ? lkcd->total_reads : 1);
	pct_hashed = (lkcd->hashed_reads * 100) / (lkcd->total_reads ? lkcd->total_reads : 1);
	lkcd_print("    cached_reads: %ld (%ld%%)\n", lkcd->cached_reads, pct_cached);
	lkcd_print("    hashed_reads: %ld (%ld%%)\n", lkcd->hashed_reads, pct_hashed);
	...
	if (LKCD_DEBUG(1)) {
		for (i = 0; i < LKCD_PAGE_HASH; i++) {
			...
			while (phe && LKCD_VALID_PAGE(phe->pg_flags)) {
				sprintf(buf, "%llx@", (ulonglong) phe->pg_addr);
				sprintf(&buf[strlen(buf)], "%llx,", (ulonglong) phe->pg_hdr_offset);
				lkcd_print("%18s", buf);
		...
		for (i = 0; i < LKCD_PAGE_HASH; i++) {
			...
			wrap = 0;
			while (phe && LKCD_VALID_PAGE(phe->pg_flags)) {
				lkcd_print(BITS32()? "%9llx," : "%9lx,", phe->pg_addr);
				phe = phe->next;
				if (phe && (++wrap == 7)) {
					lkcd_print("\n        ");
					wrap = 0;
...
static int save_offset(uint64_t paddr, off_t off)
{
	uint64_t zone, page;
	int ii, ret;
	int max_zones;
	...
	if (lkcd->num_zones == 0) {
		...
		lkcd->zones[0].pages = malloc((ZONE_SIZE >> lkcd->page_shift) * sizeof(struct page_desc));
		if (!lkcd->zones[0].pages) {
			return -1;	/* this should be fatal */
		}

		BZERO(lkcd->zones[0].pages, (ZONE_SIZE >> lkcd->page_shift) * sizeof(struct page_desc));
		lkcd->num_zones++;
	}
 ...
 retry:
			...
			if (lkcd->zones[ii].pages[page].offset != 0) {
				if (lkcd->zones[ii].pages[page].offset != off) {
					if (CRASHDEBUG(1)
						...
						error(INFO,
						      "LKCD: conflicting page: zone %lld, "
	...
	if (ii == lkcd->num_zones) {
		...
		if (lkcd->num_zones < lkcd->max_zones) {
			...
			lkcd->zones[ii].pages = malloc((ZONE_SIZE >> lkcd->page_shift) * sizeof(struct page_desc));
			if (!lkcd->zones[ii].pages) {
				return -1;	/* this should be fatal */
			}

			BZERO(lkcd->zones[ii].pages, (ZONE_SIZE >> lkcd->page_shift) * sizeof(struct page_desc));
			lkcd->zones[ii].pages[page].offset = off;
			ret = 1;
			lkcd->num_zones++;
		} else {
			...
			memcpy(zones, lkcd->zones, lkcd->max_zones * sizeof(struct physmem_zone));
			free(lkcd->zones);
...
static off_t get_offset(uint64_t paddr)
{
	uint64_t zone, page;
	int ii;
...
int lkcd_lseek(physaddr_t paddr)
{
	long i = 0;
	int err;
	int eof;
	...
	while (!eof) {
		...
		physaddr = lkcd->get_dp_flags() &
		    ...
		    (lkcd->get_dp_address() - lkcd->kvbase) << lkcd->page_shift : lkcd->get_dp_address() - lkcd->kvbase;
 ...
 */
long lkcd_read(void *buf, long count)
{
	char *p;
 ...
 */
static int hash_page(ulong type)
{
	struct page_hash_entry *phe;
	int index;
	...
	for (phe = &lkcd->page_hash[index]; LKCD_VALID_PAGE(phe->pg_flags); phe = phe->next) {
	...
	if (LKCD_VALID_PAGE(phe->pg_flags)) {
		if ((phe->next = malloc(sizeof(struct page_hash_entry))) == NULL)
			return FALSE;
		phe = phe->next;
	}
 ...
 */
static int page_is_hashed(long *pp)
{
	struct page_hash_entry *phe;
	int index;
 ...
 *  lkcd->curhdroffs.
 */
int set_mb_benchmark(ulong page)
{
	long mb;
...
static int cache_page(void)
{
	...
	case LKCD_DUMP_COMPRESSED:
		...
		switch (lkcd->compression) {
		case LKCD_DUMP_COMPRESS_NONE:
			lkcd_print("dump_header: DUMP_COMPRESS_NONE and " "dump_page: DUMP_COMPRESSED (?)\n");
			return FALSE;
	...
	case LKCD_DUMP_RAW:
		...
		if ((rawsz = lkcd->get_dp_size()) == 0)
			BZERO(lkcd->page_cache_hdr[i].pg_bufptr, lkcd->page_size);
		else if (rawsz == lkcd->page_size)
			bytes = read(lkcd->fd, lkcd->page_cache_hdr[i].pg_bufptr, lkcd->page_size);
		else {
			lkcd_print("cache_page: " "invalid LKCD_DUMP_RAW dp_size\n");
 ...
 */
static int lkcd_uncompress_RLE(unsigned char *cbuf, unsigned char *ucbuf, uint32_t blk_size, int *new_size)
{
	int i;
	...
	while (ri < blk_size) {
		...
		if (wi > /* PAGE_SIZE */ lkcd->page_size) {
			lkcd_print("Attempted to decompress beyond page boundaries: file corrupted!\n");
			return (0);
		}
	}
...
static int uncompress_recover(unsigned char *dest, ulong destlen, unsigned char *source, ulong sourcelen)
{
	int byte, bit;
	...
	for (byte = 0; byte < sourcelen; byte++) {
		for (bit = 0; bit < 8; bit++) {
			...
			if (uncompress(dest, &retlen, source, sourcelen) == Z_OK && retlen == destlen) {
				good_decomp++;
 ...
 */
static int lkcd_uncompress_gzip(unsigned char *dest, ulong destlen, unsigned char *source, ulong sourcelen)
{
	ulong retlen = destlen;
	int rc = FALSE;
	...
	case Z_BUF_ERROR:
		lkcd_print("uncompress: " "Z_BUF_ERROR (not enough room in output buffer)\n");
		rc = FALSE;
		break;
	...
	if (rc == FALSE) {
		uncompress_errloc = uncompress_recover(dest, destlen, source, sourcelen);
	}
	return rc;
}
 ...
 */
int lkcd_load_dump_page_header(void *dp, ulong page)
{
	uint32_t dp_flags;
	...
	if (read(lkcd->fd, dp, lkcd->page_header_size) != lkcd->page_header_size) {
		if (page > lkcd->total_pages)
			lkcd_dumpfile_complaint(page, lkcd->total_pages, LKCD_DUMPFILE_EOF);
	 ...
	 */
	if ((page == 0) && !(lkcd->flags & LKCD_VALID) && (lkcd->version == LKCD_DUMP_V1) && (dp_address == 0xc0000000))
		lkcd->kvbase = dp_address;
#endif

	physaddr = dp_flags & (LKCD_DUMP_MCLX_V0 | LKCD_DUMP_MCLX_V1) ?
	    (dp_address - lkcd->kvbase) << lkcd->page_shift : dp_address - lkcd->kvbase;
 ...
 */
void lkcd_dumpfile_complaint(uint32_t realpages, uint32_t dh_num_pages, int retval)
{
	if (lkcd->flags & LKCD_BAD_DUMP)
		return;
