...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "vd:")) > 0) {
		switch (c) {
		case 'v':
			printf("%s %s\n", basename(argv[0]), BASELEVEL_REVISION);
			exit(0);
 ...
 */
static void daemon_socket_options(int sockfd)
{
	int nodelay;
...
static void handle_connection(int sock)
{
	...
	while (TRUE) {
		...
		if (STRNEQ(recvbuf, "OPEN ")) {
			...
			if (i < MAX_REMOTE_FDS) {
				if ((fds[i] = open(file, O_RDWR)) < 0) {
					if ((fds[i] = open(file, O_RDONLY)) < 0)
						...
						sprintf(buf1, " %d O_RDONLY %ld", fds[i], daemon_filesize(fds[i]));
						strcat(sendbuf, buf1);
					}
				} else {
					sprintf(buf1, " %d O_RDWR %ld", fds[i], daemon_filesize(fds[i]));
					strcat(sendbuf, buf1);
				}
			} else
				strcat(sendbuf, " <FAIL>");
			...
			if (!len) {
				sprintf(readbuf, "%s%07ld", FAILMSG, (ulong) errno);
				console("[%s]\n", readbuf);
			} else {
			...
			if ((len = read_netdump(UNUSED, &readbuf[DATA_HDRSIZE], len, UNUSED, addr)) < 0)
				len = 0;
			...
			sprintf(sendbuf, "%s%s", savebuf, retval ? " OK" : " <FAIL>");
			console("[%s]\n", sendbuf);
			...
			sprintf(sendbuf, "%s %s %d", recvbuf, MACHINE_TYPE, getpid());
			console("[%s]\n", sendbuf);
			daemon_send(sendbuf, strlen(sendbuf));
			continue;
			...
			sprintf(readbuf, "/usr/bin/strings %s | grep 'Linux version'", file);
			...
			while (!done) {
				...
				cnt = fread(&readbuf[DATA_HDRSIZE], sizeof(char), reqsize, pipe);

				total += cnt;

				if (feof(pipe)) {
					sprintf(readbuf, "%s%07ld", DONEMSG, (ulong) cnt);
					done = TRUE;
				} else if (ferror(pipe)) {
					sprintf(readbuf, "%s%07ld", FAILMSG, (ulong) errno);
					done = TRUE;
				} else
					sprintf(readbuf, "%s%07ld", DATAMSG, (ulong) cnt);
			...
			sprintf(sendbuf, "%s %s", savebuf, no_debugging_symbols_found(p2));
			console("[%s]\n", sendbuf);
			...
			if (strstr(recvbuf, "LIVE"))
				sprintf(sendbuf, "%s %d", recvbuf, (uint) getpagesize());
			else if (strstr(recvbuf, "NETDUMP"))
				sprintf(sendbuf, "%s %d", recvbuf, (uint) netdump_page_size());
			else if (strstr(recvbuf, "MCLXCD"))
				sprintf(sendbuf, "%s %d", recvbuf, (uint) mclx_page_size());
			else if (strstr(recvbuf, "LKCD"))
				sprintf(sendbuf, "%s %d", recvbuf, (uint) lkcd_page_size());
			else if (strstr(recvbuf, "S390D"))
				sprintf(sendbuf, "%s %d", recvbuf, s390_page_size());
			...
			if (daemon_find_module(p1, p2, buf1)) {
				if (daemon_checksum(buf1, &total))
					sprintf(sendbuf, "%s %s %lx", savebuf, buf1, total);
				else
					sprintf(sendbuf, "%s %s %lx", savebuf, buf1, (ulong) 0xdeadbeef);
			} else
				sprintf(sendbuf, "%s <FAIL>", savebuf);
			...
			while (!done) {
				...
				cnt = fread(&readbuf[DATA_HDRSIZE], sizeof(char), reqsize, tmp);
			...
			sprintf(sendbuf, "%s %s", savebuf, netdump_init(p3, NULL) ? "OK" : "<FAIL>");
			...
			sprintf(sendbuf, "%s %s", savebuf, lkcd_dump_init(NULL, atoi(p2), p3) ? "OK" : "<FAIL>");
			...
			sprintf(sendbuf, "%s %s", savebuf, s390_dump_init(p3) ? "OK" : "<FAIL>");
			...
			sprintf(sendbuf, "%s %s", savebuf, s390x_dump_init(p3) ? "OK" : "<FAIL>");
			...
			if ((len = read_s390_dumpfile(UNUSED, &readbuf[DATA_HDRSIZE], len, UNUSED, addr)) < 0)
				len = 0;
...
static void daemon_send(void *buffer, int len)
{
 ...
 */
char *no_debugging_symbols_found(char *file)
{
	FILE *pipe;
	char buf[BUFSIZE];
 ...
 */
static int daemon_proc_version(char *buf)
{
	FILE *pipe;
	struct stat sbuf;
	...
	if (fread(buf, sizeof(char), BUFSIZE - 1, pipe) <= 0)
		return FALSE;
 ...
 */
static int daemon_is_elf_file(char *s)
{
	int fd, is_elf;
 ...
 */
static ulong daemon_htol(char *s)
{
	long i, j;
	ulong n;
...
static int daemon_find_booted_kernel(char *namelist)
{
	char kernel[BUFSIZE];
...
static char **daemon_build_searchdirs(int create)
{
	int i;
	int cnt;
	DIR *dirp;
	...
	if ((dirp = opendir("/usr/src"))) {
		...
		for (dp = readdir(dirp); dp != NULL; dp = readdir(dirp)) {
			if (STREQ(dp->d_name, "linux") || STREQ(dp->d_name, ".") || STREQ(dp->d_name, ".."))
				continue;
 ...
 *  available.
 */
static int daemon_mount_point(char *name)
{
	int i;
 ...
 */
static int daemon_is_directory(char *file)
{
	struct stat sbuf;
 ...
 */
static int daemon_file_readable(char *file)
{
	struct stat sbuf;
	long tmp;
	int fd;
 ...
 */
int daemon_parse_line(char *str, char *argv[])
{
	int i, j;
	int string;
	int expression;
...
static char *daemon_clean_line(char *line)
{
	char buf[BUFSIZE];
	char *p;
...
int monitor_memory(long *a1, long *a2, long *a3, long *a4)
{
	return FALSE;
}

static int daemon_find_module(char *release, char *filename, char *retbuf)
{
	char dir[BUFSIZE];
	int found;
...
int daemon_search_directory_tree(char *directory, char *file, char *retbuf)
{
	char command[BUFSIZE];
	char buf[BUFSIZE];
	FILE *pipe;
	...
	if (!daemon_file_exists("/usr/bin/find", NULL) ||
	    !daemon_file_exists("/bin/echo", NULL) || !daemon_is_directory(directory))
		return FALSE;

	sprintf(command, "/usr/bin/find %s -name %s -print; /bin/echo search done", directory, file);
	...
	while (fgets(buf, BUFSIZE - 1, pipe) || !found) {
		...
		if (!found && STREQ((char *)basename(strip_linefeeds(buf)), file)) {
			strcpy(retbuf, buf);
			found = TRUE;
		}
	}
...
static int daemon_file_exists(char *file, struct stat *sp)
{
	struct stat sbuf;
...
static int daemon_checksum(char *file, long *retsum)
{
	int i;
	int fd;
	ssize_t cnt;
...
int is_remote_daemon(char *dp)
{
	...
	if (connect(pc->sockfd, (struct sockaddr *)&serv_addr, sizeof(struct sockaddr_in)) < 0) {
		herror(hp->h_name);
	...
	if (file2) {
		...
		case TYPE_ELF:
			if (pc->server_namelist)
				error(FATAL, "two remote namelists entered: %s and %s\n", file1, file2);
			pc->server_namelist = file2;
			break;
		case TYPE_NETDUMP:
			if (pc->server_memsrc)
				error(FATAL, "neither %s or %s is an ELF file\n", file1, file2);
 ...
 */
static int remote_file_type(char *file)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
 ...
 */
void remote_socket_options(int sockfd)
{
	int rcvbuf, optlen;

	pc->rcvbufsize = rcvbuf = READBUFSIZE;

	if (setsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, (char *)&rcvbuf, sizeof(rcvbuf)) < 0) {
		error(INFO, "SO_RCVBUF setsockopt error\n");
		return;
	}

	optlen = sizeof(rcvbuf);
	if (getsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, (char *)&rcvbuf, (socklen_t *) & optlen) < 0) {
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_RCVLOWAT, (char *)&rcvbuf, sizeof(rcvbuf)) < 0) {
		/*
	...
	optlen = sizeof(rcvbuf);
	if (getsockopt(sockfd, SOL_SOCKET, SO_RCVLOWAT, (char *)&rcvbuf, (socklen_t *) & optlen) < 0) {
 ...
 */
static int remote_file_open(struct remote_file *rfp)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
	char *p1;
 ...
 */
static int remote_file_close(struct remote_file *rfp)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
 ...
 */
static int remote_proc_version(char *buf)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
...
void remote_fd_init(void)
{
	...
	if ((pc->namelist || pc->server_namelist) && pc->namelist_debug && pc->system_map) {
		error(INFO,
		      "too many namelist options:\n       %s\n       %s\n       %s\n",
		      pc->namelist ? pc->namelist : pc->server_namelist, pc->namelist_debug, pc->system_map);
		program_usage(SHORT_FORM);
	}
	...
	if (pc->flags & MEMORY_SOURCES) {
		...
		if (remote_file_open(rfp)) {
			...
			if ((pc->flags & REM_NETDUMP) && !remote_netdump_init())
				error(FATAL, "%s: remote initialization failed\n", pc->server_memsrc);

			if ((pc->flags & REM_LKCD) && !remote_lkcd_dump_init())
				error(FATAL, "%s: remote initialization failed\n", pc->server_memsrc);

			if ((pc->flags & REM_S390D) && !remote_s390_dump_init())
				error(FATAL, "%s: remote initialization failed\n", pc->server_memsrc);
		...
		if (REMOTE_ACTIVE() && !remote_proc_version(kt->proc_version))
			error(WARNING, "daemon cannot access /proc/version\n\n");
	}
	...
	if (pc->namelist) {
		...
		if (!remote_find_booted_kernel(rfp))
			error(FATAL, "remote daemon cannot find booted kernel\n");
		if (!remote_file_open(rfp))
 ...
 */
static void copy_to_local_namelist(struct remote_file *rfp)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
	...
	if ((pc->nfd = open(pc->namelist, O_RDWR | O_CREAT | O_TRUNC, S_IRWXU)) < 0) {
...
static char *create_local_namelist(struct remote_file *rfp)
{
	char buf[BUFSIZE];
	char *p1;
...
static int identical_namelist(char *file, struct remote_file *rfp)
{
	...
	if (remote_file_checksum(rfp) && file_checksum(file, &csum) && (csum == rfp->csum))
		return TRUE;
	...
	if (CRASHDEBUG(1)) {
		...
		fprintf(fp, "%s vs. %s => %s\n", file, rfp->filename, STREQ(vers, readbuf) ? "IDENTICAL" : "DIFFERENT");
	}
 ...
 */
static int remote_file_checksum(struct remote_file *rfp)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
	char *p1;
 ...
 */
int get_remote_file(struct remote_file *rfp)
{
	int i;
	char local[BUFSIZE];
	char readbuf[READBUFSIZE];
	...
	if (STREQ(rfp->filename, "dumpfile") ||
	    ...
	    STREQ(rfp->filename, basename(pc->server_memsrc)) || STREQ(rfp->filename, readbuf))
		goto dumpfile_save;
	...
	if (STREQ(rfp->filename, "kernel") ||
	    ...
	    STREQ(rfp->filename, pc->server_namelist) || STREQ(rfp->filename, readbuf))
		goto kernel_save;
 ...
 generic_file_save:
	...
	while (file_exists(local, NULL)) {
		...
		if (file_checksum(local, &csum) && (csum == rfp->csum)) {
			if (CRASHDEBUG(1))
				error(NOTE, "local %s checksum matches -- using it\n", local);
			strcpy(rfp->local, local);
			return TRUE;
		}
		sprintf(local, "%s@%s_%d", basename(rfp->filename), pc->server, ++cnt);
	}
	...
	if (copy_remote_file(rfp, sfd, local, rfp->flags & REMOTE_VERBOSE ?
		...
		if (rfp->flags & REMOTE_VERBOSE)
			fprintf(stderr, "\rremote file saved as: \"%s\"                                \n", local);
		retval = TRUE;
		rfp->flags |= REMOTE_COPY_DONE;
	} else {
		fprintf(stderr, "\r%s NOT saved                                             \n", rfp->filename);
		retval = FALSE;
	}
 ...
 dumpfile_save:
	...
	if (copy_remote_file(rfp, sfd, local, "please wait... (copying remote dumpfile: ")) {
		fprintf(stderr, "\rdumpfile saved as: \"%s\"                                \n", local);
 ...
 */
static int remote_find_booted_kernel(struct remote_file *rfp)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
	char *p1;
...
uint remote_page_size(void)
{
	...
	if (REMOTE_ACTIVE())
		...
		error(FATAL, "cannot determine remote page size (unknown memory source)\n");
	...
	if (psz > MAXRECVBUFSIZE)
		error(FATAL, "remote page size %d is larger than MAXRECVBUFSIZE!\n", psz);
 ...
 */
static int copy_remote_file(struct remote_file *rfp, int fd, char *file, char *ttystr)
{
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
	char readbuf[READBUFSIZE];
	...
	for (offset = 0; offset < filesize;) {
		...
		if (write(fd, readbuf, size) != size) {
			error(INFO, "%swrite to local file \"%s\" failed", ttystr ? "\n" : "", file);
			close(fd);
			return FALSE;
		}
		...
		if (ttystr) {
			...
			if (pct > last) {	/* readline work-around... */
				if (last < 0)
					sprintf(readbuf, "echo -n \'%s0%%)\'", ttystr);
				else if (last >= 0 && last < 10)
					sprintf(readbuf, "echo -e -n \"\\b\\b\\b%ld%%)\"", pct);
				else if (last < 100)
					sprintf(readbuf, "echo -e -n \"\\b\\b\\b\\b%ld%%)\"", pct);
...
static void copy_remote_gzip_file(struct remote_file *rfp, char *file, char *ttystr)
{
	int done;
	char sendbuf[BUFSIZE];
	char readbuf[READBUFSIZE];
	...
	while (!done) {
		...
		while (req) {
			...
			if (!tot) {
				if (STRNEQ(bufptr, FAILMSG)) {
					fprintf(fp, "copy_remote_gzip_file: %s\n", bufptr);
					tot = -1;
					break;
				}
		...
		if (ttystr && (stat(pc->namelist, &sbuf) == 0)) {
			...
			fprintf(stderr, "\r%s%ld%%)%s", ttystr, pct, CRASHDEBUG(1) ? "\n" : "");
		}
	}
 ...
 */
int find_remote_module_objfile(struct load_module *lm, char *module, char *retbuf)
{
	int absolute;
	char sendbuf[BUFSIZE];
	char recvbuf[BUFSIZE];
	...
	if (absolute) {
		if ((p1 = strstr(module, "@"))) {
			*p1 = NULLCHAR;
			...
			error(FATAL, "module file name must have \"@server-name\" attached\n");
		}
		...
		BZERO(sendbuf, BUFSIZE);
		BZERO(recvbuf, BUFSIZE);
		sprintf(sendbuf, "FIND_MODULE %s %s", kt->utsname.release, module);
		send(pc->sockfd, sendbuf, strlen(sendbuf), 0);
		recv(pc->sockfd, recvbuf, BUFSIZE - 1, 0);
		if (strstr(recvbuf, "<FAIL>")) {
	...
	if (get_remote_file(rfp)) {
		if (!is_elf_file(rfp->local)) {
			error(INFO, "%s@%s: not an ELF format object file\n", rfp->filename, pc->server);
			return FALSE;
		}
 ...
 */
int remote_memory_dump(int verbose)
{
	char sendbuf[BUFSIZE];
	char readbuf[READBUFSIZE];
	...
	BZERO(sendbuf, BUFSIZE);
	sprintf(sendbuf, "MEMORY_DUMP %ld %s%s", pc->rcvbufsize, type, verbose ? "_VERBOSE" : "");
	send(pc->sockfd, sendbuf, strlen(sendbuf), 0);
	...
	while (!done) {
		...
		while (req) {
			...
			if (!tot) {
				if (STRNEQ(bufptr, FAILMSG)) {
					fprintf(fp, "remote_memory_dump: %s\n", bufptr);
					tot = -1;
					break;
				}
				if (STRNEQ(bufptr, DONEMSG)
 ...
 *  a page in length.
 */
int remote_memory_read(int rfd, char *buffer, int cnt, physaddr_t address)
{
	char sendbuf[BUFSIZE];
	char readbuf[READBUFSIZE];
	char datahdr[DATA_HDRSIZE];
...
void remote_clear_pipeline(void)
{
	...
	if (FD_ISSET(pc->sockfd, &rfds)) {
		...
		if (CRASHDEBUG(1))
			error(INFO, "remote_clear_pipeline(%d): %d bytes discarded\n", pc->sockfd, ret);
	}
}
