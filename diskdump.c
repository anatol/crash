...
void map_cpus_to_prstatus_kdump_cmprs(void)
{
	...
	for (i = 0, j = 0; i < nrcpus; i++) {
		if (in_cpu_map(ONLINE, i)) {
			...
			dd->num_prstatus_notes = MAX(dd->num_prstatus_notes, i + 1);
		}
	}
...
#define DDL_SIZE 16
	...
	if (dd_list == NULL) {
		...
		for (i = 0; i < num_dumpfiles; i++) {
			...
			if (same_file(ddp->filename, name))
				error(FATAL, "split dumpfiles are identical:\n" "  %s\n  %s\n", ddp->filename, name);
			if (memcmp(ddp->header, dd->header, sizeof(struct disk_dump_header)))
				error(FATAL,
	...
	if (CRASHDEBUG(1))
		fprintf(fp, "%s: start_pfn=%lu, end_pfn=%lu\n", name,
			dd->sub_header_kdump->start_pfn, dd->sub_header_kdump->end_pfn);
}
...
{
	return header->bitmap_blocks >= divideup(divideup(header->max_mapnr, 8), dd->block_size) * 2;
}
...
void process_elf32_notes(void *note_buf, unsigned long size_note)
{
	Elf32_Nhdr *nt;
	size_t index, len = 0;
	int num = 0;
...
void process_elf64_notes(void *note_buf, unsigned long size_note)
{
	Elf64_Nhdr *nt;
	size_t index, len = 0;
	int num = 0;
...
static void arm_kdump_header_adjust(int header_version)
{
...
static int read_dump_header(char *file)
{
 ...
 restart:
	...
	if (!memcmp(header->signature, DISK_DUMP_SIGNATURE, sizeof(header->signature))) {
		dd->flags |= DISKDUMP_LOCAL;
	} else if (!memcmp(header->signature, KDUMP_SIGNATURE, sizeof(header->signature))) {
	...
	if (CRASHDEBUG(1))
		fprintf(fp, "%s: header->utsname.machine: %s\n",
			DISKDUMP_VALID()? "diskdump" : "compressed kdump", header->utsname.machine);

	if (STRNEQ(header->utsname.machine, "i686") && machine_type_mismatch(file, "X86", NULL, 0))
		goto err;
	else if (STRNEQ(header->utsname.machine, "x86_64") && machine_type_mismatch(file, "X86_64", NULL, 0))
		goto err;
	else if (STRNEQ(header->utsname.machine, "ia64") && machine_type_mismatch(file, "IA64", NULL, 0))
		goto err;
	else if (STREQ(header->utsname.machine, "ppc") && machine_type_mismatch(file, "PPC", NULL, 0))
		goto err;
	else if (STREQ(header->utsname.machine, "ppc64") && machine_type_mismatch(file, "PPC64", NULL, 0))
		goto err;
	else if (STRNEQ(header->utsname.machine, "arm") && machine_type_mismatch(file, "ARM", NULL, 0))
		goto err;
	else if (STRNEQ(header->utsname.machine, "s390x") && machine_type_mismatch(file, "S390X", NULL, 0))
		goto err;
	else if (STRNEQ(header->utsname.machine, "aarch64") && machine_type_mismatch(file, "ARM64", NULL, 0))
		goto err;

	if (header->block_size != block_size) {
		block_size = header->block_size;
		if (CRASHDEBUG(1))
			fprintf(fp, "retrying with different block/page size: %d\n", header->block_size);
		goto restart;
	}
	dd->block_size = header->block_size;
	dd->block_shift = ffs(header->block_size) - 1;

	if ((DISKDUMP_VALID() && (sizeof(*header) + sizeof(void *) * header->nr_cpus > block_size))
	    || header->nr_cpus <= 0) {
		error(WARNING, "%s: invalid nr_cpus value: %d\n",
		      DISKDUMP_VALID()? "diskdump" : "compressed kdump", header->nr_cpus);
	...
	if (CRASHDEBUG(8))
		fprintf(fp, "%s: memory bitmap offset: %llx\n",
			DISKDUMP_VALID()? "diskdump" : "compressed kdump", (ulonglong) offset);
	...
	if (dump_is_partial(header))
		memcpy(dd->dumpable_bitmap, dd->bitmap + bitmap_len / 2, bitmap_len / 2);
	else
	...
	if (machine_type("ARM"))
		...
		error(INFO, "%s: unsupported machine type: %s\n",
		      DISKDUMP_VALID()? "diskdump" : "compressed kdump", MACHINE_TYPE);
		goto err;
	}
	...
	if (KDUMP_CMPRS_VALID() && !(dd->flags & NO_ELF_NOTES) &&
	    ...
	    (sub_header_kdump->offset_note) && (sub_header_kdump->size_note) && (machdep->process_elf_notes)) {
		...
		if ((dd->nt_prstatus_percpu = malloc(NR_CPUS * sizeof(void *))) == NULL)
			error(FATAL, "compressed kdump: cannot malloc pointer" " to NT_PRSTATUS notes\n");
	...
	if (KDUMP_CMPRS_VALID() && (dd->header->header_version >= 5) &&
	    (sub_header_kdump->offset_eraseinfo) && (sub_header_kdump->size_eraseinfo))
		pc->flags2 |= ERASEINFO_DATA;

	if (KDUMP_CMPRS_VALID() && (dd->header->header_version >= 3) &&
	    dd->sub_header_kdump->offset_vmcoreinfo && dd->sub_header_kdump->size_vmcoreinfo)
		pc->flags2 |= VMCOREINFO;

	/* For split dumpfile */
	if (KDUMP_CMPRS_VALID()) {
		is_split = ((dd->header->header_version >= 2) && (sub_header_kdump->split));

		if ((is_split && (num_dumpfiles != 0) && (dd_list == NULL)) || (!is_split && (num_dumpfiles != 0))) {
			clean_diskdump_data();
			goto err;
		}
...
static ulong pfn_to_pos(ulong pfn)
{
...
int is_diskdump(char *file)
{
	...
	for (i = 0; i < DISKDUMP_CACHED_PAGES; i++)
		dd->page_cache_hdr[i].pg_bufptr = &dd->page_cache_buf[i * dd->block_size];
 ...
 *  At a minimum
 */
int diskdump_init(char *unused, FILE * fptr)
{
 ...
 */
int diskdump_phys_base(unsigned long *phys_base)
{
	if (KDUMP_CMPRS_VALID()) {
 ...
 */
static int page_is_cached(physaddr_t paddr)
{
	int i;
 ...
 *  uncompressed data.
 */
static int cache_page(physaddr_t paddr)
{
	int i, ret;
	int found;
	ulong pfn;
	...
	if (!found) {
		...
		dd->evict_index = (dd->evict_index + 1) % DISKDUMP_CACHED_PAGES;
		dd->evictions++;
	}
	...
	seek_offset = dd->data_offset + (off_t) (desc_pos - 1) * sizeof(page_desc_t);
	...
	if (pd.flags & DUMP_DH_COMPRESSED_ZLIB) {
		retlen = block_size;
		ret =
		    uncompress((unsigned char *)dd->page_cache_hdr[i].pg_bufptr,
			       &retlen, (unsigned char *)dd->compressed_page, pd.size);
		if ((ret != Z_OK) || (retlen != block_size)) {
			error(INFO, "%s: uncompress failed: %d\n",
			      DISKDUMP_VALID()? "diskdump" : "compressed kdump", ret);
			return READ_ERROR;
		}
		...
		if (!(dd->flags & LZO_SUPPORTED)) {
			error(INFO,
			      "%s: uncompress failed: no lzo compression support\n",
			      DISKDUMP_VALID()? "diskdump" : "compressed kdump");
			return READ_ERROR;
		}
#ifdef LZO
		retlen = block_size;
		ret =
		    lzo1x_decompress_safe((unsigned char *)dd->compressed_page,
					  pd.size,
					  (unsigned char *)dd->page_cache_hdr[i].pg_bufptr, &retlen,
		...
		if (!(dd->flags & SNAPPY_SUPPORTED)) {
			error(INFO,
			      "%s: uncompress failed: no snappy compression support\n",
			      DISKDUMP_VALID()? "diskdump" : "compressed kdump");
			return READ_ERROR;
		}
#ifdef SNAPPY
		ret = snappy_uncompressed_length((char *)dd->compressed_page, pd.size, &retlen);
		if (ret != SNAPPY_OK) {
			error(INFO, "%s: uncompress failed: %d\n",
			      DISKDUMP_VALID()? "diskdump" : "compressed kdump", ret);
			return READ_ERROR;
		...
		ret = snappy_uncompress((char *)dd->compressed_page, pd.size,
					(char *)dd->page_cache_hdr[i].pg_bufptr, &retlen);
		...
		memcpy(dd->page_cache_hdr[i].pg_bufptr, dd->compressed_page, block_size);
 ...
 */
int read_diskdump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	int ret;
	physaddr_t curpaddr;
	...
	if ((pfn >= dd->header->max_mapnr) || !page_is_ram(pfn)) {
		if (CRASHDEBUG(8)) {
			fprintf(fp, "read_diskdump: SEEK_ERROR: " "paddr/pfn: %llx/%lx ", (ulonglong) paddr, pfn);
	...
	if (!page_is_dumpable(pfn)) {
		if ((dd->flags & (ZERO_EXCLUDED | ERROR_EXCLUDED)) == ERROR_EXCLUDED) {
			if (CRASHDEBUG(8))
		...
		if (CRASHDEBUG(8))
			fprintf(fp, "read_diskdump: zero-fill: " "paddr/pfn: %llx/%lx\n", (ulonglong) paddr, pfn);
		memset(bufptr, 0, cnt);
		return cnt;
	}
		...
		if ((ret = cache_page(curpaddr)) < 0) {
			if (CRASHDEBUG(8))
				fprintf(fp, "read_diskdump: "
					...
					ret == SEEK_ERROR ? "SEEK_ERROR" : "READ_ERROR", (ulonglong) curpaddr);
			return ret;
		}
 ...
 */
int write_diskdump(int fd, void *bufptr, int cnt, ulong addr, physaddr_t paddr)
{
	return 0;
}
...
static void get_diskdump_regs_ppc(struct bt_info *bt, ulong * eip, ulong * esp)
{
	Elf32_Nhdr *note;
	int len;

	if (KDUMP_CMPRS_VALID())
		ppc_relocate_nt_prstatus_percpu(dd->nt_prstatus_percpu, &dd->num_prstatus_notes);
	if (KDUMP_CMPRS_VALID() &&
	    (bt->task == tt->panic_task || (is_task_active(bt->task) && dd->num_prstatus_notes > 1))) {
...
static void get_diskdump_regs_ppc64(struct bt_info *bt, ulong * eip, ulong * esp)
{
...
static void get_diskdump_regs_arm(struct bt_info *bt, ulong * eip, ulong * esp)
{
	machdep->get_stack_frame(bt, eip, esp);
}

static void get_diskdump_regs_arm64(struct bt_info *bt, ulong * eip, ulong * esp)
{
	machdep->get_stack_frame(bt, eip, esp);
}
...
void get_diskdump_regs(struct bt_info *bt, ulong * eip, ulong * esp)
{
...
static void dump_vmcoreinfo(FILE * fp)
{
	...
	if ((buf = malloc(size_vmcoreinfo)) == NULL) {
		error(FATAL, "compressed kdump: cannot malloc vmcoreinfo" " buffer\n");
	}
...
static void dump_eraseinfo(FILE * fp)
{
	char *buf = NULL;
	unsigned long i = 0;
	...
	if ((buf = malloc(size_eraseinfo)) == NULL) {
		error(FATAL, "compressed kdump: cannot malloc eraseinfo" " buffer\n");
	}
...
static void dump_nt_prstatus_offset(FILE * fp)
{
	...
	if (KDUMP_CMPRS_VALID() && !(dd->flags & NO_ELF_NOTES) &&
		...
		for (tot = cnt = 0; tot < size; tot += len) {
			if (machine_type("X86_64") || machine_type("S390X") || machine_type("ARM64")) {
 ...
 */
int __diskdump_memory_dump(FILE * fp)
{
	...
	if ((dsh = dd->sub_header)) {
		...
		if ((pc->flags & RUNTIME)
			...
			fprintf(fp, "%d (0x%x) %s", dump_level, dump_level, dump_level ? "(" : "");
...
#define DUMP_SAVE_PRIVATE  0x00000010	/* Save private pages */
			...
			if (dump_level & DUMP_EXCLUDE_CACHE)
				fprintf(fp, "%sDUMP_EXCLUDE_CACHE", others++ ? "|" : "");
			if (dump_level & DUMP_EXCLUDE_CLEAN)
				fprintf(fp, "%sDUMP_EXCLUDE_CLEAN", others++ ? "|" : "");
			if (dump_level & DUMP_EXCLUDE_FREE)
				fprintf(fp, "%sDUMP_EXCLUDE_FREE", others++ ? "|" : "");
			if (dump_level & DUMP_EXCLUDE_ANON)
				fprintf(fp, "%sDUMP_EXCLUDE_ANON", others++ ? "|" : "");
			if (dump_level & DUMP_SAVE_PRIVATE)
				fprintf(fp, "%sDUMP_SAVE_PRIVATE", others++ ? "|" : "");
			fprintf(fp, "%s\n\n", dump_level ? ")" : "");
		} else
			fprintf(fp, "%s\n\n", pc->flags & RUNTIME ? "(unknown)" : "(undetermined)");
...
#define DL_EXCLUDE_FREE         (0x010)	/* Exclude Free Pages */
			...
			if (dump_level & DL_EXCLUDE_ZERO)
				fprintf(fp, "%sDUMP_EXCLUDE_ZERO", others++ ? "|" : "");
			if (dump_level & DL_EXCLUDE_CACHE)
				fprintf(fp, "%sDUMP_EXCLUDE_CACHE", others++ ? "|" : "");
			if (dump_level & DL_EXCLUDE_CACHE_PRI)
				fprintf(fp, "%sDUMP_EXCLUDE_CACHE_PRI", others++ ? "|" : "");
			if (dump_level & DL_EXCLUDE_USER_DATA)
				fprintf(fp, "%sDUMP_EXCLUDE_USER_DATA", others++ ? "|" : "");
		...
		if (dh->header_version >= 3) {
			...
			fprintf(fp, "     size_vmcoreinfo: %lu (0x%lx)\n",
				dd->sub_header_kdump->size_vmcoreinfo, dd->sub_header_kdump->size_vmcoreinfo);
			if (dd->sub_header_kdump->offset_vmcoreinfo && dd->sub_header_kdump->size_vmcoreinfo) {
				dump_vmcoreinfo(fp);
			}
		}
		if (dh->header_version >= 4) {
			...
			fprintf(fp, "           size_note: %lu (0x%lx)\n",
				dd->sub_header_kdump->size_note, dd->sub_header_kdump->size_note);
		...
		if (dh->header_version >= 5) {
			...
			fprintf(fp, "      size_eraseinfo: %lu (0x%lx)\n",
				dd->sub_header_kdump->size_eraseinfo, dd->sub_header_kdump->size_eraseinfo);
			if (dd->sub_header_kdump->offset_eraseinfo && dd->sub_header_kdump->size_eraseinfo) {
				dump_eraseinfo(fp);
			}
		}
		fprintf(fp, "\n");
 ...
 */
int diskdump_memory_dump(FILE * fp)
{
	int i;
 ...
 */
ulong get_diskdump_switch_stack(ulong task)
{
	return 0;
}
...
void show_split_dumpfiles(void)
{
	...
	for (i = 0; i < num_dumpfiles; i++) {
		ddp = dd_list[i];
		fprintf(fp, "%s%s %s",
			i ? "              " : "", ddp->filename, is_partial_diskdump()? "[PARTIAL DUMP]" : "");
...
static char *vmcoreinfo_read_string(const char *key)
{
	...
	if ((buf = malloc(size_vmcoreinfo + 1)) == NULL) {
		error(INFO, "compressed kdump: cannot malloc vmcoreinfo" " buffer\n");
		goto err;
	}
...
void diskdump_display_regs(int cpu, FILE * ofp)
{
	Elf32_Nhdr *note32;
	Elf64_Nhdr *note64;
	char *user_regs;
	...
	if (machine_type("X86_64")) {
		...
		fprintf(ofp,
			...
			USHORT(user_regs + OFFSET(user_regs_struct_cs)), USHORT(user_regs + OFFSET(user_regs_struct_ss))
		    );
	}
