...
struct supported_gdb_version {
	...
	"GDB=gdb-5.3post-0.20021129.36rh",
		    ...
		    "GDB_PATCH_FILES=", "GDB_FLAGS=-DGDB_5_3", "GPLv2"}, {
	"GDB=gdb-6.0",
		    "6.0",
		    "GDB_FILES=${GDB_6.0_FILES}",
		    "GDB_OFILES=${GDB_6.0_OFILES}", "GDB_PATCH_FILES=", "GDB_FLAGS=-DGDB_6_0", "GPLv2"}, {
	"GDB=gdb-6.1",
		    "6.1",
		    "GDB_FILES=${GDB_6.1_FILES}",
		    "GDB_OFILES=${GDB_6.1_OFILES}", "GDB_PATCH_FILES=gdb-6.1.patch", "GDB_FLAGS=-DGDB_6_1", "GPLv2"}, {
	"GDB=gdb-7.0", "7.0", "GDB_FILES=${GDB_7.0_FILES}",
		    "GDB_OFILES=${GDB_7.0_OFILES}", "GDB_PATCH_FILES=gdb-7.0.patch", "GDB_FLAGS=-DGDB_7_0", "GPLv3"}, {
	"GDB=gdb-7.3.1", "7.3.1", "GDB_FILES=${GDB_7.3.1_FILES}",
		    "GDB_OFILES=${GDB_7.3.1_OFILES}",
		    "GDB_PATCH_FILES=gdb-7.3.1.patch", "GDB_FLAGS=-DGDB_7_3_1", "GPLv3"}, {
"GDB=gdb-7.6", "7.6", "GDB_FILES=${GDB_7.6_FILES}",
		    "GDB_OFILES=${GDB_7.6_OFILES}", "GDB_PATCH_FILES=gdb-7.6.patch", "GDB_FLAGS=-DGDB_7_6", "GPLv3"},};
...
void target_rebuild_instructions(struct supported_gdb_version *sp, char *target)
{
...
void arch_mismatch(struct supported_gdb_version *sp)
{
	fprintf(stderr,
...
void get_current_configuration(struct supported_gdb_version *sp)
{
	FILE *fp;
	static char buf[512];
	char *p;
	...
	if (target_data.target_as_param) {
		if ((target_data.target == X86 || target_data.target == X86_64)
			 ...
			 */
			target_data.target = ARM;
		} else if ((target_data.target == X86_64) && (name_to_target((char *)target_data.target_as_param)
							      == X86)) {
			/*
			 ...
			 */
			target_data.target = ARM64;
		} else if ((target_data.target == PPC64) && (name_to_target((char *)target_data.target_as_param)
							     == PPC)) {
			/*
			...
			fprintf(stderr,
				...
				target_data.target_as_param, target_to_name(target_data.host));
			exit(1);
		}
	}
	 ...
	 */
	if (target_data.initial_gdb_target && (target_data.target != target_data.initial_gdb_target)) {
		if ((target_data.initial_gdb_target == ARM) && (target_data.target != ARM)) {
		...
		if ((target_data.target == ARM) && (target_data.initial_gdb_target != ARM))
			arch_mismatch(sp);

		if ((target_data.initial_gdb_target == X86) && (target_data.target != X86)) {
			if (target_data.target == X86_64)
				target_data.target = X86;
			else
				arch_mismatch(sp);
		}
		if ((target_data.target == X86) && (target_data.initial_gdb_target != X86))
			arch_mismatch(sp);

		if ((target_data.initial_gdb_target == ARM64) && (target_data.target != ARM64)) {
			if (target_data.target == X86_64)
				target_data.target = ARM64;
			else
				arch_mismatch(sp);
		}
		if ((target_data.target == ARM64) && (target_data.initial_gdb_target != ARM64))
			arch_mismatch(sp);

		if ((target_data.initial_gdb_target == PPC) && (target_data.target != PPC)) {
			if (target_data.target == PPC64)
				target_data.target = PPC;
			else
				arch_mismatch(sp);
		}
		if ((target_data.target == PPC) && (target_data.initial_gdb_target != PPC))
			arch_mismatch(sp);
	}
 ...
 get_release:
	...
	if (file_exists(".rh_rpm_package")) {
		if ((fp = fopen(".rh_rpm_package", "r")) == NULL) {
			...
			if (fgets(buf, 512, fp)) {
				...
				if (strlen(buf)) {
					...
					fprintf(stderr, "WARNING: .rh_rpm_package file is empty!\n");
			} else
				fprintf(stderr, "WARNING: .rh_rpm_package file is empty!\n");
			fclose(fp);
		...
		fprintf(stderr, "WARNING: .rh_rpm_package file does not exist!\n");
	...
	while (fgets(buf, 512, fp)) {
		if (strncmp(buf, "#define BASELEVEL_REVISION", strlen("#define BASELEVEL_REVISION")) == 0) {
			p = strstr(buf, "\"") + 1;
			strip_linefeeds(p);
			p[strlen(p) - 1] = '\0';
...
void show_configuration(void)
{
	...
	if (strlen(target_data.program)) {
		for (i = 0; i < (strlen("TARGET") - strlen(target_data.program)); i++)
			printf(" ");
...
void build_configure(struct supported_gdb_version *sp)
{
	FILE *fp1, *fp2;
	char buf[512];
	char *target;
	...
	while (fgets(buf, 512, fp1)) {
		if (strncmp(buf, "TARGET=", strlen("TARGET=")) == 0)
			fprintf(fp2, "%s\n", target);
		else if (strncmp(buf, "TARGET_CFLAGS=", strlen("TARGET_CFLAGS="))
			 == 0)
			fprintf(fp2, "%s%s%s\n", target_CFLAGS, cflags ? " " : "", cflags ? cflags : "");
		else if (strncmp(buf, "GDB_CONF_FLAGS=", strlen("GDB_CONF_FLAGS="))
			 == 0)
			fprintf(fp2, "%s\n", gdb_conf_flags);
			...
			fprintf(fp2, "GPL_FILES=%s\n", strcmp(sp->GPL, "GPLv2") == 0 ? "COPYING" : "COPYING3");
...
void release_configure(char *gdb_version, struct supported_gdb_version *sp)
{
	FILE *fp1, *fp2;
	int found;
	char buf[512];
	...
	sprintf(gdb_files, "GDB_%s_FILES", &gdb_version[strlen("gdb-")]);
 ...
 */
void make_rh_rpm_package(char *package, int release)
{
	char *p, *cur;
	FILE *fp;
	char buf[256];
	...
	if ((strcmp(package, "remove") == 0)) {
		if (file_exists(".rh_rpm_package")) {
			if (unlink(".rh_rpm_package")) {
				perror("unlink");
				fprintf(stderr, "cannot remove .rh_rpm_package\n");
				exit(1);
			}
		}
		return;
...
void gdb_configure(struct supported_gdb_version *sp)
{
	FILE *fp1, *fp2;
	char buf[512];
...
void unconfigure(void)
{
	...
	while (fgets(buf, 512, fp1)) {
		if (strncmp(buf, "TARGET=", strlen("TARGET=")) == 0)
		...
		else if (strncmp(buf, "GDB_CONF_FLAGS=", strlen("GDB_CONF_FLAGS=")) == 0)
			...
			fprintf(fp2, "LDFLAGS=\n");
		else if (strncmp(buf, "WARNING_ERROR=", strlen("WARNING_ERROR="))
			 == 0) {
			shift_string_right(buf, 1);
			buf[0] = '#';
			fprintf(fp2, "%s", buf);
		} else if (strncmp(buf, "WARNING_OPTIONS=", strlen("WARNING_OPTIONS=")) == 0) {
			shift_string_right(buf, 1);
			buf[0] = '#';
			fprintf(fp2, "%s", buf);
		} else
...
void set_warnings(int w)
{
	...
	while (fgets(buf, 512, fp1)) {
		if (strncmp(buf, "#WARNING_ERROR=", strlen("#WARNING_ERROR="))
		    == 0) {
			switch (w) {
			case 'W':
		...
		if (strncmp(buf, "WARNING_ERROR=", strlen("WARNING_ERROR=")) == 0) {
			switch (w) {
			case 'n':
			case 'w':
				shift_string_right(buf, 1);
		...
		if (strncmp(buf, "#WARNING_OPTIONS=", strlen("#WARNING_OPTIONS="))
		    == 0) {
			switch (w) {
			case 'W':
			case 'w':
...
void makefile_setup(FILE ** fp1, FILE ** fp2)
{
...
void makefile_create(FILE ** fp1, FILE ** fp2)
{
	fclose(*fp1);
	fclose(*fp2);
	...
	if (chown("Makefile", target_data.statbuf.st_uid, target_data.statbuf.st_gid) == -1) {
		fprintf(stderr, "Makefile: cannot restore original owner/group\n");
	}
}
 ...
 */
char *lower_case(char *s, char *buf)
{
	char *p1, *p2;
...
void make_build_data(char *target)
{
	char *p;
...
void make_spec_file(struct supported_gdb_version *sp)
{
	...
	printf
	    ("ExclusiveArch: %%{ix86} alpha ia64 ppc ppc64 ppc64pseries ppc64iseries x86_64 s390 s390x arm aarch64\n");
...
struct supported_gdb_version *store_gdb_defaults(struct
...
void set_initial_target(struct supported_gdb_version *sp)
{
	FILE *fp;
	...
	sprintf(crash_target, "%s/crash.target", &sp->GDB[strlen("GDB=")]);

	if (!file_exists(crash_target)) {
		if (target_data.target_as_param && file_exists(&sp->GDB[strlen("GDB=")])) {
...
char *get_extra_flags(char *filename, char *initial)
{
	FILE *fp;
	char inbuf[512];
	char buf[512];
