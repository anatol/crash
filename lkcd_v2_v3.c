 ...
 */
int lkcd_dump_init_v2_v3(FILE * fp, int fd)
{
	int i;
	int eof;
	uint32_t pgcnt;
	...
	if (read(lkcd->fd, dha, sizeof(dump_header_asm_t)) != sizeof(dump_header_asm_t))
		return FALSE;
	...
	for (pgcnt = 0, eof = FALSE; !eof; pgcnt++) {
		...
		if (dp->dp_flags & ~(DUMP_COMPRESSED | DUMP_RAW | DUMP_END | LKCD_DUMP_MCLX_V0)) {
...
void dump_dump_page_v2_v3(char *s, void *dpp)
{
	dump_page_t *dp;
	uint32_t flags;
	int others;
 ...
 */
void dump_lkcd_environment_v2_v3(ulong arg)
{
	int others;
	dump_header_t *dh;
	dump_header_asm_t *dha;
